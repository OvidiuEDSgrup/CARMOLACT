CREATE VIEW [dbo].[IntrariLapteCantitativScriptic] AS
SELECT 
p.Subunitate,
p.Tip, 
p.Numar,
...
RTRIM(ISNULL(Punct_livrare, '')) AS Ruta,
...
dbo.iauPropStocGestCodIntr(identificator, 'G') AS gras_teren,
dbo.iauPropStocGestCodIntr(identificator, 'G')*p.cantitate AS cant_UG_teren,
dbo.iauPropStocGestCodIntr(identificator, 'G') AS gras_fabrica,
dbo.iauPropStocGestCodIntr(identificator, 'G')*p.cantitate AS cant_UG_fabrica,
