CREATE VIEW [dbo].[IesiriLapteCantitativ] AS
SELECT 
p.Subunitate,
p.Tip, 
p.Numar,
...
Comanda AS Masina,
CONVERT(INT, CASE ISNUMERIC(LEFT(LTRIM(p.numar),1)) WHEN 1 THEN REPLACE(LEFT(LTRIM(p.numar),1),'.','') ELSE 0 END) AS Tura,
p.cod AS Tip_lapte,
CONVERT(INT, CASE ISNUMERIC(LEFT(RIGHT(RTRIM(p.cod_intrare),3),1)) 
			WHEN 1 THEN LEFT(RIGHT(RTRIM(p.cod_intrare),3),1) ELSE 0 END) AS Compartiment,
