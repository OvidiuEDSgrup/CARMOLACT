...
*/  
	...
	insert into #datecitite(idPozPlin,tip_antet,subunitate,cont,data,numar,plata_incasare,tert,factura,cont_corespondent,suma,valuta,curs,suma_valuta,nr_pozitie_primit,  
		cota_tva,suma_tva,explicatii,loc_de_munca,comanda,indbug,cont_dif,suma_dif,achitat,_de_spart,_update,jurnal,detalii,detalii_antet,tip_tva,marca,decont,efect,suma_de_spart, subtip, idplaja)  
	select  isnull(idPozPlin,0),tip_antet,@sub,isnull(nullif(cont_pozitii,''),isnull(nullif(cont_antet,''),'')) as cont,isnull(data_pozitii,data_antet),numar,subtip,
		...
		cota_tva,suma_tva,explicatii,lm,comanda,isnull(indbug,''),isnull(cont_dif,'') as cont_dif,0 as suma_dif,achitfact as achitat,  
		(case when subtip in ('PF','PV','IB','IV','PS','IS') and ISNULL(factura,'')='' then 1 else 0 end), _update,isnull(jurnal,isnull(jurnal_poz,'')),detalii as detalii, detalii_antet as detalii_antet, 
		...
		isnull(nullif(efect_pozitii,''),nullif(efect_antet,'')) as efect,(case when suma_valuta>0 and valuta!='' then suma_valuta else suma end) as suma_de_spart , subtip, idplaja
 ...
 */  
	...
	select  pd.idPozPlin,@sub,pd.cont,pd.data,pd.numar,pd.plata_incasare,fc.tert,fc.factura,fc.cont_de_tert,  
		...
		ROW_NUMBER() OVER (partition by pd.cont,pd.data,pd.numar order by pd.nr_pozitie_primit) as numar_pozitie_calculat,pd.cont_dif,pd.suma_dif,isnull(pd.achitat,0),pd.jurnal,
	...
	select   
		...
		@utilizator,convert(datetime, convert(char(10), getdate(), 104), 104),RTrim(replace(convert(char(8), getdate(), 108), ':', '')),  
		ROW_NUMBER() OVER (partition by pd.cont,pd.data order by pd.nr_pozitie_primit),pd.cont_dif,pd.suma_dif,isnull(pd.achitat,0),pd.jurnal,
		(case when @detalii_antet is not null then @detalii_antet else pd.detalii end),isnull(pd.Tip_tva,0),pd.Marca,pd.Decont,pd.Efect,
...
-->	pentru cazuri in care se fac plati/incasari reprezentand avansuri (daca suntem pe subtip de avans), TVA-ul se extrage din suma introdusa in macheta (model CGplus si wScriuPozplin).
 ...
 -->Tipul din subtip  
	update #descris set plata_incasare=(case when plata_incasare='PV' 
												or plata_incasare='PX'/*PX->subtip de plata avans*/ 
												or plata_incasare='PL'/*PL->subtip de plata factura in alta valuta decat valuta facturii*/ then 'PF' 
											when plata_incasare='IV' 
												or plata_incasare='IX'/*IX->subtip de incasare avans*/ 
												or plata_incasare='IL'/*IL->subtip de incasare factura in alta valuta decat valuta facturii*/
												then 'IB' 
...
-->	Scos formarea indicatorului bugetar. Indicatorul va fi completat doar in tabela conturi (detalii).
 ...
 -->Procesare valuta  
	IF EXISTS(select 1 from #descris where valuta!='' or nullif(detalii.value('(/row/@valutafalta)[1]','varchar(20)'),'') is not null)
	BEGIN  
...
-->	transpunere suma valuta in suma lei pornind de la curs.	
		...
		update #descris set Achit_fact=Suma_valuta where plata_incasare in ('PF','IB','PS','IS') and valuta!='' and subtip not in ('PL','IL')
		
		update #descris set  
			suma=round(convert(decimal(18, 5), #descris.suma_valuta*#descris.curs),2), 
			Achit_fact=(case when subtip in ('PL','IL') then #descris.Achit_fact else #descris.Suma_valuta end)
		from facturi f 
...
-->Tratat curs la valuta facturii, pentru plata/incasare facturi in alta valuta decat valuta facturilor (subtip PL si IL).
		update #descris set Curs_la_valuta_facturii=convert(DECIMAL(10, 4),Suma/Achit_fact) where plata_incasare in ('PF','IB') and subtip in ('PL','IL')

-->Calcul diferente de curs. Pentru subtipurile PL/IL diferentele de curs se determina functie de campul Curs_la_valuta_facturii. In rest ca si pana acuma functie de campul curs.
		update #descris set  
			Suma_dif=convert(decimal(11,2),(case when f.valuta='' then 0 
				else #descris.Achit_fact*(case when subtip in ('PL','IL') then #descris.Curs_la_valuta_facturii else #descris.curs end)-#descris.Achit_fact*f.curs end))
		from facturi f 
		where #descris.Subunitate=f.Subunitate and #descris.tert=f.Tert and #descris.Factura=f.Factura and #descris.tip_fact=f.Tip
			and #descris.plata_incasare in ('PF','IB','PS','IS') and #descris.tert!='' and #descris.Factura!=''
			and (#descris.valuta!='' or nullif(#descris.detalii.value('(/row/@valutafalta)[1]','varchar(20)'),'') is not null)
...
-->	calcul diferente de curs la deconturi in valuta
		...
		from deconturi d 
		where #descris.Subunitate=d.Subunitate and #descris.marca=d.Marca and #descris.Decont=d.Decont and #descris.valuta!='' and #descris.subtip not in ('PA'/*,'IA'*/)	-- la restituire trebuie dif. de curs
		...
		from deconturi d 
		where #descris.Subunitate=d.Subunitate and #descris.marca=d.Marca and #descris.Decont=d.Decont and #descris.valuta!='' and #descris.subtip not in ('PA'/*,'IA'*/)
...
-->	stocam diferenta de curs si contul in pozplin.detalii (pentru a nu se suprapune cu diferenta de curs/contul de diferente de curs de la facturi)
	...
	-->Tratat curs la valuta facturii, pentru plata/incasare facturi in alta valuta decat valuta facturilor (subtip PL si IL).
	update #descris set Curs_la_valuta_facturii=convert(DECIMAL(10, 4),Suma/Achit_fact) where plata_incasare in ('PF','IB') and subtip in ('PL','IL') and valuta=''
	...
	begin tran scriupozplin	
				...
				begin
					...
					select top 1 @tipPentruNr=tip_antet,@lm=loc_de_munca,@jurnal=jurnal,@subtip=plata_incasare, @idplaja = idplaja
					...
					set @fXML.modify ('insert attribute idplaja {sql:variable("@idplaja")} into (/row)[1]')
