...
declare @Tip varchaR(2),@Numar char(20), @Data datetime, @Tert char(13),
	...
	@Data_scad_prest datetime,@Cont_fact_prest varchar(40),@Valoare_prest float,@Cota_TVA_prest float,@SumaTVA float, @Tip_TVA_prest int,@Update int,
	@ValoareFaraTVAvaluta_prest float,@o_ValoareFaraTVAvaluta_prest float,@ValoareFaraTVAlei_prest float,@Numar_pozitie int,@stergere int,@subtip varchar(2),@tipRepartizarePrestari int, @detalii xml,
...
begin try
	...
	*/
	...
	select 
		...
		@SumaTVA=ISNULL(@parXML.value('(/row/row/@valTVA)[1]', 'float'), 0),
		...
		@o_ValoareFaraTVAvaluta_prest=ISNULL(@parXML.value('(/row/row/@o_ValoareFaraTVAvaluta_prest)[1]', 'float'), 0),
		...
		if @Update=0
			set @Factura_prest=ltrim(rtrim(@Factura_prest))
	...
	declare @Sb char(9),@CodPS int,@RPGreu int,@NrPozitie int,@DVE int,@Utilizator char(10),@CDTVA varchar(40),@CNEEXREC varchar(40),
...
--	apelez validarea pentru receptiile de mijloace fixe pentru cazul in care s-a inceput amortizarea pentru acestea
	...
	if (@Valoare_prest=0 or @ValoareFaraTVAvaluta_prest<>@o_ValoareFaraTVAvaluta_prest) and @ValoareFaraTVAvaluta_prest<>0
		set @Valoare_prest=convert(decimal(17,4),@ValoareFaraTVAvaluta_prest*(case when @Valuta_prest<>'' then @Curs_prest else 1 end))
	
	if @SumaTVA=0 and @Cota_TVA_prest>0
	...
	begin
		...
		select @Data_scad_prest = isnull(dateadd(day,coalesce(ext.discount,0),@Data_fact_prest),convert(char(10),getdate(),101))
		from terti t 
		left outer join infotert ext on ext.subunitate=t.Subunitate and ext.tert=t.tert and ext.identificator='' 
		where t.Subunitate=@Sb and t.tert=@Tert_prest 
			and (@Data_scad_prest=@Data_fact_prest or convert(char(10),@Data_scad_prest,101)=convert(char(10),getdate(),101) or @Data_scad_prest='01/01/1901')
