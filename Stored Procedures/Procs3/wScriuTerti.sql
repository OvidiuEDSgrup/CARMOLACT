...
as
declare @iDoc int, @Sub varchar(9), @AdrComp int, @CodTertCodFisc int, @ContFImpl varchar(40), @ContBImpl varchar(40), 
	@mesaj varchar(200), @tert varchar(13), @cod_fiscal varchar(16), @alt_tert varchar(13), @referinta int, @tabReferinta int, @mesajEroare varchar(100), 
	@UltTert int,@locTerti int ,@JudTerti int, @eroare int,@tip_tert varchar(1), @detalii xml,@email varchar(2000), @dataAzi datetime, @TipTVAUnitate varchar(1)
...
begin try
	...
	set @dataAzi=convert(datetime,convert(varchar(10), getdate(),101), 101)
	...
	begin
		...
		while exists (select 1 from terti where subunitate=@Sub and tert=RTrim(LTrim(convert(varchar(9), @UltTert))))
			set @UltTert = @UltTert + 1
		update top (1) x
		set tert=RTrim(LTrim(convert(varchar(9), @UltTert)))
		from #xmlt x
	...
	begin
		...
		begin
			...
			insert into #validCUI (cui)
			select rtrim(@cod_fiscal)
			
			exec pValidareCodFiscal
			if (select top 1 cod_eroare from #validCUI)!=0 
			begin
				select top 1 @mesajEroare=den_eroare from #validCUI
				raiserror(@mesajEroare, 16, 1)
			end
		end
	end
