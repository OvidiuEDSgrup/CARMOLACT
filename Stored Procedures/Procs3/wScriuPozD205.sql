...
begin try
	...
	@venit_brut float, @deduceri_personale float, @deduceri_alte float, @baza_impozit float, @impozit float, @venit_net float, 
	...
	select  @an=ISNULL(@parXML.value('(/row/@an)[1]','int'),''),
		...
		@venit_net=isnull(@parXML.value('(/row/row/@venitnet)[1]','decimal(10)'),0),
...
--	daca tabela par este view inseamna ca se lucreaza cu parametrii pe locuri de munca (in aceeasi BD sunt mai multe unitati)	
	...
	begin
		insert into DateD205 (an,loc_de_munca,tip_venit,tip_impozit,marca,cnp,nume,tip_functie,venit_brut,Deduceri_personale,Deduceri_alte,Baza_impozit,Impozit, Venit_net)
		values (@an,@lm,@tip_venit,@tip_impozit,@marca,@cnp,@nume,@tip_functie,@venit_brut,@deduceri_personale,@deduceri_alte,@baza_impozit,@impozit,@venit_net)
	end
	else --modificare
	begin 
		update DateD205 set Tip_venit=@tip_venit, Tip_impozit=@tip_impozit, marca=@marca, cnp=@cnp, nume=@nume, tip_functie=@tip_functie, 
			Venit_brut=@venit_brut, Deduceri_personale=@deduceri_personale, Deduceri_alte=@deduceri_alte, 
			Baza_impozit=@baza_impozit, impozit=@impozit, venit_net=@venit_net
