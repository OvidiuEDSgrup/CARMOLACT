...
CREATE PROCEDURE wScriuPozOrdineDePlata @sesiune VARCHAR(50), @parXML XML OUTPUT
AS
BEGIN TRY
	DECLARE 
	...
	**/
	...
	IF NOT EXISTS (	SELECT 1 FROM conturi	WHERE cont = @cont	) and @idOPInitial iS NULL
		RAISERROR ('Contul contabil introdus este invalid!', 11, 1)

	IF @data IS NULL and @idOPInitial IS NULL
	...
	SELECT 
		...
		Pozitii.poz.value('@data_scadentei', 'datetime') data_scadentei,
		Pozitii.poz.value('@soldscadent', 'float') soldscadent,
...
--							return
	...
	IF @idOPInitial is not null and exists (select 1 from #setPozitiiOP)
		INSERT INTO PozOrdineDePlata (idOP, tert, marca, factura, decont, banca, IBAN,  explicatii, suma, stare, detalii)
		SELECT s.idOP, s.tert, s.marca, s.factura, s.decont, s.banca, s.IBAN, s.explicatii, s.suma, s.stare, s.detalii
		FROM #setPozitiiOP s
		LEFT JOIN PozOrdineDePlata p on p.idOP=@idOPInitial and p.factura=s.factura and p.tert=s.tert
		where p.idPozOp is null
	...
	BEGIN
		...
		IF @tipOP='EX'
			EXEC wIaPozOrdineDePlataExtras @sesiune = @sesiune, @parXML = @docPozitii
	END
END TRY
