...
begin
	...
	--set @intervalreptm=isnull(@document.value('(/row/row/@intervalreptm)[1]','varchar(60)'),isnull(@document.value('(/row/@intervalreptm)[1]','varchar(60)'),''))
	...
	--	return -1
	--end
	--if (@document.exist('/row/row')=1 or @document.exist('/row')=1) and @intervalreptm in ('OrePeLuna','OrePeSaptamina','OrePeZi') and @Grupa_de_munca in ('N','D','S')
	--begin
