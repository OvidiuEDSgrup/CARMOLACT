...
create procedure wScriuPersoaneContactSP @sesiune varchar(50), @parXML xml
as
...
begin try
	select identity (int, 1, 1) as numar_unic, isnull(ptupdate, 0) as ptupdate, upper(tert) as tert, isnull(tert_vechi, tert) as tert_vechi, 
		...
		numar, bloc, scara, apartament, cont_in_banca3, indicator, grupa13, data_nasterii, discount, zile_incasare, observatii, detalii
	into #xmlpersct
	...
	if @parXML.exist('/row/@_butonAdaugare')=1 and @tertDelegat is not null	-- Daca se adauga un tert din form de tip ACA, se citeste tertul delegat din detalii
	begin
		update x
		set tert=@tertDelegat
		from #xmlpersct x
	end
	...
	(Subunitate, Tert, Identificator, 
		...
		Grupa13, Sold_ben, Discount, Zile_inc, Observatii,detalii)
	select 'C'+@Sub, x.tert, x.identificator, left(rtrim(isnull(x.persoana_contact, '')) + ' ' + rtrim(isnull(x.nume_delegat, '')), 30), 
		...
		isnull(x.discount, 0), isnull(x.zile_incasare, 0), isnull(x.observatii, ''), x.detalii
	from #xmlpersct x
	where x.ptupdate=0
	
	set identity_insert delegexp on
	insert delegexp (Numele_delegatului,Seria_buletin,Numar_buletin,Eliberat,Loc_de_munca,Marca,id)
	select left(rtrim(isnull(x.persoana_contact, '')) + ' ' + rtrim(isnull(x.nume_delegat, '')), 30), 
		rtrim(isnull(x.serie_buletin, '')), rtrim(isnull(x.numar_buletin, '')), isnull(x.eliberat_buletin, ''), 
		isnull(x.lm, ''), '', x.identificator
	from #xmlpersct x
	where x.ptupdate=0 and x.tert=@tertDelegat
	set identity_insert delegexp off
	...
	set tert=isnull(x.tert, i.tert), identificator=isnull(x.identificator, i.identificator), 
		...
		zile_inc=isnull(x.zile_incasare, i.zile_inc), observatii=isnull(x.observatii, i.observatii), i.detalii=x.detalii
	...
	update i
	set Numele_delegatului=isnull(nullif(left(rtrim(isnull(x.persoana_contact, '')) + ' ' + rtrim(isnull(x.nume_delegat, '')), 30)
			,''),i.Numele_delegatului), 
		Loc_de_munca=isnull(x.lm, i.Loc_de_munca), 
		Seria_buletin=rtrim(isnull(x.serie_buletin, i.Seria_buletin)),
		Numar_buletin=rtrim(isnull(x.numar_buletin, i.Numar_buletin)), 
		Eliberat=isnull(x.eliberat_buletin, i.eliberat)
	from delegexp i, #xmlpersct x
	where x.ptupdate=1 and rtrim(abs(i.id))=x.identificator_vechi and x.tert_vechi=@tertDelegat
...
begin catch
	set @mesaj = ERROR_MESSAGE()+ ' (wScriuPersoaneContactSP)'
