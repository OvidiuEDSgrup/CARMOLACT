...
declare @iDoc int,@Sub char(9),@mesaj varchar(200),@update int,@lm char(8),@comanda char(20),@indbug char(20),@ComandaDeScris char(40),@utilizator varchar(30),@factura varchar(20),
		...
		@o_tert varchar(13),@o_cont varchar(40),@o_lm char(8),@an_impl int,@luna_impl int,@mod_impl int,@valoare_valuta float,@achitat_valuta float,@valoare_lei float,@achitat_lei float,
		@sold_valuta float,@valtva_lei float,@valtva_valuta float,@mesajEroare varchar(200),@_cautare varchar(100)
...
begin try	
	select
		...
		@o_cont=@parXML.value('(/row/row/@o_cont_de_tert)[1]', 'varchar(40)')	,
		@o_lm=@parXML.value('(/row/row/@o_lm)[1]', 'varchar(13)')			
...
--	validare campuri
	...
	--select @achitat,@tiptert,@o_factura,@o_tert
		...
		WHERE Subunitate=@Sub and tip=(case @tiptert when 'B' then 0x46 when 'F' then 0x54 else 0 end)
			and Factura=@o_factura and tert=@o_tert and Cont_de_tert=@o_cont and Loc_de_munca=@o_lm
	end
	else
	begin
