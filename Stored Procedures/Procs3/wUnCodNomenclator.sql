...
declare @cod varchar(100), @categoriePret int, @cantitate decimal(12,3), @vanzareFaraStoc bit, @ruleazaSelect bit, @barcode varchar(50), @UM varchar(3),
		...
		@trebuieCantarit bit, @durataInput int, @clipboardIsSimilar bit 
...
begin 
	 ...
	 */
	if LEN(@codInitial)>20 and exists (select * from CarduriFidelizare c where c.UID=@codInitial)
	begin
		set @xmlFinal=@parXML
	...
	begin
		set @mesaj='Codul introdus ('+ @cod + case when @codInitial<>'' then ' sau ' + @codInitial  else '' end + ') nu poate fi gasit.'
