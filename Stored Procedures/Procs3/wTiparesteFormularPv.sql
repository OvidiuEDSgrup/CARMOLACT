...
begin
	exec @returnValue = wTiparesteFormularPvSP @sesiune,@parXML
	return @returnValue 
end
...
begin try
	...
	if exists(select * from sysobjects where name='wTiparesteFormularPvSP1' and type='P')      
	begin
		exec @returnValue = wTiparesteFormularPvSP1 @sesiune=@sesiune,@parXML=@parXML output
		return @returnValue 
	end
	...
	select @factura=b.Factura, @cDataDoc=convert(char(10) ,b.Data_facturii, 101), @tert=b.Tert, @eBon=b.Chitanta, @numar=convert(varchar(30),b.Numar_bon)
	from antetBonuri b
	...
	if @factura is null 
		raiserror('Numarul documentului nu poate fi identificat!',11,1)
	
	if @parXML.value('(/row/@tip)[1]','varchar(50)') is null
		set @parXML.modify ('insert attribute tip {"AP"} into (/row)[1]')
	else
		set @parXML.modify('replace value of (/row/@tip)[1] with "AP"')
