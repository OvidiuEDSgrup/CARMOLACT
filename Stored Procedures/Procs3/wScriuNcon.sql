...
--> citire si organizare parametri:
	...
	select 	@subunitate,
		...
		(case when isnull(suma,0)=0 and isnull(valuta,'')<>''
			then isnull(suma_valuta,0)*isnull(curs,0)
			else isnull(suma,0) end)  as suma,
...
--> prelucrari de date:
		...
		*/
		...
		if exists (select 1 from #pozncon where Loc_munca='' and Comanda<>'')
			update p1 set loc_munca=c.Loc_de_munca
			from #pozncon p1
				left outer join comenzi c on c.subunitate=@Subunitate and c.Comanda=p1.Comanda
			where p1.Loc_munca='' and p1.Comanda<>''
			...
			if (select count(*) from #pozncon where _update=1)>0 /*Se va modifica pozitia din tabela pozncon*/
				update pozncon  
					set Cont_debitor=isnull(#pozncon.cont_debitor,pozncon.cont_debitor), 
						...
						Comanda=isnull(#pozncon.comanda+(case when #pozncon.tip in ('AO','AL','AT') then #pozncon.indbug else '' end),pozncon.comanda), 
				...
				select p1.Subunitate,p1.Tip,p1.Numar,p1.Data,p1.Cont_debitor,p1.Cont_creditor,p1.Suma,p1.Valuta,p1.Curs,p1.Suma_valuta,p1.Explicatii,p1.Utilizator,p1.Data_operarii,p1.Ora_operarii,
					...
					p1.Loc_munca,p1.Comanda+(case when p1.tip in ('AO','AL','AT') then p1.indbug else '' end),p1.Tert,p1.Jurnal,p1.Detalii
				from #pozncon p1
