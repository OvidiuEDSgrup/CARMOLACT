--***
Create procedure wValidareConcodih (@sesiune varchar(50), @document xml)
as 
begin
	declare @subtip varchar(2), @marca varchar(6), @data datetime, @tipconcediu varchar(2), @datainceput datetime, @datasfarsit datetime, 
	...
	set @subtip=@document.value('(/row/row/@subtip)[1]', 'varchar(2)')
	...
	set @datainceput=isnull(@document.value('(/row/row/@datainceput)[1]','datetime'),dbo.bom(@data))
	set @datasfarsit=isnull(@document.value('(/row/row/@datasfarsit)[1]','datetime'),dbo.eom(@data))
	...
	if @tipconcediu in ('3','6') and @subtip='ON'
		set @ZileCO=@document.value('(/row/row/@zileco)[1]', 'int')
	else
