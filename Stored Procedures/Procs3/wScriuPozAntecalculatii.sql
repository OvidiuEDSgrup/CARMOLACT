...
CREATE PROCEDURE [dbo].[wScriuPozAntecalculatii] @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	DECLARE @idAntec INT, @idElement INT, @cod VARCHAR(20), @cantitate FLOAT, @data DATETIME, @valuta VARCHAR(10), @curs FLOAT, @update 
		BIT, @numarDoc VARCHAR(8), @fXML XML, @utilizator VARCHAR(20), @docGenerare XML, @pretElement FLOAT, @cantitateElement FLOAT
		, @mesaj VARCHAR(500), @element VARCHAR(20), @tipElement VARCHAR(2), @scriuPret BIT
	...
	SET @cantitate = ISNULL(@parXML.value('(/row/@cantitate)[1]', 'float'), 0)
	SET @curs = ISNULL(@parXML.value('(/row/@curs)[1]', 'float'), 1)
	...
	BEGIN
		...
		SET @docGenerare = (
				SELECT @idAntec AS id, @data AS data, @numarDoc AS numarDoc, @cantitateElement procent, @element element, @scriuPret 
					scriuPret
				FOR XML raw
				)
	...
	ELSE
		...
		BEGIN
			...
			SET @docGenerare = (
					SELECT @cod AS cod, @numarDoc AS numarDoc, @data AS data, @scriuPret scriuPret, @valuta valuta, @curs curs
					FOR XML raw
					)

			EXEC GenerareAntecalculatii @sesiune = @sesiune, @parXML = @docGenerare

			SELECT TOP 1 @idAntec = idAntec
			FROM antecalculatii
			WHERE numar = @numarDoc
				AND cod = @cod
				AND convert(DATE, data) = @data
			ORDER BY idAntec DESC
		END

	DECLARE @docXMLIaPozAntec XML

	SET @docXMLIaPozAntec = (
			SELECT @idAntec AS idAntec
			FOR XML raw
			)
