...
--***
CREATE PROCEDURE yso_wScriuTE @parXmlScriereIesiri XML
OUTPUT AS
...
BEGIN TRY
	DECLARE @Numar CHAR(8), @Data DATETIME, @GestPred CHAR(9), @GestPrim CHAR(9), @GestDest CHAR(20), 
		...
		@Serie CHAR(20), @Utilizator CHAR(10), @PastrCtSt INT, @Valoare FLOAT, @TotCant FLOAT, @NrPozitie INT, @CtCoresp CHAR(13), 
		@CtIntermediar CHAR(13), @TVAnx FLOAT, @update BIT, @subtip VARCHAR(2), @mesaj VARCHAR(200), @iDoc INT, @docInserate XML,
		@docDetalii XML, @detalii xml

	EXEC sp_xml_preparedocument @iDoc OUTPUT, @parXmlScriereIesiri

	SELECT @Numar = isnull(numar, ''), @Data = data, @GestPred = gestiune, @GestPrim = gestiune_primitoare, @GestDest = contract, @Cod = 
		cod, @CodIntrare = cod_intrare, @CodIPrim = (CASE WHEN codiPrim = '' THEN NULL ELSE codiPrim END), @Cantitate = 
		cantitate, @LocatiePrim = locatie, @PretAmPrim = pret_amanunt, @CategPret = ISNULL(categ_pret, ''), @Valuta = valuta, @Curs = 
	...
	FROM OPENXML(@iDoc, '/row') WITH (
			subtip CHAR(2) '@subtip', numar CHAR(8) '@numar', data DATETIME '@data', tert CHAR(13) '@tert', factura CHAR(20) 
			...
			cota_TVA FLOAT '@cota_TVA', locatie CHAR(30) '@locatie', numar_pozitie INT '@numar_pozitie', suprataxe FLOAT 
			'@suprataxe', cont_corespondent CHAR(13) '@cont_corespondent', contintermediar CHAR(13) '@contintermediar', 
	...
	DECLARE @Sb CHAR(9), @TabPreturi INT, @TLitR INT, @Accize INT, @CtAccCR CHAR(13), @FaraTVAnx INT, @Ct348 CHAR(13), @DifPProd INT, 
		@CtIntTE CHAR(13), @Ct378 CHAR(13), @AnGest378 INT, @AnGr378 INT, @Ct4428 CHAR(13), @AnGest4428 INT, @TipNom CHAR(1), @CtNom 
		CHAR(13), @PStocNom FLOAT, @PAmNom FLOAT, @GrNom CHAR(13), @CoefConv2Nom FLOAT, @CategNom INT, @GreutSpecNom FLOAT, @TVANom 
		FLOAT, @TipGestPred CHAR(1), @TipGestPrim CHAR(1), @CtGestPrim CHAR(13), @PretSt FLOAT, @CtStoc CHAR(13), @PretAmPred FLOAT, 
	...
	SELECT @PretSt = pret, @CtStoc = cont, @TVAnx = (CASE WHEN tip_gestiune = 'A' THEN tva_neexigibil ELSE @TVAnx END), 
		...
		are_documente_in_perioada, @PVanzSt = pret_vanzare
	FROM stocuri
	WHERE @TipGestPred <> 'V'
		AND subunitate = @Sb
		AND tip_gestiune = @TipGestPred
		AND cod_gestiune = @GestPred
		AND cod = @Cod
	...
	WHERE @TabPreturi = 1
		AND cod_produs = @Cod
		AND UM = (CASE WHEN @CategPret <> 0 THEN @CategPret ELSE 1 END)
		AND tip_pret = '1'
	...
	IF isnull(@CodIPrim, '') = ''
		...
		SET @CodIPrim = dbo.yso_cautareCodIntrare(isnull(@Cod, ''), isnull(@GestPrim, ''), @TipGestPrim, isnull(@CodIntrare, ''), 
