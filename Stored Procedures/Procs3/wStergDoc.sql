...
Declare @cSub char(9), @mesajeroare varchar(100), @eroare xml, @TipPlataTVAUnitate char(1), @factura_vama varchar(20), @tert_vama varchar(20), @data datetime
...
--	Daca firma este cu TVA la incasare, atunci factura de TVA vama s-a scris prin wOPDVI in TVAPeTerti cu tip_tva='P'.
	select @factura_vama=Factura_vama, @tert_vama=tert_vama, @data=dvi.data_receptiei 
	from DVI, @doc dx  
	...
	from TvaPeTerti where tipf='B' and tert is null and @data>dela
	order by dela desc

	if isnull(@TipPlataTVAUnitate,'P')='I'
		delete from TVAPeTerti where tipf='F' and tert=@tert_vama and factura=@factura_vama and dela=@data and tip_tva='P'
