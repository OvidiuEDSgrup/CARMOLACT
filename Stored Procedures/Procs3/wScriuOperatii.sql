...
BEGIN
	DECLARE @cod VARCHAR(10), @denumire VARCHAR(50), @o_cod VARCHAR(20), @UM VARCHAR(50), @tipoperatie VARCHAR(50), @categorie 
		...
		@err VARCHAR(100), @modificare INT, @detalii xml
	...
	BEGIN
		...
		SET Cod = @cod, Denumire = @denumire, UM = @UM, Tip_operatie = @tipoperatie, Numar_persoane = @nrpersoane, Numar_pozitii = 
			@nrpozitii, Tarif = @tarif, Categorie = @categorie, detalii = @detalii
		WHERE Cod = @o_cod
	END
	...
	BEGIN
		...
		END
		ELSE
			INSERT INTO catop (Cod, Denumire, UM, Tip_operatie, Numar_pozitii, Numar_persoane, Tarif, Categorie, detalii)
			VALUES (@cod, @denumire, @um, @tipoperatie, @nrpozitii, @nrpersoane, @tarif, @categorie, @detalii)
	END
