...
BEGIN TRY
	...
	IF EXISTS (SELECT *	FROM sysobjects	WHERE NAME = 'wScriuPozInventarSP')
		exec wScriuPozInventarSP @sesiune=@sesiune, @parXML=@parXML OUTPUT

	DECLARE 
		@gestiune VARCHAR(20), @data DATETIME, @cod VARCHAR(20), @stoc_faptic FLOAT, @utilizator VARCHAR(20), 
		@idInventar INT, @idPozInventar INT, @update BIT, @mesaj VARCHAR(500),@barcod varchar(100),@grupa varchar(13), @detalii XML, @cautare varchar(500)
	...
	select @idInventar = @parXML.value('(/*/@idInventar)[1]', 'int'),
		...
		@barcod = @parXML.value('(/*/*/@barcod)[1]', 'varchar(100)'),
	...
	--se verifica compatibilitate dintre stocul introdus si cel care se citeste in wIaPozInventar unde se face convert la decimal(15,2)
	begin try
		select convert(DECIMAL(15, 2),@stoc_faptic)
	end try
	begin catch
			RAISERROR ('Stocul faptic nu este valid!', 11, 1)
	end catch
