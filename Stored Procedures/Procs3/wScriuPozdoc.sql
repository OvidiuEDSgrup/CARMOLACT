...
declare @userASiS varchar(20), @fara_luare_date varchar(1),@sub varchar(20), @tip char(2), @numar char(20), @data datetime, @tipGrp char(2), @numarGrp char(20), @dataGrp datetime, 
	...
	@jurnalProprietate varchar(3), 	@NrAvizeUnitar int, @NumarDocPrimit int, @gestiune char(9), @gestiune_primitoare varchar(40), @cod char(20), @cantitate decimal(17,5), @cod_intrare char(20), 
...
set @cod_intrare=ISNULL(@parXML.value('(/row/row/@codintrare)[1]', 'char(20)'), '')
...
begin
	...
	begin
		--tratare sold si valoare tva
		select @pret_valuta=ff.sold, @suma_TVA=convert(decimal(15,2),ff.sold*(ff.TVA_11+ff.TVA_22)/(ff.Valoare+ff.TVA_11+ff.TVA_22))
		from facturi ff 
		where ff.Factura=@cod_intrare and ff.Tert=@tert and (ff.Tip=0x54 and @tip_doc in ('RM','RS') or ff.Tip=0x46 and @tip_doc in ('AP','AS'))

		set @pret_valuta=@pret_valuta-@suma_TVA
...
if @tip_doc in ('RM', 'RS', 'RC') and not exists (select 1 from doc where Subunitate=@sub and tip=@tip and cod_tert=@tert and factura=@factura and (numar=@numar and data=@data)) 
	begin
		set @rec_factura_existenta=(select max(numar) from doc where Subunitate=@sub and tip=@tip and cod_tert=@tert and factura=@factura and not (numar=@numar and data=@data)) 
...
begin
	declare @data_facturii datetime, @data_scadentei datetime, @lmprim char(9), 
		...
		@barcod char(100), @cont_corespondent varchar(40), @DVI char(25), @categ_pret int, @cont_intermediar varchar(40), @cont_venituri varchar(40), @TVAnx float, 
	...
	begin try
		...
		select 
			...
			(
				...
				cod_intrare char(20) '@codintrare',	
			...
			begin
				...
				begin
					...
					begin
						...
						if nullif(@cont_factura,'') is null	--Daca cont factura este completat, acesta este mai tare
		...
		begin
			...
			update doc set 
				detalii=(case when @detalii_antet IS NOT NULL then @detalii_antet else detalii end),
				idplaja=(case when @idPlajaPrimit IS NOT NULL then @idPlajaPrimit else idplaja end)
