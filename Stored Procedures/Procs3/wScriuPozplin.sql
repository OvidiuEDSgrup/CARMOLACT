...
select	@fara_luare_date=ISNULL(@parXML.value('(//@fara_luare_date)[1]', 'char(1)'), '0'),
		...
		@jurnal=ISNULL(@parXML.value('(/row/row/@jurnal)[1]', 'varchar(20)'), ISNULL(@parXML.value('(/row/@jurnal)[1]', 'varchar(20)'), '')),
		@valuta=ISNULL(@parXML.value('(/row/@valuta)[1]', 'varchar(3)'), ''),
		@curs=ISNULL(nullif(@parXML.value('(/row/row/@curs)[1]', 'float'),0),ISNULL(@parXML.value('(/row/@curs)[1]', 'float'), 0)),
...
select @plata_incasare=
	(case 
		when @tip in ('RE','RV') and (@subtip='PN' or @subtip='PV' or @subtip='PX'/*PX->subtip de plata avans*/) 
			or @subtip='PL'/*PL->subtip de plata factura in alta valuta decat valuta facturii*/ then 'PF'
		when @tip in ('RE','RV') and (@subtip='IX'/*IX->subtip de incasare avans*/ 
			or @subtip='IL'/*IL->subtip de incasare factura in alta valuta decat valuta facturii*/) then 'IB'
		when @tip in ('RE','RV') and @subtip in ('PA', 'PE') then 'PD'
		when @tip in ('RE','RV') and @subtip in ('IA', 'IE') then 'ID'
...
if @tip in ('RE','EF','DR','DE','RV') and @subtip not in ('PX','IX')/*pentru avansuri nu apelam macheta de incasari/plati facturi selectv*/ 
...
begin 
	...
	--jurnal
	if @parXML.value('(/row/@jurnal)[1]', 'varchar(20)') is not null                          
		set @parXML.modify('replace value of (/row/@jurnal)[1] with sql:variable("@jurnal")') 
	else
		set @parXML.modify ('insert attribute jurnal {sql:variable("@jurnal")} into (/row)[1]') 	
	...
	set @docXMLIaPozplin = (
	select
			rtrim(@sub) subunitate, rtrim(@tip) tip, rtrim(@cont) cont, convert(char(10), @data, 101) data, @jurnal jurnal, 
			(case when @tip='EF' then rtrim(@tert) end) tert, (case when @tip='EF' then rtrim(LEFT(@plata_incasare,1)) end) tipefect,
			(case when @tip='EF' then rtrim(@efect_antet) end) efect	
	for xml raw, type)
...
begin
	declare 
		@tipptIa varchar(2),@dataPtIa datetime, @contPtIa varchar(40), @marcaPtIa varchar(6), @decontPtIa varchar(40), @tertPtIa varchar(13), @efectPtIa varchar(20), @tipefectPtIa varchar(2),
		@jurnalPtIa varchar(20)
	select 
		@tipptIa=@parXML.value('(/row/@tip)[1]', 'varchar(2)'),
		...
		@jurnalPtIa=@parXML.value('(/row/@jurnal)[1]', 'varchar(20)'),
		@tipefectPtIa=LEFT(@parXML.value('(/row/row/@subtip)[1]', 'varchar(20)'),1)

	set @docXMLIaPozplin = (select
		rtrim(@sub) subunitate, rtrim(@tipptIa) tip, rtrim(@contPtIa) cont, convert(char(10), @dataPtIa, 101) data, @jurnalPtIa jurnal, 
		(case when @tipptIa ='DE' then rtrim(@marcaPtIa) end) marca, (case when @tipptIa ='DE' then rtrim(@decontPtIa) end) decont,
		(case when @tipptIa='EF' then rtrim(@tertPtIa) end) tert, (case when @tipptIa='EF' then rtrim(@tipefectPtIa) end) tipefect,
		(case when @tipptIa='EF' then rtrim(@efectPtIa) end) efect	
	 for xml raw)
