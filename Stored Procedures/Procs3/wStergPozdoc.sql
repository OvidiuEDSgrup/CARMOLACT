...
declare @iDoc int, @eroare xml ,@serie varchar(20),@prop1 varchar(20),@prop2 varchar(20),@cod varchar(20),@tip varchar(2),@update bit,@subtip varchar(2),
		...
		@userAsis varchar(13),@tert varchar(13), @tip_doc varchar(2), @idpromotie int, @multiselect int, @root varchar(500), @cautare varchar(100)


begin try
begin transaction
	set @multiselect=0
	if isnull(@parXML.value('count(/row/@idpozdoc)','int'),0)=0
	begin
		set @root = '/row/row'
	...
	if @multiselect=0 and exists (select 1 from sysobjects where [type]='P' and [name]='wStergMFdinCG')
	Begin
	...
	select
		 ...
		 @cautare=ISNULL(@parXML.value('(//@_cautare)[1]', 'varchar(200)'), '')
	...
	IF @multiselect=0 and @idPromotie IS NOT NULL
	begin
	...
	IF @multiselect=0 and EXISTS(SELECT name FROM sys.tables WHERE name = 'penalizarifact') and @tip='AS'--daca exista tabela specifica de penalitai si dobanzi
		if exists (select 1 from sysobjects where [type]='P' and [name]='wUpdateStergerePenDob')
		begin
			exec wUpdateStergerePenDob @sesiune,@parXML
		end
		
	--if @subtip='SE' --suntem pe linie de serie - nu mai tratam nimic...
	--begin
    ...
    --begin	
	--if @multiselect=0 and (select UM_2  from nomencl where cod=@cod)='Y' and @serie<>''--daca avem serii pe acesta pozitie stergem toate seriile
	--begin
	...
	-- de tratat legaturistornare pe multiselect!!!!!!
	
	--daca exista id pe pozdoc si exista tabela LegaturiStornare
	IF @multiselect=0 and EXISTS (	SELECT *FROM sys.objects WHERE NAME = 'LegaturiStornare' AND type = 'U'	) 
	...
	--delete from pozdoc where Subunitate=@subunitate and tip=@tip_doc and Numar=@numar and data=@data and (@tip<>'RC' or jurnal='RC') and Numar_pozitie=@numar_pozitie  and @tip not in ('RP','RZ') --stergere si pozitie din pozdoc	
	...
	delete pozdoc
		...
		OPENXML (@iDoc, @root)
			WITH
			(idpozdoc int '@idpozdoc') as dx
		where p.idpozdoc = dx.idpozdoc
	...
	if exists (select 1 from pozdoc where tip in ('RP','RZ') and Subunitate=@subunitate and Numar=@numar and data=@data) and @tip_doc in ('RM','RS')
	begin
		/**
			Daca este ultima pozitie de RM care "se sterge" stergem si toate prestarile care ar mai fi ramas
		*/
		IF NOT EXISTS (select 1 from pozdoc where Subunitate=@subunitate and tip=@tip_doc and numar=@numar AND data=@data)
	...
	SET @docXMLIaPozdoc = (select rtrim(@subunitate) subunitate, rtrim(@tip) tip, rtrim(@numar) numar, convert(char(10), @data, 101) data, @cautare _cautare for xml raw)
