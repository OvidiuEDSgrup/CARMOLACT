...
begin try
	...
	select marca,nume,codfct,lm,denlm,salinc,salor,tipsal,denfct,comanda,isnull(dataangajarii,'01/01/1901') as dataangajarii,
		...
		cnp,isnull(datanasterii,'1901-01-01') as datanasterii,isnull(o_datanasterii,'1901-01-01') as o_datanasterii,(case when (left(cnp,1)='1' or left(cnp,1)='5') then 1 else 0 end) as sex,
	...
	o_datanasterii datetime '@o_datanasterii',
	sex bit '@sex',
...
--	validare CNP
	...
	update x
	set datanasterii=(case when datanasterii='01/01/1901' or @datanasterii<>o_datanasterii then @datanasterii else datanasterii end),sex=@sex
	from #xmlpersonal x

	if exists (select 1 from #xmlpersonal where DateADD(year,15,datanasterii)>getdate()) --	zilierul nu are 15 ani
		select 'Persoana nu are 15 ani. O persoana cu varsta de pana in 15 ani nu poate fi incadrata ca zilier!' as textMesaj for xml raw, root('Mesaje')
