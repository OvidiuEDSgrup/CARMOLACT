...
as
if exists (select 1 from sysobjects where [type]='P' and [name]='wScriuPuncteLivrareSP')
begin
	exec wScriuPuncteLivrareSP @sesiune=@sesiune, @parXML=@parXML
	return
end
...
begin try
	select identity (int, 1, 1) as numar_unic, isnull(ptupdate, 0) as ptupdate, upper(tert) as tert, isnull(tert_vechi, tert) as tert_vechi, 
		...
		mijloc_transport, adresa2, telefon_fax, judet, tara, e_mail e_mail, (case when @AdrPLiv=1 then e_mail else adresa end) adresa, banca, cont_in_banca, 
		ruta, cont_client, indicator, grupa13, sold_ben, discount, zile_incasare, nr_autorizatie, detalii
	into #xmlpctliv
	...
	select @Sub, x.tert, x.punct_livrare, isnull(x.descriere, ''), isnull(x.lm, ''), isnull(x.persoana_contact, ''), 
		...
		isnull(x.grupa13, ''), isnull(x.sold_ben, 0), isnull(x.discount, 0), isnull(x.zile_incasare, 0), isnull(x.nr_autorizatie, ''), x.detalii
	from #xmlpctliv x
	where x.ptupdate=0
	...
	set tert=isnull(x.tert, i.tert), identificator=isnull(x.punct_livrare, i.identificator), 
		...
		zile_inc=isnull(x.zile_incasare, i.zile_inc), observatii=isnull(x.nr_autorizatie, i.observatii), i.detalii=x.detalii
