...
begin try
	...
	@docXMLIaSalariiZilieri xml, @mesajAvertizare varchar(1000), @dataIncAn datetime, @dataSfAn datetime
	...
	select	@datajos=dbo.Bom(@data), @datasus=dbo.Eom(@data), 
			@dataIncAn=dbo.BOY(@data), @dataSfAn=dbo.EOY(@data)

	if @update=0 and exists (select 1 from SalariiZilieri sz where sz.data=@data and sz.marca=@marca)
	begin
		set @mesajAvertizare='Zilierul introdus a fost deja adaugat pe aceasta data!' 
		select @mesajAvertizare as textMesaj for xml raw, root('Mesaje')
	end

	if @update=0 and isnull((select count(distinct data) from SalariiZilieri sz where sz.data between @dataIncAn and @dataSfAn and sz.marca=@marca),0)>90
	begin
		set @mesajAvertizare='Zilierul introdus a fost pontat in acest mai mult de 90 de zile!' 
		select @mesajAvertizare as textMesaj for xml raw, root('Mesaje')
	end
	...
	if @tipsalor='B' 
	begin
		set @venittotal=round(convert(decimal(12,2),@orelucrate * @salor),0)
		...
		if @tipsalor='N'	
		begin
			set @restdeplata=round(convert(decimal(12,2),@orelucrate * @salor),0)
