	...
	EXEMPLU APEL 
		exec yso_rapFactura @sesiune='',@tip='AP', @numar='16113355', @data='2016-11-04', @nrexemplare=1
	*/
CREATE procedure [dbo].[yso_rapFactura] (@sesiune varchar(50)=null, @tip varchar(2), @numar varchar(20), @data datetime, @nrExemplare int=1,
...
begin try
	...
	declare 
		...
		@detalii xml, @data_expedierii datetime, @ora_expedierii varchar(5), @cData_expedierii varchar(30), @data_exec datetime
	set @data_exec=getdate()
	select 
		@nrmaxim=500
	select
	...
	create table #filtre 
		(subunitate varchar(20), tip varchar(2), numar varchar(20), data datetime, utilizator varchar(50), numeutilizator varchar(50) default null, cnputilizator varchar(50) default null)
	...
	--select '2',datediff(s,GETDATE(),@data_exec)
	...
	*/
	...
	create table #DocFiltr(
		...
		tert varchar(20), factura varchar(20), data_facturii datetime, data_scadentei datetime, total float, total_tva float, total_tva_fdisc float,
		total_discount float, total_tva_nediscountat float, 
		valuta varchar(20), curs float, detalii xml, gestiune varchar(20), gestiune_primitoare varchar(20), [contract] varchar(20))	
	create table #PozDocFiltr (tip varchar(2), numar varchar(20), data datetime, tert varchar(50),
		...
		,[contract] varchar(20), numar_pozitie int)
	...
	select
		...
		sum(round(convert(decimal(18,5),p.cantitate*p.pret_valuta*(case when p.curs>0 then p.curs else 1 end)),2)
			-round(convert(decimal(18,5),p.cantitate*p.pret_vanzare),2)),
	...
	group by p.numar, p.data, p.tert, p.factura, p.cod, p.pret_vanzare, p.pret_valuta, p.Discount, p.Comanda, p.[Contract]
	...
	*/
	...
	select
		...
		round(convert(decimal(18,5),intoc.cantitate*intoc.pret_valuta*(case when intoc.curs>0 then intoc.curs else 1 end)),2), 
		avize.pret_valuta,
		intoc.curs curs ,
		...
		round(convert(decimal(18,5),intoc.cantitate*intoc.pret_valuta*(case when intoc.curs>0 then intoc.curs else 1 end)),2)
			-round(convert(decimal(18,5),intoc.cantitate*intoc.pret_vanzare),2),
		intoc.discount,
		rtrim(avize.subtip), 
	...
	insert into #DocFiltr (tip, numar, data, tert, factura, data_facturii, data_scadentei, total, total_tva, total_tva_fdisc, total_discount, total_tva_nediscountat, valuta, curs, detalii, gestiune, gestiune_primitoare
		, [contract])
	select
			p.tip,
			p.numar, 
			...
			sum(round(convert(decimal(18,5),p.cantitate*p.pret_valuta*(case when p.curs>0 then p.curs else 1 end)),2)) ,
			sum(p.tva_deductibil),
			convert(decimal(12,2),sum(p.pret_valuta*p.cantitate*p.Cota_TVA/100)),
...
--select d.factura, af.Numar_factura, af.Numele_delegatului
--from #PozDocFiltr p
...
--		and del.tert=isnull(rtrim(d.detalii.value('(/row/@tertdelegat)[1]', 'varchar(20)')), d.tert) and del.subunitate='C1'
--	LEFT JOIN anexafac af on af.subunitate=@subunitate and af.numar_factura=d.factura
	...
	select 
		...
		rtrim(it.observatii) as furnizor,
		...
		isnull(rtrim(inf.Descriere), '') as pctlivrare,
		isnull(ISNULL((select rtrim(denumire) from judete where cod_judet=inf.telefon_fax2),RTRIM(inf.telefon_fax2))
			+','+RTRIM(inf.pers_contact)+','+RTRIM(inf.e_mail), '') as adrpctlivrare,
		...
		round((case t.tert_extern when 0 then d.total else d.total/ISNULL(NULLIF(d.curs,0),1) end),2) totalfdisc,
		round((case t.tert_extern when 0 then d.total_tva_fdisc else d.total_tva_fdisc/ISNULL(NULLIF(d.curs,0),1) end),2) totaltvafdisc , 
		d.valuta,
		d.curs,
		p.cota_tva,
		...
		--> footer, delegat, samd
		isnull(convert(varchar(10), d.detalii.value('(/row/@data_expedierii)[1]', 'datetime'), 103)
			, rtrim(CONVERT(CHAR(10), af.data_expedierii, 103))) as data_expedierii,
		isnull(rtrim(del.descriere), rtrim(af.numele_delegatului)) as delegat,
		isnull(d.detalii.value('(/row/@ora_expedierii)[1]', 'varchar(6)')
			, rtrim(left(af.ora_expedierii, 2) + ':' + substring(af.ora_expedierii, 3, 2))) as ora_expedierii,
		--isnull(nullif(dbo.fStrToken(del.Buletin, 1, ','),''), rtrim(af.numar_buletin)) as ci_delegat,
		--isnull(nullif(dbo.fStrToken(del.Buletin, 2, ','),''), rtrim(af.seria_buletin)) as cis_delegat,
		rtrim(af.numar_buletin) as ci_delegat,
		rtrim(af.seria_buletin) as cis_delegat,
		isnull(rtrim(del.Eliberat), rtrim(af.eliberat)) as eliberat_delegat,
		isnull(rtrim(del.Mijloc_tp), rtrim(af.mijloc_de_transport))  as auto_delegat,
	...
	--outer apply (select top (1) cod_de_bare from codbare cb where cb.Cod_de_bare<>'' and cb.Cod_produs=p.cod) cb
	left join codbare cb on cb.Cod_de_bare<>'' and cb.Cod_produs=p.cod
	...
	Select * from #date_factura order by factura, cod, nr, nrpoz
