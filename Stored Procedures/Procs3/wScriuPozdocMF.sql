...
declare @sub char(9),@datal datetime, @tip char(2),@subtip char(2),@numar char(8),@data datetime,
	...
	@valinv float, @valam float, @valamcls8 float, @valamneded float, @valamist float, 
...
--	La reevaluari sumatva contine Dif.am.cls.8. Sa ramina cea introdusa in macheta.
	(case when isnull(sumatva,0)=0 or isnull(pretvaluta,0)<>0 and isnull(pretvaluta,0)<>isnull(o_pretvaluta,0) and subtip<>'VI' and subtip<>'RE'	
...
--	Lucian: la documente cu pret in valuta tratat sa recalculeze valoarea de inventar functie de pret valuta*curs si la modificare pozitie
	...
	while @fetchstatus = 0
	begin
		set @sumatvam=(case when @tiptva in (4,5) then @sumatva-round(@sumatva/2,2) else @sumatva end)
...
*/		SELECT @ctamm=md.cod_de_clasificare	--	aceasta variabila este nefolosita
		...
		if @tip='MT' and (@contcor='' or abs(@valinv-@valam)<0.01 and @contcor not like '482%')
			set @farapozdoc=1
