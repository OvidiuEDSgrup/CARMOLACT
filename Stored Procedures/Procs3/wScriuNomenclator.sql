...
begin try
	...
	Declare @update bit, @cod varchar(20), @grupa varchar(13), @denumire varchar(80), @note varchar(80), @um varchar(3),@cont varchar(40), @cotatva float, @pretvanznom float,
		@codbare varchar(100),@areserii int,@o_areserii int,@Serii int,@pret_stocn float,@observatii varchar(21),@stocmin decimal(12,3),@detalii xml,
		@docDetalii xml, @greutate float,@categorie int,@stocmax decimal(12,3),@tip varchar(1),@um_1 varchar(3),@valuta varchar(3),
		@o_codbare varchar(100), @o_stocmin decimal(12,3), @o_stocmax decimal(12,3), @coeficient_conversie_1 float
--		,@codcpv int
		, @cotaTvaImplicita float, @gestiune varchar(20)

	exec luare_date_par 'GE','COTATVA',0, @cotaTvaImplicita output, ''
	...
	Set @cotatva = isnull(@parXML.value('(/row/@cotatva)[1]','float'),@cotaTvaImplicita)
	...
	Set @codbare = upper(@parXML.value('(/row/@codbare)[1]','varchar(100)'))
	...
	Set @o_codbare = @parXML.value('(/row/@o_codbare)[1]','varchar(100)')
	Set @coeficient_conversie_1 = isnull(@parXML.value('(/row/@coeficient_conversie_1)[1]','float'),0)
	SET @gestiune = upper(isnull(@parXML.value('(/row/@gestiune)[1]','varchar(20)'),''))
	...
	begin  
		update nomencl set Tip = @tip, Grupa=@grupa, Denumire=@denumire, Loc_de_munca=@note, UM=@um, Cont=@cont, UM_1=@um_1, coeficient_conversie_1=@coeficient_conversie_1,
				...
				Greutate_specifica=@greutate, Categorie=@categorie, gestiune=@gestiune
	...
	begin    
		...
		values (@cod_par, @tip, @grupa, @denumire, @um,@um_1,@coeficient_conversie_1,case when @areserii=1 and @Serii=1 then 'Y' else '' end,
			0,@cont,@valuta,0,convert(decimal(17,5),@pret_stocn),@pretvanznom,0,@cotatva,0,0,@greutate,'',@note,@gestiune,@categorie,@observatii)  

		select 'S-a adaugat codul nou '+rtrim(@cod_par)+' - '+rtrim(@denumire)+' !' as textMesaj, 'Finalizare adaugare' as titluMesaj for xml raw, root('Mesaje')
