...
CREATE PROCEDURE yso_wOPImprimFactContractePrePopulare @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	DECLARE @tert VARCHAR(20), @lm VARCHAR(20), @gestiune VARCHAR(20), @grupa VARCHAR(20), @mesaj VARCHAR(400), 
		...
		@utilizator varchar(50), @idContractFiltrat int, @factura varchar(20)
	...
	(
		select convert(char(10), @dataJos, 101) as datajos, convert(char(10), @dataSus, 101) as datasus,
			@valuta as valuta, @tipContract as tip, 
			@factura as factura, @idContractFiltrat as idContract, 
			@tert as tert, @punct_livrare as punct_livrare,
			@lm as lm, @gestiune as gestiune
		for xml raw --,root('row')
	)
	
	SELECT 'Operatie pentru listare facturi contracte'  nume, 'CL' codmeniu, 'D' tipmacheta,'CL' tip,'LS' subtip,'O' fel,
	 (SELECT @dateInitializare ) dateInitializare FOR XML RAW('deschideMacheta'), ROOT('Mesaje')

END TRY
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (yso_wOPImprimFactContractePrePopulare)'
