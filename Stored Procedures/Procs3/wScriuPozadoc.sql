...
declare @tip_antet char(2), @numar char(8), @data datetime, @tert char(13), @factura_antet char(13), 
	...
	@detaliipoz xml, @idPlajaPrimit int
BEGIN TRY
	...
	if @tip in ('CO') and @ptupdate<>1 and @apelDinProcedura=0 and @factura_stinga='' and @factura_dreapta=''
	begin 
		delete from SelectieFacturiPtCompensari where utilizator=@utilizator
		...
		(
			select 'PF' as tipOperatiune, 'AD' as tipdoc,RTRIM(@tert) as tert, convert(char(10), @data, 101) as data, RTRIM(@lm) as lm, 
		...
		set @docXMLIaPozadoc='<row subunitate="'+rtrim(@sub)+'" tip="CO" numar="'+rtrim(@numar)+'" data="'+convert(char(10), @data, 101)+'" />'
		
		select @docXMLIaPozadoc for xml raw('dateAntet'), root('Mesaje')

		SELECT 'Compensare facturi: Selectie facturi furnizor'  nume, 'PI' codmeniu, 'D' tipmacheta, 'RE' tip,'PI' subtip, 'O' fel,
	...
	isnull(numar_pozitie, 0) as numar_pozitie, isnull(jurnal, '') as jurnal, isnull(data_facturii, isnull(data, '01/01/1901')) as data_facturii, 
	isnull(data_scadentei, isnull(data_scad_ant,'01/01/1901')) as data_scadentei,diftva,o_diftva,
	...
	(
		...
		data_scad_ant char(13) '../@datascadentei', 
	...
	begin
		...
		begin
				...
				exec wIauNrDocFiscale @parXML=@fXML, @Numar=@NrDocFisc output, @NrDoc=@NrDocFiscPrimit output, @idPlaja=@idPlajaPrimit output
		...
		set @parXmlpozadoc.modify ('insert 
					...
					attribute idplaja {sql:variable("@idPlajaPrimit")}
					)					
					into (/row)[1]')		
