...
begin try
	...
	select p.idPozDoc,p.subunitate,p.numar,p.data, p.tip, p.numar_DVI, 
		...
		p.Pret_vanzare,p.pret_de_stoc,p.Pret_valuta,p.Discount,p.Valuta,p.Curs,
		(case when n.Greutate_specifica=0 and n.um in ('T', 'TO') then 1000 when n.Greutate_specifica=0 and n.um='KG' then 1 else n.Greutate_specifica end) AS Greutate_specifica,
		p.numar_pozitie,p.cantitate,p.cod,
	...
	/** SP complementar pentru modificari tabel temporar si posibilitatea de a repartiza pe cantitati negative;
		altfel, procedura merge ca inainte. **/
	IF EXISTS (SELECT 1 FROM sys.sysobjects WHERE name = 'repartizarePrestariServiciiSP')
		EXEC repartizarePrestariServiciiSP @tip = @tip, @numar = @numar, @data = @data
	ELSE
		DELETE FROM #pozitiiRM
		WHERE (cantitate * Pret_valuta * (1 + Discount/100) * (CASE WHEN ISNULL(Valuta, '') <> '' THEN Curs ELSE 1 END)) < 0
	...
	from #calcul_prestari c
		outer apply (
			select sum(p.Cantitate*p.pret_stoc_anteriorDVIcuPrestCoduri) as valRMFaraTaxe,
				sum(p.Cantitate * p.Greutate_specifica) as greutateRM,
