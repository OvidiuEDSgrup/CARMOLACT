...
*/
declare @cGest varchar(20), @cCod varchar(20), @cCodI varchar(20), @nPret float,@cContDeStoc varchar(40), @nPretAm float, @tipGestiuneIntrare char(1), @binar varbinary(128)
...
set CONTEXT_INFO @binar -- pentru cazurile documentelor definitive, prin operatia de propagare are sens sa se modifice aceste documente in lant.
...
--Conditionat update-ul de existenta diferentelor pe pozdoc. Asa pare ca merge mai rapid daca sunt multe pozitii de inlocuit.
if exists (select 1 from pozdoc p where p.Subunitate=@subunitate and p.Gestiune=@cGest and cod=@cCod and Cod_intrare=@cCodI
	and (p.Tip_miscare='E' or p.Tip_miscare='I' and p.cantitate<0 or p.tip='AI') and p.Cont_de_stoc!=@cContDeStoc)
begin
	update p
	...
	where p.Subunitate=@subunitate and p.Gestiune=@cGest 
		...
		and (p.Tip_miscare='E' or p.Tip_miscare='I' and p.cantitate<0 or p.tip='AI')
		and p.Cont_de_stoc!=@cContDeStoc
end

/*TI-uri*/
--Conditionat update-ul de existenta diferentelor pe pozdoc. Asa pare ca merge mai rapid daca sunt multe pozitii de inlocuit.
if exists (select 1 from pozdoc p where	p.Subunitate=@subunitate and Gestiune_primitoare=@cGest and p.tip='TE'
		and cod=@cCod and (case when grupa<>'' then grupa else Cod_intrare end)=@cCodI and p.Tip_miscare='E'
...
if exists (select 1 from pozdoc p
	...
	left outer join gestiuni g on p.tip='TE' and g.subunitate=p.subunitate and g.cod_gestiune=p.gestiune_primitoare
	where p.subunitate=@subunitate and p.cod=@cCod and (p.tip_miscare='E' or p.tip='AI')
...
begin
	...
	left outer join gestiuni g on p.tip='TE' and g.subunitate=p.subunitate and g.cod_gestiune=p.gestiune_primitoare
	where p.subunitate=@subunitate and p.cod=@cCod
	and (p.tip_miscare='E' or p.tip='AI')
	and (p.pret_de_stoc<>@nPret /*or (case when tip='TE' then p.Pret_amanunt_predator else p.Pret_cu_amanuntul end)<>@nPretAm*/)
end
