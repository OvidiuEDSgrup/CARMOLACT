...
declare @Sub varchar(20),@userASiS varchar(100),@lista_lm int, @rulajePeLocm int, @lista_jurnale bit
...
left join PropUtiliz pu on pu.utilizator=@userASIS and pu.proprietate='JURNAL' and pu.valoare=pd.jurnal
...
where pd.subunitate=@Sub and pd.data between c.dataInc and c.datamax 
	...
	and (@lista_jurnale = 0 or pu.valoare is not null)
...
left join PropUtiliz pu on pu.utilizator=@userASIS and pu.proprietate='JURNAL' and pu.valoare=pc.jurnal
...
--=====Insereaza la calcul acele perechi (cont, valuta, data) care au rulaj in lei dar nu si in valuta, sau invers
--Ex. Conturile in valuta din ziua in care au miscare doar pe lei - fara acest insert nu se aduc soldurile in valuta pe zilele cand sunt doar miscari pe lei
insert into  #calcule (cont, valuta, data)
select ct.cont, ct.valuta, ct.data 
from #conturi ct
where not exists (select * from #calcule cl where ct.cont=cl.cont and ct.valuta=cl.valuta and ct.data=cl.data)
--====================================
