...
/*Glich destul de rau pentru schimbare cota TVA*/
if @cotaTVA=20 and @dataSus<'01/01/2016'
	set @cotaTVA=24
...
select tip,tert,factura,cota_tva,sum(tvad) as tvad,sum(valoare) as valoare, eCuTLI 
	...
	(select (case when p.tip in ('AP','AS') then 0x46 else 0x54 end) as tip,p.tert,p.Factura,p.cota_tva,tva_deductibil as tvad,
		round(p.cantitate*(case when p.tip in ('AP','AS') then p.Pret_vanzare -- avize
			...
			+(case when p.procent_vama=0 or p.tip in ('RM','RS','RP') and p.procent_vama>=2 then p.TVA_deductibil else 0 end) as valoare, 
	...
	union all
	select 0x54,p.tert_vama,p.Factura_vama,@cotaTVA,@cotaTVA as tvad,@cotaTVA as valoare, 1 as eCuTLI 
	from #FacturiDeTratatGrupate ft
	inner join DVI p on p.Subunitate=@cSub and p.tert_vama=ft.tert and p.Factura_vama=ft.factura
	where ft.tipf=0x54 
	union all
