...
begin
	...
	@DateSalGrpPeLMNivel1 int, @NivelGrupare int
	...
	left(rtrim(isnull(i.Loc_de_munca,'')),@NivelGrupare) as lmantet, rtrim(max(isnull(lm.denumire,''))) as denlmantet, count(a.Marca) as nrsal, 
	...
	group by a.data, left(rtrim(isnull(i.loc_de_munca,'')),@NivelGrupare)
	union all
	...
	left(i.loc_de_munca,@NivelGrupare) as lmantet, rtrim(max(isnull(lm.denumire,''))) as denlmantet,
	...
	from istpers as i
		left outer join lm on left(i.loc_de_munca,@NivelGrupare)=lm.cod
	...
	where i.data=@datasus and (@data is null or i.data=@data) and 
		...
		and (dbo.f_areLMFiltru(@userASiS)=0 or lu.cod is not null)
	group by i.data, left(i.loc_de_munca,@NivelGrupare)
