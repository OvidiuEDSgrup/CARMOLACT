--***
create procedure rapBalantaContabilaLocm 
/**	Procedura folosita de rapoartele web "Balanta contabila" si "Balanta contabila pe locuri de munca"
*/
		@sesiune varchar(50)=null, @ContJos varchar(40), @ContSus varchar(40), @pLuna int, @pAn int,
		...
		@conturiRecompuse int=0	--> 0= "Plan de conturi": asa cum este in tabela conturi
								-->	1= "Recompunere bugetari": se recompun si se regrupeaza dupa tabela de reguli - max. 22 caractere 
								-->	2= "Recompunere bugetari pe terti": se recompun si se regrupeaza dupa tabela de reguli; in plus, se recompun conturile pe terti folosind pFacturi
...
begin try
	...
	*/
	...
	create table #datebaza(		--> tabela care contine toate sumele ce trebuie pe mai departe
		...
		loc_de_munca varchar(20), data datetime, valuta varchar(20), indbug varchar(100), marcat bit
	)
	 ...
	 Begin
	  ...
	  Begin
	   ...
	   insert into #datebaza(tip_suma, subunitate, cont, rulaj_debit, rulaj_credit, loc_de_munca, data, valuta, indbug)
		...
		if @conturiRecompuse>0 and OBJECT_ID('rapBalantaContabilaRecompunereBugetari') is not null
		begin
			declare @xmlR xml
			set @xmlR = (select @ContJos ContJos, @ContSus ContSus, @p_dData_sf_luna p_dData_sf_luna, 
								@direct direct, @conturiRecompuse conturiRecompuse, @indicator indicator, @peIndbug peIndbug, @cLM lm for xml raw)
			exec rapBalantaContabilaRecompunereBugetari @parXML=@xmlR
		end
