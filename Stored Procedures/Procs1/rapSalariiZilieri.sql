...
Create procedure rapSalariiZilieri
	@dataJos datetime, @dataSus datetime, @marca varchar(6)=null, @locm char(9)=null, @strict int=0, @centralizare char(1)='', @ordonare int=0, @alfabetic int=1
as
begin
	...
	max(rtrim(z.Nume)) as Nume, max(z.Cod_functie) as Cod_functie, max(f.Denumire) as DenFunctie, rtrim(max(z.Cod_numeric_personal)) as cnp, 
