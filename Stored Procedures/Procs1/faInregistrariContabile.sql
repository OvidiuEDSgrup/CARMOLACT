...
begin 
	...
	*/
	declare 
		...
		,@tabela int -- 0=toate, 1=pozdoc, 2=pozplin, 3=pozadoc, 4=pozncon, 5=alop
	set @tabela=ISNULL(@parXML.value('(/row/@tabela)[1]','int'), 0)
	...
	*/
	...
	begin try -- daca da eroare la generarea rapida sa stiu ce eroare 
		if @compUrmaPozincon=0 and exists(select 1 from sys.objects where name='InregDoc' and type='P') and @tabela in (0,1)
			exec InregDoc '',''

		if @compUrmaPozincon=0 and exists(select 1 from sys.objects where name='InregPlin' and type='P') and @tabela in (0,2)
			exec InregPlin '',''

		if @compUrmaPozincon=0 and exists(select 1 from sys.objects where name='InregADoc' and type='P') and @tabela in (0,3)
			exec InregADoc '',''

		if @compUrmaPozincon=0 and exists(select 1 from sys.objects where name='inregNCon' and type='P') and @tabela in (0,4)
			exec inregNCon '',''
		
		if exists (select 1 from sys.objects where name='inregALOP' and type='P') and exists(select 1 from sys.objects where name='ALOP' and type='U') and @tabela in (0,5)
			exec inregALOP '',''
	end try
	BEGIN catch
	...
	begin
		...
		begin
			begin try
				...
				--if @tip in ('IC','MA','ME','MI','MM','NC','DP','AO','AT','PS','UA') exec inregNC @dataj=@data, @datas=@data, @nrdoc=@numar, @tipdoc=@tip
