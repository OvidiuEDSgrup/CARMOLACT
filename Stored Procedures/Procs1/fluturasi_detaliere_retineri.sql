...
Begin
	...
	/*	Apelaz procedura care verifica daca exista un calcul de lichidare in derulare. Sa nu se poata rula raportul daca exista calcul intrucat s-ar afisa rezultate incompletete. */
	declare @parXML xml, @locm varchar(9), @multiFirma int, @DataImpTicJ datetime, @DataImpTicS datetime
...
--	daca tabela par este view inseamna ca se lucreaza cu parametrii pe locuri de munca (in aceeasi BD sunt mai multe unitati)	
	...
	set @locm=(case when @multiFirma=1 and @Filtru_locm=1 then @locm_jos end)
	set @parXML=(select convert(char(10),@dataSus,101) as datal, rtrim(@locm) as locm, 'RS' as tip, rtrim(OBJECT_NAME(@@PROCID)) as obiectSQL, 1 as verificare for xml raw)
	...
	exec pTichete @dataJos=@DataImpTicJ, @dataSus=@DataImpTicS, @marca=@Marca_jos, @DeLaCalculLichidare=1
	...
	from brut a
		...
		left outer join infopers ip on ip.Marca=c.marca
		, istpers i
	where a.marca=b.marca and a.data=c.data and a.marca=c.marca and a.Data=i.Data and a.Marca=i.Marca
		...
		and (@Rest_plata_poz=0 or c.rest_de_plata>0) and (@Un_tip_stat=0 or isnull(b.tip_stat,ip.Religia)=@Tip_stat) 
...
--	apelare procedura specifica pentru alterare tabela tmpFluturi.
	if exists (select * from sys.objects WHERE object_id = OBJECT_ID(N'fluturasi_detaliere_retineriSP1') and type='P')
		exec fluturasi_detaliere_retineriSP1 
			@Host_ID=@Host_ID, @dataJos=@dataJos, @dataSus=@dataSus, @Ordonare=@Ordonare, 
			@Filtru_marci=@Filtru_marci, @Marca_jos=@Marca_jos, @Marca_sus=@Marca_sus, @Filtru_locm=@Filtru_locm, @Locm_jos=@Locm_jos, @Locm_sus=@Locm_sus, 
			@Fitru_functie=@Fitru_functie, @Functia=@Functia, @lGrupa_de_munca=@lGrupa_de_munca, @cGrupa_de_munca=@cGrupa_de_munca, @lGrupa_de_munca_exceptata=@lGrupa_de_munca_exceptata, 
			@Salarii_conducere=@Salarii_conducere, @Lista_conducere=@Lista_conducere, @Filtru_mandatar=@Filtru_mandatar, @Mandatar=@Mandatar, @Rest_plata_poz=@Rest_plata_poz,  
			@Un_tip_stat=@Un_tip_stat, @Tip_stat=@Tip_stat, @Un_sir_de_marci=@Un_sir_de_marci, @Sir_marci=@Sir_marci, @Un_card=@Un_card, @Card=@Card, @Det_comp_saln=@Det_comp_saln, 
			@ltip_salarizare=@ltip_salarizare, @ctip_sal=@ctip_sal
