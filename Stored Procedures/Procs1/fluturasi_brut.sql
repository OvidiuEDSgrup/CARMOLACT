...
Begin
	...
	if object_id('tempdb..#pontajFBrut') is not null 
		drop table #pontajFBrut

	select p.marca, max(p.tip_salarizare) as tip_salarizare, max(case when convert(char(1),p.loc_munca_pentru_stat_de_plata)='1' then p.Coeficient_acord else 0 end) as Coeficient_acord
	into #pontajFBrut
	from pontaj p
		inner join tmpfluturi t on t.Host_ID=@Host_ID and t.marca=p.marca and t.Loc_de_munca=p.Loc_de_munca
	...
	select a.Marca,max(p.nume),max(i.cod_functie),max(i.loc_de_munca),max(i.tip_salarizare),max(i.salar_de_incadrare),max(n.loc_de_munca),max(a.loc_de_munca),max(isnull(pr.Valoare,'')),
		...
		sum(a.Ore_delegatie),sum(a.Realizat_delegatie),isnull(max(pt.tip_salarizare),'') as tip_salarizare, isnull(max(pt.Coeficient_acord),0) as Coeficient_acord
	from tmpfluturi a
		...
		left outer join #pontajFBrut pt on pt.marca=a.marca
	where a.Host_ID=@Host_ID
	group by a.data,a.marca
	order by (case when @SupervisorPeMarca=1 then max(isnull(pr.Valoare,'')) else '' end), 
		(case when @Ordonare='Nume' then max(i.nume)+a.marca when @Ordonare='Loc de munca; Nume' 
