...
Begin try 
	...
	@Dafora int,@Colas int,@Pasmatex int,@Drumor int,@Plastidr int, @lmCorectie varchar(9), @nrLMFiltru int, @CursEUROPensiiF decimal(10,4)

 	set @utilizator = dbo.fIaUtilizator(null)
	select @nrLMFiltru=count(1) from LMfiltrare where utilizator=@utilizator and cod in (select cod from lm where Nivel=1)
...
--	citire parametrii
	...
	where Tip_parametru='PS' and Parametru in ('SIND%','REGCOMAN','COLAS','OINTNRN','LOCMSALAR',
			...
			'CHINDPON','CHINDLCRT','CHINDVEN','BAZANEROT','COMPSALN','SALNETV','SALNPO-CM','CIMPZNEG','TICHETE','NUDPSH','ADVNET-Q','DIUNEIMP')
...
--	citire parametrii lunari
	select @Salar_minim=max(case when Parametru='S-MIN-BR' then Val_numerica else 0 end),
		...
		@CursEUROPensiiF=max(case when Parametru='CURSEURPF' then Val_numerica else 0 end)
	from par_lunari
	where Data=@dataSus and tip='PS' 
		and Parametru in ('S-MIN-BR','SALMBRUT','SOMAJ-ISR','ORE_LUNA','CASINDIV','CASSIND','SOMAJIND',
			'CASGRUPA3','CASGRUPA2','CASGRUPA1','COTACCI','CASSUNIT','3.5%SOMAJ','FONDGAR','0.5%ACCM','1%-CAMERA',
			'STOUG28','DJIMPZTIC','DSIMPZTIC','CURSEURPF')
	...
	@OUG13 int,@OUG6 int,@FaraAltVenitPtCASS int,@uMarca2CNP int,@uMarca2CNPCM int,@CNP char(13),
	@Pensmax_ded_legala float,@Pensmax_ded float,@Pensded_lun float,@Pensded_ant float,@Pensluna float,@SalComp float,@AlocHrana float,
	...
	@PensieFUnitate float, @Part_profit float, @SomajTehnicSusp float, @OrdinePlafonareCAS float, @OreCMSubvSomaj int, @ExpatriatCuA1 int, @DataNasterii datetime
	...
	if object_id('tempdb..#personalCVN') is not null drop table #personalCVN
	if OBJECT_ID('tempdb..#ptichete') is not null drop table #ptichete
	if OBJECT_ID('tempdb..#PensiiFAnt') is not null drop table #PensiiFAnt
	if OBJECT_ID('tempdb..#PensiiFLuna') is not null drop table #PensiiFLuna
	if OBJECT_ID('tempdb..#nrPersintr') is not null drop table #nrPersintr
...
--	pun datele din personal in tabela temporara ca sa nu fac apel la prefiltare din LMFiltrare in fiecare select din cursorul de mai jos (necesar pentru multifirma)
	select p.*, convert(char(1),isnull(p.detalii.value('(/row/@functpublic)[1]','int'),convert(int,ip.Actionar))) as PersNecontractual 
	into #personalCVN
	from personal p 
...
--	pun datele din brut in tabela temporara ca sa nu fac apel la prefiltare din LMFiltrare in fiecare select din cursorul de mai jos (necesar pentru multifirma)
	...
	set @Pensmax_ded_legala=400	--	Pensie maxima deductibila 400 EURO/an
--	calcul pensii deductibile anterior.
	select n.marca, sum(convert(decimal(12,2),n.ded_baza/(case when @dataSus>='01/01/2016' and pl.val_numerica is not null then pl.val_numerica else 1 end))) as Pensded_ant
	into #PensiiFAnt
	from net n 
		inner join #personalCVN p on p.marca=n.marca
		left outer join par_lunari pl on pl.data=dbo.EOM(n.data) and pl.tip='PS' and pl.Parametru='CURSEURPF' and pl.val_numerica<>0
	where n.data between @Data1_an and @dataSus_ant and day(n.data)=1
		and exists (select 1 from brut b where b.data=@dataSus and b.Marca=n.Marca)
	group by n.marca

--	pensii facultative in luna curenta (sa nu fie facut subselect in selectul pe tabela brut).
	select r.marca, sum(r.Retinere_progr_la_avans+r.Retinere_progr_la_lichidare) as PensieLuna
	into #PensiiFLuna
	from resal r 
		inner join #personalCVN p on p.marca=r.marca
	where r.data between @dataJos and @dataSus 
		and r.cod_beneficiar in (select cod_beneficiar from benret where @Subtipret=0 and tip_retinere='5' 
			or @Subtipret=1 and tip_retinere in (select subtip from tipret where tip_retinere='5'))
	group by r.marca

--	numarare persoane in intretinere
	select s.marca, count(1) as nrpersintr
	into #nrPersintr
	from persintr s 
		inner join #personalCVN p on p.marca=s.marca
	where s.data between @dataJos and @dataSus and s.coef_ded<>0
	group by s.marca

	create table #ptichete (data datetime)
	exec CreeazaDiezSalarii @numeTabela='#ptichete'
	exec pTichete @dataJos=@DataImpTicJ, @dataSus=@DataImpTicS, @marca=@marcaJos, @DeLaCalculLichidare=1
...
--	cursor
	...
	max(isnull(e.data_inf,'01/01/1901')), maX(isnull(npi.nrPersintr,0)) as nrPersintr, max(p.cod_numeric_personal) as CNP, max(p.PersNecontractual) as PersNecontractual, 
...
--	cursor 1
	...
	isnull((select count(1) from #brut b1 
		...
		where max(p.Tip_colab) not in ('DAC','CCC','ECT') and b1.data=@dataSus 
			...
			and exists (select p1.marca from #personalCVN p1 where p1.Marca=b1.marca and p1.cod_numeric_personal=max(p.cod_numeric_personal) and (p1.loc_ramas_vacant=0 or p1.Data_plec>@dataJos) 
	...
	/*and (select count(1) from #brut b2 where b2.data=@dataSus and b2.marca>b.marca 
		and exists (select p2.marca from #personalCVN p2 where p2.Marca=b2.marca and p2.cod_numeric_personal=max(p.cod_numeric_personal) and (p2.loc_ramas_vacant=0 or p2.Data_plec>@dataJos)))=0*/),0),
	isnull((select count(1) from #brut b1 where b1.data=@dataSus and b1.marca<b.marca /* am inlocuit in conditia de mai inainte <> cu < */
		and exists (select p1.marca from #personalCVN p1 where p1.Marca=b1.marca and p1.cod_numeric_personal=max(p.cod_numeric_personal) and p1.Grupa_de_munca in ('N','D','S','C','P'))),0) as uMarca2CNPCM,
	max(convert(float,isnull(nullif(e1.val_inf,''),@Pensmax_ded_legala))) as Pensie_max_ded, max(isnull(e1.procent,0)) as Pensded_lun, max(isnull(pfa.Pensded_ant,0)) as Pensded_ant, max(isnull(pfl.PensieLuna,0)) as PensieLuna,
	...
	(case when year(@dataSus)<=2010 
		then isnull((select count(1) from #brut b1 where max(p.Tip_colab) not in ('DAC','CCC','ECT') and max(p.Grupa_de_munca)<>'C' and max(p.Somaj_1)<>0 and b1.data=@dataSus 
			and b1.marca<b.marca and exists (select p1.marca from #personalCVN p1 where p1.Marca=b1.marca and p1.cod_numeric_personal=max(p.cod_numeric_personal) 
			...
			and (select count(1) from #brut b2 where b2.data=@dataSus and b2.marca>b.marca 
			and exists (select p2.marca from #personalCVN p2 where p2.Marca=b2.marca and p2.cod_numeric_personal=max(p.cod_numeric_personal) and p2.Somaj_1<>0 and p2.Grupa_de_munca<>'C' 
		...
		where max(p.Somaj_1)<>0 and b1.data=@dataSus 
			and (isnull(op1.ordineCAS,0)<max(isnull(op.ordineCAS,0)) or isnull(op1.ordineCAS,0)=max(isnull(op.OrdineCAS,0)) and b1.marca<b.marca)
			and exists (select p1.marca from #personalCVN p1 left outer join #tipAsigurat a2 on b1.data=a2.data and b1.marca=a2.marca 
		...
		and (select count(1) from #brut b2 
			...
			where b2.data=@dataSus 
				...
				and exists (select p2.marca from #personalCVN p2 left outer join #tipAsigurat a3 on b2.data=a3.data and b2.marca=a3.marca 
	...
	max(isnull(op.ordineCAS,0)) as OrdinePlafonareCAS, isnull(max(m.zcm_subv_somaj),0)*max(b.spor_cond_10) as OreCMSubvSomaj, max(p.Data_nasterii) as DataNasterii
	from #brut b
		left outer join #personalCVN p on p.marca=b.marca
		left outer join infopers ip on ip.marca=b.marca
		left outer join net n on n.Data=b.Data and n.marca=b.marca
		left outer join #nrPersintr npi on npi.marca=b.marca
		...
		left outer join #PensiiFAnt pfa on pfa.marca=b.marca
		left outer join #PensiiFLuna pfl on pfl.marca=b.marca
		...
		left outer join #ptichete ft on @ImpozitTichete=1 and b.Marca=ft.Marca /*and b.Data=ft.Data*/
	...
	fetch next from calcnet into @Data,@Marca,@SalarDeBaza,@OreCFS,@OreCO,@OreCM,@Invoiri,@Nemotivate,@OreJust,@IndFAMBP,@CMunit,@CMCAS,@Diurna,@SumaImpoz,@ConsAdmin,@SumaImpsep,@AjDeces,
		...
		@AvantajeMat,@AvantajeMatImpozabile,@DiurneNeimpoz,@PensieFUnitate,@Part_profit,@SomajTehnicSusp,@OrdinePlafonareCAS,@OreCMSubvSomaj,@DataNasterii
...
--	calcul impozit, deducere personala, venit net
		exec psCalcul_impozit_vennet
			...
			@PersNecontractual,@Pensmax_ded,@Pensded_lun,@Pensded_ant,@Pensluna,@AvantajeMat,@AvantajeMatImpozabile,@DiurneNeimpoz,@DataNasterii,@CheltDed,@CursEUROPensiiF
