--***
CREATE procedure calculcost @dDataJos datetime,@dDataSus datetime as
set arithabort off
set ansi_warnings off
declare @cLm char(13),@cCom char(13),@cArt char(13),@gLM char(13),@gCom char(13),@gArt char(13)
...
begin
	...
	update costuri set rezolvat=1 where rezolvat=0 and not exists(select lm_sup,comanda_sup from costtmp where costtmp.art_sup not in ('P','S','N','R','A') and parcurs=0 and lm_sup=costuri.lm and comanda_sup=costuri.comanda)
	...
	if @nDecRegL<2 --Se pune baza din articole de calculatie
	...
	else  --Se pune baza din venituri
		...
		where costuri.lm=calcule.lm and costuri.comanda=''
	...
	update costuri set rezolvat=2
		where rezolvat=1 and lm<>'' and comanda='' and lm not in (select distinct left(lm_sup,len(lm)) from costtmp where parcurs=0 and art_sup not in ('L','G') and art_sup in (select articol_de_calculatie from artcalc where baza_pt_regia_sectiei=1))
	...
	update costuri set cantitate=
	(select isnull(sum(cantitate),0) from costtmp where costtmp.comanda_inf=costuri.comanda and
	costtmp.lm_inf=costuri.lm and costtmp.art_inf='T' and costuri.rezolvat=1 and costtmp.tip<>'CX')
	...
	where 
	(costtmp.art_inf='T' and costtmp.comanda_inf=costuri.comanda and costtmp.lm_inf=costuri.lm
...
if @nMaiAre>0
	update costuri set nerezolvate=(select count(*) from costtmp where costtmp.comanda_sup=costuri.comanda and
	costtmp.lm_sup=costuri.lm and costtmp.parcurs=0)
