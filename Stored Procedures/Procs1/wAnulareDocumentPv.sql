...
declare @ErrorMessage NVARCHAR(4000), @ErrorSeverity INT, @ErrorState INT, @debug bit, @utilizator varchar(10), 
		@idAntetBon int, @numarDoc varchar(20), @tipDoc char(2), @dataDoc datetime, @UID varchar(50)
...
begin try
	...
	begin
		...
		delete from pozdoc where Subunitate='1' and tip=@tipDoc and Numar=@numarDoc and data=@dataDoc and stare=5
		
		-- sterg tranferul aferent(daca s-a facut)
		if @tipDoc='AC'
			delete from pozdoc where Subunitate='1' and tip='TE' and Numar=@numarDoc and data=@dataDoc and stare=5
	end
