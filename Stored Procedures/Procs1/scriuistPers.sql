...
Begin try
	...
	Begin
		...
		where (@pMarca='' or a.Marca=@pMarca) and (@pLocm='' or a.loc_de_munca like rtrim(@pLocm)+'%') 
			...
			and (convert(char(1),a.loc_ramas_vacant)='0' or a.data_plec>=@DataJos or left(isnull(a.detalii.value('(/row/@lmnou)[1]','varchar(100)'),isnull(ip.Loc_munca_nou,'')),7)='DETASAT')
			and (@lista_lm=0 or lu.cod is not null)
