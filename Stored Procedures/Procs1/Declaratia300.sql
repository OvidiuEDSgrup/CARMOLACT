...
set @eroare=''
begin try
	declare @utilizator varchar(20), @datasus datetime, @datajos datetime, @parXML xml, @an int, 
			@temei int, @bifa_mob char(1), @bifa_disp char(1), @bifa_cons char(1),
...
--	stabilesc cheia de control pentru numarul de evidenta platii
	...
	select @temei=0, @bifa_mob='N', @bifa_disp='N', @bifa_cons='N'
...
--	stabilesc daca exista operatiuni cu cereale (doar daca se ruleaza operatia cu optiunea de calcul)
	...
	begin
		...
		create table #D394plus
			(codtert varchar(50), cuiP varchar(50), dentert varchar(200), tipop varchar(3), nrfacturi int, baza decimal(15), tva decimal(15), cod varchar(20), denumirecod varchar(200))
		exec Declaratia394
			@data=@data
			...
			,@tip_D394=@tip_D300 
			,@genRaport=1 
		set @bifa_cereale=(case when exists (select 1 from #D394plus where tipop in ('V','C') 
			and (@data<'07/01/2016' and cod<>'' or @data>='07/01/2016' and cod in ('1001','1002','1003','1004','1005','1201','1205','120600','121291','10086000','120400'))) then 'D' else 'N' end)
		set @bifa_mob=(case when @data>='07/01/2016' and exists (select 1 from #D394plus where tipop in ('V','C') and cod in ('29')) then 'D' else 'N' end)
		set @bifa_disp=(case when @data>='07/01/2016' and exists (select 1 from #D394plus where tipop in ('V','C') and cod in ('30')) then 'D' else 'N' end)
		set @bifa_cons=(case when @data>='07/01/2016' and exists (select 1 from #D394plus where tipop in ('V','C') and cod in ('31')) then 'D' else 'N' end)
		if object_id('tempdb..#D394plus') is not null drop table #D394plus
	end
...
--	parametrii din macheta de generare a jurnalelor in baza carora rezulta sumele de TVA
	...
	set @parXML=(select @datajos datajos, @datasus datasus, (case when @OptiuniGenerare=0 then 1 else 0 end) as calcul,
		...
		@bifa_cereale as bifa_cereale, @bifa_mob as bifa_mob, @bifa_disp as bifa_disp, @bifa_cons as bifa_cons, @solicit_ramb as solicit_ramb, 
		@data_scadentei as data_scadentei, @tip_D300 as tip_D300 for xml raw)
...
--	formare continut XML pentru generare fisier
	...
	select @totalPlata_A=SUM(convert(decimal(15),valoare)+convert(decimal(15),TVA)) 
		from deconttva where data=@datasus and Rand_decont not in ('53','53.1','54','54.1') and (@multiFirma=0 or Loc_de_munca=@lmUtilizator)

--	format fisier pentru an >= 2016
	if @an>=2016
	begin
		select @continutXml=(
			select 'mfp:anaf:dgti:d300:declaratie:v4'as [@ptxmlns],
				month(@datasus) as [@luna], @an as [@an] 
				,convert(char(1),@bifa_interne) as [@bifa_interne]
				,convert(char(1),@temei) as [@temei]
				...
				,@cui [@cui], @den [@den], @adresa [@adresa]
					...
					,max((case when Rand_decont='TELEFOANE' then RTRIM(Denumire_indicator) end)) as [@bifa_mob]
					,max((case when Rand_decont='DISPOZITIV' then RTRIM(Denumire_indicator) end)) as [@bifa_disp]
					,max((case when Rand_decont='CONSOLE' then RTRIM(Denumire_indicator) end)) as [@bifa_cons]
					...
					,max((case when Rand_decont='12.1' then convert(decimal(15),valoare) end)) as [@R12_1_1]
					,max((case when Rand_decont='12.1' then convert(decimal(15),tva) end)) as [@R12_1_2]
					,max((case when Rand_decont='12.2' then convert(decimal(15),valoare) end)) as [@R12_2_1]
					,max((case when Rand_decont='12.2' then convert(decimal(15),tva) end)) as [@R12_2_2]
					,max((case when Rand_decont='12.3' then convert(decimal(15),valoare) end)) as [@R12_3_1]
					,max((case when Rand_decont='12.3' then convert(decimal(15),tva) end)) as [@R12_3_2]
					...
					,max((case when Rand_decont='25.1' then convert(decimal(15),valoare) end)) as [@R25_1_1]
					,max((case when Rand_decont='25.1' then convert(decimal(15),tva) end)) as [@R25_1_2]
					,max((case when Rand_decont='25.2' then convert(decimal(15),valoare) end)) as [@R25_2_1]
					,max((case when Rand_decont='25.2' then convert(decimal(15),tva) end)) as [@R25_2_2]
					,max((case when Rand_decont='25.3' then convert(decimal(15),valoare) end)) as [@R25_3_1]
					,max((case when Rand_decont='25.3' then convert(decimal(15),tva) end)) as [@R25_3_2]
					...
					,max((case when Rand_decont='53.1' and Valoare<>0 then convert(decimal(15),Valoare) end)) as [@valoare_a1]
					,max((case when Rand_decont='53.1' and TVA<>0 then convert(decimal(15),tva) end)) as [@tva_a1]
					...
					,max((case when Rand_decont='54.1' and Valoare<>0 then convert(decimal(15),Valoare) end)) as [@valoare_b1]
					,max((case when Rand_decont='54.1' and TVA<>0 then convert(decimal(15),tva) end)) as [@tva_b1]
					,max((case when Rand_decont='55' and Valoare<>0 then convert(decimal(15),Valoare) end)) as [@nr_facturi_primite]
					,max((case when Rand_decont='56' and Valoare<>0 then convert(decimal(15),Valoare) end)) as [@baza_primite]
					,max((case when Rand_decont='57' and TVA<>0 then convert(decimal(15),tva) end)) as [@tva_primite]
				from deconttva where data=@datasus and (@multiFirma=0 or Loc_de_munca=@lmUtilizator)
				group by Data
				for xml path('declaratie300'), type)
	end
	else 
	begin
	--	format fisier pentru an >= 2013 si <=2015
		if @an>=2013 and @an<=2015
		begin
			select @continutXml=(
