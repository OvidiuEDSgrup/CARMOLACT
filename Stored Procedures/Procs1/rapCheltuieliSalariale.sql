...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	else (select sum((case when t.tip_operatie='R' then -1 else 1 end)*nr_tichete) from tichete t where t.data_lunii=n.data and t.marca=n.marca 
	...
	isnull((case when @optichm=0 then (select sum(ore__cond_6)*dbo.iauParLN(n.data,'PS','VALTICHET') from pontaj o where o.data between dbo.bom(n.Data) and n.Data and o.marca=n.marca) 
		else (select sum((case when t.tip_operatie='R' then -1 else 1 end)*t.nr_tichete*t.valoare_tichet) from tichete t where t.data_lunii=n.data and t.marca=n.marca 
		and (@tichetePersonalizate=1 and (t.Tip_operatie in ('C','S','R') or @Remarul=0 and t.Tip_operatie='P')
...
-->	calculez procent de repartizare pe locuri de munca de cheltuiala
	update #cheltsal1 set Procent=(case when venit_total_brut>0 and venit_total_brut-Somaj_tehnic_pozitie-cm_fnuass_pozitie-cm_fambp_pozitie>0
		then (venit_total_net-Somaj_tehnic_marca-cm_fnuass_marca-cm_fambp_marca)/(venit_total_brut-Somaj_tehnic_pozitie-cm_fnuass_pozitie-cm_fambp_pozitie) 
		else 1 end),
	...
	chelt_sal=venit_net-cm_fnuass_marca-cm_fambp_marca+pensie_suplimentara_3+somaj_1+impozit+asig_sanatate_din_net+cas+somaj_5+fond_de_risc_1+cci+asig_sanatate_pl_unitate+fond_garantare
