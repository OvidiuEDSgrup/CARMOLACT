...
Create procedure scriuResal 
	...
	@Retinere_progr_la_lichidare float, @Procent_progr_la_lichidare float, @Retinut_la_avans float, @Retinut_la_lichidare float, @detalii xml)
As
Begin 
	If isnull((select count(1) from Resal where Data=@Data and Marca=@Marca and Cod_beneficiar=@Cod_beneficiar and Numar_document=@Numar_document),0)=1
		update Resal set Data_document=@Data_document, Valoare_totala_pe_doc=@Valoare_totala_pe_doc, 
			...
			Procent_progr_la_lichidare=@Procent_progr_la_lichidare, detalii=isnull(@detalii,detalii)
	...
	Else 
		insert into Resal
			...
			Retinere_progr_la_avans, Retinere_progr_la_lichidare, Procent_progr_la_lichidare, Retinut_la_avans, Retinut_la_lichidare, detalii)
