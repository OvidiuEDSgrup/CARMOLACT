...
begin try
	...
	declare @Subunitate char(9), @FurnBenef char(1), @dDataJos datetime, @dDataSus datetime, @Tert char(13),
		...
		,@inclFacturiNe bit, @refacere bit
	...
	select	@FurnBenef=isnull(@parXML.value('(row/@furnbenef)[1]','varchar(1)'),'')
			...
			, @refacere = ISNULL(@parXML.value('(row/@refacere)[1]','bit'),0)
	...
	/*	Am scos pentru bugetari filtrarea pe loc de munca in momentul selectarii datelor, pentru a functiona spargerea platilor/incasarilor anterioare anului 2014 pe indicatori bugetari 
		...
		select fi.subunitate,fi.tert,fi.factura,'SI',fi.factura,@dDImpl,fi.valoare,fi.tva_11+fi.tva_22,fi.achitat,fi.valuta,fi.curs,fi.valoare_valuta,
				fi.achitat_valuta,fi.loc_de_munca,fi.comanda,fi.cont_de_tert,'1','',isnull(fi.explicatii,'Sold initial'),0,'',fi.data,fi.data_scadentei,'','','' as contTVA,
		...
		select p.subunitate,p.tert,p.factura,p.plata_incasare,p.numar,p.data,0,0,
				...
				isnull(nullif(p.detalii.value('(/row/@valutafalta)[1]','varchar(20)'),''),p.valuta),
				(case when nullif(p.detalii.value('(/row/@valutafalta)[1]','varchar(20)'),'') is not null then p.Curs_la_valuta_facturii else p.curs end),0,
				(case when p.valuta='' and Achit_fact=0 or isnull(t.tert_extern,0)=0 then 0 when plata_incasare in ('PS','IS') then-1 else 1 end)*p.achit_fact,p.loc_de_munca,
		...
		select a.subunitate,b.tert_vama,isnull(nullif(b.detalii.value('(/row/@facturacomis)[1]','varchar(20)'),''),b.Factura_vama),a.tip,a.numar,b.data_DVI,b.suma_com_vam+b.dif_com_vam,0,0,'',0,0,0,a.loc_munca,
				a.comanda,isnull((case when c.Sold_credit=1 then nullif(b.cont_com_vam,'') end),(case when b.cont_tert_vama<>b.cont_vama and b.cont_tert_vama<>'' then b.cont_tert_vama else b.cont_vama end)),
		...
		from doc a 
			...
			left join terti t on b.subunitate=t.subunitate and b.tert_vama=t.tert
			left join conturi c on a.subunitate=c.subunitate and b.Cont_com_vam=c.cont
		where /*not (@CtFactVamaDVI=1 or b.cont_vama<>b.cont_tert_vama) and - Renuntam la citirea setarilor, si vom aduce pozitii separate pentru fiecare componenta a DVI-ului. */
			...
			and isnull((case when c.Sold_credit=1 then nullif(b.cont_com_vam,'') end),(case when b.cont_tert_vama<>b.cont_vama and b.cont_tert_vama<>'' then b.cont_tert_vama else b.cont_vama end)) like rtrim(@ContFactura)+'%' 
			and b.factura_comis in ('','D')
			and (@filtrareUser=0 or pr.utilizator=@userASiS)
			and (@bugetari=1 or @locm='%' or a.Loc_munca like @locm)
			and (@q_cuFltLocmStilVechi=0 or f.factura is not null)
	...
	begin
		...
		select p.subunitate,p.tert,p.factura,p.plata_incasare,p.numar,p.data,0,0,
				...
				isnull(nullif(p.detalii.value('(/row/@valutafalta)[1]','varchar(20)'),''),p.valuta),
				(case when nullif(p.detalii.value('(/row/@valutafalta)[1]','varchar(20)'),'') is not null then p.Curs_la_valuta_facturii else p.curs end),0,
				(case when p.valuta='' and Achit_fact=0 or isnull(t.tert_extern,0)=0 then 0 when plata_incasare in ('PS','IS') then -1 else 1 end)*achit_fact,p.loc_de_munca,
				left(p.comanda,20)+space(20),
...
*/
	...
	begin
		...
		select furn_benef,subunitate,tert,factura,tip,numar,data,valoare,tva
			...
			(case when determinant=1 then '0' when tip in ('FF','FB','CB','CF') then '1' when tip in ('SI','AP','AS','RM','RS','RP','RQ','SF','IF') then '2' else '3' end)
				+convert(char(8),99999999-convert(int,convert(char(8),data,112)))+str(numar_pozitie) as ordine_cont,
		...
		set 
			...
			contFact=(case when d.ordine_cont=d1.ordine_cont then d.cont_de_tert else d.contFact end), 
		...
		from #tmpdocfac d, (select d2.grp, min(d2.ordine) as ordine, min(d2.ordine_cont) as ordine_cont, max(case when d2.Achitat<0 and isnull(d2.tip,'')<>'SI' then '' else d2.ordine_valuta end) as ordine_valuta from #tmpdocfac d2 group by d2.grp) d1
		where d.grp=d1.grp and (d.ordine=d1.ordine or d.ordine_cont=d1.ordine_cont or d.ordine_valuta=d1.ordine_valuta)
		...
		select
			...
			max(isnull(nullif(valutafact,''),valuta)) valuta, max(isnull(nullif(cursfact,0),curs)) curs,
