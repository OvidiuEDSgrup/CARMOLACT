...
begin try
	...
	declare @parXMLinreg xml, @tabela int
	set @tabela=4 -- note contabile
	set @parXMLinreg=(select @tabela tabela for xml raw)
	...
	/*Scriu in tabela de parametri data ultimei inchideri 121
		...
		Lucian (discutat cu Ghita): Am skipat scrierea in par pentru a nu da eroare pe BD unde tabela par este view. Ar fi si solutia ca pe acele BD sa facem scrierea in parlm.
	*/
...
if exists (select 1 from sys.objects o where o.name='inchidere121SP2AnteScriere')
begin
	exec inchidere121SP2AnteScriere @data, @locm, @inlocm, @stergTVA, @inchidTVA, @inchid4423, @sterg121, @inchid121, @inv711_121
end
...
if (@inchid4423=1 and (@inchidlm=0 or @nu481inc=1))
begin
	exec fainregistraricontabile @dinTabela=1,@dataSus=@data,@parXML=@parXMLinreg
