...
CREATE procedure rapfisaterti(@sesiune varchar(50)=null,
	...
	@centralizare int=0,	--> prin parametrul centralizare se determina care grupari se aduc;
		/*	4=nivel1,
			3=nivel2,
			2=facturi,
			1=documente,
...
--select 'test',* from #tfacturi
			...
			--> daca apar gunoaie in data scadentei: 
			update #docfacturi
				set data_scadentei='1901-1-1'
				where data_scadentei<'1901-1-1'
...
begin
	...
	create table #raport(sursa varchar(200), denumire varchar(200), oras varchar(200), furn_benef varchar(1), subunitate varchar(20), tert varchar(20),
		...
		barcod varchar(500), pozitie varchar(20), peSold int, primSold int,
...
--*/
	...
	if @grupare='CT' or @grupare1='CT' or @grupare2='CT'
	...
	update #raport set ordonare=
				...
				(case when @centralizare=1 then '|'+convert(varchar(20), data,102)
				else '' end)+
					('|'+convert(varchar(20), data_facturii,102)+'|'+rtrim(factura))
					+(case when tip='SI' then '1' else '2' end)
					+'|'+convert(varchar(20), data,102)
					+'|'+convert(varchar(20),pozitie)	--> necesar sa fie ordonarea "pana in panzele albe" ca sa nu se greseasca la sold cumulat
	...
	select (case when @centralizare in (2,0) then sursa else '' end) sursa,
		denumire, oras, furn_benef, subunitate, tert
		,(case when @centralizare in (2,0) then factura else '' end) factura
		,(case when @centralizare in (1,0) then tip else '' end) tip
		,rtrim(case when @centralizare in (1,0) then numar else '' end) numar
		,(case when @centralizare in (1,0) then data else '1901-1-1' end) data,
		...
		gestiune
		,(case when @centralizare in (2,0) then data_facturii else '1901-1-1' end) data_facturii
		, data_scadentei, nr_dvi, barcod, pozitie, peSold, soldi+tva+valoare-achitat soldf, sold_cumulat_lei,
		0 soldi_valuta, 0 valoare_valuta, 0 tva_valuta, 0 total_valuta, 0 achitat_valuta, 0 soldf_valuta, 0 sold_cumulat_valuta,
		ordonare, achitat_efect, indicator, grupare1, denumire1
		,(case when @centralizare=4 then '' else grupare2 end) grupare2
		,denumire2
	from #raport
	order by ordonare
	...
	select (case when @centralizare in (2,0) then sursa else '' end) sursa,
		...
		gestiune
		,(case when @centralizare in (2,0) then data_facturii else '1901-1-1' end) data_facturii
		, data_scadentei, nr_dvi, barcod, pozitie, peSold, soldi+tva+valoare-achitat soldf, sold_cumulat_lei sold_cumulat,
	...
	select sursa,
		max(rtrim(f.denumire)) denumire, max(oras) oras, max(furn_benef) furn_benef, max(f.subunitate) subunitate, max(f.tert) tert, rtrim(factura) factura, tip,
		rtrim(numar) numar, data,
		sum(soldi) soldi, sum(valoare) valoare, sum(tva) tva, sum(total) total, sum(achitat) achitat, max(valuta) valuta, max(curs) curs, max(rtrim(loc_de_munca)) loc_de_munca,
		max(left(comanda,20)) comanda, max(rtrim(cont_de_tert)) cont_de_tert, max(fel) fel, max(rtrim(cont_coresp)) cont_coresp
		, max(rtrim(explicatii)) explicatii, max(numar_pozitie) numar_pozitie,
		max(gestiune) gestiune, data_facturii data_facturii, max(data_scadentei) data_scadentei, max(nr_dvi) nr_dvi, max(barcod) barcod, max(pozitie) pozitie
		, max(case when peSold=1 then 1 else 0 end) peSold, sum(soldf) soldf, sum(case when f.linie_cumulat=1 then sold_cumulat else 0 end) sold_cumulat,
		sum(soldi_valuta) soldi_valuta, sum(valoare_valuta) valoare_valuta, sum(tva_valuta) tva_valuta, sum(total_valuta) total_valuta, sum(achitat_valuta) achitat_valuta, sum(soldf_valuta) soldf_valuta,
		sum(case when f.linie_cumulat=1 then 0 else sold_cumulat_valuta end) sold_cumulat_valuta, max(ordonare) ordonare, max(f.indicator) indicator,
		max(rtrim(t.grupa)) as gtert, max(rtrim(g.denumire)) dengtert, max(rtrim(l.denumire)) as denlm,
		max(rtrim(t.grupa)) as grupa, max(rtrim(i.descriere)) as den_pctlivrare, grupare1, max(denumire1) denumire1, grupare2, max(denumire2) denumire2
	from 
		--> pentru sold cumulat ma intereseaza sa aflu care e ultima linie din gruparea cea mai detaliata; pentru asta voi folosi un camp bazat pe row_number in ordine inversa a campului ordine, cu partitionare pe campurile de grupare:
		--> cu subquery ca imi trebuie si pt valuta
		(select *
			,row_number() over (partition by f.grupare1, f.grupare2, f.factura, f.sursa, f.data_facturii, f.numar, f.tip, f.data
								order by f.ordonare desc) as linie_cumulat
				--> linie_cumulat=1 inseamna ca voi lua valoarea sold_cumulat, altfel iau 0; apoi insumez si ajung la ce am nevoie
			from #fisa f) f
		left join terti t on t.subunitate=f.subunitate and f.tert=t.tert
		left join gterti g on t.grupa=g.grupa
		left join lm l on f.loc_de_munca=l.cod
		left join infotert i on i.subunitate=t.subunitate and i.tert=f.tert and i.identificator=f.nr_dvi
	group by f.grupare1, f.grupare2
		,f.factura, f.sursa, f.data_facturii
		,f.numar, f.tip, f.data
	union all -- anulez achitarile cu efecte, daca efectul este pe sold
	select sursa,
		max(rtrim(f.denumire)) denumire, max(oras) oras, max(furn_benef) furn_benef, max(f.subunitate) subunitate, max(f.tert) tert, rtrim(factura) factura, tip, rtrim(numar) numar, data,
		sum(soldi) soldi, sum(achitat_efect) valoare, sum(tva) tva, 0 total, -sum(achitat_efect) achitat, max(valuta) valuta, max(curs) curs, max(rtrim(loc_de_munca)) loc_de_munca,
		max(left(comanda,20)) comanda, max(rtrim(cont_de_tert)) cont_de_tert, max(fel) fel, max(rtrim(cont_coresp)) cont_coresp,
		max(rtrim(explicatii)) explicatii, max(numar_pozitie) numar_pozitie,
		max(gestiune) gestiune, data_facturii data_facturii, max(data_scadentei) data_scadentei, max(nr_dvi) nr_dvi, max(barcod) barcod, max(pozitie) pozitie
		, max(case when peSold=1 then 1 else 0 end) peSold, sum(achitat_efect) soldf,  sum(case when f.linie_cumulat=1 then sold_cumulat else 0 end) sold_cumulat,
		sum(soldi_valuta) soldi_valuta, sum(valoare_valuta) valoare_valuta, sum(tva_valuta) tva_valuta, sum(total_valuta) total_valuta, sum(achitat_valuta) achitat_valuta, sum(soldf_valuta) soldf_valuta,
		sum(case when f.linie_cumulat=1 then 0 else sold_cumulat_valuta end) sold_cumulat_valuta, max(ordonare) ordonare, max(f.indicator) indicator,
		max(rtrim(t.grupa)) as gtert, max(rtrim(g.denumire)) dengtert, max(rtrim(l.denumire)) as denlm,
		max(rtrim(t.grupa)) as grupa, max(rtrim(i.descriere)) as den_pctlivrare, grupare1, max(denumire1) denumire1, grupare2, max(denumire2) denumire2
	...
	group by grupare1, grupare2
		,factura, sursa, data_facturii
		,numar, tip, data
	order by max(f.ordonare)
