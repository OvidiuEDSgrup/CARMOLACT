...
begin	--> infftrt este o tabela sinteza cu facturile pe terti; infftrt se foloseste ca sa nu ia mult timp calculul (cum s-ar intampla daca s-ar apela ftert pt fiecare indicator in parte)
...
drop table infftrt create table infftrt (fb char(1),tip varchar(2),tert varchar(13),factura varchar(20),cont_coresp varchar(20))
	...
	select 'F' as fb,tip,tert,factura,cont_coresp 
		from ftert ('f','1901-1-1','2100-1-1',null,null,null,0,0,0, null) f group by tip,tert,factura,cont_coresp union all 
	select 'B' as fb,tip,tert,factura,cont_coresp 
		from ftert ('b','1901-1-1','2100-1-1',null,null,null,0,0,0,null) f group by tip,tert,factura,cont_coresp
end
