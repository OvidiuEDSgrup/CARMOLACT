...
Begin try
	...
	Begin
		select @TBazaCASCN=sum(n.Baza_CAS_cond_norm), @TBazaCASCD=sum(n.Baza_CAS_cond_deoseb), @TBazaCASCS=sum(n.Baza_CAS_cond_spec), @TotalCAS=sum(n.CAS)
			...
			,@TBazaCCIfambp=sum(cm.indFambp), @TotalCCIfambp=sum(n1.Ded_suplim), @TBazaFG=sum(case when n1.CM_incasat=0 and year(n.data)<=2012 then n.Asig_sanatate_din_cas else n1.CM_incasat end), @TotalFG=sum(n1.Somaj_5)
		from net n
