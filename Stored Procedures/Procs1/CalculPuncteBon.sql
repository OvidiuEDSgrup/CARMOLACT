...
begin try
	...
	--select (case when tip='21' then total else 0 end),
	--		(case when tip='37' then total else 0 end)
	--	from bp 
	--	where idAntetBon=@idAntetBon
	
	--select @valoareVanzariBon, @valoareIncasariPePuncte, @idAntetBon, @valoarePunct
	...
	if abs(@valoareIncasariPePuncte)>0.01 and @valoarePunctIncasare<>0
		insert into PvPuncte(idAntetBon, UID_card, tip, Puncte)
			values(@idAntetBon, @UID_card, 'C', convert(int,floor(@valoareIncasariPePuncte/@valoarePunctIncasare)))
	
	-- adaug puncte in functie de produsele vandute
	if abs(@valoareVanzariBon)>0.01 and @valoarePunctVanzare<>0
		insert into PvPuncte(idAntetBon, UID_card, tip, Puncte)
			values(@idAntetBon, @UID_card, 'D', convert(int,floor(@valoareVanzariBon/@valoarePunctVanzare)))
