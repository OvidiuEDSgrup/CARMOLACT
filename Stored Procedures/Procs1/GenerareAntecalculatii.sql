...
CREATE PROCEDURE [dbo].[GenerareAntecalculatii] @sesiune VARCHAR(50), @parXML XML
AS
DECLARE @nRanduri INT, @calculat INT, @id INT, @cod VARCHAR(20), @grupa VARCHAR(20), @utilizator VARCHAR(100), @mesaj VARCHAR(500)
...
*/
	...
	IF EXISTS (
			SELECT *
			FROM sysobjects
			WHERE NAME = 'anteclcpecoduri'
			)
	BEGIN
		IF (
				SELECT TOP 1 val_logica
				FROM par
				WHERE Tip_parametru = 'MP'
					AND Parametru = 'INCLCAC'
				) = 0
	...
	BEGIN
		...
		SELECT MIN(pt.id), @utilizator, (CASE WHEN MIN(LEFT(n.Cont, 3)) = '345' THEN 'P' ELSE 'S' END), t.codnomencl, 
			NULL, NULL, 0, 1
	...
	BEGIN -- Pentru modificare => avem @id de antecalculatie
		...
		INNER JOIN dbo.nomencl n ON ac.Cod = n.Cod
		WHERE ac.idAntec = @id
	END
