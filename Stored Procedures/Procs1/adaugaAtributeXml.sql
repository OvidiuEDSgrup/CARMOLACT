...
declare @atrTable table(nume varchar(8000), valoare varchar(max), pt_update bit)
...
begin try
	...
	SELECT
		xA.atribut.value('localname[1]', 'varchar(8000)') AS nume,
		xA.atribut.value('value[1]', 'varchar(max)') AS valoare,
		0 pt_update
	FROM 
	( 
	SELECT x.query('
	...
	from @atrTable a,
		...
		(SELECT x.query('
			...
			CROSS APPLY q1.atribute.nodes('/atribute') AS xA(atribut)
			where xA.atribut.value('localname[1]', 'varchar(8000)') not like 'o_%' ) x
	where x.nume=a.nume
	...
	begin
		...
		declare @crsPoz cursor, @nume varchar(8000), @valoare varchar(max)
