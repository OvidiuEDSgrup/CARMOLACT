--***
	--/*
create procedure rapStatDePlata(@datajos datetime, @datasus datetime, 
	@locm char(20)=null, @strict int=0,	-->	strict loc de munca
	...
	@tipstat varchar(30)=null,	-->	filtru dupa tip stat de plata
	@modangajare varchar(30)=null	-->	filtru dupa mod de angajare (durata contract) null-Toti, N-Nedeterminata, D-Determinata
	)
as--*/
...
*/
declare @q_datajos datetime,@q_datasus datetime, @q_locm varchar(20), @q_strict int, 
	@q_marci varchar(300),@q_functii varchar(300),@q_grupare int, @q_centru varchar(1), @q_tipstat varchar(30), 
	@ImpozitTichete int, @tich_datajos datetime, @tich_datasus datetime, @q_tipSalarizare char(1), @q_tipangajat char(1), @q_modangajare char(1)
set @q_datajos=dbo.bom(@datajos) set @q_datasus=dbo.eom(@datasus) set @q_locm=@locm set @q_strict=@strict
	 set @q_marci=@marci set @q_functii=@functii set @q_tipSalarizare=@tipSalarizare 
	 set @q_tipangajat=@tipangajat set @q_tipstat=@tipstat set @q_modangajare=@modangajare
...
Begin
	...
	exec pTichete @dataJos=@dataImpozJos, @dataSus=@dataImpozSus, @marca=@q_Marci, @DeLaCalculLichidare=1
...
select i.data,p.loc_ramas_vacant,max(i.nume) as nume,max(p.cod_numeric_personal) as cod_numeric_personal,max(i.cod_functie) as cod_functie,
		...
		from istpers i 
			...
			left join infopers ip on ip.marca=i.marca
		...
		--	and (p.loc_ramas_vacant=0 or p.data_plec>@q_datajos)	--> conditie scoasa pentru cei de la Arges cu plati retroactive
			...
			and (@activitate is null or i.Activitate=@activitate)
			and (@q_tipstat is null or isnull(nullif(p.Tip_stat,''),ip.religia)=@q_tipstat)
			and (@q_modangajare is null or i.Mod_angajare=@q_modangajare)
...
delete p from #personal p where not ((p.loc_de_munca like rtrim(@q_locm)+(case when @q_strict=1 then '' else '%' end) or @q_locm is null) 
...
select e.data, rtrim(isnull(e.nume,'')) as nume,rtrim(isnull(lm.denumire,''))+space(20) as nume_lm, rtrim(isnull(f.denumire,'')) as nume_functie, 
	...
	isnull(convert(float,m.zile_unitate),0)/isnull(nn.nr,1000000000) as zi_bo,
	convert(float,isnull(m.zi_fnuass,0))/isnull(nn.nr,100000000) as zi_bs,
	isnull(m.zi_faampb,0) as zi_ba,
	...
	(case when isnull(p.ore_lucrate,0)=0 then 0.1 else p.ore_lucrate end))*isnull(p.ore_lucrate,0),0) as sal_cuv, 0 as ind_cond,
	...
	isnull(m.zile_unitate*p.regim_de_lucru,0)/isnull(nn.nr,100000000) as ore_bo, 
	isnull(b.Ind_c_medical_unitate,0)/isnull(nn.nr,100000000) as suma_bo,
	isnull(m.zi_fnuass,0)*p.regim_de_lucru/isnull(nn.nr,10000000) as ore_bs,
	isnull(m.indemnizatie_fnuass,0)/isnull(nn.nr,100000000) as suma_bs,
	isnull(zi_faampb*p.regim_de_lucru,0) as ore_ba, isnull(m.suma_faambp,0) as suma_ba,
	isnull(p.ore_nemotivate,0) as ore_ne,isnull(p.ore_concediu_fara_salar,0) as ore_cfs,
...
-- c
	left join (select data, marca, sum(m.zile_cu_reducere) as zile_unitate, sum(m.zile_lucratoare) as zile_lucratoare,
					sum(case when not(tip_diagnostic='2-' or tip_diagnostic='3-' or tip_diagnostic='4-') then m.zile_lucratoare-m.zile_cu_reducere else 0 end) as zi_fnuass,
					sum(case when tip_diagnostic='2-' or tip_diagnostic='3-' or tip_diagnostic='4-' then m.zile_lucratoare-m.zile_cu_reducere else 0 end) as zi_faampb,
					sum(isnull(m.indemnizatie_unitate,0)) as indemnizatie_unitate, 
					sum(case when not(tip_diagnostic='2-' or tip_diagnostic='3-' or tip_diagnostic='4-') then m.indemnizatie_cas else 0 end)as indemnizatie_fnuass,
					sum(case when tip_diagnostic='2-' or tip_diagnostic='3-' or tip_diagnostic='4-' then m.indemnizatie_cas else 0 end) as suma_faambp
...
*/
--	am apelat functia pentru a citi fie din pontaj, fie din tichete. Am utilizat procedura pTichete pentru a putea filtra dupa data impozitarii tichetelor.
...
select s.Data, isnull(s.nume,''), isnull(s.nume_lm,''), isnull(s.nume_functie,''), isnull(s.nivel,''), isnull(s.cod_numeric_personal,''), 
	...
	isnull(b.ore_concediu_de_odihna/s.regim_de_lucru,''),isnull(s.zi_ev,''), isnull(s.zi_bo,''), 
	...
	isnull(s.ore_bo,''), isnull(b.ind_c_medical_unitate,''), isnull(s.ore_bs,''), isnull(b.Ind_c_medical_CAS,''), isnull(s.ore_ba,''), isnull(b.spor_cond_9,''), 
	isnull(b.ore_nemotivate,''), isnull(b.Ore_concediu_fara_salar,''),
...
--	selectez din functii_lm pozitiile valabile la data generarii raportului
	...
	from functii_lm where (@q_locm is null or Loc_de_munca like rtrim(@q_locm)+(case when @q_strict=1 then '' else '%' end))
