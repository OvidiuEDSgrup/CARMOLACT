--***
create procedure [dbo].[repAdmDesf] @dDataJos datetime,@dDataSus datetime 
as  
declare @sub char(9)
declare @nDifD float,@nDifA float 
declare @cArtAdm char(13), @cArtDesf char(13), @bazaArt int 
...
where p.subunitate=@sub and p.cont_debitor like '6%' and p.data between @dDataJos and @dDataSus and
p.loc_de_munca<>'' and p.comanda<>'' and k.tip_comanda in ('P','R') and
exists
(select * from costtmp where lm_sup='' and art_sup in ('P','R') and p.loc_de_munca=costtmp.lm_inf and p.comanda=costtmp.comanda_inf)
...
where @bazaArt=0 and pozincon.cont_Debitor=conturi.cont and
conturi.logic=1 and conturi.articol_de_calculatie in (@cArtAdm,@cArtDesf) and
pozincon.cont_debitor like '6%' and pozincon.data between @dDataJos and @dDataSus and
(pozincon.comanda='' or pozincon.comanda in (select comanda from comenzi where tip_comanda='L' and subunitate = @sub))
...
set @nDifA=ISNULL((select sum(suma) from pozincon,conturi where pozincon.cont_debitor=conturi.cont and
conturi.logic=1 and conturi.articol_de_calculatie=@cArtAdm and pozincon.data between @dDataJos and @dDataSus),0)
-ISNULL((select sum(valoare) from cost where tip_inregistrare='PE' and data_lunii=@dDataSus and articol_de_calculatie=@cArtAdm),0)
-ISNULL((Select sum(cheltuieli_initiale) from coefic where tip_coeficient='Z' and data_lunii=@dDataSus and baza_initiala<>1),0)

set @nDifD=ISNULL((select Sum(suma) from pozincon,conturi where pozincon.cont_debitor=conturi.cont and
conturi.logic=1 and conturi.articol_de_calculatie=@cArtDesf and pozincon.data between @dDataJos and @dDataSus),0)-
ISNULL((select sum(valoare) from cost where tip_inregistrare='PE' and data_lunii=@dDataSus and articol_de_calculatie=@cArtDesf),0)-
ISNULL((Select sum(cheltuieli_initiale) from coefic where tip_coeficient='D' and data_lunii=@dDataSus and baza_initiala<>1),0)
