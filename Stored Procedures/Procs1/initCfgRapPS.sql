...
--if (select count(1) from cfgRapPS c where c.grup='R')=0
	declare @maxOrdine int, @grupareRetineri int, @subtipret int
	...
	set @subtipret=isnull((select val_logica from par where Tip_parametru='PS' and parametru like 'subtipret'),0)
	if @subtipret=1 and @grupareRetineri=0
...
--	pentru stat de plata (grupare pe cod beneficiar/tip retinere functie de parametru)
		...
		from(
			select --row_number() over (order by b.tip_retinere, b.cod_beneficiar) as ordine,
					(case when @grupareRetineri=1 then (case when @subtipret=1 then rtrim(t.subtip) else rtrim(t.tip_retinere) end) else rtrim(isnull(b.tip_retinere,'')) end)
						+'|'+(case when @grupareRetineri=1 then '' else rtrim(isnull(b.cod_beneficiar,'')) end) as subtipuri
			from dbo.fTip_retineri((case when @grupareRetineri=1 and @subtipret=1 then 0 else 1 end)) t left join benret b on b.Tip_retinere=t.Tip_retinere and @grupareRetineri=0
			) x
