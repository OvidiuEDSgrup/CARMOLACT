--***
create procedure rapDocumentepeTertiSP @sesiune varchar(50)=null, @cFurnBenef varchar(1), @cData datetime, @cTert varchar(50)=null,
			...
			@tipdoc varchar(1)='F' --/*SP
			,@cDataJos datetime, @StrictPerioada int --SP*/
as
begin
	/*	--pt teste:
	...
	exec rapDocumentepeTertiSP @cFurnBenef=@cFurnBenef, @cData=@cData
...
/*SP	set @cDataJos=dateadd(d,-1,@cDataJos) SP*/
		if(dateadd(d,-1,@cDataJos)<@dataSolduri and @avemDataJos=0)
		begin
				 -- set @soldmin_f=@soldmin
...
--SELECT convert(char(10),@cDataJos,101) as datajos, convert(char(10),@cData,101) as datasus
	...
	begin
		...
		set @parXMLFact=(select @cFurnBenef as furnbenef, convert(char(10),@cDataJos,101) as datajos, convert(char(10),@cData,101) as datasus, 
			...
			,@indicator indicator --/*SP
			,@StrictPerioada as strictperioada --SP*/
			for xml raw)
		...
		begin
			set @parXMLFact=(select @cFurnBenef as furnbenef, @cDataJos as datajos, convert(char(10),@fsolddata,101) as datasus, 1 as cen, 
...
/*				and ft.sursa='F' or ft.sursa='E' and 
	...
	begin catch
		set @eroare='rapDocumentepeTertiSP (linia '+convert(varchar(20),error_line())+'):'+CHAR(10)+ERROR_MESSAGE()
	end catch
	if object_Id('tempdb.dbo.#fFacturi') is not null drop table #fFacturi
