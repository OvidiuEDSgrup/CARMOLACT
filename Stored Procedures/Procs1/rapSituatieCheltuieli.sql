	--***
create procedure rapSituatieCheltuieli @datajos datetime, @datasus datetime, @cont varchar(100), @art_calc varchar(100), @locm varchar(100),
		@comanda varchar(100), @grcom varchar(100), @tip_document nvarchar(100)
as
/*
exec rapSituatieCheltuieli @datajos='2014-02-01', @datasus='2014-02-28', @cont='624.T.02.02', @art_calc=null, @locm=null,
...
where
	p.Subunitate=@Sub
	and p.data between @datajos and @datasus and p.cont_debitor like '6%' and 
	(@cont is null or p.cont_debitor like @cont+'%') and (@art_calc is null or c.Articol_de_calculatie=@art_calc or Tip_document in ('CM','RS','AI','PD','NC'))
	and (@comanda is null or p.comanda=@comanda) and (@grcom is null or g.cod_produs=@grcom)
...
begin
	...
	select p.cont_debitor,c.Denumire_cont as Denumire_cont,
		p.tip_document,p.numar_document,p.data,p.suma as suma,p.explicatii,p.loc_de_munca,p.comanda 
	into #chdet
	from pozincondet p
		left join conturi c on c.subunitate=p.Subunitate and p.cont_debitor=c.cont
