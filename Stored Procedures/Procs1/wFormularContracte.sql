...
CREATE PROCEDURE wFormularContracte @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100)
OUTPUT AS
...
IF EXISTS (
		...
		FROM sysobjects
		WHERE type = 'P'
			AND NAME = 'wFormularContracteSP1'
		)
BEGIN
	EXEC wFormularContracteSP1 @sesiune = @sesiune, @parXML = @parXML, @numeTabelTemp = @numeTabelTemp OUTPUT
END
