...
from indicatori i
left outer join expval e on i.cod_indicator=e.cod_indicator and (@el1 is null or e.element_1=@el1) and data between @dataJ and @dataS
full join calstd c on isnull(e.data,'01/01/2005')=c.data
full outer join #ind on #ind.codi=i.cod_indicator
where i.cod_indicator in (select codi from #ind)
order by #ind.nivel
