...
set @eroare=''
begin try
	declare @sub varchar(9), @UrmarireCantSec int, @ValidareTara int, @userASiS varchar(30), @filtrareUser bit
	...
	select @userASiS=dbo.fIaUtilizator('')
...
--	if object_id('tempdb.dbo.#intrastat') is not null drop table #intrastat
	...
	if object_id('tempdb.dbo.#docIntrastat') is not null drop table #docIntrastat
	...
	/*	Selectez datele in #docIntrastat pentru a citi din doc.detalii. Pe BD Unicarm mergea foarte greu citirea din doc in selectul din pozdoc.	*/
	select d.*
		,nullif(d.detalii.value('/row[1]/@taraexp', 'varchar(20)'),'') as tara_tert
		,nullif(d.detalii.value('/row[1]/@valstatistica', 'decimal(17,2)'),0) as val_statistica
		,nullif(d.detalii.value('/row[1]/@valprestari', 'float'),0) as valprestari
		,nullif(d.detalii.value('/row[1]/@nattranza', 'varchar(20)'),'') as natura_tranzactie_a
		,nullif(d.detalii.value('/row[1]/@nattranzb', 'varchar(20)'),'') as natura_tranzactie_b
		,nullif(d.detalii.value('/row[1]/@condlivr', 'varchar(20)'),'') as cond_livrare
		,nullif(d.detalii.value('/row[1]/@modtransp', 'varchar(20)'),'') as mod_transport
	into #docIntrastat
	from doc d
	WHERE @tipdecl<>'U' and d.subunitate=@sub and (@flux='I' and d.tip in ('RM','RS') or @flux='E' and d.Tip in ('AP','AS')) 
		and d.data between @datajos and @datasus
	create index tmpIndex on #docIntrastat (subunitate, tip, numar, data)
	...
	select p.*
		...
		--isnull(d.tara_tert,isnull(intst.cont_intermediar,'')) as tara_tert, d.val_statistica
	into #pozdocIntrastat
	from pozdoc p
		left outer join #docIntrastat d on d.subunitate=p.subunitate and p.tip=d.tip and p.numar=d.numar and p.data=d.data 
	...
	WHERE @tipdecl<>'U' and p.subunitate=@sub and p.tip=(case when @flux='I' then 'RM' else 'AP' end) and p.data between @datajos and @datasus
		and n.tip not in ('R', 'S') and (abs(p.pret_valuta)>=0.000001 or p.Valuta<>'') /*and (p.tip<>'RM' or p.numar_DVI='')*/
		and (@filtrareUser=0 or pr.utilizator=@userASiS)
	
	/* SP1 care sa permita alterarea tabelei #pozdocIntrastat. */ 
	if exists (select 1 from sysobjects where [type]='P' and [name]='rapDeclaratieIntrastatSP1')
		exec rapDeclaratieIntrastatSP1 @datajos=@datajos, @datasus=@datasus, @flux=@flux, @tipdecl=@tipdecl, @faraGrupare=@faraGrupare output, @tabela=@tabela

	alter table #pozdocIntrastat add cod_vamal varchar(20)
	alter table #pozdocIntrastat add tara_tert varchar(20)
	alter table #pozdocIntrastat add val_statistica decimal(17,2)
	
	update p set cod_vamal=isnull(nullif(p.detalii.value('/row[1]/@codvama','varchar(20)'),''),isnull(nullif(n.detalii.value('/row[1]/@codvama','varchar(20)'),''),substring(n.tip_echipament,2,20))),
		tara_tert=isnull(d.tara_tert,isnull(intst.cont_intermediar,'')),
		val_statistica=d.val_statistica
	...
	round(convert(decimal(15,3), p.cantitate*(case when p.tip in('RM','RS') then p.pret_valuta*(case when p.valuta<>'' then p.curs else 1 end)*(1.00+p.discount/100.00) else p.pret_vanzare end)), 2) as val_facturata,
	round(convert(decimal(15,3), isnull(p.val_statistica*(case when isnull(tot.total_cantitate,0)<>0 then p.cantitate/tot.total_cantitate else 0 end),
		p.cantitate*(case when p.tip in('RM','RS') then p.pret_valuta*(case when p.valuta<>'' then p.curs else 1 end)*(1.00+p.discount/100.00) else p.pret_vanzare end)
			*(1+(case when isnull(tot.total_facturat,0)<>0 then isnull(d.valprestari,isnull(intst.pret_valuta,0))/tot.total_facturat else 0 end)))), 2) as val_statistica,
	...
	isnull(d.natura_tranzactie_a,isnull(intst.gestiune,'')) as natura_tranzactie_a, 
	isnull(d.natura_tranzactie_b,isnull(intst.gestiune_primitoare,'')) as natura_tranzactie_b, 
	isnull(d.cond_livrare,isnull(intst.cont_corespondent,'')) as cond_livrare, 
	isnull(d.mod_transport,isnull(intst.cont_de_stoc,'')) as mod_transport,
...
--	stuff(t.Cod_fiscal, 1, patindex('%[0-9]%', t.Cod_fiscal)-1, '') as cif_partener, 
	...
	FROM #pozdocIntrastat p
		...
		left outer join (select p.subunitate, p.tip, p.numar, p.data, /*abs*/(sum(round(convert(decimal(15,3), p.cantitate*(case when p.tip in('RM','RS') then p.pret_valuta*(case when p.valuta<>'' then p.curs else 1 end)
							*(1.00+p.discount/100.00) else p.pret_vanzare end)), 2))) as total_facturat, sum(round(convert(decimal(15,3), p.cantitate),3)) as total_cantitate
						from #pozdocIntrastat p 
						group by p.subunitate, p.tip, p.numar, p.data) tot on tot.subunitate=p.subunitate and tot.tip=p.tip and tot.numar=p.numar and tot.data=p.data
