--***
Create procedure OPGenerareD205
	@DataJ datetime, 
	@DataS datetime,
...
--	inserez datele legate de drepturi de autor si conventii civile
	...
	select CampD205
	from fDeclaratia205 (@DataJ, @DataS, @tipdecl, @TipVenit, @LmJos, @LmSus, @ContImpozit, @ContFactura)
...
--		select @rezultat
	...
	begin /* altfel, il salvez in tabela temporara si apoi cu bcp in un fisier pe disk */
		...
		set @comandaBCP='bcp "select rtrim(coloana) from ##tmpdecl'+'" queryout "'+@cFisier+'" -T -c -t -C ACP -S '+@nServer
