...
begin try	
	...
	Begin
		...
		*/
		...
		select row_number() over(order by pd.tert,pd.cont,pd.data,pd.efect) as nrord_poz ,dense_rank() over(order by pd.tert) as nrord_plin, pd.tert, pd.dentert,
			...
			s.sumacompensata, pd.loc_de_munca, pd.Comanda, convert(varchar(8),'') as nr_plin, 0 as nr_poz
		into #tmpcompensari
	...
	--insertul final
		SELECT @ctcomp as cont, @datacomp as data, nr_plin as numar, (case when @tipefecte='P' then 'PD' else 'ID' end) as plata_incasare, tert, cont_efect_negativ as cont_corespondent, 
		...
		FROM #tmpcompensari
		union all
		SELECT @ctcomp as cont, @datacomp as data, nr_plin as numar, (case when @tipefecte='P' then 'PD' else 'ID' end) as plata_incasare, tert, cont_efect_pozitiv as cont_corespondent, 
