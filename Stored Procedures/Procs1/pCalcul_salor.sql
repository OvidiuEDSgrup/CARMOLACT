...
--			and pontaj.loc_de_munca between @LocmJos and @LocmSus 
	...
	if exists (select * from #salor s inner join extinfop e on e.marca=s.marca and e.cod_inf='TIPINTREPTM' and e.val_inf='OrePeLuna' and e.Procent<>0)
		update s set s.salar_orar=round((case when @Buget=1 then p.salar_de_baza else p.salar_de_incadrare end)/e.procent,4)
		from #salor s
		inner join extinfop e on e.marca=s.marca and e.cod_inf='TIPINTREPTM' and e.val_inf='OrePeLuna' and e.Procent<>0, personal p
		where s.marca=p.marca and p.tip_salarizare='1'
