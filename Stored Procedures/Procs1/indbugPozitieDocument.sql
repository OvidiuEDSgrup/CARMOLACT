...
/*
			...
			update ib 
				set ib.indbug=rtrim(isnull(nullif(pd.detalii.value('(/row/@indicator)[1]','varchar(20)'),''),
					(case	when pd.tip in ('RM','RS','RC','RP','RQ') then isnull(nullif(cf.detalii.value('(/row/@indicator)[1]','varchar(20)'),''),isnull(cs.detalii.value('(/row/@indicator)[1]','varchar(20)'),'')) 
							...
							when pd.tip in ('CM','AE','CI','AI','TE','PF','DF','AF') then isnull(nullif(ccor.detalii.value('(/row/@indicator)[1]','varchar(20)'),''),isnull(cs.detalii.value('(/row/@indicator)[1]','varchar(20)'),'')) 
						else '' end)))
		...
		begin
			update ib
				set ib.indbug=rtrim(case when pn.detalii.value('(/row/@fara_indicator)[1]','int')=1 then ''
					when pn.tip in ('AO','AL','AT') then substring(pn.comanda,21,20)			--pentru documente de ALOP ramine sa citim inca indicatorul bugetar din comanda
	...
	/*	Completare indicator bugetar in POZPLIN.detalii.	*/
		...
		begin
			...
			set @binar=cast('specificebugetari' as varbinary(128))-->context info pt a fenta triggerul
