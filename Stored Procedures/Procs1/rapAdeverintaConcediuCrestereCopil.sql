...
--	pun intr-o tabela temporara orele de stagiu (lucrate, ore CM, ore CO, ore nelucrate si neplatite)
	...
	select b.data, b.marca, sum(b.ore_lucrate_regim_normal+isnull(po.ore_platite,0)+b.ore_obligatii_cetatenesti) as ore_lucrate
		...
		,(case when max(b.Spor_cond_10)=0 then 8 else max(b.Spor_cond_10) end) as regim_de_lucru
	into #brutMarca
	from brut b
...
--	stabilesc perioada de concediu de maternitate aferenta nasterii copilului
	...
	from fCalendar (@datalunii_11, @datalunii) fc where data=data_lunii	
	create table #tichete 
		(marca varchar(6), data_salar datetime, data datetime, numar_tichete int, valoare_tichete float)
	declare @datal datetime, @dataImpozJos datetime, @dataImpozSus datetime
	declare tmpTich cursor for
	select datalunii, datajos, datasus
	from #perTich

	open tmpTich
	fetch next from tmpTich into @datal, @dataImpozJos, @dataImpozSus
	while @@fetch_status = 0 
	begin
		delete from #ptichete
		exec pTichete @dataJos=@dataImpozJos, @dataSus=@dataImpozSus, @marca=@marca, @DeLaCalculLichidare=1

		insert #tichete (marca, data_salar, data, numar_tichete, valoare_tichete)
		select marca, @datal, data, numar_tichete, valoare_tichete from #ptichete
...
--	selectie finala		
	select ROW_NUMBER() OVER(ORDER BY n.Data desc) as nr_crt, 
		...
		round(sum(n.venit_net+isnull(t.Valoare_tichete,0)),0) as venit_net, 
	...
	from net n 
		...
		left outer join #tichete t ON t.Marca = n.Marca and t.data_salar = n.Data
