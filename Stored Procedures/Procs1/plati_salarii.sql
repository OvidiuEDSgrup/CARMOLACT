...
as
/*
	exec plati_salarii @dIncLPrec='09/01/2015', @dSfLPrec='09/30/2015', @pTip='', @pElement=''
*/
begin try
	...
	Set @cData_curenta=isnull(rtrim((select val_alfanumerica from par where tip_parametru='PS' and parametru=rtrim(host_id())+'D')) ,'01'+'/'+(case when @nLuna<10 then '0' else '' end)
		+rtrim(convert(char(2),@nLuna))+'/'+convert(char(4),@nAn))
	...
	exec rapFluturasCentralizat @dataJos=@dIncLPrec, @dataSus=@dSfLPrec, @Grupare=''
