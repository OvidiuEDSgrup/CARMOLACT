...
Begin try
	...
	@OreProba int, @OrePreaviz int, @NrTichDePus int, @SerieInceputDePus varchar(13), @gfetch int, 
	@repartizarePeSerii int, @serieInceput int, @serieSfarsit int, @ordineRepartizare int, @ticheteXZileAng char(2)
	/*
...
--	calcul tichete pt. celelalte cazuri: parcurgere cursor din pontaj cu calcul functie de setari
	...
	Begin
		if OBJECT_ID('tempdb..#proba') is not null drop table #proba
		if OBJECT_ID('tempdb..#preaviz') is not null drop table #preaviz
		if OBJECT_ID('tempdb..#calcTichete') is not null drop table #calcTichete
		if OBJECT_ID('tempdb..#pontaj_tichete') is not null drop table #pontaj_tichete

		create table #calcTichete (marca varchar(20))
		exec CreeazaDiezSalarii @numeTabela='#calcTichete'

		select marca, Zile, Ore into #proba from dbo.fDate_pontaj_automat(@dataJos, @dataSus, @Data, 'PB', @Marca, 0, 0)
		select marca, Zile, Ore into #preaviz from dbo.fDate_pontaj_automat(@dataJos, @dataSus, @Data, 'PZ', @Marca, 0, 0)
		...
		select a.Data, a.Marca, a.Ore__cond_6, a.Loc_de_munca, a.Loc_munca_pentru_stat_de_plata,
			...
			a.Spor_cond_10, a.Ore_intrerupere_tehnologica, a.Spor_cond_8, a.Ore_nemotivate, isnull(c.Zile_lucratoare,0), a.Numar_curent, 
			isnull(pb.Zile*a.Regim_de_lucru+pb.Ore,0) as ore_proba, isnull(pz.Zile*a.Regim_de_lucru+pz.Ore,0) as ore_preaviz, 
			upper(isnull(ta.val_inf,'')) as ticheteXZileAng
		from #pontaj_tichete a
			inner join #personal p on p.Marca=a.Marca
			left outer join conmed c on c.Marca=a.Marca and c.Data=@dataSus and c.Tip_diagnostic='0-'
			left outer join #proba pb on pb.marca=a.marca
			left outer join #preaviz pz on pz.marca=a.marca
			left outer join extinfop ta on ta.marca=a.marca and ta.Cod_inf='TICHZILEANG' and upper(ta.Val_inf)='DA'
		where a.data between @dataJosCalc and @dataSusCalc
			and (@TichPontajLunaAnt=0 or (a.Ore_lucrate>0 and a.Marca not in (select marca from #personal where loc_ramas_vacant=1 and Data_plec between @dataJosCalc and @dataSusCalc)))
		...
		fetch next from TichetePontaj into
			...
			@OreDelegatie, @OreIT, @OreIntemp, @OreNemotivate, @ZileIngCopil, @NrCrt, @OreProba, @OrePreaviz, @ticheteXZileAng
...
--	calcul nr. zile lucratoare fara tichete (daca setare [X]Nu se acorda tichete pt. primele X zile calendaristice)
				set @ZileLucrFTich=isnull((select dbo.psZileFaraTichete(@dataJos, @dataSus, @Marca) where @NuTichXZileAng=1 and DateAdd(day,@XZileFaraTich,@DataAngajarii)>=@dataJos and @ticheteXZileAng<>'DA'),0)
...
--	calcul tichete in pontaj
				update #pontaj_tichete set Ore__cond_6=@NrTichete where @TichPontajLunaAnt=0 and @NrTichete>=0 and Data=@Data and Marca=@Marca and Loc_de_munca=@Lm and Numar_curent=@NrCrt
--	scriu tichete pontaj din luna anterioara
				...
				fetch next from TichetePontaj into
					...
					@NrTicheteDafora, @OreDelegatie, @OreIT, @OreIntemp, @OreNemotivate, @ZileIngCopil, @NrCrt, @OreProba, @OrePreaviz, @ticheteXZileAng
				set @gfetch=@@fetch_status
			End
...
--	scriere numar tichete in tabela tichete (daca se lucreaza cu macheta de tichete)
			...
			Begin
				...
				select	@NrTichDePus=round(@NrTicheteCuvMarca,0)-isnull(@NrTichAnt,0), 
						@SerieInceputDePus=(case when @SerieInceputAnt is null then '' else convert(char(13),convert(int,isnull(@SerieInceputAnt,''))+1) end)

				/*	Folosesc tabela temporara pentru a nu scrie pentru fiecare marca in tabela tichete. Merge mai greu din cauza triggerelor. */
				insert into #calcTichete (marca, data_lunii, tip_operatie, serie_inceput, serie_sfarsit, nr_tichete, valoare_tichet, valoare_imprimat, TVA_imprimat)
				select @gMarca, @dataSus, 'C', @SerieInceputDePus, '', @NrTichDePus, @ValTichet, @ValImprimat, @TVAImprimat
			End			
			set @gMarca=@Marca
		end
		close TichetePontaj
		...
		if @TichPontajLunaAnt=0
			update p set p.Ore__cond_6=pt.Ore__cond_6
			from pontaj p
				inner join #pontaj_tichete pt on pt.data=p.data and pt.marca=p.marca and pt.loc_de_munca=p.loc_de_munca and pt.numar_curent=pt.numar_curent
		...
		if @TichMacheta=1 
			...
			select marca, data_lunii, tip_operatie, serie_inceput, serie_sfarsit, nr_tichete, valoare_tichet, valoare_imprimat, TVA_imprimat
			from #calcTichete
	End
