...
--	Pun datele in tabela temporara pt. a face cateva operatii ulterioare
	...
	where i.data=@dataSus and (@marca is null or i.marca=@marca) and p.data_angajarii_in_unitate<=(case when @salariatiactivi=1 then @data else @dataSus end) 
		...
		and (isnull(@grupaMunca,'')='' or isnull(i.grupa_de_munca,p.grupa_de_munca)=@grupaMunca) and (isnull(@tipStat,'')='' or isnull(p.tip_stat,ip.Religia)=@tipStat)
...
--	pun numar curent la marcile inlocuitoare
	...
	(select a.Numar_curent, a.numar_curent_ordonare, isnull(i.Data,@dataSus) as Data, a.marca, a.nume, a.cod_functie, a.denumire_functie, a.loc_de_munca, a.denumire_lm, a.niv, 
		...
		isnull(i.spor_conditii_4,0) as spor_conditii_4, isnull(i.spor_conditii_5,0) as spor_conditii_5, isnull(i.spor_conditii_6,0) as spor_conditii_6, isnull(p.spor_cond_7,isnull(ip.spor_cond_7,0)) as spor_cond_7, 
		...
		isnull(round(a.salar_de_incadrare*isnull(p.spor_cond_7,ip.spor_cond_7)/100,0),0) as sp7_calc, 
