...
create procedure rapJurnalTvaVanzari(@sesiune varchar(50), @DataJ datetime, @DataS datetime
	...
	,@Provenienta varchar(1)	-->	''=Toate, I=Interne, E=Export extracomunitar, U=Export intracomunitar
								--	, V=Toate vanzarile, C=Cumparari
...
set @q_cotatvaptfiltr=@cotatvaptfiltr
...
set @TipFact=(case when @provenienta='' then @TipFact when @provenienta in ('V','I','E','U') then 'V' else 'C' end)
...
where 
	(@Provenienta='' or isnull(zile_inc, 0)=(case @Provenienta when 'E' then 2 when 'I' then 0 else 1 end))
	and (@TipFact='' or d.vanzcump=@TipFact) and (@q_cotatvaptfiltr is null or d.cota_tva=@q_cotatvaptfiltr)
	and (@TipTvaTert=0
		or (((@TipTvaTert=1) and isnull((select top 1 tt.tip_tva from tvapeterti tt where tt.tert=d.tert and tt.tipf='F' and tt.dela<=d.data and isnull(tt.factura,'')='' order by tt.dela desc),'P')='P' 
				/*isnull(it.Grupa13,0)<>1*/ and (t.tert is not null /*or d.tert='<Dim BF>'*/)
		or (@TipTvaTert=3 and @tli=1 or @TipTvaTert=1 and @tli=0) and isnull((select top 1 tt.tip_tva from tvapeterti tt where tt.tert=d.tert and tt.tipf='F' and tt.dela<=d.data and isnull(tt.factura,'')='' order by tt.dela desc),'P')='I' 
				and (t.tert is not null /*or d.tert='<Dim BF>'*/)
		or @TipTvaTert=2 and (isnull((select top 1 tt.tip_tva from tvapeterti tt where tt.tert=d.tert and tt.tipf='F' and tt.dela<=d.data and isnull(tt.factura,'')='' order by tt.dela desc),'P')='N' 
...
insert #jtvavanz
	...
	,care_jurnal, tip_doc, nr_doc, data_doc, valoare_doc, cota_tva_doc, suma_tva_doc, afara_ded_serv, baza_txinv_cump, tva_txinv_cump, baza_txinv_cump_9, tva_txinv_cump_9, baza_19_reg, tva_19_reg, baza_alte, tva_alte)
select factura numar, min(dataf) data, (case when tipDoc<>'FA' then v.tert else '' end) cod_tert
	...
	,sum(case when coloana=6 and v.cota_tva=24 and year(v.dataDoc)>=2016 then baza else 0 end) baza_19_reg
	,sum(case when coloana=6 and v.cota_tva=24 and year(v.dataDoc)>=2016 then tva else 0 end) tva_19_reg
	,sum(case when coloana=6 and v.cota_tva<>20 then baza else 0 end) baza_alte, sum(case when coloana=6 and v.cota_tva<>20 then tva else 0 end) tva_alte
from #vert v
...
if @DetalDoc=1
	insert #jtvavanz (numar, data, cod_tert, beneficiar, codfisc, total, baza_19, tva_19, baza_9, tva_9, baza_5, tva_5
		,baza_txinv, tva_txinv, regim_spec, afara_ded, afara_fara, scutite_intra_ded_1
		,scutite_intra_ded_2, scutite_ded_alte, scutite_fara, neimpozabile, explicatii, detal_doc
		,care_jurnal, tip_doc, nr_doc, data_doc, valoare_doc, cota_tva_doc, suma_tva_doc, afara_ded_serv
		,baza_txinv_cump, tva_txinv_cump, baza_txinv_cump_9, tva_txinv_cump_9, baza_19_reg, tva_19_reg)
	...
	sum(valoareDoc) valoare_doc, cota_tva cota_tva_doc, sum(case when coloana in (11,12,13,14,15,16,17,18,22) then 0 else tvaDoc end) suma_tva_doc, 0, 0, 0, 0, 0, 0, 0
	from #vert v
	left outer join terti t on v.tipDoc<>'FA' and v.sub=t.subunitate and v.tert=t.tert
...
select rtrim(numar) numar, data, rtrim(r.cod_tert) cod_tert, rtrim(beneficiar) beneficiar, rtrim(codfisc) codfisc
	,total, baza_19+(case when @dinCGplus=1 then baza_txinv_cump else 0 end)-(case when @dinCGplus=1 then 0 else baza_alte end) baza_19
	,tva_19+(case when @dinCGplus=1 then tva_txinv_cump else 0 end)-(case when @dinCGplus=1 then 0 else tva_alte end) tva_19, 
	(case when @Provenienta='1' then baza_5 else baza_9+(case when @dinCGplus=1 then baza_5 else 0 end)+(case when @dinCGplus=1 then baza_txinv_cump_9 else 0 end) end) baza_9, 
	(case when @Provenienta='1' then tva_5 else tva_9+(case when @dinCGplus=1 then tva_5 else 0 end)+(case when @dinCGplus=1 then tva_txinv_cump_9 else 0 end) end) tva_9, baza_5, tva_5
	,baza_txinv, tva_txinv, regim_spec, afara_ded, afara_fara, scutite_intra_ded_1
	,scutite_intra_ded_2, scutite_ded_alte, scutite_fara, neimpozabile, explicatii, detal_doc
	...
	,baza_alte+baza_5 baza_alte, tva_alte+tva_5 tva_alte, 1-isnull(it.zile_inc, 0) as tert_intern
from #jtvavanz r
...
order by (case when @OrdDenTert=1 then beneficiar+numar+convert(char(10),data,102) else convert(char(10),data,102)+numar end), data, numar, beneficiar, detal_doc, data_doc, tip_doc, nr_doc
