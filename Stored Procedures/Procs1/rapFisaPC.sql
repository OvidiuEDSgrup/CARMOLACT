...
create procedure rapFisaPC (@pLM varchar(20), @pCom varchar(20), @pCentru varchar(20)=null, @pNivelMax int, @pDetalDOC varchar(20),
	 @pDinf datetime, @pDsup datetime, @PeConturi int=0, @PeLuni int=0,  @art_calc_multiplu varchar(100),
	 @art_calc_nedet varchar(100),@art_calc_excep varchar(100), @siCoduri bit=0, @detaliat bit=1, @faraNivel int=-1)
as
begin
--	select @faraNivel=1
...
begin
	...
	if @maiSuntDate=1
		update #fisacmdttmp set Descriere=left('(com. ' + RTrim(@ComCurenta) + ',lm ' + RTrim(@LMCurent) + ') ' + Descriere, 100)
		where nivel=1
	...
	begin
		...
		Pret, Valoare, Tip, Cod, Locm, comanda_sup, art_sup, NrOrdP, unic, 0, 0
		from #fisacmdttmp
	end
	else begin
...
begin
	...
	from #fisatmp f
		cross apply (select top 1 luna, lunaalfa from calstd c where rtrim(c.lunaalfa)+' '=left(f.lunaanalfa,len(rtrim(c.lunaalfa)+' '))) t
