...
declare @Sb char(9),@CotaTVA int,@RotRM int,@Ct4426 varchar(40),@Ct4428 varchar(40),@CtTvaNeexPlati varchar(40),@AccImpDVI int,@ContFactVama int,@Bugetari int
	...
	,@data01072016 datetime	-->	data modificarii declaratiei 394, in raport cu care achizitiile in D394 nu mai trebuie declarate in raport cu data facturii ci cu data documentului.
	,@D390 int	-->	la apel TVACumparari dinspre Declaratia390, trebuie sa se pastreze filtrarea documentelor dupa data facturii.
...
select @marcaj=isnull(@parXML.value('(row/@marcaj)[1]','int'),0), @RPTVACompPeRM=isnull(@parXML.value('(row/@RPTVACompPeRM)[1]','int'),0)
	...
	,@data01072016='07/01/2016'
	,@D390=isnull(@parXML.value('(/*/@D390)[1]', 'int'),0)
...
select a.subunitate,a.numar+'  ',a.numar+space(5),case when a.tip='RP' then 'RM' else a.tip end,
	...
	(CASE WHEN a.cota_tva<>0 or @TipCump=9 and @DataS>=@data01072016 and @D390=0 then (case when @RecalcBaza=0 then /*(case when a.tip='RS' and a.pret_valuta<0.01 then round(convert(decimal(17,2),a.TVA_deductibil*100/@CotaTVA),2) 
		else*/ round(convert(decimal(17,5),a.cantitate*(case when a.tip='RM' and a.jurnal='RC' then round(a.pret_valuta*(1-convert(decimal(12,0),a.cota_TVA)/convert(decimal(12,0),100+a.cota_TVA)),5) 
	...
	round(convert(decimal(17,5),a.tva_deductibil/(case when @nTVAned in (0,1) and a.procent_vama=5 then 2 else 1 end)),2) as tva_22,space(50),'',/*(case when 1=0 and a.tip='RS' and a.pret_valuta<0.01 then @CotaTVA else*/ a.cota_tva /*end)*/,
	...
	a.Cont_de_stoc, a.idPozDoc, a.loc_de_munca
from pozdoc a
...
where a.subunitate=@Sb and a.tip in ('RM','RP','RS','RQ') 
	...
	and ((@TipCump<>9 or @DataS>=@data01072016 and @D390=0 or a.data_facturii='1901-1-1') and a.data between @DataJ and @DataS 
		or @TipCump=9 and (@DataS<@data01072016 or @D390=1) and a.Data_facturii between @DataJ and @DataS)
	and a.cont_factura like rtrim(@ContFFlt)+'%' and a.cont_factura<>'' 
	and @nTVAned in (2, (case when (a.tip in ('RS','RP') or a.tip='RM' and a.numar_DVI='') and (a.procent_vama in (2,3) or a.procent_vama=5 and @nTVAned=1) then 1 else 0 end)) 
...
where @nTVAex in (0,2) and @nTVAned in (0,2) and a.subunitate=@Sb and a.tip='AI' and a.tva_deductibil<>0 and @TVAAlteCont<>1 
	and ((@TipCump<>9 or @DataS>=@data01072016 and @D390=0 or a.Data_facturii='1901-1-1') and a.data between @DataJ and @DataS 
		or @TipCump=9 and (@DataS<@data01072016 or @D390=1) and a.Data_facturii between @DataJ and @DataS) 
	and a.cont_intermediar like rtrim(@ContFFlt)+'%' 
...
--	s-a inlocuit apelul functiei docTVACumpDVI cu continutul ei
		...
		select b.subunitate,b.numar_receptie,b.numar_receptie,(case when e.tip in ('RM','RS') then 'RM' else 'MI' end),(case when @OrdDataDoc=1 then b.data_receptiei else isNull(e.Data_facturii,b.data_receptiei) end),
			...
			(case when b.total_vama=1 then 1 else 0 end),'C',0,(case when e.tip in ('RM','RS') then 'RM' else 'MI' end),'','',b.cont_tert_vama,null,0,e.Data_facturii,null,null,e.loc_munca
		from DVI b
			left outer join doc e on b.subunitate=e.subunitate and e.tip in ('RM','RS') and b.numar_receptie=e.numar and b.data_DVI=e.data 
		where @nTVAex in (0,(case when b.total_vama=1 then 1 else 2 end)) and @nTVAned in (2,(case when b.total_vama=2 then 1 else 0 end)) and b.factura_comis in ('','D') and b.subunitate=@Sb 
			and ((@TipCump<>9 or @DataS>=@data01072016 and @D390=0 or e.Data_facturii='1901-1-1') and b.data_DVI between @DataJ and @DataS 
				or @TipCump=9 and (@DataS<@data01072016 or @D390=1) and e.Data_facturii between @DataJ and @DataS)	
			and b.cont_tert_vama like rtrim(@ContFFlt)+'%' 
...
--	s-a inlocuit apelul functiei docTVACumpAdoc cu continutul ei
	...
	select a.subunitate,a.numar,a.cont,'PI',(case when a.plata_incasare='PC' then isnull(a.detalii.value('(/row/@datafact)[1]','datetime'),isnull(c.data,a.data)) else a.Data end),a.factura,a.tert,
		(case when a.Plata_incasare in ('PC','PR') and a.suma=a.TVA22 and a.TVA11<>0 
			...
			then (case when left(a.numar,4)='ITVA' and abs(isnull(nullif(a.detalii.value('(/row/@valachitat)[1]','decimal(12,3)'),0),a.Curs))>=0.01 
		...
		null,a.idPozplin, a.loc_de_munca
	from pozplin a
	...
	select a.Subunitate,a.Numar_document,a.Numar_document,a.Tip,a.Data_fact,a.factura_dreapta,a.Tert,
		...
		(case when (a.TVA11=0 or a.TVA22=0) and (@TipCump<>9 or @DataS<@data01072016 and @utilizator<>'lucian') then 0 else (case when @RecalcBaza=0 then a.suma else round(a.TVA22 *100/a.TVA11,2) end) end),a.TVA22,'','',(case when a.TVA22=0 then 0 else a.TVA11 end),
		0,0,a.data,'','',(case when 1=0 and a.tip='FF' and a.tert_beneficiar<>'' then a.tert_beneficiar else @Ct4426 end),a.cont_deb,(case when a.stare=1 then 1 else 0 end),'C',a.numar_pozitie,a.tip,'',
		(case a.tip when 'SF' then a.factura_stinga else '' end),a.cont_cred, a.detalii, a.stare, a.Data_fact,null,a.idPozadoc, a.loc_munca
	from pozadoc a
	where a.subunitate=@Sb 
		and ((@TipCump<>9 or @DataS>=@data01072016 and @D390=0 or a.Data_fact='1901-1-1') and a.data between @DataJ and @DataS 
			or @TipCump=9 and (@DataS<@data01072016 or @D390=1) and a.Data_fact between @DataJ and @DataS)
...
--	Lucian: Am eliminat filtrarea dupa alte conturi de TVA, ramine de mutat in procedura rapJurnalTvaCumparari
	...
	select a.Subunitate,a.Numar_document,a.Numar_document,a.Tip,a.Data_fact,a.Factura_dreapta,a.Tert,-a.Suma+(case when 1=0 and a.TVA22<0 then -1 else 1 end)*a.TVA22,
		...
		(case when 1=0 and a.TVA22<0 then 1 else -1 end)*a.TVA22,'','',@CotaTVA,0,0,a.data,'','',@Ct4426,a.cont_deb,0,'C',a.numar_pozitie,a.tip,'','',a.cont_cred, a.detalii, 0, null, null, a.idPozadoc, a.loc_munca
	from pozadoc a
	...
	select a.subunitate,a.numar_document,a.numar_document,(case when a.tip_miscare='IAF' then 'MI' else 'MM' end),a.data_miscarii,a.factura,a.tert,
		...
		(case when 1=1 or a.tip_miscare='MFF' then isnull((select max(cont_mijloc_fix) from fisaMF where subunitate=@Sb and numar_de_inventar=a.numar_de_inventar and felul_operatiei='3'),'212') 
			else a.subunitate_primitoare end),0,'C',0,(case when a.tip_miscare='IAF' then 'MI' else 'MM' end),'','',a.cont_corespondent, null, 0, null, null, null, null
	from misMF a 
		left outer join facturi c on c.tip=0x54 and a.subunitate=c.subunitate and a.factura=c.factura and a.tert=c.tert 
...
if @FaraVanz=1 and exists (select 1 from sysobjects where [type]='TF' and [name]='docTVACumpSP')
	insert #tvacump select *,null,0,null,null,null,null from dbo.docTVACumpSP(@Sb,@DataJ,@DataS,@ContFFlt,@Gest,@LM,@LMExcep,@Jurnal,@ContCor,@TVAnx,@RecalcBaza,@nTVAex,@FFFBTVA0,@SFTVA0,@IAFTVA0,@TipCump,@TVAAlteCont,@Tert,@Factura,@CotaTVA,@Ct4426,@Ct4428,@nTVAned)
...
set @parXMLSP=(select @DataJ as datajos, @DataS as datasus, @TipCump as tipCump, @UnifFact as uniffact for xml raw)
...
if @faraReturnareDate=0
	select 
		...
		ordonare, drept_ded, cont_TVA, cont_coresp, exonerat, vanzcump, numar_pozitie, tipDoc, cod, factadoc, contf, detalii, tip_tva, dataf, cont_de_stoc, idpozitie, lm
	from #tvacump 
end
