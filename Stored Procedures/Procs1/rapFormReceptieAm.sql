...
set transaction isolation level read uncommitted
	...
	create table [dbo].[#PozDocFiltr] ([Numar] [varchar](20) NOT NULL, [Cod] [varchar](20) NOT NULL, [Data] [datetime] NOT NULL, 
		...
		[Data_facturii] [datetime] NOT NULL, [numar_pozitie] [int], [Utilizator] [varchar](200),idpozdoc int
		)

	insert into #PozDocFiltr (		
		Numar, Cod, Data, Gestiune, Cantitate, Pret_valuta, Pret_de_stoc, Pret_cu_amanuntul, 
		TVA_deductibil, tva_neexigibil, Tert, Factura, Data_facturii, Numar_Pozitie, Utilizator,idpozdoc
		)
	select
	...
	if exists(select 1 from #PozDocFiltr p inner join gestiuni g on p.Gestiune=g.cod_gestiune and g.pret_am=1)
	begin
		...
		update #PozDocFiltr set pret_cu_amanuntul=pa.pret_amanunt,tva_neexigibil=pa.cota_tva
		from #preturiam pa where pa.idpozdoc=#PozDocFiltr.idpozdoc
	end
	...
	/*
	--Nu cred ca mai este necesara deoarece am facut mai sus update-ul pe pret cu amanuntul
	...
	*/
	...
	select
		...
		round(pz.cantitate * (pz.pret_cu_amanuntul - round(pz.Pret_cu_amanuntul*pz.tva_neexigibil/(100.00+pz.tva_neexigibil),2) - pz.pret_de_stoc), 2) as AD,
		round(pz.cantitate * round(pz.Pret_cu_amanuntul*pz.tva_neexigibil/(100.00+pz.tva_neexigibil),2),2) as TVANEEX,
		...
		round((select sum(round(p.cantitate * (p.pret_cu_amanuntul- round(p.Pret_cu_amanuntul*p.tva_neexigibil/(100.00+p.tva_neexigibil),2) - p.pret_de_stoc),2)) from #pozdocFiltr p), 2) as TAD,
		round((select sum(round(p.cantitate * round(p.Pret_cu_amanuntul*p.tva_neexigibil/(100.00+p.tva_neexigibil),2),2)) from #pozdocFiltr p), 2) as TTVANEEX,
