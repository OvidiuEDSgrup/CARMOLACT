...
begin try
	...
	(marca char(6), nume char(50), cod_functie char(6), den_functie char(80), lm char(9), den_lm char(30), vechime_totala char(8), data_angajarii datetime, 
	...
	select a.marca, max(p.nume), max(p.cod_functie), max(f.denumire), max(p.Loc_de_munca), max(lm.denumire), 
		max((case when @ZileCOVechUnit=0 then dbo.fVechimeAALLZZ(p.vechime_totala) else isnull(p.vechime_la_intrare,ip.vechime_la_intrare) end)) as vechime_totala, 
