...
--	Creez tabela temporara in care extrag datele din extinfop si le aranjez pe coloana pt. fiecare salariat; merge mai rapid decat cu subselect
	...
	from istpers i 
		...
		inner join infoPers ip on ip.marca = i.marca 
	...
	where ((i.data=@dataSus or i.Data=@dataSusNext and p.Data_angajarii_in_unitate>@dataSus) --and (convert(char(1),p.loc_ramas_vacant)='0' or (p.Data_plec>=@DataRegistru or p.Data_plec>=@dataJos)) 
		...
		and (@Bugetari=0 or isnull(p.detalii.value('(/row/@functpublic)[1]','int'),convert(int,ip.Actionar))=0) and (p.Data_angajarii_in_unitate<=@DataRegistruPtAng or r.DataIncheiere<=@DataRegistru)
	...
	if exists (select * from sysobjects where name ='genRevisalSalariatiStergPlecati' and xtype='P')
		exec genRevisalSalariatiStergPlecati @dataJos=@dataJos, @dataSus=@dataSus, @DataRegistru=@DataRegistru
