...
--	Date asigurati
	if @dataset='Asigurat'
	begin
		if OBJECT_ID('tempdb..#asigurati') is null --	daca se apeleaza procedura din subreport sa nu mai creeze #asigurati. Doar sa filtreze pe CNP.
		begin
		...
		if exists (select * from sysobjects where name = 'rapDeclaratia112Anexa12SP' and xtype='P')
			exec rapDeclaratia112Anexa12SP @sesiune=@sesiune, @datalunii=@datalunii, @marca=@marca, @cnp=@cnp, @dataset=@dataset, @parXML=@parXML
		
		select Data, cnpAsig, idAsig, numeAsig, prenAsig, cnpAnt, numeAnt, prenAnt, dataAng, dataSf, casaSn, asigCI, asigSO
		from #asigurati
		where (@cnp is null or cnpAsig=@cnp)
		order by (case when @alfabetic=1 then numeAsig else cnpAsig end)
	end
