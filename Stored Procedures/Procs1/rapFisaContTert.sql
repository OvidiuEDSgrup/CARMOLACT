--***
create procedure rapFisaContTert(@sesiune varchar(50)=null, @cFurnBenef varchar(1),@cDataJos datetime,@cDataSus datetime,
	@cuefecte bit=0,	--> sa fie aduse facturile achitate prin efecte: 0, null = nu se aplica, 1 = se aduc doar cele cu efecte neachitate
	@cTert varchar(50)=null,@cContTert varchar(40),
...
/**1.	creare cursor pentru a imparti datele mai usor si mai rapid in continuare pe sold si pe rulaj*/
	...
	set @parXMLFact=(select @cFurnBenef as furnbenef, convert(char(10),@cDataJos,101) as datajos, convert(char(10),@cDataSus,101) as datasus, 
		isnull(@cuefecte,0) as efecteachitate,
		rtrim(@cTert) as tert, rtrim(@cFactura) as factura, rtrim(@cContTert) as contfactura, @lm as locm, 1 as prelconttva, @grupa as gtert,
	...
	select
		t.denumire,t.grupa,ft.furn_benef, ft.subunitate, rtrim(ft.tert) tert, ft.factura, ft.tip, ft.numar, ft.data,
		ft.valoare valoare, ft.tva tva,
		-ft.achitare_efect_in_curs achitat, ft.valuta, ft.curs, 
		ft.total_valuta total_valuta, ft.achitat_valuta achitat_valuta,
		ft.loc_de_munca, 
		isnull(f.comanda,ft.comanda) as comanda,
		ft.cont_de_tert, ft.fel, ft.cont_coresp, ft.explicatii, ft.numar_pozitie,
	...
	where 
		...
		and (@tipinc is null or exists (select nr_doc from incfact e 
			where ft.subunitate=e.subunitate and ft.tert=e.tert and ft.factura=e.numar_factura 
		and e.mod_plata=@tipinc))
		and (@cuefecte=1 and abs(ft.achitare_efect_in_curs)>0.001)
...
--/*
	...
	if exists (select 1 from sys.objects o where name='rapFisaContTertSP1')
	begin
		declare @pxml xml
		select @pxml=(select @cFurnBenef furnbenef, convert(varchar(10),@cDataJos,101) datajos, convert(varchar(10),@cDataSus,101) datasus for xml raw)
		exec rapFisaContTertSP1 @sesiune=@sesiune, @parXML=@pXML
	end
