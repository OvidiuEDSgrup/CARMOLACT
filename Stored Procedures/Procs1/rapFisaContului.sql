...
create procedure rapFisaContului(
	...
	,@subtotaluri int=0	-->	0= fara, 1=pe jurnale, 2=pe locuri de munca, 3=pe indicatori, 4=pe luni
...
create table #prelExplicatii(cont varchar(40), denumire_cont varchar(500), cont_parinte varchar(40), tip_document varchar(20),
	...
	,loc_de_munca varchar(100) default '')
...
insert into #prelExplicatii(cont, denumire_cont, cont_parinte, tip_document, numar_document, data, cont_debitor, cont_creditor,
	...
	sold_cred_valuta, explicatii, numar, jurnal, ID, subtotal, tip_cont, are_analitice, are_rulaje, valuta, numar_pozitie, loc_de_munca)
	select rtrim(cont_debitor) as cont, isnull(rtrim(c.denumire_cont), '') as denumire_cont, isnull(c.cont_parinte, '') as cont_parinte, tip_document, 
		...
		(case @subtotaluri when 1 then a.jurnal
				...
				when 4 then convert(varchar(10),year(a.data))+'-'+(case when month(a.data)<10 then '0' else '' end)+convert(varchar(10),month(a.data))
...
insert into #prelExplicatii(cont, denumire_cont, cont_parinte, tip_document, numar_document, data, cont_debitor, cont_creditor,
	suma_deb, suma_cred, sold_deb, sold_cred, suma_deb_valuta, suma_cred_valuta, sold_deb_valuta,
	sold_cred_valuta, explicatii, numar, jurnal, ID, subtotal, tip_cont, are_analitice, are_rulaje, valuta, loc_de_munca)
...
insert into #fisa(cont, denumire_cont, cont_parinte, tip_document, numar_document, data, cont_debitor, cont_creditor, suma_deb, suma_cred, sold_deb
		...
		,are_rulaje, valuta, numar_pozitie, loc_de_munca)
select cont, denumire_cont, cont_parinte, tip_document, numar_document, data, cont_debitor, cont_creditor, isnull(suma_deb,0), isnull(suma_cred,0),
	...
	rtrim(isnull(subtotal,'')), tip_cont, convert(int,are_analitice) are_analitice, convert(int,are_rulaje) are_rulaje, valuta, numar_pozitie, loc_de_munca
from #final
...
begin
	set @comanda_str='
	update f set den_subtotal=s.lunaalfa+'' ''+convert(varchar(20),year(f.data))
	from #fisa f
		inner join (select f.luna, rtrim(f.lunaalfa) lunaalfa from fcalendar(''2015-1-1'',''2015-12-31'') f group by f.luna, f.lunaalfa)
			s on month(f.data)=s.luna'
	exec (@comanda_str)
end
...
begin
	...
	select cont, max(denumire_cont) denumire_cont, max(cont_parinte) cont_parinte, '' tip_document, '' numar_document, '' data, '' cont_debitor, '' cont_creditor,
		...
		,convert(decimal(15,2), (case when @subtotaluri=1 then 0 else sum(sold_deb_valuta)-sum(sold_cred_valuta) end)) as sold_initial_valuta,
			convert(decimal(15,2), (case when @subtotaluri=1 then 0 else sum(suma_deb_valuta)-sum(suma_cred_valuta)+sum(sold_deb_valuta)-sum(sold_cred_valuta) end)) as sold_final_valuta, '' loc_de_munca
...
--/*
	...
	select cont, max(denumire_cont) denumire_cont, max(cont_parinte) cont_parinte, '' tip_document, '' numar_document, '' data, '' cont_debitor, '' cont_creditor,
		...
		,sum(sold_deb_valuta)-sum(sold_cred_valuta) sold_initial_valuta,
			sum(isnull(suma_deb_valuta,0))-sum(isnull(suma_cred_valuta,0))+sum(sold_deb_valuta)-sum(sold_cred_valuta) sold_final_valuta, '' loc_de_munca
	from #raport r where nivel=0
	group by r.cont
