...
if 	@numeTabela='#docfac'
	alter table #docfac
		add	subunitate char(9), tert char(13), factura char(20), tip char(2), numar char(20), data datetime, valoare float, tva float, achitat float, 
			...
			cont_de_tert varchar(40), fel int, cont_coresp varchar(40), explicatii char(80), numar_pozitie int, gestiune char(13), 
...
if 	@numeTabela='#pfacturi'
	alter table #pfacturi
		add	loc_de_munca varchar(13), tip binary, factura varchar(20), tert varchar(13), data datetime, data_scadentei datetime, 
			...
			achitat_valuta float, sold_valuta float, comanda varchar(40), data_ultimei_achitari datetime, achitat_interval float, achitat_interval_plata float, explicatii varchar(2000), contract varchar(20)
