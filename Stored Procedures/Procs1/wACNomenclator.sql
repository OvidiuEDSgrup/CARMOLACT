...
declare @FltStocPred int, @faraU bit, @searchText varchar(80), @subunitate varchar(9), @tip varchar(2), @gestiune varchar(20),@gestutiliz varchar(20), @categoriePret int,
		...
		@tipnom_tipgest int, @tip_gest char(1)
...
if @tipnom_tipgest=1 and @gestiune<>''
	select @tip_gest=tip_gestiune from gestiuni where subunitate=@subunitate and Cod_gestiune=@gestiune	
declare @tipGest char(1)
set @tipGest=(case when @tip in ('PF','CI','AF') then 'F' else (select top 1 tip_gestiune from gestiuni where Subunitate=@subunitate and Cod_gestiune=@gestiune) end)
...
left join stocuri on @gestiune!='' 
	and stocuri.Subunitate=@subunitate and stocuri.Tip_gestiune=@tipGest and stocuri.Cod=nomencl.cod and stocuri.Cod_gestiune=@gestiune
	and (@lista_gestiuni=0 or exists (select 1 from @gestuniUtiliz gu where gu.gestiune=stocuri.cod_gestiune))  --Se filtreaza pe gestiunile provenite din proprietati
...
and (@tip_gest is null or tip=@tip_gest) 
...
having (@FltStocPred=0 or nomencl.tip in ('R','S') or @tip not in ('PV','AP', 'AC', 'CM', 'TE', 'DF', 'AE', 'PF', 'CI') or @gestiune!='' and sum(ISNULL(stocuri.stoc, 0))>=0.001)
