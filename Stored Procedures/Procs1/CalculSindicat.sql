...
--	pun intr-o tabela temporara datele pt. calcul
	Select p.marca, p.salar_de_incadrare as SalarIncadrare, p.salar_de_baza as SalarBaza, p.sindicalist, 
	...
	isnull(b.ore_lucrate_regim_normal+(case when isnull(substring(br.cod_fiscal,10,1),'')='4' then b.ore_nelucrate else 0 end),0) as OreLucrate, isnull(b.total_ore_nelucrate,0) as total_ore_nelucrate,
	...
	(case when ms.Data_inf is not null 
		then round(isnull(i.Salar_de_incadrare,p.Salar_de_incadrare)/(@OreLuna/8)*dbo.zile_lucratoare(@dataJos,ms.Data_inf-1),0)
			+round(p.Salar_de_incadrare/(@OreLuna/8)*dbo.zile_lucratoare(ms.Data_inf,@dataSus),0) 
	...
	from personal p 
		...
		outer apply (select marca, sum(venit_total) as venit_total, sum(Suma_impozabila+Suma_imp_separat+Cons_admin) as sume_exceptate, 
			...
			sum(ore_concediu_de_odihna+ore_obligatii_cetatenesti+ore_concediu_medical+ore_intrerupere_tehnologica) as total_ore_nelucrate
...
--	calculez valoarea sindicatului conform setarilor
	...
	if exists (select 1 from sysobjects where [type]='P' and [name]='CalculSindicatSP')
		exec CalculSindicatSP @dataJos, @dataSus, @marca, @lm
...
--	calcul SP varianta preluata de la procedura de calcul anterioara.
