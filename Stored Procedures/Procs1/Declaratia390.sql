...
*/
	...
	exec TVACumparari @DataJ=@datajos, @DataS=@datasus, @ContF='', @Gest='', @LM='', @LMExcep=0, @Jurnal='', @ContCor='', @TVAnx=0, @RecalcBaza=0
			...
			,@OrdDataDoc=0, @Tert=@tert, @Factura='', @UnifFact=0, @FaraVanz=1, @nTVAned=2, @parXML='<row D390="1" />'
	...
	--> compunere continut XML
		select @continutXml=(
			select 'mfp:anaf:dgti:d390:declaratie:v1' as [@nu_am_alte_idei_decat_replace_pe_string]
				...
				,rtrim(@fax) [@fax], rtrim(@mail) [@mail], (select sum(baza)+count(distinct codfisc+'|'+tipop+'|'+rtrim(dentert))-1 from #D390) as [@totalPlata_A],
				(select 
