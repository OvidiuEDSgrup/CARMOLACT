...
WHERE (
	...
	AND n.tert = @Tert
	AND (
		ISNULL(n.detalii.value('(/row/@invalid)[1]', 'varchar(200)'), '') = '' OR
			NOT (GETDATE() BETWEEN n.detalii.value('(/row/@data_invalid_jos)[1]', 'datetime') AND n.detalii.value('(/row/@data_invalid_sus)[1]', 'datetime'))
	)
