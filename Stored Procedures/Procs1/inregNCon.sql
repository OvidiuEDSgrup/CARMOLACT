...
as 
	...
	select 
		...
		rtrim(explicatii) explicatii, utilizator, 
		...
		'' ora_operarii, loc_munca, left(comanda,20) as comanda, tert, jurnal, space(20) as indbug, detalii, idPozncon
	into #tmppozncon
	from pozncon p
	...
	begin try	
		...
		delete from #pozincon 
			where convert(decimal(12,2),abs(suma))<0.01 and convert(decimal(12,2),abs(Suma_valuta))<0.01
		...
		delete dc 
			...
			where dc.tip in ('IC','MA','ME','MI','MM','NC','DP','AO','AT','PS','UA')

		delete #DocDeContat where tip in ('IC','MA','ME','MI','MM','NC','DP','AO','AT','PS','UA')
