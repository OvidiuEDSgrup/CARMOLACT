...
as
	...
	*/
	declare 
		@userASiS varchar(20), @mesaj varchar(200), @datajos datetime,@datasus datetime,@subunitate char(13),@contcheltfurn varchar(40),
		@contvenfurn varchar(40),@contcheltben varchar(40),@contvenben varchar(40),@aninchis int,@lunainchisa int,@user char(10),
		@validcomstrict int,@comandaGenerica varchar(20),@maxpoz int,@cmaxnumar varchar(10),@maxnumar int, @parXMLFact xml, @multiFirma int, 
		@lmUtilizator varchar(20), @lm varchar(50)
	...
	begin
		...
		into #doc_sterse(subunitate, tip, numar_document, data) 
			...
			and (@multiFirma=0 or loc_munca like rtrim(@lm)+'%')
	...
	begin
		...
		*/
		...
		--	trimitem la pFacturi @datajos='01/01/1921' in loc de NULL, pentru a lua in calcul facturile incepand cu ultimul an initializat.
		--	Daca se trimite NULL se iau in calcul documentele anterioare ultimului an initializat si din cauza de modificare (corectat) rotunjiri in pFacturi pot sa apara alte valori in valuta la factura.
		set @parXMLFact=(select @tipcor as furnbenef, '01/01/1921' as datajos, convert(char(10),@datasus,101) as datasus, 1 as cen, rtrim(@partert) as tert, rtrim(@parconttert) as contfactura for xml raw)
