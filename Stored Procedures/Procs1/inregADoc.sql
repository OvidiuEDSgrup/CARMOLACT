...
as 
	...
	select 
		...
		p.utilizator, convert(datetime,convert(datetime,convert(varchar(10),p.data_operarii,101),101),108)
		...
		'' ora_operarii, p.loc_munca, left(p.comanda,20) as comanda, p.tert, p.jurnal, p.stare tip_tva, p.tva11 cota_tva, convert(decimal(17,5), p.tva22) suma_tva, convert(decimal(17,5), dif_tva) dif_tva,
		p.idPozadoc as idpozadoc, space(20) as indbug, 0 as dedublareCOmpBug, rtrim(isnull(p.detalii.value('(/row/@contintermediar)[1]', 'varchar(40)'),'')) cont_intermediar 
	into #pozadoc 	
	from pozadoc p
	...
	begin
		...
		insert into #pozadoc (subunitate, tip, numar, data, cont_debitor, cont_creditor, cont_diferenta, suma_dif, suma, valuta, curs, suma_valuta, explicatii, factura_stanga, factura_dreapta, 
			Data_fact, cont_factura, utilizator, data_operarii, ora_operarii, loc_munca, comanda, tert, jurnal, tip_tva, cota_tva, suma_tva, dif_tva, idpozadoc, indbug, dedublareCOmpBug,cont_intermediar)
	...
	*/
	...
	select
		...
		(case when tip='FF' and cont_intermediar<>'' then cont_intermediar else cont_debitor end),cont_creditor, 
	...
	*/
	insert into #pozincon(TipInregistrare, Subunitate, Tip, Numar, Data, Valuta, Curs, Suma_valuta, Utilizator, Loc_de_munca, Comanda, Jurnal,Cont_debitor, Cont_creditor, Suma,Explicatii, Data_operarii, Ora_operarii, Indbug)
	select
		'POZI',subunitate, tip, numar, data, valuta, curs, round(Suma_valuta,2), utilizator, loc_munca, comanda, jurnal, cont_debitor, cont_intermediar, round(suma,2), explicatii, Data_operarii, Ora_operarii, indbug
	from #pozadoc
	where tip='FF' and ISNULL(cont_intermediar,'')<>''
