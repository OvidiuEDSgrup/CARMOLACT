...
--	daca tabela par este view inseamna ca se lucreaza cu parametrii pe locuri de munca (in aceeasi BD sunt mai multe unitati)	
	...
	from contorOperatiiSalarii where (tip=@tipOperatie or @tipOperatie in ('CS','RS') and tip='CL') and data_lunii=@datal and (@multiFirma=0 or Loc_de_munca=@locm) 
		and DateDiff(mi,data,getdate())<=@durCalcul order by data desc
	...
	set @testMesaj=(case when @tipOperatie='CL' then 'calcul de lichidare' when @tipOperatie='CS' then 'calcul de salarii' when @tipOperatie='RS' then 'raport' else '' end)
	...
	/*	Apelam procedura specifica, pentru a putea modifica eventual durata. Sau pentru alte cazuri de validare. */
	if exists (select * from sysobjects where name ='pContorizareOperatiiSalariiSP2')
		exec pContorizareOperatiiSalariiSP2 @sesiune=@sesiune, @parXML=@parXML OUTPUT
end try
