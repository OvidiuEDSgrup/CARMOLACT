...
Begin try
	...
	declare @Detaliere_retineri int, @Lichid_ret_avans int, @Ret_procent_tl int, @Op_ret_venit_net int, @Proc_ret_venit_net float, @ACorU_restpl int, @CAS_CorU int, 
			...
			@retinere_calculata float, @marca_ant varchar(6), @codBenAvMatImpozabile varchar(20), @NumarDocAvMatImp varchar(10), @Cod_benef_gm varchar(20)
	...
	if object_id('tempdb..#pontaj_marca_locm') is not null 
		drop table #pontaj_marca_locm
	if object_id('tempdb..#personalret') is not null 
		drop table #personalret
	if object_id('tempdb..#brutret') is not null 
		drop table #brutret
	if object_id('tempdb..#calculret') is not null 
		drop table #calculret
	if object_id('tempdb..#garantiiAnt') is not null 
		drop table #garantiiAnt
...
--	selectare garantii retinute anterior lunii de lucru (pentru cele care s-au retinut complet si s-a schimbat salarul) - trebuie sa stim valoarea retinuta.
	select * into #garantiiAnt 
	from (select r.data, r.marca, r.cod_beneficiar, r.data_document, r.numar_document, r.Valoare_totala_pe_doc, r.valoare_retinuta_pe_doc, RANK() over (partition by r.Marca order by r.Data Desc) as ordine
		...
		where r.Data<@DataSus and r.cod_beneficiar=@Cod_benef_gm and r.numar_document='GARANTII') a
	where Ordine=1
...
--	selectare date din resal in tabela temporara
	select r.data, r.marca, r.cod_beneficiar as cod_beneficiar, r.numar_document, r.valoare_totala_pe_doc as val_totala, r.valoare_retinuta_pe_doc as val_retinuta, 
		...
		isnull(r.detalii.value('(/row/@retinutsep)[1]','decimal(12,2)'),0)+isnull(c.retinut_la_lichidare,0) as ret_chitanta, isnull(d.valoare_retinuta_pe_doc,isnull(ga.valoare_retinuta_pe_doc,0)) as val_retinuta_ant, 
		isnull(d.retinere_progr_la_lichidare,0) as ret_progr_lich_ant, 
		...
		(case when @Salubris=1 or @Colas=1 or br.detalii.value('(/row/@prioritate)[1]','int') is not null 
			then isnull(br.detalii.value('(/row/@prioritate)[1]','int'),convert(int,left(br.cod_fiscal,9))) else 999 end) as Ord1, 
	...
	from resal r 
		...
		left outer join #garantiiAnt ga on ga.marca=r.marca and ga.cod_beneficiar=r.cod_beneficiar --and ga.numar_document=r.numar_document
...
--	pun index pe tabela pt. a functiona update-ul pe index (sper ca merge).
	create clustered index ordonare on #calculret (marca,Ord1,Ord2)
...
--	calcul retineri de tip procentual
	...
	if exists (select * from sysobjects where name ='CalculRetineriSiRestPlataSP2')
		exec CalculRetineriSiRestPlataSP2 @dataJos, @dataSus, @marca, @lm
...
-- s-a inlocuit procedura Calcul_retinut_la_lichidare cu partea de mai jos.
	...
	if @Op_ret_venit_net=1
		update cr 
			set @Salar_net_crt=round(n.venit_net*@Proc_ret_venit_net/100,0)
				-(n.avans+n.cm_incasat+n.co_incasat+n.suma_incasata)	--	aceasta diminuare trebuie analizata altfel in cazul in care se doreste calcul retinere la procent din net.
				-n.premiu_la_avans-n.diferenta_impozit+b.aj_deces-(case when cr.marca<>@marca_ant then 0 else @retinut_anterior end),
