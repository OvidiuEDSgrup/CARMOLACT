...
/**	procedura pentru actualizare 
		...
		spor vechime specialitate (@CeLaInchLuna=5) 
			- pentru ANAR, campul vechime_in_meserie din personal.detalii este utilizat ca si vechime in specialitate 
			- pentru ANAR, campul spor_cond_8 din personal este utilizat ca si spor pentru vechime in specialitate (pentru ANAR)
*/
...
Begin
	...
	Select p.Marca, p.Loc_de_munca, p.Zile_concediu_de_odihna_an, p.Data_angajarii_in_unitate, p.Loc_ramas_vacant, p.Data_plec, 
		p.Vechime_totala, isnull(p.Vechime_la_intrare,ip.Vechime_la_intrare), isnull(p.detalii.value('(/row/@vechimemeserie)[1]','varchar(10)'),ip.Vechime_in_meserie) as vechime_in_meserie, c.Vechime_totala, c.Vechime_la_intrare, c.Vechime_in_meserie, 
...
--		scriu vechime la intrare
		if (@CeLaInchLuna='1' and @InchidereLuna=1 or @CO=1) and (@ZileCOVechUnit=1 or @IncrementVechIntr=1) and @dataSus>=@DataAngajarii and (@Plecat=0 or @DataPlec>=@dataJos)
			...
			--update personal set Vechime_la_intrare=@vVechime_la_intrare where marca=@Marca and @CO=0	pana finalizam renuntarea la infoPers
...
--		scriu vechime in meserie
		...
		begin
			...
			/*
			update personal set detalii='<row/>' where marca=@Marca and @CO=0 and detalii is null and @vVechime_in_meserie<>''
			update personal set detalii.modify ('replace value of (/row/@vechimemeserie)[1] with sql:variable("@vVechime_in_meserie")') 
			where marca=@Marca and @CO=0 and detalii.value('(/row/@vechimemeserie)[1]','varchar(6)') is not null

			update personal set detalii.modify ('insert (attribute vechimemeserie {sql:variable("@vVechime_in_meserie")}) into (/row)[1]')
			where marca=@Marca and @CO=0 and detalii.value('(/row/@vechimemeserie)[1]','varchar(6)') is null
			*/
		end
...
--		scriu spor conditii 8 (vechime in specialitate la ANAR). Momentan nu se foloseste.
			if @CeLaInchLuna='5' and 1=0
				update p set p.Spor_cond_8=isnull(@vSpor_cond_8,p.Spor_cond_8) 
				from personal p 
				where p.marca=@Marca and p.Tip_colab not in ('DAC','CCC','ECT')
		End
		fetch next from ActSporuri into @Marca, @Loc_de_munca, @ZileCOan, @DataAngajarii, @Plecat, @DataPlec, 
			@Vechime_totala, @Vechime_la_intrare, @Vechime_in_meserie, @vVechime_totala, @vVechime_la_intrare, @vVechime_in_meserie, 
			@vSpor_vechime, @vSpor_specific, @vSpor_cond_8, @vZile_CO_ramase, @vZile_CO_an
