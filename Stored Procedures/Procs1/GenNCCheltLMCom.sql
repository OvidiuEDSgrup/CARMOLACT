...
Begin try
	...
	@RepManIndCom int, @RepIndCMUnitCom int, @RepManIndComTL int, @RepManIndComTLSiCorectii varchar(200), @NumaiPontajeAcord int, 
...
--	daca tabela par este view inseamna ca se lucreaza cu parametrii pe locuri de munca (in aceeasi BD sunt mai multe unitati)	
	...
	select 
		...
		@RepManIndComTLSiCorectii=max(case when Parametru='NC-MC-TLC' then Val_alfanumerica else 'C' end),
		...
		Begin
			...
			Begin
				...
				set @ValoarePoz=@ValoarePoz*(case when @ValRealizataMarca=0 then 1 
					...
					-(case when @RepManIndComTL=1 then @VenitNelucratMLM+(case when @RepManIndComTLSiCorectii='C' then 0 else @CorectiiMLM end) else 0 end))/@ValRealizataMarca end)
