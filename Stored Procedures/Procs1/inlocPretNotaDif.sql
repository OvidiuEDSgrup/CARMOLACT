--***
create procedure inlocPretNotaDif @datajos datetime,@datasus datetime as 
...
declare @cCont348 varchar(13), @AreAn348 int, @cContParinte348 varchar(13), @cCont711 varchar(13)
if @lNotaDifPret = 1 create table #x (cont varchar(13))
...
begin
	...
	begin
		...
		if @AreAn348 = 0 begin
			...
			set @cCont711 = RTrim('711') + (case when RTrim(@cContParinte348) <> '' then substring(@cCont348, len(RTrim(@cContParinte348)) + 1, 13) else '' end) 
		end
		else begin
		...
		set @cComandaSQL = 'insert into ' + RTrim(@cPozNCon) + '(Subunitate, Tip, Numar, Data, Cont_debitor, Cont_creditor, Suma, 
			...
			(case when ' + (case when @lNotaDifInv=1 then '1' else '0' end) + '=1 and pu.pret_unitar - pd.pret_de_stoc < 0 
				then ''' + RTrim(@cCont711) + ''' + (case when ' + convert(char(1), @AreAn348) + '=1 and RTrim(c.cont_parinte)<>'''' then substring(pd.cont_de_stoc, len(RTrim(c.cont_parinte)) + 1, 13) else '''' end) 
				else ''' + RTrim(@cCont348) + ''' + (case when ' + convert(char(1), @AreAn348) + '=1 and RTrim(c.cont_parinte)<>'''' then substring(pd.cont_de_stoc, len(RTrim(c.cont_parinte)) + 1, 13) else '''' end)
			end), 
			(case when ' + (case when @lNotaDifInv=1 then '1' else '0' end) + '=1 and pu.pret_unitar - pd.pret_de_stoc < 0 
				then ''' + RTrim(@cCont348) + ''' + (case when ' + convert(char(1), @AreAn348) + '=1 and RTrim(c.cont_parinte)<>'''' then substring(pd.cont_de_stoc, len(RTrim(c.cont_parinte)) + 1, 13) else '''' end) 
				else ''' + RTrim(@cCont711) + ''' + (case when ' + convert(char(1), @AreAn348) + '=1 and RTrim(c.cont_parinte)<>'''' then substring(pd.cont_de_stoc, len(RTrim(c.cont_parinte)) + 1, 13) else '''' end)
			end), 
