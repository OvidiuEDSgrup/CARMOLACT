...
Begin try
	...
	@Avans0SPL int, @GenerareCORI int, @ReCalcMZ int, @ReCalcCOTip78 int, @multiFirma int, @parXML xml, @parXMLCO xml
...
--	trebuie apelata procedura si la calcul salarii (nu doar la Inchidere de luna) pt. datele inregistrate dupa inchiderea lunii anterioare.
	...
	if @CalculCO=1
	begin
		if exists (select 1 from sysobjects where name ='CalculConcediiDeOdihna')
		begin
			set @parXMLCO=(select convert(varchar(10),@dataJos,101) as datajos, convert(varchar(10),@datasus,101) as datasus, rtrim(@marcaJos) as marca,
				rtrim(@locmJos) as lm, @ReCalcCOTip78 as recalccolant for xml raw)
			exec CalculConcediiDeOdihna @sesiune=null, @parXML=@parXMLCO
		end
		else 
