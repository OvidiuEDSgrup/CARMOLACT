...
as  
Begin try
	declare @utilizator varchar(20), @lista_lm int, @multiFirma int, @lmFiltru varchar(9), @lmUtilizator varchar(9), @nrLMFiltru int, @denlmUtilizator varchar(9), 
	...
	@Cod_declaratie varchar(3), @DataScad datetime, @vcif varchar(13), @cif varchar(13), @rgCom varchar(14), @caen varchar(4), @den varchar(200), @adrSoc varchar(1000), @localitate varchar(100), @judet varchar(100), 
...
--	citire date unitate
	...
	if @multiFirma=0 and @nrLMFiltru=1
		select @vcif=valoare 
		from proprietati
		where tip='LM' and cod_proprietate='CODFISCAL' and cod=@lmFiltru and Valoare<>''
	if isnull(@vcif,'')=''
		select @vcif=dbo.iauParA('PS','CODFISC')
	if @vcif=''
		set @vcif=dbo.iauParA('GE','CODFISC')
	select @rgCom=dbo.iauParA('GE','ORDREG'), @caen=dbo.iauParA('PS','CODCAEN'), @den=dbo.iauParA('GE','NUME'), 
		@adrSoc=dbo.iauParA('GE','ADRESA'), @localitate=dbo.iauParA('PS','LOCALIT'), @judet=dbo.iauParA('PS','JUDET'),
		@telSoc=dbo.iauParA('GE','TELFAX'), @faxSoc=(case when dbo.iauParA('GE','FAX')='' then dbo.iauParA('GE','TELFAX') else dbo.iauParA('GE','FAX') end), 
		@mailSoc=dbo.iauParA('GE','EMAIL'), @casaAng=dbo.iauParA('PS','CODJUDETA'),
		@tRisc=dbo.iauParLN(@dataSus,'PS','0.5%ACCM')
	set @adrSoc=rtrim(@adrSoc)+(case when @localitate<>'' then ' loc.'+rtrim(@localitate) else '' end)+(case when @judet<>'' then ' jud.'+rtrim(@judet) else '' end)
...
--	sterg tabelele temporare
		...
		if object_id('tempdb..#D112Asigurat') is not null drop table #D112Asigurat
...
--	Pun tabela istpers intr-o tabela temporara pt. a nu face filtru pe loc de munca in fiecare select de mai jos
		...
		where i.Data=@dataSus 
			and (@lm='' or i.Loc_de_munca like rtrim(@lm)+'%')
			and (@lista_lm=0 or lu.cod is not null) 
			and not (convert(int,p.Loc_ramas_vacant)=1 and p.Data_plec<@datajos and upper(left(isnull(p.detalii.value('(/row/@lmnou)[1]','varchar(100)'),ip.Loc_munca_nou),7))='DETASAT')
...
--	pun in tabela temporara ultima suspendare pentru ingrijire copil pana la 2 ani aferenta fiecarei marci
		...
		from fRevisalSuspendari ('01/01/1901', @dataSus, '') where Temei_legal in ('Art51Alin1LiteraA','Art51Alin1LiteraB','Art51Alin1LiteraC')) a
		where ordine=1
...
--	inserez idasig pentru asiguratii din zilieri
		...
		select top 0 * into #D112Asigurat from D112Asigurat
--	Populare tabele asigurati
		...
		select @dataSus as data, @lmUtilizator as loc_de_munca, rtrim(p.Cod_numeric_personal) as cnpAsig, rtrim(convert(char(6),max(id.idAsig))) as idAsig,
...
--	daca exista pe CNP o marca activa, DataSf trebuie sa fie necompletata
		...
		insert into D112Asigurat 
			(Data, Loc_de_munca, cnpAsig, idAsig, numeAsig, prenAsig, cnpAnt, numeAnt, prenAnt, dataAng, dataSf, casaSn, asigCI, asigSO)
		select Data, Loc_de_munca, cnpAsig, idAsig, numeAsig, prenAsig, cnpAnt, numeAnt, prenAnt, dataAng, dataSf, casaSn, asigCI, asigSO
		from #D112Asigurat
...
--	inserez asiguratii din zilieri
		...
		from #DateAsigurat da 
			left outer join #idasig id on id.CNP=da.CNP
			left outer join #D112Asigurat a on a.cnpAsig=da.CNP
		where 'asiguratB1' like rtrim(da.TagAsigurat)+'%' 
--	Daca asiguratul (CNP-ul) este activ trebuie sa aiba valori in sectiunea D112AsiguratB1 sau sa nu fie asigurat pentru somaj/CCI
--	Am pus aici conditia pentru cazul in care se inceteaza contractul pe o marca inceteaza contractul in luna si se face altul. Prima marca nu trebuie declarata daca pe ea nu sunt sume/valori.
			and (a.DataSf<>'' or da.Baza_FG+da.Ore_lucrate+da.Ore_suspendate+da.OreSST+da.ZileSST+da.Baza_somaj+da.Zile_lucrate<>0 or a.asigSO='0' or a.asigCI='0')
		...
		if @versiune>='2'
			...
			select a.Data, @lmUtilizator, a.CNP, 
				...
				rtrim(convert(char(15),sum(a.Venit_total/*+a.Valoare_tichete*/))) as E3_8, rtrim(convert(char(15),sum(a.Contributii_sociale))) as E3_9, rtrim(convert(char(15),sum(a.Valoare_tichete))) as E3_10, 
