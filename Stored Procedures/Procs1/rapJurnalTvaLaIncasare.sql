...
if @cote_tva is null
	set @cote_tva=''
begin try
	declare @GrFact int, @cContFact varchar(40), @nSoldMin float, @nSemnSold int, @nStrictPerioada int, @locm varchar(20),@dDataInit datetime
			,@utilizator varchar(20), @filtrareUser bit, @parXMLFact xml
			,@grupaTerti varchar(20),@datafactjos datetime, @datafactsus datetime
			,@cuTabela bit
	...
	select @GrFact=null, @cContFact=null, @nSoldMin=0, @nSemnSold=0,
		...
		,@datafactjos=@parXML.value('(/*/@datafactjos)[1]', 'datetime')
		,@datafactsus=@parXML.value('(/*/@datafactsus)[1]', 'datetime')
	...
	/*-------------------------------------------
		Sold Initial
	*/-------------------------------------------
	if not exists (select 1 from SoldFacturiTLI s
				left join facturi f on f.subunitate=@cSub and f.tip=(case when @cFurnBenef='F' then 0x54 else 0x46 end) and f.factura=s.factura and f.tert=s.tert
				left join lmfiltrare lf on lf.utilizator=@utilizator and lf.cod=f.loc_de_munca
			where tipf=@cFurnBenef and datalunii=@dDataInit and (@filtrareUser=0 or lf.cod is not null)) and @dDataInit=@dDImpl
	begin
		/*
		...
		*/
		...
		set @parXMLFact=(select @cFurnBenef as furnbenef, @dDataInit as datajos, @dDataInit as datasus, 1 as cen, 
				rtrim(@tert) as tert, rtrim(@factura) as factura, rtrim(@cContFact) as contfactura, @nSoldMin as soldmin, @nSemnSold as semnsold, @loc_de_munca as locm
			for xml raw)
		...
		--from dbo.fFacturiCen(@cFurnBenef, @dDataInit, @dDataInit, @tert, @factura, @GrTert, @GrFact, @cContFact, @nSoldMin, @nSemnSold, null) f
			left join terti t on f.subunitate=t.subunitate and f.tert=t.tert
	...
	begin
		...
		select s.tert,s.factura,'SI',s.sold,s.baza,s.sold
		from SoldFacturiTLI s
			left join terti t on t.subunitate=@csub and s.tert=t.tert
			left outer join facturi f on f.Subunitate=@cSub and f.Tip=@tipFact and f.tert=s.tert and f.factura=s.factura
			left join lmfiltrare l on f.loc_de_munca=l.cod and l.utilizator=@utilizator
		where datalunii=@dDataInit and s.tipf=@cFurnBenef	
			and (@tert is null or s.tert=@tert) 
			and (@grupaterti is null or t.grupa=@grupaterti)
			and (@factura is null or s.factura=@factura)
	...
	set @parXMLFact=(select @cFurnBenef as furnbenef, @datajos as datajos, @datasus as datasus, 
			rtrim(@tert) as tert, rtrim(@factura) as factura, rtrim(@cContFact) as contfactura, 
			@nSoldMin as soldmin, @nSemnSold as semnsold, 1 as strictperioada, @loc_de_munca as locm
		for xml raw)
	exec pFacturi @sesiune=null, @parXML=@parXMLFact

	select ft.data as data,ft.tert, ltrim(ft.factura) as factura, ft.valoare+ft.tva as valcutva,ft.valoare as baza,ft.tva as tva,ft.achitat as achitat,ft.numar
	into #docTert
	from #docfacturi ft
	--from fFacturi(@cFurnBenef,@datajos,@datasus, @tert,@factura,@cContFact,@nSoldMin,@nSemnSold,1,null, null) ft
		left join terti t on t.subunitate=@cSub and ft.tert=t.tert
		left outer join facturi f on f.Subunitate=@cSub and f.Tip=@tipFact and f.tert=ft.tert and f.factura=ft.factura
		left join lmfiltrare l on f.loc_de_munca=l.cod and l.utilizator=@utilizator
	...
	insert into #fTLI
	select p.tert,ltrim(p.factura),'RC',sum(suma) as rulajc,0 as baza, 0 as tva 
	from pozplin p
		left join lmfiltrare l on p.loc_de_munca=l.cod and l.utilizator=@utilizator
		left join terti t on t.subunitate=p.subunitate and t.tert=p.tert
	where p.Subunitate=@cSub and cont=@cont4428 and p.Plata_incasare=@tipDocPI 
		...
		and (@loc_de_munca is null or p.loc_de_munca like rtrim(@loc_de_munca)+'%')
		and (@filtrareUser=0 or l.cod is not null)
	group by p.tert,ltrim(p.factura)

	-- Alte prelucrari pe #fTLI, daca trebuie
	if exists (select * from sysobjects o where o.name='rapJurnalTvaLaIncasareSPfTLI' and o.type='P')
		exec rapJurnalTvaLaIncasareSPfTLI @cFurnBenef=@cFurnBenef, @datajos=@datajos, @datasus=@datasus, @tert=@tert, @factura=@factura, @loc_de_munca=@loc_de_munca, @cote_tva=@cote_tva, @parXML=@parXML

	--Lucian: utilizam procedura tipTVAFacturi (in locul selectului de mai sus) care stabileste tipul de TVA al facturii
	select '' as tip,@cFurnBenef as tipf,f.tert,f.factura,convert(datetime,null) as data, convert(varchar(40),null) as cont,'' as tip_tva, sum(baza) as baza, sum(TVA) as TVA  
	into #facturi_cu_TLI
	from #fTLI f
	group by f.tert,f.factura
	...
	begin
		delete j
			from SoldFacturiTLI j
			left outer join facturi f on f.Subunitate=@cSub and f.Tip=@tipFact and f.tert=j.tert and f.factura=j.factura
			left join lmfiltrare l on f.loc_de_munca=l.cod and l.utilizator=@utilizator
			where datalunii=@dataSus and tipf=@cFurnBenef
				and (@loc_de_munca is null or isnull(f.loc_de_munca,@loc_de_munca) like @loc_de_munca)
				and (@filtrareUser=0 or l.cod is not null)
	...
	begin
		...
		if exists (select * from sysobjects o where o.name='rapUAJurnalTvaLaIncasare' and o.type='P') and @cFurnBenef='B' 
			and @grupaterti is null
	--		insert into #jurnalTLI
				exec rapUAJurnalTvaLaIncasare @datajos=@datajos, @datasus=@datasus, @tert=@tert, @factura=@factura, @loc_de_munca=null, @sesiune=null, @D394=0

		--select * into ##j2 from #jurnalTLI where abs(isnull(sold_tli,0))>0.01  --Pentru Depanare pentru cunoscatori

		-- Alte prelucrari pe #jurnalTLI, daca trebuie
		if exists (select * from sysobjects o where o.name='rapJurnalTvaLaIncasareSP' and o.type='P')
			exec rapJurnalTvaLaIncasareSP @cFurnBenef=@cFurnBenef, @datajos=@datajos, @datasus=@datasus, @tert=@tert, @factura=@factura, @loc_de_munca=@loc_de_munca,
				@cote_tva=@cote_tva, @parXML=@parXML


if APP_NAME() like '%Management Studio%'
	and @utilizator not like 'luci.maier' --> din ceva motiv selecturile suplimentare incetinesc rularea testelor multisor (de la 0:53 la 2:53)
begin

select 'test1' test1, 'Sold contabilitate '+str(sum(rulaj_debit)-sum(rulaj_credit)) from rulaje where cont=@CtTvaNeexPlati and data between dbo.boy(@datasus) and @datasus 
		and (@loc_de_munca is null or loc_de_munca like @loc_de_munca)


select 'test2' test2, 'Sold raport '+str(sum(sold_tli)) from #jurnalTLI -- sum(sold_tli)
	where abs(sold_tli)>0.001 

select 'test3' test3, 'Sold tabela '+str(sum(s.sold)) from SoldFacturiTLI s -- sum(s.sold)
			left join terti t on t.subunitate=@csub and s.tert=t.tert
			left outer join facturi f on f.Subunitate=@cSub and f.Tip=@tipFact and f.tert=s.tert and f.factura=s.factura
			left join lmfiltrare l on f.loc_de_munca=l.cod and l.utilizator=@utilizator
	where datalunii=@datasus and s.tipf=@cFurnBenef	
			and (@tert is null or s.tert=@tert) 
			and (@grupaterti is null or t.grupa=@grupaterti)
			and (@factura is null or s.factura=@factura)
			and (@loc_de_munca is null or f.loc_de_munca like @loc_de_munca)
	...
	select 'test4' test4, 'doar in tabela',* from SoldFacturiTLI s 
			left join terti t on t.subunitate=@csub and s.tert=t.tert
			left outer join facturi f on f.Subunitate=@cSub and f.Tip=@tipFact and f.tert=s.tert and f.factura=s.factura
			left join lmfiltrare l on f.loc_de_munca=l.cod and l.utilizator=@utilizator
	where datalunii=@datasus and s.tipf=@cFurnBenef	
			...
			and (@loc_de_munca is null or f.loc_de_munca like @loc_de_munca)
			and (@filtrareUser=0 or l.cod is not null)
			and s.tert+s.factura not in (select tert+factura from #jurnalTLI where abs(sold_tli)>0.001)


	select 'test5' test5,'doar in raport',* from #jurnalTLI
		where abs(sold_tli)>0.001 
			and tert+factura not in 
			(select s.tert+s.factura from SoldFacturiTLI s 
			left join terti t on t.subunitate=@csub and s.tert=t.tert
			left outer join facturi f on f.Subunitate=@cSub and f.Tip=@tipFact and f.tert=s.tert and f.factura=s.factura
			left join lmfiltrare l on f.loc_de_munca=l.cod and l.utilizator=@utilizator
	where datalunii=@datasus and s.tipf=@cFurnBenef	
...
--> select final pentru raport:
		insert into #rapJurnalTvaLaIncasare_Tabela(nrcrt, factura, data, denumireTert, codFiscal, totalFactura,
			baza, tva, docIncasare, dataDocInc, sumaIncasata, soldInitTLI,
			rulajDebitTLI, rulajCreditTLI, bazaSoldTLI, soldTLI, ordonare, cota_tva)
		select row_number() over (order by (case when @ordonare=1 then convert(char(10),j.data,102) else denTert end)), 
			...
			convert(decimal(12,3),(case when (baza is null or abs(baza)<0.009) and year(j.data)>=2016 or abs(isnull(tva,0)/isnull(nullif(baza,0),1)*100-20)<0.05 then 20 
				when (baza is null or abs(baza)<0.009) and year(j.data)<2016 or abs(isnull(tva,0)/baza*100-24)<0.05 then 24 
		...
		where (@loc_de_munca is null or f.loc_de_munca like @loc_de_munca)
			and (@filtrareUser=0 or l.cod is not null)
			and (j.data between isnull(@datafactjos,'1901-1-1') and isnull(@datafactsus,'2999-1-1'))
			and (@cote_tva='' 
		...
		if @cuTabela=0
			select * from #rapJurnalTvaLaIncasare_Tabela order by ordonare
	end
end try
begin catch
