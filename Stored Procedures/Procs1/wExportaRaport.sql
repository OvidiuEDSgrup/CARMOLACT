...
/*
	...
	-- pentru impersonare useri
	insert into par select 'AR', 'SHORTADN', 'Nume domeniu impersonat', 0,0, 'CLUJ'
	insert into par select 'AR', 'FULLADN', 'Nume lung domeniu imp.', 0,0, 'CLUJ.asw.local'
...
DECLARE @mesaj VARCHAR(200), @userASiS VARCHAR(50), @mesajEroare VARCHAR(100), @pathReportToPdf VARCHAR(1000), @splitPage INT, @p2 
	...
	@numeFisierTxt varchar(120), @UserImpersonat varchar(500), @ShortDomainName varchar(50), @FullDomainName varchar(500)
...
BEGIN TRY
	...
	-- impersonare user spre reporting -> xp_cmdshell pierde userul autentificat
	select @ShortDomainName=rtrim(val_alfanumerica) from par where tip_parametru='AR' and parametru='SHORTADN'
	select @FullDomainName=rtrim(val_alfanumerica) from par where tip_parametru='AR' and parametru='FULLADN'

	if len(@FullDomainName+@ShortDomainName)>0
	begin
		select @UserImpersonat = rtrim(observatii) from utilizatori where id=@userAsis
		set @UserImpersonat = replace(@UserImpersonat, @ShortDomainName+'\','')+'@'+@FullDomainName

		if len(@userImpersonat)>0
			SET @parXML.modify('insert attribute UserImpersonat {sql:variable("@UserImpersonat")} into (/row)[1]')
	end
