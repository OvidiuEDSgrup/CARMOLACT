...
begin
	...
	select @searchText=ISNULL(@parXML.value('(/row/@searchText)[1]', 'varchar(80)'), ''), 
		...
		@valuta=ISNULL(ISNULL(NULLIF(@parXML.value('(/row/detalii/row/@valutafalta)[1]', 'varchar(3)'),''),@parXML.value('(/row/@valuta)[1]', 'varchar(3)')), ''),
	...
	set @inValuta=(case when (@tip in ('RM', 'RS', 'AP', 'AS', 'CF', 'CB', 'CO', 'C3') or @tip in ('RE', 'RV', 'DE', 'EF') and @subtip in ('PV', 'PL', 'IV', 'IL')) and @valuta<>'' then 1 else 0 end)
