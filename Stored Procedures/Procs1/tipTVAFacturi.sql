--***
create procedure tipTVAFacturi @dataJos datetime=null/*nu se mai foloseste*/, @dataSus datetime=null /*nu se mai foloseste*/, @TLI int=null, @parXML xml=null
as
/*
...
set nocount on
declare @dataTLI datetime, @parXMLFact xml, @subunitate varchar(9)
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
select @subunitate=(case when parametru='SUBPRO' then val_alfanumerica else @subunitate end)
from par where tip_parametru='GE' and parametru in ('SUBPRO')
...
select row_number() over (partition by tt.tert order by tt.dela) as nr,tt.tert,tt.dela,tt.dela as panala,tt.tip_tva,tt.tipf,t.detalii.value('(/row/@tert_grup)[1]','int') as tert_grup
into #tvapeterti
from tvapeterti tt
left outer join terti t on t.subunitate=@subunitate and t.tert=tt.tert
where nullif(tt.factura,'') is null	--> se vor lua doar datele tva necesare, adica pentru tert=null si tert existent in #facturi_cu_TLI, altfel simpla inchidere a machetei de pozitii AP poate dura pana la 7 sec, pe Orto de exemplu
	and (tt.tert is null or exists (select 1 from #facturi_cu_TLI f where f.tert=tt.tert))
order by tt.dela
...
from #facturi_cu_TLI fTLI
	inner join facturi f on f.subunitate=@subunitate and fTLI.tert=f.tert and fTLI.factura=f.factura and fTLI.tipf=(case when f.tip=0x54 then 'F' else 'B' end)
...
select ft.tip,ft.tipf,ft.tert,ft.factura,
	(case	when left(ft.Cont,3) in ('408','418') then 'P' 
			...
			when isnull(tvatb.tip_tva,'P')='I' and nullif(tvat.tert_grup,0) is null then isnull(tvatf.tip_tva,'I')	--Daca BD este cu TLI, si tertul este afiliat, nu se aplica TLI. Daca nu este afiliat se aplica TLI.
...
outer apply (select top 1 tvat.tip_tva, tvat.dela, tvat.tert_grup from #TvaPeTerti tvat where tvat.tipf=(case when ft.tipf='B' and isnull(tvatb.tip_tva,'P')='P' then 'B' else 'F' end)
