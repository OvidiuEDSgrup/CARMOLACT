...
begin try 
	...
	DECLARE @utilizator varchar(50), @lista_lm int, @lm varchar(9), @multiFirma int, @mesaj varchar(1000), @lmFiltru varchar(9), @nrLMFiltru int
	...
	select @lmFiltru=nullif(@lmFiltru,'')
...
--	Date contributii
	if @dataset='C'
		...
		where data between @datajos and @datasus
			and (@multiFirma=0 and @lmFiltru is null or lu.cod is not null) 
