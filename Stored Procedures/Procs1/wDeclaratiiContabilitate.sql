...
declare @tip char(2), @datajos datetime, @datasus datetime, @lunaalfa varchar(15), @luna int, @an int, @userASiS varchar(10),
	@dataDecl datetime, @flux varchar(10), @multiFirma int, @lmUtilizator varchar(9), @denlmUtilizator varchar(100)
...
if exists (select * from sysobjects where name ='par' and xtype='V')
	set @multiFirma=1
if @multiFirma=1 
	select @lmUtilizator=rtrim(isnull(min(Cod),'')) from LMfiltrare where utilizator=@userASiS and cod in (select cod from lm where Nivel=1)
select @denlmUtilizator=rtrim(denumire) from lm where cod=@lmUtilizator
...
begin try  
	select @tip as tip, convert(char(10),d.Data,101) as datalunii, rtrim(dbo.fDenumireLuna(d.data)) + ' ' + convert(char(4),year(d.data)) as numeluna, month(d.data) as luna, convert(char(4),year(d.data)) as an, 
		rtrim(d.Loc_de_munca) as lm, rtrim(lm.Denumire) as denlm, 
	...
	where d.data between @datajos and @datasus and (@tip='YM' and d.Cod='394' or @tip='YL' and d.Cod='390' or @tip='YO' and d.Cod='300' or @tip='YS' and d.Cod like 'Intrastat_%')
		...
		and (@multiFirma=0 or d.Loc_de_munca=@lmUtilizator)
	union all
	select distinct @tip as tip, convert(char(10),Data_lunii,101) as datalunii, rtrim(LunaAlfa) + ' ' + convert(char(4), an) as numeluna, Luna as luna, convert(char(4),an) as an,
		@lmUtilizator as lm, @denlmUtilizator as denlm, 
	...
	where data=Data_lunii
		and not exists (select 1 from declaratii d where (@multiFirma=0 or d.Loc_de_munca=@lmUtilizator) and d.Data=c.Data 
	...
	where data=Data_lunii and @tip='YS'
		and not exists (select 1 from declaratii d where (@multiFirma=0 or d.Loc_de_munca=@lmUtilizator) and d.Data=c.Data and @tip='YS' and d.Cod='Intrastat_E' 
			and detalii.value('/row[1]/@flux', 'varchar(1)')='E')
