...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	if object_id('tempdb.dbo.#avans') is not null
		drop table #avans
	...
	select n.data, n.marca, max(rtrim(isnull(i.Nume,p.Nume)))+(case when @afisarecnp=1 then ' - '+rtrim(max(p.Cod_numeric_personal)) else '' end) as nume, 
		...
		(case when @ordonare='3' then max(isnull(m.mandatar,'')) else '' end)+(case when @ordonare='2' then '' else max(n.loc_de_munca) end)
			+(case when @alfabetic=1 then max(p.nume) else n.marca end) as ordonare, 
		rtrim(max(p.Cod_numeric_personal)) as cnp
	into #avans
	from net n 
	...
	if exists (select * from sysobjects where name ='rapAvansSalariiSP' and xtype='P')
		exec rapAvansSalariiSP @dataJos=@dataJos, @dataSus=@dataSus, @MarcaJ=@MarcaJ, @locm=@locm, @strict=@strict
	
	select data, marca, nume, loc_de_munca, denumire_lm, lm_parinte, cod_functie, denumire_functie, salar_de_incadrare, 
		avans, premiu_la_avans, retinere_la_avans, suma_corectie_M, suma_corectie_N, suma_corectie_O, 
		exista_avans_exceptie, avans_exceptie_Dafora, ore_avans_exceptie, zile_lucratoare_CM, avans_de_afisat, suma_corectieC_Salubris, 
		rest_de_plata_avans, banca, categoria_salarizare, Grupa_de_munca, Mandatar, nume_mandatar, ordonare, cnp 
	from #avans
	order by ordonare
end try
