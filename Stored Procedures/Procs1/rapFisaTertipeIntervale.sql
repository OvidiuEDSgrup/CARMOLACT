...
CREATE procedure rapFisaTertipeIntervale(
--***	Procedura pentru raportul CG\Financiar\Fisa terti pe intervale
/**	--> exemplu de apel:
declare @cFurnBenef nvarchar(1),@cData datetime,@cTert nvarchar(8),@cFactura nvarchar(4000),@cContTert nvarchar(4000),@soldmin nvarchar(4),@soldabs int,@dDataFactJos nvarchar(4000),@dDataFactSus nvarchar(4000),@dDataScadJos nvarchar(4000),@dDataScadSus nvarchar(4000),@aviz_nefac nvarchar(1),@grupa nvarchar(4000),@grupa_strict nvarchar(1),@exc_grupa nvarchar(4000),@fsolddata1 bit,@comanda nvarchar(4000),@indicator nvarchar(4000),@cDataJos datetime,@tipdoc nvarchar(1),@locm nvarchar(4000),@punctLivrare nvarchar(4000),@fsold bit,@moneda nvarchar(1),@valuta nvarchar(4000),@centralizare nvarchar(1),@gestiune nvarchar(4000)
select @cFurnBenef=N'F',@cData='2006-10-31 00:00:00',@cTert=N'1',@cFactura=null,@cContTert=NULL,@soldmin=N'0.01',@soldabs=0,@dDataFactJos=NULL,@dDataFactSus=NULL,
		@dDataScadJos=NULL,@dDataScadSus=NULL,@aviz_nefac=N'0',@grupa=NULL,@grupa_strict=N'0',@exc_grupa=NULL,@fsolddata1=0,@comanda=NULL,@indicator=NULL,
		@cDataJos='2001-02-28 00:00:00',@tipdoc=N'F',@locm=NULL,@punctLivrare=NULL,@fsold=0,@moneda=N'0',@valuta=NULL,@centralizare=N'0',@gestiune=NULL

exec rapFisaTertipeIntervale @cFurnBenef=@cFurnBenef,@cData=@cData,@cTert=@cTert,@cFactura=@cFactura,@cContTert=@cContTert,@soldmin=@soldmin,
	...
	@fsold=@fsold, @moneda=@moneda, @valuta=@valuta, @centralizare=@centralizare, @gestiune=@gestiune
...
*/
	...
	@ordonare int = 1,		-->0=cod, 1=denumire, 2=sold tert (din raport)
	...
	*/	--*/
	...
	if isnull(@parXML.value('(row/@specific)[1]','bit'),0)=1
		...
		select tert, factura, data_facturii, data_scadentei, valoare, denumire, tipValori, denTipValori, interval, denInterval, isnull(comanda,'') comanda, isnull(loc_de_munca,'') loc_de_munca, ordine, data_intervale
				, (case @ordonare when 0 then tert when 1 then denumire else factura end) ordonare
