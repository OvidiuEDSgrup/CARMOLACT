...
As
Begin try
	declare @CorL_SREAC int, @MachetaCO int, @MachetaCO_TipOpMach varchar(100) 

	select 	@CorL_SREAC=max(case when parametru='SREAC-L' then Val_logica else 0 end)
	...
	from par where tip_parametru in ('PS') and parametru in ('OPZILECOM','SREAC-L')
...
--	Am mutat aici apelul procedurii scriu_brut_din_CO (pana acuma era apelata la final in pRecalcul_sporuri
	if @MachetaCO=1 or @MachetaCO_TipOpMach<>''
		exec scriu_brut_din_CO @dataJos, @dataSus, @MarcaJos, @lmJos
