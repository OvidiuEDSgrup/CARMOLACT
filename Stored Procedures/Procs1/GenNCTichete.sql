...
Begin
	...
	create table #NCtichete (data datetime)
	exec CreeazaDiezSalarii @numeTabela='#NCtichete'

	if exists (select 1 from sysobjects where name='pTichete' and xtype='P')
	begin
		if OBJECT_ID('tempdb..#ptichete') is not null 
			drop table #ptichete
		create table #ptichete (data datetime)
		exec CreeazaDiezSalarii @numeTabela='#ptichete'
		exec pTichete @dataJos=@dataJos, @dataSus=@dataSus, @marca=@pMarca, @DeLaCalculLichidare=2
		
		insert into #NCtichete
