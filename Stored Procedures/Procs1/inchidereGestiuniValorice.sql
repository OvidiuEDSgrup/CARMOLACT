...
as
/*	Apel procedura specifica inlocuitoare. */
if exists (select * from sysobjects where name ='inchidereGestiuniValoriceSPInloc')
begin
	exec inchidereGestiuniValoriceSPInloc @sesiune=@sesiune, @parXML=@parXML
	return
end
if not exists (select cod_gestiune from gestiuni where tip_gestiune='V')
	return
begin try
	SET NOCOUNT ON
