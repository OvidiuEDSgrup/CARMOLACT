...
exemplu apel:
declare @p xml
select @p=(select null dDataJos, '2016-12-31' dDataSus, '03100023' cCod, '3170' cGestiune, null cCodi
				,null cGrupa, 'D' TipStoc, null cCont, 0 Corelatii, null Locatie, null Comanda, null Furnizor
				,null lm, null grupGestiuni, null Lot
	for xml raw)
...
else set @eLmUtiliz=isnull((select max(1) from lmfiltrare where utilizator=@utilizator),0)
...
create table #pozdoc
	...
	schimb int, contractdinpozdoc_pebune varchar(20),idIntrareFirma int,idIntrare int,idPozDoc int,modificarepretam int,tip varchar(2))
...
insert #pozdoc (subunitate, gestiune, cont, cod, data, cod_intrare, pret, tip_document,	numar_document,	cantitate, cantitate_UM2, tip_miscare, in_out, predator, codi_pred, jurnal,
	...
	cont_corespondent, schimb, contractdinpozdoc_pebune,idIntrareFirma,idIntrare,idPozDoc,modificarepretam,tip)
select i.subunitate, i.cod_gestiune, i.cont, i.cod, i.data, i.cod_intrare, i.pret, 'SI','',i.stoc,
		...
		i.TVA_neexigibil, i.pret_vanzare, 0, 
		--i.loc_de_munca
		(case
			when @TipStoc='F' then rtrim(case when @SubgestFolLM=1 then i.loc_de_munca else pers.Loc_de_munca end) -- folosinta
			else isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') end) -- depozit
			as loc_de_munca
		,i.comanda, i.contract, 
		isnull(nullif(case when ip.tip='RM' then ip.tert else '' end,''),i.furnizor), 
		isnull(nullif(ip.lot,''),i.lot), 
		0, '', 0, '' Contract,i.idIntrareFirma,i.idIntrare,si.idpozdoc,0,'IS'
...
where @Corelatii<>1 and i.data_lunii=@dDataIstoric and i.subunitate=@cSub and i.tip_gestiune not in ('V','I') 
	and (@cCod is null or i.cod like rtrim(@cCod))
	...
	and (@LM='' 
		or @TipStoc='F' and rtrim(case when @SubgestFolLM=1 then i.loc_de_munca else pers.Loc_de_munca end) like @LM_like -- folosinta
		or @TipStoc!='F' and isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') like @LM_like) -- depozit 
	...
	and (@eLmUtiliz=0 or
			@tipstoc='F' and
				(@SubgestFolLM=0 and exists (select 1 from @LMUtiliz pr where rtrim(pr.marca)=rtrim(i.Cod_gestiune))
					or @SubgestFolLM=1 and exists (select 1 from @LMUtiliz pr where rtrim(pr.valoare)=rtrim(i.loc_de_munca)))
			or @tipstoc<>'F' and 
				-- fie gestiunile au locul de munca al utilizatorului
				(exists (select 1 from lmfiltrare l where l.utilizator=@utilizator and l.cod=isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') 
				-- fie gestiunile nu au completat loc de munca
				or isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'')=''))
		)
union all	--*/
select a.subunitate,a.gestiune,a.cont_de_stoc,a.cod,a.data,a.cod_intrare,a.pret_de_stoc,a.tip,a.numar,a.cantitate,
	...
	(case when a.tip_miscare='E' then isnull(ds.locatie,isnull(s.locatie,a.locatie)) else a.locatie end),
	(case when a.tip_miscare='E' then isnull(nullif(ds.data_expirarii,'1901-01-01'),isnull(nullif(i.data_expirarii,'1901-01-01'),a.data_expirarii)) else a.data_expirarii end),
	...
	a.accize_cumparare,
	--(case when @SubgestFolLM=1 and a.tip in ('PF','CI') then isnull(nullif(ds.loc_de_munca,''),isnull(nullif(s.loc_de_munca,''),a.loc_de_munca)) else a.loc_de_munca end)
	(case
		when @TipStoc='F' then rtrim(case when @SubgestFolLM=1 and a.tip in ('PF','CI') then isnull(isnull(nullif(ds.detalii.value('(/row/@lmprim)[1]','varchar(200)'),''),ds.loc_de_munca),isnull(s.loc_de_munca,'')) when @SubgestFolLM=1 then a.loc_de_munca else pers.Loc_de_munca end)
		else isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') end) -- depozit 
	,
	(case when a.tip='TE' then ds.comanda else a.comanda end) comanda,--pentru transferuri, comanda predatoare este comanda de pe documentul de intrare in gestiune
	...
	a.procent_vama, a.contract,isnull(a.idIntrareFirma,a.idPozDoc) as idIntrareFirma,isnull(a.idIntrare,a.idPozDoc),a.idPozDoc,(case when a.tip='TE' and a.gestiune=a.gestiune_primitoare then 1 else 0 end),a.tip
from pozdoc a
	left join pozdoc i on i.idpozdoc=a.idintrarefirma --and i.subunitate=@cSub --and i.tip='rm'
	left join pozdoc ds on ds.idpozdoc=a.idintrare 
	...
	-- urmatorul join este lasat pt. compatibiltate in urma (unde nu e completat idIntrare)
...
where a.subunitate=@cSub 
	...
	and (@subtipGestiune is null or isnull(b.detalii.value('(/row/@custodie)[1]', 'int'),0)=1)
	and (@cCod is null or a.cod like rtrim(@cCod))
	...
	and (@Locatie='' or 
		...
		(case when a.tip in ('TE','DF','PF') then isnull(ds.locatie,isnull(s.locatie,a.locatie)) else a.locatie end) like @Locatie)
	and (@LM='' 
		or @TipStoc='F' and rtrim(case when @SubgestFolLM=1 and a.tip in ('PF','CI') then isnull(isnull(nullif(ds.detalii.value('(/*/@lmprim)[1]','varchar(20)'),''),ds.loc_de_munca),isnull(s.loc_de_munca,'')) 
										when @SubgestFolLM=1 then a.loc_de_munca else pers.Loc_de_munca end) like @LM_like
		or @TipStoc!='F' and isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') like @LM_like) -- depozit 
	...
	and (@eLmUtiliz=0 
		or @tipstoc='F' and
		( @SubgestFolLM=0 and exists (select 1 from @LMUtiliz pr where rtrim(pr.marca)=rtrim(a.gestiune))
			or @SubgestFolLM=1 and exists (select 1 from @LMUtiliz pr where rtrim(pr.valoare)=rtrim(case when a.tip in ('PF','CI') then isnull(isnull(nullif(ds.detalii.value('(/*/@lmprim)[1]','varchar(20)'),''),ds.loc_de_munca),isnull(s.loc_de_munca,'')) else a.loc_de_munca end)))
			or @tipstoc<>'F' and 
				-- fie gestiunile au locul de munca al utilizatorului
				(exists (select 1 from lmfiltrare l where l.utilizator=@utilizator and l.cod=isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') 
				-- fie gestiunile nu au completat loc de munca
...
select a.subunitate,a.gestiune_primitoare,a.cont_corespondent,a.cod,a.data,(case when a.grupa='' then a.cod_intrare else a.grupa end),
	...
	(case when a.tip='DF' and a.procent_vama<>0 then (1-a.procent_vama/100) else 1 end),
	...
	a.locatie,
	a.data_expirarii,
	a.TVA_neexigibil,a.pret_amanunt_predator,a.accize_cumparare,
									--(case when @SubgestFolLM=1 then isnull(a.detalii.value('(/*/@lmprim)[1]','varchar(20)'),a.loc_de_munca) else pers.Loc_de_munca end)
	(case when @TipStoc='F' then rtrim(case when @SubgestFolLM=1 then isnull(nullif(a.detalii.value('(/*/@lmprim)[1]','varchar(20)'),''),a.loc_de_munca) else pers.Loc_de_munca end)
		else isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') end)
	,a.comanda,
	...
	a.numar_pozitie,a.cont_de_stoc,a.procent_vama, a.contract,ISNULL(a.idIntrareFirma,a.idpozdoc),a.idPozdoc,a.idPozDoc,(case when a.tip='TE' and a.gestiune=a.gestiune_primitoare then 1 else 0 end),'TI'
from pozdoc a
...
where a.subunitate=@cSub and (a.tip in ('DF','PF') or a.tip='TE' and isnull(tip_gestiune,'') <> 'I' and (@Corelatii=1 or isnull(tip_gestiune,'')<>'V'))
	...
	and (@LM='' 
		or @TipStoc='F' and rtrim(case when @SubgestFolLM=1 then isnull(nullif(a.detalii.value('(/*/@lmprim)[1]','varchar(20)'),''),a.loc_de_munca) else pers.Loc_de_munca end) like @LM_like 
		or @TipStoc!='F' and isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') like @LM_like) -- depozit 
	and (@Comanda='' or a.comanda in('',@Comanda))
	...
	and (@eLmUtiliz=0 
		or @tipstoc='F' and
		( @SubgestFolLM=0 and exists (select 1 from @LMUtiliz pr where rtrim(pr.marca)=rtrim(a.gestiune_primitoare))
			or @SubgestFolLM=1 and exists (select 1 from @LMUtiliz pr where rtrim(pr.valoare)=rtrim(isnull(nullif(a.detalii.value('(/*/@lmprim)[1]','varchar(20)'),''),a.loc_de_munca))))
			or @tipstoc<>'F' and 
				-- fie gestiunile au locul de munca al utilizatorului
				(exists (select 1 from lmfiltrare l where l.utilizator=@utilizator and l.cod=isnull(b.detalii.value('(/row/@lm)[1]','varchar(200)'),'') 
				-- fie gestiunile nu au completat loc de munca
...
select a.subunitate,a.tert,cont_corespondent,a.cod,a.data,cod_intrare,a.pret_de_stoc,a.tip,numar,a.cantitate,
	...
	(case when a.tip in ('AI') and isnull(a.lot,'')<>'' then isnull(a.lot,'') when a.tip='AI' then a.grupa else '' end),numar_pozitie,a.cont_de_stoc,a.procent_vama, a.Contract,a.idIntrareFirma,a.idPozDoc,a.idPozDoc,(case when a.tip='TE' and a.gestiune=a.gestiune_primitoare then 1 else 0 end),'TI'
from pozdoc a 
...
where a.subunitate=@cSub and a.tip in ('AI','AP') and (@C35=1 and left(cont_corespondent,2)='35' or @C8=1 and left(cont_corespondent,1)='8')
	and (@subtipGestiune is null or isnull(b.detalii.value('(/row/@custodie)[1]', 'int'),0)=1)
	and isnull(n.tip,'') not in ('R','S') and isnull(b.tip_gestiune,'') not in ('V','I') 
	and (@cCod is null or a.cod like rtrim(@cCod))
...
/*Pentru gestiunile de tip C cu Evaluare - noul tip de gestiuni Amanunt*/
if exists(select 1 from #PozDoc p inner join gestiuni g on p.Gestiune=g.cod_gestiune and g.pret_am=1)
begin
	
	select p.idpozdoc,p.cod,p.tip,'' as gestiune
	into #preturiam
	from #PozDoc p
	inner join gestiuni g on p.Gestiune=g.cod_gestiune and g.pret_am=1
	where modificarepretam!=1

	exec CreazaDiezPreturiAmanunt
	exec wIaPreturiAmanunt @sesiune,'<row/>'
		
	update p set pret_cu_amanuntul=pa.pret_amanunt,tva_neexigibil=pa.cota_tva
	from #PozDoc p
	inner join #preturiam pa on pa.idpozdoc=p.idpozdoc and pa.tip=p.tip
end
/*Gata tip C cu Evaluare - noul tip de gestiuni Amanunt*/
...
insert into #docstoc (subunitate, gestiune, cont, cod, data, data_stoc, cod_intrare, pret, tip_document, numar_document, cantitate, cantitate_UM2, tip_miscare, in_out, 
			...
			data_expirarii, TVA_neexigibil, pret_vanzare, accize_cump, loc_de_munca, comanda, [contract], furnizor, lot, numar_pozitie, cont_corespondent, schimb, idIntrareFirma, idIntrare,idPozDoc)
select subunitate, gestiune, cont, cod, data, data, cod_intrare, pret, tip_document, numar_document, cantitate, cantitate_UM2, tip_miscare, in_out,
	...
	data_expirarii, TVA_neexigibil, pret_vanzare, accize_cump, isnull(loc_de_munca,''), comanda, [contract], furnizor, lot, numar_pozitie, cont_corespondent, schimb, idIntrareFirma, idIntrare,idPozDoc
from #pozdoc
...
begin
	...
	insert #docstoccen
	select d.subunitate,d.gestiune,d.cont,d.cod,d.data,d.data_stoc,d.cod_intrare,d.pret,d.tip_document,d.numar_document,d.cantitate,d.cantitate_UM2,d.tip_miscare,d.in_out,d.predator,d.jurnal,d.tert,d.serie,
		d.pret_cu_amanuntul,d.tip_gestiune,(case when d.tip_miscare='E' then '' else d.locatie end),d.data_expirarii,d.TVA_neexigibil,d.pret_vanzare,d.accize_cump,d.loc_de_munca,d.comanda,d.[contract],d.furnizor,d.lot,d.numar_pozitie,
		d.subunitate+d.tip_gestiune+d.gestiune+d.cod+d.cod_intrare grp,
		(case when d.tip_document = 'SI' then '0' else '1' end)
			+(case when d.tip_miscare='I' and d.tip_document not in ('AI','TI') or d.tip_miscare='E' and d.cantitate<0 then '0' when d.tip_document='AI' then '1' else '2' end)
			+convert(char(8),d.data,112)+str(d.numar_pozitie) ordineIntrareFirma, 	-- SI, apoi intrari (exclusiv TI), apoi AI, apoi iesiri
		(case when d.tip_miscare='I' and d.tip_document<>'AI' or d.tip_miscare='E' and d.cantitate<0 then '2' when d.tip_document = 'SI' then '1' else '0' end)
			+convert(char(8),d.data,112)+str(d.numar_pozitie) ordineIntrariCustodie, -- iesiri, apoi SI, apoi intrari = ordinea pt. "ultima intrare"
		(case when d.tip_document = 'SI' then '0' else '1' end)
			+(case when d.tip_miscare='I' and d.tip_document not in ('AI') or d.tip_miscare='E' and d.cantitate<0 then '0' when d.tip_document='AI' then '1' else '2' end)
			+(case when g.pret_am=1 then str(999999999-convert(int,convert(char(8),d.data,112)),9) else convert(char(8),d.data,112) end)+str(d.numar_pozitie) ordineIntrare ,-- SI, apoi intrari (inclusiv TI),apoi AI, apoi iesiri
		'01/01/2999', '01/01/2999', 0, '', '01/01/2999', 0, '', '', '', '', '', '', '',idIntrareFirma,idIntrare, null, null
	from #docstoc d
	left join gestiuni g on d.Gestiune=g.cod_gestiune
