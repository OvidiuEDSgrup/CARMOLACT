...
set transaction isolation level read uncommitted
	...
	create table [dbo].[#PozDocFiltr] ([Numar] [varchar](20) NOT NULL, [Cod] [varchar](20) NOT NULL, [Data] [datetime] NOT NULL, 
		...
		[Locatie] [varchar](50), [Utilizator] [varchar](200), idPozdoc int
		)

	insert into #PozDocFiltr (
		Numar, Cod, Data, Gestiune, Cantitate, Pret_formular, Pret_cu_amanuntul, 
		Cod_intrare, Tert, Gestiune_primitoare, numar_pozitie, Locatie, Utilizator, idPozdoc
		)
	select 
		...
		Locatie, rtrim(Utilizator), idPozdoc
	from pozdoc pz
	...
	update pz 
		set pret_formular=(case when ge.tip_gestiune in ('M','P','O','C') then pz.pret_formular 
	...
	select
		...
		isnull(@detalii.value('(/row/@dentertdelegat)[1]', 'varchar(300)'), '') AS TERTDELEGAT,
		...
		isnull(nullif(rtrim(del.Mijloc_tp), ''), isnull(@detalii.value('(/row/@nrauto)[1]', 'varchar(30)'), '')) as nrmij,
