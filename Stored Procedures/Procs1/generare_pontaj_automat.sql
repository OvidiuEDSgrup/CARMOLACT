...
--	sterg din realcom daca se lucreaza cu [X]Incadrare salariati pe comenzi
	If @SalPeComenzi=1 
		...
		where a.data between @dataJ and @dataS 
			...
			and (@pTipStat='' or isnull(p.tip_stat,ip.Religia)=@pTipStat)
	...
	Begin
		...
		round((case when a.Tip_salarizare in ('1','3','6') 
			then (case when @lPontajZilnic=1 
				...
				else (case when p.ore<>0 then p.ore else (p.Zile-isnull(cd.Zile,0)-isnull(r.Zile,0)-isnull(fp.Zile,0)-isnull(dt.Zile,0)-isnull(sc.Zile,0))*p.RL end) end) 
		...
		isnull(isnull(a.Spor_cond_7,ip.Spor_cond_7),0), isnull(isnull(a.Spor_cond_8,ip.Spor_cond_8),0)+(case when @CampOreITptOreSusp=3 then isnull(sc.Zile,0)*p.RL else 0 end), 
		(case when @PontajW=1 then 0 else isnull(dt.Zile,0)*p.RL end), --isnull(a.Spor_cond_9,0)
		...
		where (@pmarca='' or a.marca=@pmarca) and (@pLocm='' or a.Loc_de_munca like rtrim(@pLocm)+'%')
			and (@pTipStat='' or isnull(a.tip_stat,ip.religia)=@pTipStat) and dbo.eom(@dataJ)>=a.Data_angajarii_in_unitate
		...
		If @SalPeComenzi=1
			...
			select a.Marca, a.Loc_de_munca, 'PS'+rtrim(convert(char(10),a.Numar_curent)), a.Data, isnull(p.comanda,ip.Centru_de_cost_exceptie), '', '', a.Ore_regie+a.Ore_acord, 1, 0, a.Salar_orar
			from #pontaj_automat a
				inner join personal p on a.Marca=p.Marca
				inner join infopers ip on a.Marca=ip.Marca
	End
