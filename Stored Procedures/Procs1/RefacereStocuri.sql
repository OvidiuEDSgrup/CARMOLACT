...
declare @LenGestiune int, @UrmCant2 int, @p xml
...
if @TipStoc='' or @TipStoc='D'
begin
	delete stocuri
		where tip_gestiune not in ('F','T')
			and (isnull(@GestFiltru,'')='' or cod_gestiune=@GestFiltru)
			and (isnull(@cCod,'')='' or cod=@cCod)

	select @p=(select @dData dDataSus, @cCod cCod, @GestFiltru cGestiune, 1 GrCod, 1 GrGest, 1 GrCodi, 'D' TipStoc for xml raw)
...
begin
	...
	select @p=(select @dData dDataSus, @cCod cCod, @GestFiltru cGestiune, 1 GrCod, 1 GrGest, 1 GrCodi, 'F' TipStoc for xml raw)
...
if @TipStoc='' or @TipStoc='T'
begin
	delete stocuri
		where stocuri.tip_gestiune='T'
			and (isnull(@GestFiltru,'')='' or stocuri.cod_gestiune=@GestFiltru)
			and (isnull(@cCod,'')='' or stocuri.cod=@cCod)

	select @p=(select @dData dDataSus, @cCod cCod, @GestFiltru cGestiune, 1 GrCod, 1 GrGest, 1 GrCodi, 'T' TipStoc for xml raw)
...
begin
	...
	if (@ProcMed = 1 and exists (select 1 from sysobjects where type='P' and name='PROCMED')) -- un fel de procedura specifica 
