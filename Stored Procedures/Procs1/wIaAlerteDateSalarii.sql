...
--	citire date din xml
	...
	IF OBJECT_ID('tempdb..#personalAlerte') IS NOT NULL drop table #personalAlerte
...
--	TREBUIE lasat acest select dupa apelare SP.	
	select p.*
		,p.detalii.value('(/row/@dataexpCI)[1]','datetime') as dataexpCI
	into #personalAlerte
	from personal p
...
--	suspendari contracte
	...
	from fRevisalSuspendari (@dataJos, @datasus, '') s
		left outer join #personalAlerte p on p.Marca=s.Marca
...
--	expirare tipuri autorizatii salariati straini
	...
	from fRevisalAutorizatii (@dataJos, @datasus, '') a
		inner join #personalAlerte p on p.Marca=a.Marca
	where DataSfarsit<DateADD(day,@zileref,getdate())

--	data expirare buletin
	insert into #alerte
	select 'CI', 'Expirare buletin', p.loc_de_munca, lm.denumire, p.Marca, p.Nume, 
		convert(varchar(10),@datajos,101), convert(varchar(10),p.dataexpCI,101) as data_sfarsit, 
		'Buletinul salariatului expira peste '+rtrim(convert(char(4),DateDIFF(day,getdate(),p.dataexpCI)))+' zile!'
	from #personalAlerte p
		left outer join lm on lm.cod=p.Loc_de_munca
	where p.dataexpCI is not null and p.dataexpCI<DateADD(day,@zileref,getdate())
		and (p.Loc_ramas_vacant=0 or p.Data_plec>@dataSus)

/*	Pare dublata partea de mai jos
	if @inchidereluna=1
		insert into #alerte
		select 'SS', 'Modificare procent '+rtrim(TipSpor), Loc_de_munca, Denumire_loc_de_munca, Marca, Nume, @datajos, @datasus, 
			'Modificare '+rtrim(TipSpor)+' de la '+rtrim(convert(char(4),SporLunaAnt))+'%'+' la '+rtrim(convert(char(4),SporLunaCrt))+'%!'
...
--	incetare contracte pe perioada determinata
	create table #contracte (marca varchar(6), nume varchar(50), cnp varchar(13), salar_de_baza float, cod_functie varchar(6), den_functie varchar(80), lm varchar(9), den_lm varchar(30), 
...
--	salariati ce se pensioneaza 
	...
	where coef_invalid<>5	-- nu se iau in calcul pensionarii
...
--	persoanele in intretinere cu varsta de pana in 18 ani care implinesc aceasta varsta in luna urmatoare lunii de calcul ii afisam pentru a fi anuntati salariatii cu privire la cine ramine in intretinere
--	pana la 18 ani poti fi trecuti in intretinere la ambii parinti
	union all 
	select 'PI', 'Persoane in intretinere (copii) ce implini 18 ani luna viitoare', 
		isnull(i.loc_de_munca,''), isnull(lm.Denumire,''), p.marca, p.nume_pren, p.Data_nasterii, DateADD(year,18,p.data_nasterii), 
		'Pers. in intretinere va implini 18 ani luna viitoare. De stabilit la care parinte va ramine in intretinere'
	from persintr p
		left outer join istpers i on i.data=p.data and i.marca=p.marca
		left outer join lm on lm.cod=i.Loc_de_munca
		inner join #personalAlerte pa on pa.marca=p.marca
	where p.data between @datajos and @dataSus and p.tip_intretinut in ('C','U')
		and DateADD(year,18,p.data_nasterii) between GETDATE() and DateADD(day,@zileref,getdate())
	...
	begin
		...
		from istpers i
			inner join #personalAlerte p on p.Marca=i.Marca
...
--	returnare date
	...
	(select (case when Tip_alerta='S' then 'Incetare suspendare contract' 
			...
			when Tip_alerta='M' then 'Salariati ce vor implini/au implinit 18 ani' 
			when Tip_alerta='CI' then 'Expirare carte identitate' 
			when Tip_alerta='PI' then 'Persoane in intretinere (copii) ce vor implini 18 ani' else e.alerta end) as dentipalerta, 
		rtrim(e.lm) as lm, rtrim(e.denumire_lm) as denlm, rtrim(e.marca) as marca, rtrim(e.Nume) as densalariat, 
		rtrim(convert(char(10),data_inceput,101)) as datainceput, rtrim(convert(char(10),data_sfarsit,101)) as datasfarsit, 
		rtrim(explicatii) as explicatii
	from #alerte e
		left outer join #personalAlerte p on p.Marca=e.Marca
		left outer join lm on lm.Cod=p.Loc_de_munca
	where (@filtruMarca='' or e.Marca like @filtruMarca+'%' or p.Nume like '%'+replace(@filtruMarca,' ','%')+'%')
		...
		and (@filtruAlerta='' 
			or (case when Tip_alerta='S' then 'Incetare suspendare contract' when Tip_alerta='C' then 'Incetare contract pe per. determinata' 
				...
				when Tip_alerta='M' then 'Salariati ce vor implini/au implinit 18 ani' 
				when Tip_alerta='CI' then 'Expirare carte identitate' 
				when Tip_alerta='PI' then 'Persoane in intretinere (copii) ce vor implini 18 ani' end) like '%'+REPLACE(@filtruAlerta,' ','%')+'%')
