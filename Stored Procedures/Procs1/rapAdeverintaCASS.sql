...
**/
create procedure rapAdeverintaCASS (@sesiune varchar(50), @marca varchar(6), @datajos datetime, @datasus datetime, @dataset char(2), @centrCM int=0, @parXML xml='<row/>')
AS
/*
...
begin try 
	...
	*/
	...
	set @data_12=dbo.BOM(DateADD(month,-11,@datajos))
	select @data_12=dbo.EOM(data_angajarii_in_unitate)
	from personal where marca=@marca and data_angajarii_in_unitate>@data_12
--	date salariat
	if @dataset='S'	
	begin
...
--	date concedii medicale pe perioade
	if @dataset like 'CM%'
	begin
-- Termotec cere sa apara si codurile 08 si 09. Asa rezulta si din Norme de aplicare OUG 158/2005, art. 65 alin.2.
		select data, dbo.fDenumireLuna(cm.Data) as luna, year(cm.data) as an, dbo.fDenumireLuna(cm.data)+' - '+convert(char(4),year(cm.data)) as perioada, marca,
			convert(char(10),data_inceput,103) as data_inceput, convert(char(10),data_sfarsit,103) as data_sfarsit, cm.tip_diagnostic, 
		...
		if exists (select * from sys.objects where name = 'rapAdeverintaCASSSPCM')
			exec rapAdeverintaCASSSPCM @sesiune=@sesiune, @marca=@marca, @datajos=@datajos, @datasus=@datasus, @dataset=@dataset, @parXML=@parXML

	--	Din raportul web Adverinta CASS legea 95.RDL se apeleaza cu @centrCM=0. Din Adverinta CASS.RDL se apeleaza cu @centrCM=1
		if @centrCM=1
			select	marca, tip_concediu, 
					max(data) as data, max(luna) as luna, max(an) as an, max(perioada) as perioada, max(data_inceput) as data_inceput, max(data_sfarsit) as data_sfarsit, 
					sum(zile_cm) as zile_cm, sum(zile_calend_cm) as zile_calend_cm, max(total_zile_cm) as total_zile_cm, max(total_zile_calend_cm) as total_zile_calend_cm
			from #adevcassCM
			group by marca, tip_concediu
		else
			select data, luna, an, perioada, marca,	data_inceput, data_sfarsit, tip_concediu,
				zile_cm, zile_calend_cm, total_zile_cm,	total_zile_calend_cm
			from #adevcassCM
	end
