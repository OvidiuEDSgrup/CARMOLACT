...
begin
	...
	begin
		declare @userASiS varchar(20), @datasus datetime,@datajos datetime,@dataincan datetime,@subunitate char(13),@aninchis int,
			@lunainchisa int,@validcomstrict int,@comandaGenerica varchar(20), @multiFirma int, @lmUtilizator varchar(20)
		...
		begin
			...
			if @lmUtilizator<>''
				set @lm=@lmUtilizator
			if @lm=''
				raiserror ('Trebuie sa selectati un loc de munca!',11,1)
		end

		if (dbo.f_arelmfiltru(dbo.fIaUtilizator(null))=1) and @multiFirma=0
		begin
...
-->	creez #DocDeContat pentru a o utiliza si la stergere dif. si dupa generare dif.
		...
		begin
			...
			and (@multiFirma=0 or loc_de_munca like rtrim(@lm)+'%')
		...
		left outer join (select r.cont,r.valuta,sum(r.rulaj_debit) as rulaj_debit,SUM(r.rulaj_credit) as rulaj_credit 
			from rulaje r where r.subunitate=@subunitate and r.data between @dataincan and @datasus and r.valuta='' and (@multiFirma=0 or r.loc_de_munca like rtrim(@lm)+'%')
			group by r.cont,r.valuta) d on a.Cont=d.Cont/* and a.Valuta=d.Valuta*/
		left outer join (select r.cont,r.valuta,sum(r.rulaj_debit) as rulaj_debit,SUM(r.rulaj_credit) as rulaj_credit 
			from rulaje r where r.subunitate=@subunitate and r.data between @dataincan and @datasus and r.valuta<>'' and (@multiFirma=0 or r.loc_de_munca like rtrim(@lm)+'%')
			group by r.cont,r.valuta) e on a.Cont=e.Cont and a.Valuta=e.Valuta
		...
		and a.data between dbo.BOY(@datajos) and @datasus 
		and a.cont in (select cont from conturi where are_analitice=0 and sold_credit=0)
		and (@multiFirma=0 or loc_de_munca like rtrim(@lm)+'%')
...
-->	pun in tabela temporara sumele gata calculate ca sa nu dublez conditia de where cand se scrie in #DocDeContat
		select cont,
			...
			convert(varchar(20),'') as comanda
		into #DifCursFinal
		from #DifCurs 
		where ABS(sold_debit_valuta*curs-sold_debit_lei)>0.01 or abs(sold_credit_valuta*curs-sold_credit_lei)>0.01
		
		if exists (select * from sysobjects where name ='DifCursDispSP')
			exec DifCursDispSP @parcont=@parcont, @parvaluta=@parvaluta, @datadoc=@datadoc, @contfav=@contfav,
				@contnefav=@contnefav, @sterg_pl_inc_ant=@sterg_pl_inc_ant, @lm=@lm, @curs=@curs
		...
		select @subunitate,cont,@datadoc,'DIF.C.V.',max(tip),'','',MAX(a.contcor),sum(suma),
			...
			@lm,max(case when @validcomstrict=1 and a.comanda='' then @comandaGenerica else a.comanda end),@user,
