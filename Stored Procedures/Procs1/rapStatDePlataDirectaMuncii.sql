...
Create procedure rapStatDePlataDirectaMuncii
	...
	@tipimpozitare char(1)=null	-->	tip impozitare
	)
/*
...
begin try
	...
	declare @utilizator varchar(20), @impozitareTichete int, @datajosTich datetime, @datasusTich datetime,
		...
		@Helvetica int, @Colas int, @Stoehr int, @listaDreptCond char(1), @areDreptCond int, @lungLm int, @parXMLTichete xml
	...
	if object_id('tempdb..#brut1') is not null drop table #brut1
	if object_id('tempdb..#ticheteTotal') is not null drop table #ticheteTotal
	if object_id('tempdb..#ticheteSupl') is not null drop table #ticheteSupl
...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	exec pTichete @dataJos=@datajosTich, @dataSus=@datasusTich, @marca=@Marca, @DeLaCalculLichidare=1

	select * into #ticheteTotal from #ptichete

	/*	Scriere in tabela separata a tichetelor suplimentare. Pentru a putea afisa aceste tichete separat in statul de plata. */
	set @parXMLTichete=(select 'S' as tipoperatie for xml raw)
	delete from #ptichete
	exec pTichete @dataJos=@datajosTich, @dataSus=@datasusTich, @marca=@Marca, @DeLaCalculLichidare=1, @parXML=@parXMLTichete
	select * into #ticheteSupl from #ptichete
	
--	creare tabela temporara in care se cumuleaza sumele din tabela brut pe marca
	select max(b.marca) as marca, max(b.data) as data, max(case when b.spor_cond_10=0 then 8 else b.spor_cond_10 end) as regim_lucru, 
	...
	sum(round (b.ore_lucrate_regim_normal/(case when b.spor_cond_10=0 then 8 else b.spor_cond_10 end),2,0)) as tzile_regim_normal_lucru, 
	...
	sum(round (b.ore_intrerupere_tehnologica,2,0)) as tore_intrerupere_teh_m, sum(ind_intrerupere_tehnologica+ind_invoiri) as ind_intrerupere_tehnologica_m, 
	sum(b.ind_regim_normal) as ind_o_regim_normal, sum(round(b.ind_obligatii_cetatenesti,0)) as tind_obligatii_cetatenesti_m, 
	...
	sum(po.Zile_lucrate) as Zile_lucrate, sum(b.Diurna) as Diurna, sum(round(b.sp_salar_realizat,0)) as Sp_salar_realizat, 
	max(isnull(t.Valoare_tichete,0)) as Valoare_tichete, max(isnull(ts.Valoare_tichete,0)) as Valoare_tichete_supl, 
	...
	from brut b 
		...
		left outer join (select marca, loc_de_munca, sum(spor_cond_8) as Ore_intemperii, 
			sum(case when regim_de_lucru<1 then Coeficient_de_timp else (Ore_regie+Ore_acord)/regim_de_lucru end) as Zile_lucrate, 
			sum(Ore__cond_4) as ore_spc_4, sum(Ore__cond_5) as ore_spc_5
		...
		left outer join #ticheteTotal t on @impozitareTichete=1 and b.Marca=t.Marca /*b.Data=t.Data and */
		left outer join #ticheteSupl ts on @impozitareTichete=1 and b.Marca=ts.Marca /*b.Data=t.Data and */
	...
	select n.data, n.marca, left((case when 1=0 then n.loc_de_munca else i.loc_de_munca end),@lungLm) as lm, rtrim(lm.denumire) den_lm, 
		...
		n.VENIT_BAZA, n.impozit, n.VENIT_NET, n.avans, n.debite_externe, n.rate, n.debite_interne, n.cont_curent, n.REST_DE_PLATA, 
		convert(decimal(10,2),b.Valoare_tichete) as Valoare_tichete, convert(decimal(10,2),b.Valoare_tichete_supl) as Valoare_tichete_supl, 
		n.Baza_CAS, n.CAS+isnull(n1.CAS,0) as cas, n.fond_de_risc_1, n.asig_sanatate_din_CAS, n.asig_sanatate_pl_unitate, 
		n.Camera_de_Munca_1, n.somaj_5, n1.somaj_5 as fond_garantare, b.loc_de_munca, b.spor_cond_10, b.zile_lucrate, 
		b.tore_lucrate_m, b.tore_regim_normal_lucru, b.tore_co_m, b.tore_CFS_m, b.tore_cm_m, b.tore_invoiri_m, b.tore_nemotivate_m, b.total_zile_CO_m, b.total_zile_CM_m, 
		b.total_zile_CFS_m, b.total_zile_nemotivate_m, b.total_zile_invoiri_m, b.total_zile_fara_plata_m, 
		b.total_zile_oblig_cetatenesti_m, b.total_ore_oblig_cetatenesti_m, b.total_zile_oblig_intrerupere_m, b.tore_oblig_intrerupere_m, 
		total_ore_suplimentare_1_m, total_ore_suplimentare_2_m, total_ore_suplimentare_3_m, total_ore_suplimentare_4_m, b.total_ore_suplimentare_m, 
		...
		b.venit_impozabil_m, b.tpremiu_m, b.zile_intrerupere_teh_m, b.ind_intrerupere_tehnologica_m, b.ind_o_regim_normal, b.tind_obligatii_cetatenesti_m, 
		...
		p.cod_numeric_personal, p.somaj_1, (case when isnull(rtrim(x.val_inf),'')<>'' then isnull(rtrim(x.val_inf),'') else isnull(p.Nr_contract,ip.Nr_contract) end) as nr_contract, 
		...
		(case when @ordonare in ('2','4') then i.Loc_de_munca when @ordonare in ('3') then i.cod_functie when @ordonare='5' then i.Tip_impozitare else '' end) grupare, 
		regim_lucru, Aj_deces
	--into #de_regrupat
	from net n 
	...
	where n.data = @datasus
		...
		and (@tipimpozitare is null or @tipimpozitare='3' and i.Tip_impozitare=@tipimpozitare or @tipimpozitare='1' and i.Tip_impozitare<>'3')
