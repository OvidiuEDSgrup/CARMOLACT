...
--	variabile pt. generare NC contributii angajat+avans din net per total unitate
	...
	select @nLunaBloc=isnull(@nLunaBloc,1), @nAnulBloc=isnull(@nAnulBloc,1901)
...
--	sterg tabele temporare
		...
		if object_id('tempdb..#realcom') is not null drop table #realcom
		declare @parXML xml
...
--		insert into #net 
		...
		if exists (select * from sysobjects where name ='PSGenNCSalariiSPInc')
		begin
			set @parXML=(select convert(varchar(10),@dataJos,101) as datajos, convert(varchar(10),@dataSus,101) as datasus for xml raw)
			exec PSGenNCSalariiSPInc @sesiune=null, @parXML=@parXML
		end
...
--		creez cursor pentru a genera cate un numar de document pentru fiecare luna rectificata
		...
		begin
			set @parXML=(select convert(varchar(10),@dataJos,101) as datajos, convert(varchar(10),@dataSus,101) as datasus for xml raw)
			exec PSGenNCSalariiSP @sesiune=null, @parXML=@parXML
		end
