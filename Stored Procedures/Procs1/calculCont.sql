...
if exists(select * from proprietati where tip='LM' and cod_proprietate='LMBILANT' and Valoare='1')
	set @defalclm=1
create table #lmptbil(cod varchar(20))
insert into #lmptbil select cod from proprietati where tip='LM' and cod_proprietate='LMBILANT' and Valoare='1'
...
*/
  ...
  if left(@camp,2) in ('SI','RL','RC','SC')
   begin
		if exists (select cod from tmp_calculat where cod = rtrim(@camp) and hostid =@cHostID )
			return
