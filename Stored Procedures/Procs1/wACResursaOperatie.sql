...
AS
DECLARE @searchText VARCHAR(50), @codOp VARCHAR(20)
...
SELECT RTRIM(rs.cod) AS cod, RTRIM(rs.descriere) AS denumire, 'Tip: ' + RTRIM(rs.tip) AS info
FROM Resurse rs
INNER JOIN OpResurse ors ON rs.id = ors.idRes
	AND ors.cod = @codOp
