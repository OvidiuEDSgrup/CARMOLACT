...
Begin
	...
	if object_id('tempdb..#rapretineri') is not null 
		drop table #rapretineri
	...
	rtrim(r.cod_beneficiar) as cod_beneficiar, r.Numar_document, r.Data_document, r.Valoare_totala_pe_doc, r.Valoare_retinuta_pe_doc, 
	...
	b.tip_retinere, rtrim(b.denumire_beneficiar) as denumire_beneficiar, b.obiect_retinere, b.cod_fiscal, b.banca, b.cont_banca, e.Val_inf as cont_banca2, e.Data_inf, 
	...
	from resal r  
		...
		left outer join tipret f on b.tip_retinere=f.subtip and @detret=1
	...
	if exists (select * from sysobjects where name ='rapRetineriSP1')
		exec rapRetineriSP1 @dataJos=@dataJos, @dataSus=@dataSus, @marca=@marca, @locm=@locm, @strict=@strict, @benret=@benret, 
							@retinerela=@retinerela, @subtipret=@subtipret, @tipretinere=@tipretinere, @grupare=@grupare, @ordonare=@ordonare

	select Data, marca, nume, salar_de_incadrare, loc_ramas_vacant, loc_de_munca, denumire_lm, 
		cod_beneficiar, Numar_document, Data_document, Valoare_totala_pe_doc, Valoare_retinuta_pe_doc, 
		Retinere_progr_la_avans, Retinere_progr_la_lichidare, procent_progr_la_lichidare, Retinut_la_avans, Retinut_la_lichidare, 
		Numar_chitanta, Valoare_chitanta, venit_net, retinere_cu_procent, diferenta, valoare_ramasa,
		tip_retinere, denumire_beneficiar, obiect_retinere, cod_fiscal, banca, cont_banca, cont_banca2, Data_inf, 
		subtip, denumire_subtip, tip_retinere_din_tipret, obiect_subtip_retinere, afisat, se_afiseaza,
		Ordonare_codb1, Ordonare_codb2, Ordonare_lm, Ordonare_tip_retinere, tip_retinere_grup, Denumire_tip, tip_subtip, nr_ordine
	from #rapretineri
	order by Ordonare_lm, Ordonare_tip_retinere, Ordonare_codb1, (case when @ordonare=2 then nume else marca end), Ordonare_codb2, numar_document, data
