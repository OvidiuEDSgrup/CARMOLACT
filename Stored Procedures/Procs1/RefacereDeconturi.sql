...
create procedure RefacereDeconturi @dData datetime='2999-01-01', @cMarca char(6)='', @cDecont varchar(40)=''
as
...
begin try 
	...
	select
		subunitate, 'T', isnull(marca,''), isnull(decont,''), max(cont), min(data), min(data_scadentei),  sum(valoare), max(valuta), max(curs), 
	...
	group by subunitate, isnull(marca,''), isnull(decont,'')
end try
