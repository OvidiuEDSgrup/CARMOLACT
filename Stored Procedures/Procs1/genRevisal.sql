...
--	apelez scrierea in istoric pesonal (din istpers se iau datele pt. procedurile genRevisalContracte si genRevisalSalariati)
	...
	@fax=(case when dbo.iauParA('GE','FAX')='' then dbo.iauParA('GE','TELFAX') else dbo.iauParA('GE','FAX') end), @email=dbo.iauParA('GE','EMAIL')
...
--	creez tabela temporara ##extinfop in care pun datele din extinfop cu codurile de informatie necesare pt. generarea registrului
	...
	Begin
		...
		from extinfop e
			left outer join personal p on p.marca=e.marca and e.cod_inf<>'#CODCOR'
	...
	create table #personalDetalii (marca varchar(6))
...
Begin
	select c.Cnp as Cnp, c.Marca as Marca, rtrim(c.CodCOR) as CodCOR, @VersiuneCOR as Versiune,
		...
		StareCurenta, convert(char(19),c.DataIncStareCurenta,127)+'Z' as DataIncStareCurenta, StarePrecedenta, DataIncetareStarePrecedenta, TemeiLegal, DetaliiContract
	into #tmpContracte
...
--	formare XML cu datele salariatilor
	...
	(select rtrim((case when s.Cetatenie in ('UESEE','Alta') then @AdresaAng else s.Adresa end)) as Adresa, rtrim(s.CNP) as Cnp, rtrim(s.CNPVechi) as CnpVechi, 
		(select (select rtrim(c.CodCOR) as Cod, c.Versiune as Versiune for XML path('Cor'), type),
			...
			(case when s.Cetatenie in ('UESEE','Alta') then rtrim(s.Adresa)+(case when s.Localitate<>'' then ' loc. '+rtrim(s.Localitate) else '' end) 
				when nullif(c.DetaliiContract,'') is not null then rtrim(c.DetaliiContract) end) as Detalii,
...
--	functie de aceste date (functii_lm.Pozitie_stat) se va face ordonarea datelor in raport
	...
	select rtrim(s.Nume)+' '+rtrim(s.Prenume) as nume, c.cnp, rtrim(s.nationalitate) as nationalitate, rtrim(s.adresa)+', '+rtrim(s.localitate) as adresa, 
		...
		rtrim(c.TipDurata) as tip_durata, (case when c.TipDurata='Determinata' and c.DataSfarsitContract<>'01/01/1901' then convert(varchar(10),c.DataSfarsitContract,103) else '' end) as data_sfarsit_contract, 
		rtrim(c.Cod_functie) as Cod_functie, rtrim(c.CodCOR) as cod_cor, rtrim(c.DenumireCOR) as denumire_cor, 
