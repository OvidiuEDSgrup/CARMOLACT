...
begin try
	...
	--> Apelare SP complementar
	if exists (select 1 from sys.sysobjects where name = 'rapIntrariIesiripeTertiSP')
		exec rapIntrariIesiripeTertiSP @sesiune = @sesiune, @pDinf = @pDinf, @pDsup = @pDsup,
			@tip_doc_str = @tip_doc_str, @cLM = @cLM, @nr_doc = @nr_doc, @comanda = @comanda,
			@factura = @factura, @tert = @tert, @ord = @ord, @cod = @cod, @gestiune = @gestiune
