...
begin try	
	...
	if exists (select 1 from #concedii i where dbo.eom(i.Data_sfarsit)<dbo.eom(i.Data) and i.fel<>'CM')
	begin
		select @mesajeroare='Eroare operare '+rtrim(nume_tabela)+': Data de sfarsit trebuie sa fie in luna de lucru!'
		from #concedii i where dbo.eom(i.Data_inceput)<>dbo.eom(i.Data)	
		raiserror(@mesajeroare,16,1)
	end
...
--	validare suprapunere cu informatii pontaj
	if exists (select 1 from infopontaj ip 
		...
		where (i.fel='CA' and (case when ip.Tip='7' then '2' when ip.Tip='8' then '1' else ip.Tip end)<>i.Tip_concediu 
					or i.fel='CM' and ip.Tip not in ('1','T') or i.fel='CO' and ip.Tip not in ('2','T'))
