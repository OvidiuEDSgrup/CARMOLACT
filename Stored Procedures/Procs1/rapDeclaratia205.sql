...
Create procedure rapDeclaratia205 
	...
	@grupare char(2)='', @alfabetic int=1, @marca varchar(6)=null, @angajatiPrinDetasare int=null, @cnp varchar(13)=null, 
	@sirDeMarci varchar(1000)=null, @dinAdeverinta int=0)
as  
/*
...
--	am tratat pentru Rematinvest 3 cazuri intrucat s-au generat 3 variante in pozdoc pt. impozit.
		...
		Group by t.Cod_fiscal
		union all
--	inserez asiguratii care au realizat venituri din activitati agricole - doar incepand cu veniturile anului 2012. Incepand cu 2015, ar trebui declarat tip venit=15 (venituri din arendare).
		select @dataSus, (case when @an>=2015 then '15' else '14' end) as tip_venit, '' as tip_salar, '2', CNP, Nume, '', 0, 0, 0, Baza, Contributie, 0
...
--	inserez asiguratii care au realizat venituri din conventii civile/drepturi de autor/expertiza contabila si tehnica
		union all
		select (case when @marca is null and @cnp is null and @dinAdeverinta=0 then @dataSus else n.Data end), 
		...
		group by (case when @marca is null and @cnp is null and @dinAdeverinta=0 then @dataSus else n.Data end), p.Cod_numeric_personal, 
...
--	inserez, incepand cu veniturile anului 2012 salariatii cu contract de munca 
		select (case when @marca is null and @cnp is null and @dinAdeverinta=0 then @dataSus else n.Data end), '07', isnull(id.tipsalar,'1') as tip_salar, '2', p.Cod_numeric_personal, max(p.Nume) as Nume, 
		...
		group by (case when @marca is null and @cnp is null and @dinAdeverinta=0 then @dataSus else n.Data end), p.Cod_numeric_personal, isnull(id.tipsalar,'1'), 
			(case when i.grupa_de_munca in ('N','C') and i.tip_colab='FDP' or charindex(i.grupa_de_munca,'OP')<>0 then '2' else '1' end)
		union all
--	inserez, incepand cu veniturile anului 2012 impozitul aferent zilierilor
		select (case when @marca is null and @cnp is null and @dinAdeverinta=0 then @dataSus else dbo.EOM(s.Data) end), '07', '1' as tip_salar, '2', z.Cod_numeric_personal, max(z.Nume) as Nume, '2', 
		...
		group by (case when @marca is null and @cnp is null and @dinAdeverinta=0 then @dataSus else dbo.EOM(s.Data) end), z.Cod_numeric_personal
		union all
--	inserez date NEGESTIONATE in ASiS pe parcursul anului
		select @dataSus, d.tip_venit, (case when d.tip_venit='07' then '1' else '' end) as tip_salar, d.tip_impozit, isnull(p.Cod_numeric_personal,d.cnp), isnull(p.nume,d.nume), 
			tip_functie, venit_brut, deduceri_personale, deduceri_alte, baza_impozit, impozit, isnull(d.venit_net,0)
		from DateD205 d
