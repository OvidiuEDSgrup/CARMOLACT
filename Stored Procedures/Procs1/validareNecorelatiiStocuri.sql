--***
create procedure validareNecorelatiiStocuri @Subunitate varchar(20),@Tip varchar(2),@Numar varchar(20),@Data datetime,@Numar_pozitie int,
...
as
	...
	*/
	declare @PretMediu int, @nExcepPM int, @cGestExcepPM char(1000), 
		@GestFaraPM int -- gestiune fara pret mediu
	exec luare_date_par 'GE', 'MEDIUP', @PretMediu output, @nExcepPM output, @cGestExcepPM output
	set @cGestExcepPM=','+LTrim(RTrim(@cGestExcepPM))+','
	set @GestFaraPM=(case when sign(charindex(','+rtrim(@Gest)+',',@cGestExcepPM))!=sign(@nExcepPM) then 1 else 0 end)
	
	if @PretMediu=1 and @GestFaraPM=0
		return		

	declare @mesajeroare varchar(250)
	--if @Tip='TE'
	--	set @Tip_miscare='E'	
	...
	if exists (select 1 from stocuri s where s.subunitate=@Subunitate and s.cod=@cod and s.Tip_gestiune=@Tip_gest and s.cod_gestiune=@Gest 
			...
			and ((abs(convert(decimal(17,5),s.Pret)-convert(decimal(17,5),@Pret_stoc))>=0.0001) or ((s.cont<>@Cont_stoc and @Cont_stoc<>'')))) --daca pretul de stoc sau contul de stoc difera
	begin
