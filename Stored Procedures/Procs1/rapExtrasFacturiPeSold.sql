...
/*	if (@tipdoc<>'F' and @efecteAchitate=1)
	...
	select a.subunitate, a.tert, @FurnBenef as FurnBenef, factura, data, 
		sold, 
	...
	begin
		...
		set @parXMLFact=(select @FurnBenef as furnbenef, null as datajos, @data as datasus
			, 1 as cen
			, rtrim(@tert) as tert, rtrim(@cont) as contfactura, @gtert gtert
			,1 as grtert, 1 as grfactura, @efecteAchitate as efecteachitate for xml raw)
		exec pFacturi @sesiune=null, @parXML=@parXMLFact

		if @gtert is not null
		delete e from #pfacturi e left join terti t on e.tert=t.tert where (isnull(t.grupa,'')<>@gtert)
	...
	begin
		...
		if @gtert is not null
		delete e from #efecte e left join terti t on e.tert=t.tert where (isnull(t.grupa,'')<>@gtert)
...
/*			create table #fltContEfecte(tert varchar(20), factura varchar(20), sold decimal(20,5))
			...
			*/
			...
			/* Se permite modificarea tabelei pentru a altera datele inainte ca ele sa fie returnate*/	
			
		select @parxml=(select @sesiune sesiune, @data data, @datajos datajos, @datasus datasus
				,@facturiLaZi facturiLaZi
				,@FurnBenef FurnBenef
				,@validareResponsabilCaMarca validareResponsabilCaMarca
				--> filtre:
				,@gtert gtert
				,@tert tert
				,@soldjos soldjos, @soldsus soldsus
				,@valuta valuta
				,@cont cont
				,@grupa grupa
				,@locm locm, @responsabil responsabil
				,@efecteAchitate efecteAchitate
				,@cont_efecte cont_efecte
				,@tipdoc tipdoc
				-->	web:
				,@datascadJos datascadJos
				,@datascadSus datascadSus
				,@GrContFact GrContFact
				,@filtruSoldAbs filtruSoldAbs
				,@existaDate existaDate for xml raw)
		
			IF EXISTS (select 1 from sysobjects where name='rapExtrasFacturiPeSoldSP')
				exec rapExtrasFacturiPeSoldSP @sesiune=@sesiune, @parxml=@parxml
				
			if (select count(1) from #extras)=0 and @tert is not null
			begin
				set @soldjos=0
				insert into #extras (subunitate, tert, sold)
				select '1',@tert, 0.00
			end
...
--GO
----***
--/**	Prima rulare a functiei "in gol" e necesara pentru a ocoli eventuala constructie defectuoasa a "query plan", 
--	operatie efectuata de sql server la prima rulare a unei functii nou create; in mod normal, daca ar fi caz ideal
--	si totul ar merge pe indecsi nu ar trebui facut acest pas*/
--exec rapExtrasFacturiPeSold @data='2000-1-1',
--	@facturiLaZi=0
--	,@FurnBenef=null
--	,@validareResponsabilCaMarca=0
--	,@tert='dasfstgr'
--	,@GrContFact=0
--	,@filtrusoldabs=01
--	,@soldjos=0.01
