...
Create procedure calculOrdonantariSalarii
	...
	@tipCalcul int=1,	--	1 = contributii pe coduri bugetare
						...
						--	5 = componenta venit brut pe tipuri de sume
...
Begin try
	...
	declare @utilizator varchar(20), @lista_lm int, @IndCondSalBaza int, @SpSpecSalBaza int, @Sp1SalBaza int, @ProcentImpozit decimal(10,2), @AjDecesUnitate int, 
		@CreditCMCas1 varchar(20), @DebitCMCas2 varchar(20), @CreditCMCas2 varchar(20), @optiuneTichete int, @BugetariCG int

	set @utilizator = dbo.fIaUtilizator(null)
	set @lista_lm=dbo.f_areLMFiltru(@utilizator)
	select	@BugetariCG=max(case when parametru='BUGETARI' then val_logica else @BugetariCG end), 
			@SpSpecSalBaza=max(case when parametru='S-BAZA-SP' then val_logica else @SpSpecSalBaza end), 
	...
	from par 
	where tip_parametru='GE' and parametru in ('BUGETARI') or tip_parametru='PS' and parametru in ('S-BAZA-SP','SBAZA-IND','S-BAZA-S1','AJDUNIT-R')
	...
	select b.Data, b.Marca, (case when @tipCalcul in (3,5) or @fltLocmChelt=1 then b.Loc_de_munca else n.lm end) as lm, max(n.Grupa_de_munca) as Grupa_de_munca, sum(b.Venit_total) as Venit_total, 
		...
		sum(round(b.Spor_cond_8,0)) as SporCond8,
	...
	group by b.data, b.marca, (case when @tipCalcul in (3,5) or @fltLocmChelt=1 then b.Loc_de_munca else n.lm end)
	...
	set @optiuneTichete=(case when @tipCalcul=5 then 2 else 1 end)
	exec pTichete @dataJos=@dataJos, @dataSus=@dataSus, @marca=@Marca, @DeLaCalculLichidare=@optiuneTichete

	insert into #sumeDet (Data, TipSuma, Marca, lm, Suma, Indicator, Explicatii)	
	select t.data, 'TICHETE', t.Marca, t.Loc_de_munca, t.Valoare_tichete as suma, isnull(nullif(c.indicator,''),isnull(cn.Comanda,'')) as indicator, rtrim(cn.Denumire) as explicatii
	from #ptichete t
	...
	if @tipcalcul<>5 and @BugetariCG=1
	begin
	...
	begin
		...
		insert into #sume (Data, TipSuma, Suma, Indicator, ExplicatiiSuma)	
		select n.Data, 'CASSPTFAAMBP', sum(n.CassPtFaambp) as suma, max(isnull(nullif(c.indicator,''),cn.Comanda)) as indicator, 
		...
		if not (@locm is not null and @fltLocmChelt=1)
			...
			from par p
				left outer join config_nc cn on cn.Numar_pozitie=150 and nullif(cn.loc_de_munca,'') is null
				left outer join #conturi c on c.cont=cn.Cont_debitor
...
*/
	...
	Begin
		...
		from #contributii where @BugetariCG=1
		union all 
		select data, tipSuma, marca, lm, indicator, suma, explicatii as ExplicatiiSuma, tipSuma as TipContributii, explicatii as ExplicatiiContributii, idpoz
		from #sumeDet where @BugetariCG=0 and tipDate='ASIGANGAJAT'
		union all 
	...
	Begin
		if @BugetariCG=1
			...
			select row_number() over (order by marca, data, tipSuma, numar) as nrord_poz, data, marca, indicator, 
					tipSuma, tipSuma as tipContributii, suma, explicatii, explicatii as explicatiiRetineri, numar
			from #sumeDet
			where tipDate='RETINERI'
	End

	if @tipcalcul=5	--	nota contabila cheltuieli venit brut pe componente
	begin
		select data, tipSuma, marca, lm, suma, indicator, explicatii, numar, idpoz
		from #sumeDet
		where tipDate='BRUT' and tipSuma!='TICHETE'
	end
End try
