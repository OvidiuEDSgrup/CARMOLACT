	--***
Create procedure rapDeclaratia394v2015 @sesiune varchar(50)=null	-->	parametrul sesiune nu va avea efect pana ce nu-l vom trimite catre ftert
...
declare @facturi varchar(1000), @expandare int, @tert_generare varchar(100),
		@raport bit, @userASiS varchar(20), @multiFirma int, @lmUtilizator varchar(20), @f_codfisc varchar(20), @f_dentert varchar(100)
...
select	@raport=(case when isnull(@parxml.value('(row/@iddeclaratie)[1]','varchar(100)'),'')='' then 1 else 0 end)
		...
		,@data=isnull(isnull(@parxml.value('(row/@datalunii)[1]','datetime'),@parxml.value('(row/@data)[1]','datetime')),@data)
...
begin
	...
	from declaratii d
		...
		where d.Cod = '394'
			...
			and (@lmUtilizator is null or d.Loc_de_munca=@lmUtilizator)
...
*/	order by data desc
	...
	where Cod = '394'
		...
		and (@tert = '' or @tert_generare<>'' or rtrim(T.N.value('(@cuiP)[1]','varchar(80)')) like '%' + @tert + '%' 
						or rtrim(T.N.value('(@denP)[1]','varchar(200)')) like '%' + @tert + '%')
		and (@tert_generare = '' or rtrim(T.N.value('(@cuiP)[1]','varchar(80)')) like '%' + @f_codfisc + '%' 
						or rtrim(T.N.value('(@denP)[1]','varchar(200)')) like '%' + @f_dentert + '%')

	--> filtrez pe tert dupa denumire si/sau cod
	if rtrim(@tert)<>'%' and @tert_generare=''
	delete d
	...
	delete d from #D394det d where not exists (select 1 from #d394xml x where x.codfisc=d.codfisc and x.tipop=(case	when d.invers=1 and d.tipop='L' then 'V'
