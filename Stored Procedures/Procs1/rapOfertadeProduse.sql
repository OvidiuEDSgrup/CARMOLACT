...
begin
	...
	select @fltCod=(case when @cod is null then 0 else 1 end),
		...
		@fltCategoria=(case when @categoria is not null and @prodcucategoria=1 then 1 else 0 end)
		--/*
	select cod, denumire, UM into #nomencl 
	from nomencl n 
	where (@fltCod=0 or cod=@cod) 
		and (@fltGrupa=0 or Grupa=@Grupa) 
		and (@fltFurnizor=0 or n.Furnizor=@furnizor)
		and (@fltCategoria=0 or exists (select 1 from preturi p where p.Cod_produs=n.cod and p.Data_superioara>='2999-1-1' and p.um=@categoria))
	...
	where (@fltCod=0 or s.cod=@cod)
		...
		and (@fltGestiune=0 or s.Cod_gestiune=@gestiunea)
...
*/
	...
	from #nomencl n 
		left join #preturi p on n.Cod=p.Cod
