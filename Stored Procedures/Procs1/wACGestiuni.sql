...
begin
	...
	select	@tip=ISNULL(@parXML.value('(/row/@tip)[1]', 'varchar(2)'), ''), 
			...
			@tiprap=isnull(@parXML.value('(/row/@tiprap)[1]','varchar(200)'),isnull(@parXML.value('(/row/@tipstoc)[1]','varchar(200)'),isnull(@parXML.value('(/row/@tipgest)[1]','varchar(200)'),''))),	--="F"
	...
	select	@folosinta=(case when ( ((@raport in ('Balanta stocuri','Stocuri la data','Inventar comparativa','Fisa stocuri') or @raport like '%folosinta%') and @tiprap='F') or (@meniu='DO' and @tip='CI')) then 1 else 0 end),
