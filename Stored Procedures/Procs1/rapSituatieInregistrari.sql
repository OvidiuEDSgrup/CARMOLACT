...
where data between @datajos and @datasus and subunitate='1' 
	...
	and (@cont_debitor='' /*or isnull(p.cont_debitor,'')=''*/ or exists (select 1 from #tmp t where t.tip='D' and t.cont=p.cont_debitor and p.cont_debitor<>''))
	and	(@cont_creditor='' /*or isnull(p.cont_creditor,'')=''*/  or exists (select 1 from #tmp t where t.tip='C' and t.cont=p.cont_creditor and p.Cont_creditor<>''))
