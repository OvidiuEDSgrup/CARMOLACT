...
/**	proc. cursor1 brut	*/
Create
procedure  psCursor1_brut  @DataJ datetime,@DataS datetime,@MarcaJ char(6),@LocmJ char(9)
As
Begin
	...
	if (select count(1) from tempdb..sysobjects where name='##cursor1_brut')>0 
		and isnull((select tempdb..syscolumns.length from tempdb..syscolumns,tempdb..sysobjects 
		where tempdb..sysobjects.name='##cursor1_brut' and tempdb..sysobjects.id=tempdb..syscolumns.id and tempdb..syscolumns.name='PensieFUnitate'),0)=0 
	...
	else isnull((select count(1) from brut b1 where max(p.Somaj_1)<>0 and b1.data=@DataS and b1.marca<b.marca and b1.marca in (select p1.marca from personal p1 left outer join fDeclaratia112TagAsigurat (@DataJ, @DataS, @LocmJ) a2 on b1.data=a2.data and b1.marca=a2.marca where p1.cod_numeric_personal=max(p.cod_numeric_personal) and a2.Tip_asigurat=max(a1.Tip_asigurat) and a2.Tip_contract=max(a1.Tip_contract) and (p1.loc_ramas_vacant=0 or p1.Data_plec>@DataJ) and p1.Somaj_1<>0 and p1.Tip_colab not in ('CCC','DAC')) 
	and (select count(1) from brut b2 where b2.data=@DataS and b2.marca>b.marca and b2.marca in (select p2.marca from personal p2 left outer join fDeclaratia112TagAsigurat (@DataJ, @DataS, @LocmJ) a3 on b2.data=a3.data and b2.marca=a3.marca where p2.cod_numeric_personal=max(p.cod_numeric_personal) and a3.Tip_asigurat=max(a1.Tip_asigurat)  and a3.Tip_contract=max(a1.Tip_contract) and p2.Somaj_1<>0  and (p2.loc_ramas_vacant=0 or p2.Data_plec>@DataJ)))=0),0) end),
	...
	from brut b
		...
		left outer join fDeclaratia112TagAsigurat (@DataJ, @DataS, @LocmJ) a1 on b.data=a1.data and b.marca=a1.marca 
