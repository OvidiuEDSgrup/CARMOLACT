--***
create procedure [dbo].[regii] @dDataJos datetime,@dDataSus datetime as  
begin  
/*Se insereaza decontari de tip regie loc de munca sau generala pentru cele ce nu au asa ceva*/  
declare @cRegieNedec char(20),@cArtGen char(13),@cArtLm char(13),@lNeterminata int,@lDafora int,@lItinerar int  
declare @bucla int,@nLm int, @cArtX char(13), @subunitate char(9)  
...
begin  
 ...
 select distinct @dDataSus,LEFT(LOC_DE_MUNCA_beneficiar,@nLm),comanda_beneficiar,  
 (case when art_calc_benef='' then @cArtX else art_calc_benef end),  
 lm_sup,comanda_sup,'T',1,0,0,'DA','' from costtmp t1,comenzi  
 where comenzi.subunitate = @subunitate and
 t1.comanda_sup=comenzi.comanda and   
 comenzi.tip_comanda='X' and   
 not exists  
 (select * from costtmp t2 where  
 t1.lm_sup=t2.lm_inf and t1.comanda_sup=t2.comanda_inf)  
 set @bucla=@@rowcount  
end  
...
'T',lm_sup,comanda_sup,'T',1,0,0,'DL','' from costtmp t1,comenzi  
where comenzi.subunitate = @subunitate and  
t1.COMANDA_inf<>'' and   
t1.comanda_sup=comenzi.comanda and   
comenzi.tip_comanda not in ('P','R','A','L','S')   
...
insert into costtmp (DATA,LM_SUP,COMANDA_SUP,ART_SUP,LM_INF,COMANDA_INF,ART_INF,CANTITATE,VALOARE,PARCURS,Tip,Numar) 
select distinct @dDataSus,lm_sup,'','T',lm_sup,comanda_sup,'T',1,0,0,'DL','' from costtmp t1,comenzi  
where comenzi.subunitate = @subunitate  and
t1.COMANDA_inf<>'' and   
t1.comanda_sup=comenzi.comanda and   
comenzi.tip_comanda='L'   
...
INSERT INTO COSTTMP (DATA,LM_SUP,COMANDA_SUP,ART_SUP,LM_INF,COMANDA_INF,ART_INF,CANTITATE,VALOARE,PARCURS,Tip,Numar) 
select distinct @dDataSus,t1.lm_sup,t1.comanda_sup,'L',t2.lm_sup,'','T',0,0,0,'RL','' from costtmp t1,costtmp t2 where t1.lm_sup<>''  
and t2.lm_sup<>'' and t2.comanda_sup='' and t1.comanda_sup<>'' and   
t1.lm_sup like rtrim(t2.lm_sup)+'%'  
and (select tip_comanda from comenzi where comanda=t1.comanda_sup and subunitate = @subunitate ) not in ('L','G')   
and t1.lm_sup+t1.comanda_sup not in (select lm_sup+comanda_sup from costtmp where   
exists (select 1 from costtmp t2 where costtmp.lm_sup=t2.lm_inf and costtmp.comanda_sup=t2.comanda_inf and t2.tip='DY'))  
-- 3 linii mai sus: Ghita, 22.04.2004-20.05.2004  
  
/*Sterg regia loc de munca la cele care dau regie loc de munca*/  
delete from costtmp  
where   
comanda_inf='' and lm_inf in (select loc_de_munca from speciflm where RTrim(left(comanda,20))<>'') or   
exists  
(select * from costtmp t2 where  
costtmp.lm_inf=t2.lm_sup and costtmp.comanda_inf='' and costtmp.art_sup='L' and costtmp.tip='RL' and   
t2.comanda_sup='' and  
costtmp.lm_sup=t2.lm_inf and costtmp.comanda_sup=t2.comanda_inf)  
  
/*Sterg regia locului de munca la cele ce nu au baza pentru a primi regie loc de munca*/  
DELETE FROM COSTTMP WHERE  
ART_SUP='L' AND TIP='RL' and NOT EXISTS  
(SELECT * FROM COSTTMP T2 WHERE COSTTMP.LM_SUP=T2.LM_SUP AND   
COSTTMP.COMANDA_SUP=T2.COMANDA_SUP AND   
(T2.ART_SUP IN (SELECT ARTICOL_DE_CALCULATIE FROM ARTCALC WHERE BAZA_PT_REGIA_SECTIEI=1) OR  
T2.ART_INF IN (SELECT ARTICOL_DE_CALCULATIE FROM ARTCALC WHERE BAZA_PT_REGIA_SECTIEI=1)  
))  
...
(case when @lDafora=1 then isnull((select RTrim(left(comanda,20)) from speciflm where speciflm.loc_de_munca=left(lm_sup,2)),'') else '' end),'T',lm_sup,comanda_sup,'T',1,0,0,'GL','' from costtmp t1  
where t1.lm_sup<>'' and t1.comanda_sup='' and t1.lm_sup not in  
(select lm_inf from costtmp t2 where t2.comanda_inf='')  
...
begin  
 delete from costtmp where lm_inf<>'' and comanda_inf in   
 (select comanda from comenzi where tip_comanda='D' and subunitate = @subunitate)  
  
 insert into costtmp (DATA,LM_SUP,COMANDA_SUP,ART_SUP,LM_INF,COMANDA_INF,ART_INF,CANTITATE,VALOARE,PARCURS,Tip,Numar)
 select distinct @dDataSus,costtmp.lm_sup,costtmp.comanda_sup,'D',  
 t2.lm_sup,  
 t2.comanda_sup,  
 'T',0,0,0,'DE','' from costtmp,comenzi,  
 (select distinct lm_sup,comanda_sup from costtmp,comenzi c2 where c2.comanda=costtmp.comanda_sup and c2.tip_comanda='D') as t2   
 where comenzi.subunitate = @subunitate and 
 costtmp.comanda_sup=comenzi.comanda and  
...
begin  
 ...
 from costuri,comenzi where comenzi.subunitate = @subunitate and costuri.comanda=comenzi.comanda and comenzi.tip_comanda in ('P','R','A','S') and  
 not exists  
 (select * from costtmp where costtmp.lm_inf=costuri.lm and costtmp.comanda_inf=costuri.comanda and art_inf<>'N')  
...
CREATE TABLE costuri (lm char(13),comanda char(13),costuri float,cantitate float,pret float(53),  
 rezolvat int,nerezolvate int)  
...
from costuri,comenzi where comenzi.subunitate = @subunitate and  
costuri.comanda=comenzi.comanda and comenzi.tip_comanda in ('P','R','A','S') and  
not exists  
(select * from costtmp where costtmp.lm_inf=costuri.lm and costtmp.comanda_inf=costuri.comanda and art_inf<>'N')  
