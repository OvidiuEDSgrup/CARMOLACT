...
begin try
	...
	@dataSusAnt datetime, @dataJosNext datetime, @dataSusNext datetime, @DataRegistruPtAng datetime, @OreLuna int, @Dafora int, @Colas int, @ModifDateSalAmanate int, @NrZileAmanare int
...
--	Creez tabela temporara in care extrag datele din extinfop si le aranjez pe coloana pt. fiecare salariat	
	create table #DateRevisalContracte (Marca char(6), Cod_inf char(13), Cetatenie varchar(80), TipContract varchar(60), DataSfarsit datetime, ExceptieDataSfarsit varchar(60), 
		IntervalTimp varchar(60), Durata int, Repartizare varchar(60), TemeiIncetare varchar(60), TextTemeiIncetare varchar(60), DetaliiContract varchar(500), 
	...
	select e.Marca, max((case when e.Cod_inf='#CODCOR' then e.Cod_inf end)),
		isnull(substring(max((case when e.Cod_inf='RCETATENIE' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,80),'Romana') as Cetatenie, 
		substring(max((case when e.Cod_inf='DATAINCH' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,60) as TipContract,
		max((case when e.Cod_inf='DATASFCONDET' then e.Data_inf end)) as DataSfarsit,  
		isnull(substring(max((case when e.Cod_inf='EXCEPDATASF' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,60),'') as ExceptieDataSfarsit,
		isnull(substring(max((case when e.Cod_inf='TIPINTREPTM' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,60),'OrePeZi') as IntervalTimp,  
		isnull(max(case when e.Cod_inf='TIPINTREPTM' then e.Procent end),0) as Durata,  
		isnull(substring(max((case when e.Cod_inf='REPTIMPMUNCA' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,60),'OreDeZi') as Repartizare,  
		substring(max((case when e.Cod_inf='RTEMEIINCET' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,60) as TemeiIncetare,  
		substring(max((case when e.Cod_inf='TXTTEMEIINCET' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,60) as TextTemeiIncetare,  
		substring(max((case when e.Cod_inf='CONTRDET' then convert(char(10),e.Data_inf,111)+e.Val_inf end)),11,500) as DetaliiContract,  
		max((case when e.Cod_inf='DATAINCH' then e.Data_inf end)) as DataIncheiere,  
		isnull(max((case when e.Cod_inf='MMODIFCNTR' then e.Data_inf end)),'01/01/1901') as DataConsemnare,
		max((case when e.Cod_inf in ('DATAMFCT','DATAMDCTR','CONDITIIM','SALAR','DATAMRL') then e.Data_inf end)) as DataModificare,
		isnull(max((case when e.Cod_inf='PASAPORT' then e.Val_inf end)),'') as Pasaport,
		isnull(max((case when e.Cod_inf='CONTRACTVECHI' then e.Val_inf end)),'') as NumarContractVechi,
		isnull(max((case when e.Cod_inf='DATACNTRVECHI' then e.Data_inf end)),'') as DataContractVechi
	from Extinfop e
	LEFT OUTER JOIN personal p on p.marca=e.marca
	where e.cod_inf in ('RCETATENIE','RTEMEIINCET','TXTTEMEIINCET','CONTRDET','DATAINCH','DATASFCONDET','EXCEPDATASF','TIPINTREPTM','REPTIMPMUNCA','MMODIFCNTR','PASAPORT','CONTRACTVECHI','DATACNTRVECHI')
			or e.cod_inf in ('DATAMFCT','DATAMDCTR','CONDITIIM','SALAR','DATAMRL') and e.Data_inf<=(case when @DataRegistruPtAng>@dataSus then @DataRegistruPtAng else @dataSus end)
				and (@ModifDateSalAmanate=0 or @ModifDateSalAmanate=1 and (DateAdd(day,@NrZileAmanare,e.Data_inf)<=@DataRegistru or p.Loc_ramas_vacant=1 and p.Data_plec<=@DataRegistru)) 
	Group By e.Marca
...
--	pun datele returnate de procedura pRevisalStareContracte intr-o tabela temporara. 
	...
	select i.Data, i.Marca, (case when r.Cetatenie in ('Alta','UESEE') and r.Pasaport<>'' and p.cod_numeric_personal='' then r.Pasaport else p.Cod_numeric_personal end) as CNP, 
		...
		(case when charindex('-',isnull(p.Nr_contract,ip.Nr_contract))<>0 then left(isnull(p.Nr_contract,ip.Nr_contract),charindex('-',isnull(p.Nr_contract,ip.Nr_contract))-1) 
			when charindex('/',isnull(p.Nr_contract,ip.Nr_contract))<>0 and @Colas=0 
--	daca din sirul obtinut de dupa primul / am doua caractere / sau . inseamna ca in numarul de contract apare si data, caz in care aceasta se elimina
			and (len(substring(isnull(p.Nr_contract,ip.Nr_contract),charindex('/',isnull(p.Nr_contract,ip.Nr_contract))+1,20))-len(replace(substring(isnull(p.Nr_contract,ip.Nr_contract),charindex('/',isnull(p.Nr_contract,ip.Nr_contract))+1,20),'/',''))=2
				or len(substring(isnull(p.Nr_contract,ip.Nr_contract),charindex('/',isnull(p.Nr_contract,ip.Nr_contract))+1,20))-len(replace(substring(isnull(p.Nr_contract,ip.Nr_contract),charindex('/',isnull(p.Nr_contract,ip.Nr_contract))+1,20),'.',''))=2)
			then left(isnull(p.Nr_contract,ip.Nr_contract),charindex('/',isnull(p.Nr_contract,ip.Nr_contract))-1) else isnull(p.Nr_contract,ip.Nr_contract) end) as NumarContract,
	...
	where ((i.data=@dataSus or i.Data=@dataSusNext and p.Data_angajarii_in_unitate>@dataSus) --and (convert(char(1),p.loc_ramas_vacant)='0' or (p.Data_plec>=@DataRegistru or p.Data_plec>=@dataJos))
		...
		and (@Bugetari=0 or isnull(p.detalii.value('(/row/@functpublic)[1]','int'),convert(int,ip.Actionar))=0)		
