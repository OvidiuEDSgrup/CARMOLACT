...
Create procedure psCalcul_impozit_vennet 
	...
	@Pensded_ant float,@Pensluna float,@AvantajeMat float,@AvantajeMatImpozabile float,@DiurneNeimpoz float,@DataNasterii datetime,@CheltDed float,@CursEUROPensiiF decimal(10,4)
As
Begin try
...
--	calcul deducere personala de baza
	...
	Begin
		set @VenitDed=@Venit_total-(case when @Imps_H=1 or @NuDPSH=1 then @SumaImpoz else 0 end)
			...
			-@Indcm8915	--tratat diminuare venit pentru calcul deducere in baza codului fiscal (Art. 55, alin. 1) corelat cu OUG 158/2005, Art 2, alin. 1 litera a si c.
		exec calcul_deducere @venitBrut=@VenitDed, @nrPersIntr=@NrPersintr, @dedPers=@DedBaza output, @oreJustificate=@OreJust, @oreLuna=@OreLuna, 
			@grupaMunca=@GrpMP, @regimLucru=@RLPontaj, @venitBrutCuDed=@venitBrutCuDed, @venitBrutFaraDed=@venitBrutFaraDed, @DeducereLaOreLucrate=@ChindPont, @data=@datasus
...
*/
	...
	if exists(select * from sysobjects where name='DedBazaSP' and type='FN') 
		set @DedBaza=dbo.DedBazaSP (@dataJos, @dataSus, @Marca, @DedBaza, @dataSus)
	if exists(select * from sysobjects where name='DedBazaSP' and type='P') 
		exec DedBazaSP @dataJos=@dataJos, @dataSus=@dataSus, @Marca=@Marca, @DedBaza=@DedBaza output, @data=@dataSus
	...
	if @Pensmax_ded-@Pensded_ant>0 and not(@DedBaza>@Vennet_in_imp) and (case when @Pensded_lun=0 then @Pensluna else @Pensded_lun end)>0
	Begin
--		incepand cu 01.2016 trebuie efectuata plafonarea deducerii din baza de calcul a impozitului pentru pensii facultative in limita a 400 EURO
--		tinand cont de cursul EURO din fiecare luna in care virat pensia facultativa.
		set @DedPensie=dbo.valoare_minima((case when @Pensded_lun=0 then @Pensluna else @Pensded_lun end)/(case when @dataSus>='01/01/2016' then @CursEUROPensiiF else 1 end), @Pensmax_ded-@Pensded_ant,@DedPensie)
		set @DedPensie=(case when @dataSus>='01/01/2016' then round(@DedPensie*@CursEUROPensiiF,0) else @DedPensie end)
...
--	calcul venit baza de calcul impozit
	set @VenitBaza=@Vennet_in_imp-(case when @ProcImpoz<>10 then @CheltDed else 0 end)-(case when @GrpMP in ('C','N') and @TipColabP='FDP' then 0 else @DedSindicat end)-@DedBaza-@DedPensie-@SomajTehn+
	...
	select @VenitBaza=@Venit_total-(case when @ProcImpoz<>10 then @CheltDed else 0 end) where @GrpMP in ('O','P') and @TipColabP in ('DAC','CCC','ECT') 
...
--	calcul subventie somaj
	...
	--	pentru elevi angajati cu conventie pe perioada de vara, trebuie considerat ca norma de lucru este de 6, 7 sau 8 ore. 
				else @Oreluna-@OreSCdedSomaj end)/convert(float,@Oreluna*(case when (@ModAngP='N' and @Tipded_somajP in (1,2,3,4,9) or @ModAngP='D' and @Tipded_somajP=7) and @GrpMP in ('N','D','S') 
					and DateADD(year,18,@DataNasterii)>@dataJos and @rl>=6 then @rl/8 else 1 end)) else 1 end),0)
	End
End try
