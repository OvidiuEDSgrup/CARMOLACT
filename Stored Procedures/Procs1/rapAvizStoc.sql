...
select @mesajEroare=''
BEGIN TRY
	declare @cuTabela bit
	if object_id('tempdb..#rapAvizStoc_tabela') is null
	begin
		select @cuTabela=0
		create table #rapAvizStoc_tabela (cod varchar(100) default '')
		exec rapAvizStoc_faTabela
	end
	...
	/** Select principal */
	insert into #rapAvizStoc_tabela(cod, dencod, um, gestiune,
		dengestiune, data, stoc, pret)
	SELECT
	...
	if @cutabela=0
		select * from #rapAvizStoc_tabela
END TRY
BEGIN CATCH
	SET @mesajEroare = ERROR_MESSAGE() + ' (' + OBJECT_NAME(@@PROCID) + ')'
	insert into #rapAvizStoc_tabela(cod, dencod)
