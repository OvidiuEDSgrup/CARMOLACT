...
begin try
    if exists (select 1 from sysobjects where type='P' and name = 'GenerareCompensariStocuriSP')
    begin
        exec GenerareCompensariStocuriSP 
			@stergerecomp=@stergerecomp, @generarecomp=@generarecomp, @datastoc=@datastoc, @datacomp=@datacomp, @tipcomp=@tipcomp, 
			@nrcomp=@nrcomp, @ctcomp=@ctcomp, @gestfiltru=@gestfiltru, @codfiltru=@codfiltru, @ctstocfiltru=@ctstocfiltru, 
