...
--	am facut SUM (in loc de MAX) pe Baza_stagiu pt. cazul in care sunt 2 marci pe acelasi CNP cu concediu medical (trebuie declarat stagiul cumulat)
		SUM(case when a.Indemnizatia_zi=0 and Indemnizatii_calc_manual=1 then 0 
			else isnull(nullif(a.detalii.value('(/row/@bazastagiu)[1]','decimal(10)'),0),s.baza_stagiu) end) as Baza_calcul, 
		MAX(isnull(nullif(a.detalii.value('(/row/@zilestagiu)[1]','int'),0),s.zile_stagiu)) as Zile_baza_calcul, 
