--***
create procedure rapBalantaTerti 
/*	Procedura pentru Financiar/Balanta terti.rdl
	exemplu de apel
declare @datajos datetime,@datasus datetime,@tip nvarchar(1),@cont nvarchar(4),@tert nvarchar(4),@tipdoc nvarchar(1),@ordonare nvarchar(1),@compensari nvarchar(1),@centralizare nvarchar(1),
	@doar_facturi_pe_sold nvarchar(1)
select @datajos='2010-01-01 00:00:00',@datasus='2010-01-31 00:00:00',@tip=N'F',@cont=N'4091',@tert=N'1185',@tipdoc=N'F',@ordonare=N'0',@compensari=N'1',@centralizare=N'1',@doar_facturi_pe_sold=N'0'
exec rapBalantaTerti @datajos=@datajos, @datasus=@datasus
*/
	(@sesiune varchar(50)=null, @datajos datetime,@datasus datetime
		...
		,@valuta varchar(20)=null
			--> filtre
		,@cont varchar(40)='',@tert varchar(40)=null, @locm varchar(20)=null
		,@gtert varchar(100)=null
...
begin
	...
	select @tipef=(case when @tip='F' then 'P' else 'I' end)
			,@epsilon=0.009 ,@detTVA=1 ,@semn_sold=(case when @tip='B' then 1 else -1 end)
...
/**1.	Se citesc datele in tabela intermediara #fFacturi pentru a fi mai usor de impartit pe solduri si rulaje:	*/
	...
	where (@tert is null or p.tert=@tert)
...
/**3.	Se trimit datele catre raport, aplicandu-se filtre finale	*/
	select max(sursa) sursa, max(numar) numar, 
			...
			max(tert) tert, max(Denumire) Denumire, min(data) data, cont_de_tert cont 
	from #final
		where (@doar_facturi_pe_sold=0 or @doar_facturi_pe_sold=1 and (ABS(rulaj_debit-rulaj_credit)>@epsilon and abs(sold_initial-(rulaj_debit-rulaj_credit))>@epsilon or ABS(rulaj_debit-rulaj_credit)<@epsilon and abs(sold_initial)>@epsilon))
