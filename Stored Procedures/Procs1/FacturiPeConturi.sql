...
BEGIN TRY
	DECLARE @tert VARCHAR(20), @factura VARCHAR(20), @mesaj VARCHAR(400), @tipOperatiune VARCHAR(2), @utilizator varchar(50), 
		@valuta varchar(3), @sub varchar(9), @bugetari int, @rullm int, @facturiPeConturi int, @parXMLFact xml 
	...
	set @rullm=isnull((select max(convert(int,Val_logica)) from par where Tip_parametru='GE' and Parametru='RULAJELM'),0)
	...
	if exists (select 1 from sysobjects where [type]='P' and [name]='FacturiPeConturiSP')
	begin
		-- scopul acestei proceduri este sa permita ocolirea executarii procedurii de mai jos, in anumite conditii
		-- de ex. daca procedura va stabili ca pentru IB nu se executa decat daca tertul este '12345' atunci procedura va fi urmatoarea:

		--CREATE PROCEDURE FacturiPeConturiSP @sesiune VARCHAR(50), @parXML XML output
		--AS

		--DECLARE @tert VARCHAR(20), @tipOperatiune VARCHAR(2), @faraFacturiPeConturi int
	
		--SET @tipOperatiune = isnull(@parXML.value('(/*/*/@tipOperatiune)[1]', 'varchar(2)'),'') 
		--SET @tert = isnull(@parXML.value('(/*/*/@tert)[1]', 'varchar(20)'),'')

		--set @faraFacturiPeConturi=1
		--if @tipOperatiune<>'IB' or @tert='12345'
		--	set @faraFacturiPeConturi=0
		
		--if @parXML.value('(/*/*/@faraFacturiPeConturi)[1]','varchar(2)') IS NULL
		--	set @parXML.modify ('insert attribute faraFacturiPeConturi {sql:variable("@faraFacturiPeConturi")} into (/row/row)[1]')
		--else 
		--	set @parXML.modify ('replace value of (/row/row/@faraFacturiPeConturi)[1] with sql:variable("@faraFacturiPeConturi")')

		exec FacturiPeConturiSP @sesiune=@sesiune, @parXML=@parXML output
		if @parXML.value('(/*/*/@faraFacturiPeConturi)[1]','varchar(2)')=1
			return
	end
	...
	begin
		...
		begin
			...
			group by f.tert, f.factura, f.cont_de_tert, f.indbug, (case when @rullm=1 then f.loc_de_munca else '' end)
