...
Begin try
	SET NOCOUNT ON
	declare 
		@cSub varchar(20),@CtTvaNeexPlati varchar(40),@CtTvaNeexIncasari varchar(40),@CtTvaCol varchar(40),@CtTvaDed varchar(40),
		@CtTvaNeexDocAvans varchar(40),@CtTvaNeexNesosite varchar(40),  @CtTvaNeexNeintocmite varchar(40),  @CtDocFaraFact varchar(200),
...
--	RM, RS, AP, AS, FF, SF, FB, IF
	update ctva 
		set ctva.contTVA=isnull(nullif(pd.detalii.value('/row[1]/@cont_tva','varchar(50)'),''),(case 
			when cdff.cont is not null then (case when ctva.tipf='B' then @CtTvaNeexNeintocmite else @CtTvaNeexNesosite end) 
	...
	if exists (select * from sysobjects where name ='contTVAPozDocumentSP2' and xtype='P')
		exec contTVAPozDocumentSP2 @sesiune=@sesiune, @parXML=@parXML
end try
