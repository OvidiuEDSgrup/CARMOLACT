...
begin try
	...
	select rtrim(d.Factura) as Factura, max(d.Data) as data, max(rtrim(isnull(t.cod_fiscal,d.Tert))) as Tert,
			...
			sum(d.greutate) greutate, d.data_facturii, max(d.data_scadentei) data_scadentei
	from #date d 
