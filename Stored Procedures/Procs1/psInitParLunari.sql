...
--	inserare parametru pt. ore lucratoare luna
		SET @comandaSql = N'
			delete from '+(case when @multiFirma=1 then 'par_lunari_lm' else 'par_lunari' end)+' where @deLaInchidere=1 and Tip='+'''PS'''+' and Parametru='+'''ORE_LUNA'''+' and data=@dataSus '
			+(case when @multiFirma=1 then ' and Loc_de_munca=@lm ' else '' end)
		exec sp_executesql @statement=@comandaSql, @params=N'@dataSus datetime, @lm varchar(9), @deLaInchidere int', @dataSus=@dataSus, @lm=@lm, @deLaInchidere=@deLaInchidere
