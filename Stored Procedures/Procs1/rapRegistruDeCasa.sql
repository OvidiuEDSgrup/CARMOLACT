...
begin try
	...
	select c.cont, isnull(p.valoare,'') valuta 
		from arbconturi(@cont) c
		left join proprietati p on p.tip='cont' and p.cod_proprietate='invaluta' and p.valoare<>'' and c.cont=p.cod
		where (@valuta is null or rtrim(isnull(p.valoare,''))=@valuta)
	...
	--union all -- diferenta de curs la facturi
	--select subunitate, (case when Cont_dif=@cont then Cont_dif else cont end) as cont, data, numar, 
	--	(case when Cont_dif like '7%' then 'Iz' else 'Pz' end) as plata_incasare, tert, factura, isnull(marca,'') as marca, 
	--	Cont_dif as cont_corespondent, Suma_dif as suma,
	--	(case when left(plata_incasare,1)='I' and Cont_dif like '7%' then suma_dif else 0 end) as sumai,
	--	(case when left(plata_incasare,1)='P' and Cont_dif like '6%' then suma_dif else 0 end) as sumap,
	--	'' valuta, 0 curs, 0 suma_valuta,
	--	0 as sumavi,
	--	0 as sumavp,
	--	explicatii, utilizator, jurnal, numar_pozitie,0 as sold,0 as soldv, 0 as sold_prec, 0 as soldv_prec
	--from pozplin p
	--where subunitate='1' and exists (select 1 from #conturi c where (c.cont=p.cont or c.cont=p.cont_dif))
	--	and ((@eContUtiliz=0 or exists (select 1 from @ContUtiliz u where cont like u.valoare+'%'))
	--		or (@eContUtiliz=0 or exists (select 1 from @ContUtiliz u where Cont_dif like u.valoare+'%')))
	--	and data between @datajos and @datasus and (@utilizator is null or utilizator=@utilizator)
	--	and (@jurnal is null or rtrim(jurnal)=@jurnal)
	--	and (@locm is null or p.Loc_de_munca like @locm+'%')
	--	and (@eLmUtiliz=0 or exists (select 1 from lmfiltrare lf where lf.utilizator=@utilizatorAsis and p.Loc_de_munca=lf.cod))
	--	and p.suma_dif<>0 
	union all -- diferenta de curs la deconturi
	select subunitate, (case when isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'')=@cont then isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'') else cont end) as cont, data, numar, 
		(case when isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'') like '7%' then 'Iz' else 'Pz' end) as plata_incasare, tert, factura, isnull(marca,'') as marca, 
		isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'') as cont_corespondent, 
		(case when isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'') like '6%' then -1 else 1 end)*isnull(p.detalii.value('(/*/@_difcursdec)[1]', 'float'),0) as suma,
		(case when isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'') like '7%' then isnull(p.detalii.value('(/*/@_difcursdec)[1]', 'float'),0) else 0 end) as sumai,
		(case when isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'') like '6%' then -isnull(p.detalii.value('(/*/@_difcursdec)[1]', 'float'),0) else 0 end) as sumap,
		'' valuta, 0 curs, 0 suma_valuta,
		0 as sumavi,
		0 as sumavp,
		explicatii, utilizator, jurnal, numar_pozitie,0 as sold,0 as soldv, 0 as sold_prec, 0 as soldv_prec
	from pozplin p
	where subunitate='1' and exists (select 1 from #conturi c where (c.cont=p.cont or c.cont=isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'')))
		and ((@eContUtiliz=0 or exists (select 1 from @ContUtiliz u where cont like u.valoare+'%'))
			or (@eContUtiliz=0 or exists (select 1 from @ContUtiliz u where isnull(p.detalii.value('(/*/@_contdifdec)[1]', 'varchar(100)'),'') like u.valoare+'%')))
		and data between @datajos and @datasus and (@utilizator is null or utilizator=@utilizator)
		and (@jurnal is null or rtrim(jurnal)=@jurnal)
		and (@locm is null or p.Loc_de_munca like @locm+'%')
		and (@eLmUtiliz=0 or exists (select 1 from lmfiltrare lf where lf.utilizator=@utilizatorAsis and p.Loc_de_munca=lf.cod))
		and isnull(p.detalii.value('(/*/@_difcursdec)[1]', 'float'),0)<>0
	union all
	...
	select @datat=isnull(@datat, @datajos)
	declare @soldi decimal(15,5), @soldvi decimal(15,5)
	
	exec pRulajeConturi @nivelplancontabil=1, @dData=@datat, @ccont=@cont, @cJurnal=@jurnal, @cLM=@locm, @datajos='1901-1-1', @cvaluta='', @sesiune=@sesiune
	delete from #pRulajeConturi_t where cont not in (select cont from #conturi)
	...
	begin
		...
		delete from #pRulajeConturi_t where cont!=@cont
	...
	create table #temp_solduri (data datetime, sold float, soldv float, sold_prec float, soldv_prec float, valuta varchar(10))
	insert into #temp_solduri(data, sold, soldv, sold_prec, soldv_prec, valuta)
		select d.data,
				@soldi+sum((case @tipC when 'P' then -1 else 1 end)*s.suma),
				@soldvi+sum((case @tipC when 'P' then -1 else 1 end)*s.suma_valuta),		
				@soldi+sum(case when d.data=s.data then 0 else (case @tipC when 'P' then -1 else 1 end)*s.suma end),
				@soldvi+sum(case when d.data=s.data then 0 else (case @tipC when 'P' then -1 else 1 end)*s.suma_valuta end)
				,max(d.valuta) valuta
				from 
			...
			where d.data>=s.data group by d.data
		union all
			--> daca nu exista linie la datajos se creeaza:
		select @datajos, @soldi, @soldvi, @soldi, @soldvi, @valuta
			where not exists (select 1 from #inreg d where d.data=@datajos)
	 order by data
	--if suser_name() like '%luci.maier%' select * from #temp_solduri
...
--> centralizare: 0 = grupat pe document
	...
	insert into #rapRegistruDeCasa_Tabela(subunitate, cont, data, numar, plata_incasare, tert, factura, marca, cont_corespondent, 
		suma, sumai, sumap, valuta, curs, suma_valuta, sumavi, sumavp,
		explicatii, utilizator, jurnal,
		numar_pozitie, sold, soldv, sold_prec, soldv_prec, 
		observatiiFooter 
		, antet)
	select --*  
		...
		(case when max(explicatii)=min(explicatii) then max(explicatii) else '<multiple>' end) explicatii, max(utilizator) utilizator, max(jurnal) jurnal,
		max(numar_pozitie) numar_pozitie, 0 sold, 0 soldv, 0 sold_prec, 0 soldv_prec, 
	...
	--> antet
	union all 
	select '','',data,'','','','','','','',0,0,'','',0,0,0,'','','','',sold,soldv,sold_prec, soldv_prec, '' as observatiiFooter, 1 as antet from #temp_solduri
	--where (@valuta is null or valuta=@valuta)
	order by data, antet desc, numar_pozitie
	
	if @cutabela=0
	select * from #rapRegistruDeCasa_Tabela
