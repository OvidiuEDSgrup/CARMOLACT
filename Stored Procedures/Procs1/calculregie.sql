--***
create procedure [dbo].[calculregie] @dDataJos datetime,@dDataSus datetime as
begin
...
if (select rezolvat from costuri where lm='' and comanda='')=1
	begin 
		...
		begin
			...
			from pozincon p1
			where p1.cont_creditor like '7%' and p1.cont_debitor not like '121%' and data between @dDataJos and @dDataSus and 
			exists (select * from costtmp t2 where t2.art_sup='G' and left(p1.loc_de_munca,@nLm)=t2.lm_sup and p1.comanda=t2.comanda_sup ))
			where lm='' and comanda=''
			update costuri set cantitate=cantitate-
			(select isnull(sum(suma),0) 
			from pozincon p1
			where p1.cont_debitor like '711%' and p1.cont_creditor not like '121%' and data between @dDataJos and @dDataSus and 
			exists (select * from costtmp t2 where t2.art_sup='G'
			and left(p1.loc_de_munca,@nLM)=t2.lm_sup and p1.comanda=t2.comanda_sup ))
		...
		if @nDecReg=1
			update costtmp set cantitate=(select isnull(sum(tmp1.cantitate*(case when nomencl.greutate_specifica=0 then 1 else nomencl.greutate_specifica end)),0) from costtmp tmp1,nomencl,comenzi,pozcom
		...
		begin
			update costtmp set
			cantitate=(select isnull(sum(suma),0) 
			from pozincon p1 where p1.cont_creditor like '7%' and p1.cont_debitor not like '121%' and data between @dDataJos and @dDataSus and 
			left(p1.loc_de_munca,@nLm)=costtmp.lm_sup and p1.comanda=costtmp.comanda_sup)
			where art_sup='G' and parcurs=0 and costtmp.comanda_inf='' and costtmp.lm_inf=''
			update costtmp set cantitate=cantitate-(select isnull(sum(suma),0) 
			from pozincon p1 where p1.cont_debitor like '7%' and p1.cont_creditor not like '121%' and data between @dDataJos and @dDataSus and 
			left(p1.loc_de_munca,@nLm)=costtmp.lm_sup and p1.comanda=costtmp.comanda_sup)
			where art_sup='G' and parcurs=0 and valoare<>0 and costtmp.comanda_inf='' and costtmp.lm_inf=''
		end
		else if @nDecReg=3
