...
begin try
	...
	union all 
	select a.subunitate, left(a.marca, 6), (case when @GrMarcaCont=1 then rtrim(a.cont_corespondent)+ltrim(rtrim(a.valuta)) else rtrim(a.decont) end), a.plata_incasare, left(a.numar,20), a.data, 
		(case when a.data between @dDataJos and @dDataSus then '2' else '1' end), 
		(case when a.plata_incasare='ID' then (case when @RestitCaAchit=1 then 0 else -a.suma+isnull(a.detalii.value('(/row/@_difcursdec)[1]','float'),0) end) else a.suma end), 
		(case when a.plata_incasare='ID' and @RestitCaAchit=1 then a.suma-isnull(a.detalii.value('(/row/@_difcursdec)[1]','float'),0) else 0 end), 
	...
	from pozplin a
		...
		left join #fltdec fd on fd.Marca=a.Marca and fd.decont=(case when @GrMarcaCont=1 then rtrim(a.cont_corespondent)+ltrim(rtrim(a.valuta)) else rtrim(a.decont) end) and fd.Cont=a.cont_corespondent
	where a.subunitate=@Sb and isnull(c.sold_credit, 0) = 9 and a.data between @dDataJosDoc and @dDataSus
		and (isnull(@cMarca, '')='' or a.marca=@cMarca) and (isnull(@cDecont, '')='' or (case when @GrMarcaCont=1 then rtrim(a.cont_corespondent)+ltrim(rtrim(a.valuta)) else a.decont end)=@cDecont) 
		and (isnull(@cCont, '')='' or a.cont_corespondent like rtrim(@cCont)+'%')
		and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
		and (@tabela_fltdec=0 or fd.decont is not null and a.Data<=fd.data)

	union all 
	select a.subunitate, left(a.marca, 6), (case when @GrMarcaCont=1 then rtrim(a.cont)+ltrim(rtrim(a.valuta)) else rtrim(a.decont) end), a.plata_incasare, left(a.numar,20), a.data, 
		(case when a.data between @dDataJos and @dDataSus then '2' else '1' end), 
	...
	from pozplin a
		...
		left join lmfiltrare pr on pr.cod=a.loc_de_munca and pr.utilizator=@userASiS
		left join #fltdec fd on fd.Marca=a.Marca and fd.decont=(case when @GrMarcaCont=1 then rtrim(a.cont)+ltrim(rtrim(a.valuta)) else rtrim(a.decont) end) and fd.Cont=a.cont
	where a.subunitate=@Sb and isnull(c.sold_credit, 0) = 9 and a.data between @dDataJosDoc and @dDataSus 
		and (isnull(@cMarca, '')='' or a.marca=@cMarca) and (isnull(@cDecont, '')='' or (case when @GrMarcaCont=1 then ltrim(rtrim(a.cont))+ltrim(rtrim(a.valuta)) else a.decont end)=@cDecont) 
