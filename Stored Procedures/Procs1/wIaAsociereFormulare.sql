...
SELECT wcf.idAsociere AS idAsociere, RTRIM(wcf.meniu) meniu, RTRIM(wcf.tip) AS tipDoc, RTRIM(wcf.cod_formular) AS formular,
	RTRIM(af.Denumire_formular) AS denformular, RTRIM(isnull(wcm.Nume,wcf.meniu)) AS denmeniu, isnull(RTRIM(wft.nume), rtrim(wcf.tip)) AS dentip,
	RTRIM(wcf.loc_munca) as lm, RTRIM(lm.denumire) as denlm
...
left join lm on lm.cod=wcf.loc_munca
WHERE (
		isnull(@f_tip, '') = ''
