...
*/
	...
	*/
	...
	declare @userASiS varchar(100),@lista_lm int
	...
	*/
	select 
		...
		(case when p.tip='DF' and left(p.Cont_de_stoc,3)<>'371' or p.tip in ('RM','RS','RP','RZ') and (left(p.Cont_intermediar,3)='442' or ci.subunitate is null or ci.are_analitice=1) then '' else p.Cont_intermediar end) Cont_intermediar,p.Cont_factura,
		...
		CONVERT(decimal(17,5),(case when p.valuta<>'' and (t.tert_extern=1 or p.tip='AI' and p.jurnal='MFX' or n.tip='F') then p.pret_valuta*(1.00+p.discount/100.00) else 0 end)) as PretFurnizorValuta,
		...
		-->	Punem in data_operarii si ora_operarii, altfel nu avem corect in pozincon data si ora operarii. Am tratat la ora_operarii astfel intrucat pot fi cazuri de documente importate unde nu este completata complet ora operarii
		convert(datetime,convert(datetime,convert(varchar(10),p.data_operarii,101),101),108)
			+(case when ora_operarii='' then '00:00:00' 
				else convert(datetime,left(ora_operarii,2)+':'+SUBSTRING(ora_operarii,3,2)+':'+(case when SUBSTRING(ora_operarii,5,2)='' then '00' else SUBSTRING(ora_operarii,5,2) end),114) end) Data_operarii,
		'' Ora_operarii,
		...
		(case when @bugetari=1 and p.tip='AP' then cs.detalii.value('(/row/@indicator)[1]','varchar(20)') end) as IndbugContStoc, 
	...
	LEFT JOIN conturi cs on p.Subunitate=cs.Subunitate and p.Cont_de_stoc=cs.cont
	LEFT JOIN conturi ci on p.Subunitate=ci.Subunitate and p.Cont_intermediar=ci.cont
	...
	*/
	insert into #pozdoc 
		...
		ContAm, ContMFIstoric, ContCorespMF, ContMF8045, AmortGradNeutiliz, ValAmortizata, RezerveReevaluare, IndbugContcoresp, IndbugContStoc, cont_dif_av, dif_curs_av
		)
	select 
		...
		--am modificat gestiunea primitoare pentru a nu da efect pozitia cu I la DADAOS si DTVANEEX. Am pastrat tot timpul inversarea gestiunilor.
		(case when p.tip in ('AT','AN') or 1=1 then p.Gestiune else p.gestiune_primitoare end),	
		...
		-->	Punem in data_operarii si ora_operarii, altfel nu avem corect in pozincon data si ora operarii. Am tratat la ora_operarii astfel intrucat pot fi cazuri de documente importate unde nu este completata ora operarii
		convert(datetime,convert(datetime,convert(varchar(10),p.data_operarii,101),101),108)
			+(case when ora_operarii='' then '00:00:00' 
				else convert(datetime,left(ora_operarii,2)+':'+SUBSTRING(ora_operarii,3,2)+':'+(case when SUBSTRING(ora_operarii,5,2)='' then '00' else SUBSTRING(ora_operarii,5,2) end),114) end) data_operarii,
		'' ora_operarii,
		...
		null as ContAm, '' as ContMFIstoric, '' as ContCorespMF, '' as ContMF8045, 0 as AmortGradNeutiliz, 0 as ValAmortizata, 0 as RezerveReevaluare, null as IndbugContcoresp, null as IndbugContStoc, 
	...
	*/
	...
	/*Pentru gestiunile cu amanuntul de tip C cu evaluare*/
	if exists(select 1 from #PozDoc p inner join gestiuni g on p.Gestiune=g.cod_gestiune and g.pret_am=1 where not (p.tip='TE' and p.gestiune=p.gestiune_primitoare))
	begin
		select p.idpozdoc,p.cod,p.tip,p.Gestiune
		into #preturiam
		from #PozDoc p
		inner join gestiuni g on p.Gestiune=g.cod_gestiune and g.pret_am=1

		exec CreazaDiezPreturiAmanunt
		exec wIaPreturiAmanunt @sesiune,'<row/>'
		
		update p set pret_cu_amanuntul=pa.pret_amanunt,tva_neexigibil=pa.cota_tva,Pret_amanunt_predator=pa.pret_amanunt,
		TvaNeexUnitarIntrare=round(convert(decimal(17,5),pa.Pret_amanunt*pa.cota_tva/(100.00+pa.cota_tva)),@zecTvaNeex),
		TvaNeexUnitarIesire=round(convert(decimal(17,5), pa.Pret_amanunt*pa.cota_tva)/(100.00+pa.cota_tva),@zecTvaNeex)
		from #PozDoc p
		inner join #preturiam pa on pa.idpozdoc=p.idpozdoc and p.tip=pa.tip

		/*Tratam aici doar modificarile de pret*/
		update p set pret_amanunt_predator=porig.pret_amanunt_predator,
		TvaNeexUnitarIntrare=round(convert(decimal(17,5),porig.Pret_cu_amanuntul*p.TVA_neexigibil/(100.00+p.tva_neexigibil)),@zecTvaNeex),
		TvaNeexUnitarIesire=round(convert(decimal(17,5), porig.Pret_amanunt_predator*p.TVA_neexigibil)/(100.00+p.TVA_neexigibil),@zecTvaNeex)
		from #pozdoc p
		inner join pozdoc porig on p.idpozdoc=porig.idpozdoc
		where p.tip='TI' and p.gestiune=p.gestiune_primitoare
		
		delete p from #pozdoc p where p.tip='TE' and p.gestiune=p.gestiune_primitoare
	end
	/* 
	...
	*/
	...
	p.Cont_de_stoc,(case when p.cont_intermediar='' then p.ContCorespondentIntrare else p.cont_intermediar end),
	...
	p.Cont_de_stoc,(case when p.cont_intermediar='' then p.ContCorespondentIntrare else p.cont_intermediar end),p.valuta,p.curs,p.jurnal,p.indbug

	/*
		ISTOC2	- Incarcare stoc in pret de intrare 	  			Cont intermediar = Cont Corespondent Intrare			in lei si valuta
		Pret intrare= pret furnizor sau pret furnizor cu cota aplicata sau pret de stoc predare/intrare
	*/
	insert into #pozincon(TipInregistrare,Subunitate, Tip, Numar, Data, Valuta, Curs, Suma_valuta, Utilizator, Loc_de_munca, Comanda, Jurnal,Cont_debitor, Cont_creditor, Suma, Explicatii, Data_operarii, Ora_operarii, Indbug)
	select 'ISTOC2',p.subunitate,p.tip,p.numar,p.data,p.valuta,p.curs,sum(round(convert(decimal(17,5),p.cantitate*p.PretFurnizorValuta),2)),max(p.utilizator),p.loc_de_munca,p.comanda,p.jurnal,
	p.cont_intermediar,p.ContCorespondentIntrare,
	sum(round(convert(decimal(17,5),p.cantitate*p.PretFurnizorLei),2)) as valoare,
	isnull(max(p.explicatiidindetalii),isnull(max(p.explicatii_old),isnull(max(t.denumire),max(n.denumire)))) as explicatii,
	max(p.data_operarii), max(p.ora_operarii), p.indbug
	from #pozdoc p
	inner join nomencl n on p.cod=n.cod
	left join terti t on p.Subunitate=t.Subunitate and p.tert=t.tert
	where (p.Tip_miscare='I' or p.tip in ('RM','RS','AI')) and p.tip not in ('TE','DF') -- TE-ul este tratat ca iesire, deci "DSTOC". Se pare ca si DF-ul trebuie tratat ca iesire.
		and not(p.tip='AI' and (p.jurnal='MFX' or n.tip='F'))	--incarcarea pentru AI cu mijloace fixe se trateaza in inregDocMF. Sunt anumite particularitati.
		and p.cont_intermediar!=''
	group by p.subunitate,p.tip,p.numar,p.data,p.loc_de_munca,p.comanda,p.valuta,p.curs,
	p.cont_intermediar,p.ContCorespondentIntrare,p.valuta,p.curs,p.jurnal,p.indbug
	...
	*/
	declare 
		...
		@CtTVANedeductibilChelt varchar(40), @CtTvaNeexDocAvans varchar(40),  @CtTvaNeexNesosite varchar(40),  @CtTvaNeexNeintocmite varchar(40),  @CtDiscountSeparat varchar(40), 
	...
	set @CtTvaNeexNesosite= isnull(nullif((select top 1 RTRIM(Val_alfanumerica) from par where Tip_parametru='GE' and Parametru='CNEEXNESO'),''),@CtTvaNeexDocAvans)
	set @CtTvaNeexNeintocmite= isnull(nullif((select top 1 RTRIM(Val_alfanumerica) from par where Tip_parametru='GE' and Parametru='CNEEXNEIN'),''),@CtTvaNeexDocAvans)
...
*/
	/** Tratat pe partea de AI, AE si DF dupa modelul din TVACumparari si TVAVanzari. Sa ia in calcul doar documentele cu TVA. */
	IF OBJECT_ID('tempdb..#contTVAPozitieDoc') IS NOT NULL
		drop table #contTVAPozitieDoc
	select tip as tip,(case when tip in ('RM','RS','RP','AI') then 'F' else 'B' end) tipf,tert,factura,'pozdoc' as tabela,idPozdoc as idPozitieDoc,'' as tip_tva,contTva,convert(varchar(100),'') as sursaf
	into #contTVAPozitieDoc
	from #pozdoc
	where (tip in ('RM','RS') and left(Cont_factura,1) not in ('','8') or tip in ('RP','AP','AS','AN') or tip in ('AI','AE') and tva_deductibil<>0 or tip='DF' and Cota_TVA>0)
	...
	update #pozdoc set contTva=isnull(contTVA,'')
	...
	*/
	...
	(case when CHARINDEX(LEFT(p.cont_factura,3),@ConturiDocFaraFact)<>0 then @CtTvaDed else p.contTva end),
	sum(round(convert(decimal(17,5),p.TVA_deductibil),2)) as valoare,
	...
	group by p.subunitate,p.tip,p.numar,p.data,p.loc_de_munca,p.comanda,p.valuta,p.curs,
	p.cont_de_stoc,p.cont_factura,p.procent_vama,p.valuta,p.curs,p.jurnal,p.procent_vama,p.cota_tva,p.contTva,p.indbug

	/*
			TVADN	- Inregistrare TVA Deductibil nedeductibil cont de stoc (in functie de TIPTVA=4,5, pentru inceput documente care vin dinspre imobilizari).
	*/
	insert into #pozincon(TipInregistrare,Subunitate, Tip, Numar, Data, Valuta, Curs, Suma_valuta, Utilizator, Loc_de_munca, Comanda, Jurnal,Cont_debitor, Cont_creditor, Suma, Explicatii, Data_operarii, Ora_operarii, Indbug)
	...
	(case when p.procent_vama=5 then p.cont_de_Stoc else @CtTVANedeductibilChelt end),	
	(case when CHARINDEX(LEFT(p.cont_factura,3),@ConturiDocFaraFact)<>0 then @CtTvaDed else p.contTva end),
	...
	inner join terti t on p.Subunitate=t.Subunitate and p.tert=t.tert
	where 
		p.tip in ('RM','RS') and p.procent_vama in (4,5)
	group by p.subunitate,p.tip,p.numar,p.data,p.loc_de_munca,p.comanda,p.valuta,p.curs,
	p.cont_de_stoc,p.cont_factura,p.procent_vama,p.valuta,p.curs,p.jurnal,p.procent_vama,p.cota_tva,p.contTva,p.indbug
	...
	**/
	...
	/**
		ITDFF: Inregistrarea "suplimentara"	( vezi parametri: CNEEXREC si NEEXDOCFF) [408,418]
			Cont de stoc	=	@CtTvaNeexNesosite  (RM, RS)
			@CtTvaNeexNeintocmite =	Cont de stoc	(AP, AS)
	...
	**/
	...
	(case when p.tip in ('RM','RS') then p.Cont_de_stoc when p.tip in ('AP','AS') then @CtTvaNeexNeintocmite end),
	(case when p.tip in ('RM','RS') then @CtTvaNeexNesosite when p.tip in ('AP','AS') then p.Cont_de_stoc end),
	sum(round(convert(decimal(17,5),p.TVA_deductibil),2)) as valoare,
	isnull(max(p.explicatiidindetalii),max(t.denumire)) as explicatii,
	max(p.data_operarii), max(p.ora_operarii), p.indbug
	from #pozdoc p
...
*/
	...
	insert into #contTVAPozitieDoc 
		(tip,tipf,tert,factura,tabela,idPozitieDoc,tip_tva,contTva,sursaf)
	select tip as tip,'F' as tipf,tert,factura,'pozdoc' as tabela,idPozdoc as idPozitieDoc,'' as tip_tva,convert(varchar(40),'') as contTva,null as sursaf
	from pozdoc 
	where subunitate=@sub and (tip='RP' or tip='RZ' and TVA_deductibil<>0) 
		and idPozDoc in (select rp.idPozPrestare from RepartizarePrestari rp inner join #pozdoc rm on rp.idpozdoc=rm.idpozdoc)
	exec contTVAPozDocument @sesiune=null, @parXML=@parXMLTva
	...
	*/
	...
	(case when p.tip='TI' and p.gestiune=p.gestiune_primitoare and max(convert(int,isnull(g.pret_am,0)))=1 then /*Modificare de pret*/sum(convert(decimal(17,5),round(cantitate*(p.Pret_cu_amanuntul-p.tvaneexunitarintrare)-cantitate*(pret_amanunt_predator-tvaneexunitariesire),2)))
	else
		sum(round(convert(decimal(17,5),cantitate*p.Pret_cu_amanuntul),2)-round(convert(decimal(17,5),cantitate*pret_de_stoc),2)
			-round(convert(decimal(17,5),cantitate*p.TvaNeexUnitarIntrare),2)) end )as valoare,
	...
	inner join nomencl n on p.Cod=n.Cod
	where
		p.tip in ('RM','AI','TI','AT','AN') AND (g.tip_gestiune in ('A','V') or g.pret_am=1)
		/* Doar la conturile de stoc de marfa */
		and LEFT(p.Cont_de_stoc,3) IN ('371','357')
	group by p.subunitate,p.tip,p.numar,p.data,p.loc_de_munca,p.comanda,p.valuta,p.curs,
	p.Gestiune,p.gestiune_primitoare,p.Cont_de_stoc,(case when @anGestAdaos=1 then '.'+rtrim(p.Gestiune) when @anGrupaAdaos=1 then '.'+rtrim(n.Grupa) else '' end),p.valuta,p.curs,p.jurnal
	/*
	...
	*/
	...
	inner join nomencl n on p.Cod=n.Cod
	where
		p.tip in ('AP','AC','TE','AE','CM','AT','AN') AND (g.tip_gestiune in ('A','V') or g.pret_am=1)
		/* Doar la conturile de stoc de marfa */
		...
		and not (p.gestiune=p.gestiune_primitoare and isnull(g.pret_am,0)=1)
	group by p.subunitate,p.tip,p.numar,p.data,p.loc_de_munca,p.comanda,p.valuta,p.curs,
	...
	*/
	...
	sum(round(convert(decimal(17,5),p.Cantitate*(p.TvaNeexUnitarIntrare-/*Modificare de pret*/
		(case when p.tip='TI' and p.gestiune=p.gestiune_primitoare then p.TvaNeexUnitarIesire else 0 end))),2)) as valoare,
	isnull(max(p.explicatiidindetalii),max(g.denumire_gestiune)) as explicatii,
	max(p.data_operarii), max(p.ora_operarii)
	from #pozdoc p
	inner join gestiuni g on p.Subunitate=g.Subunitate and p.gestiune=g.cod_gestiune
	...
	*/
	...
	sum(round(convert(decimal(17,5),p.Cantitate*(case when p.tip='AE' then p.Pret_de_stoc when @DiscountSeparat=1 then p.ValLeiDinValuta else p.Pret_vanzare end)),2)) as valoare,
	isnull(max(p.explicatiidindetalii),isnull(max(t.denumire),'AE')) as explicatii,
	max(p.data_operarii), max(p.ora_operarii), p.indbug
	from #pozdoc p
	left join terti t on p.Subunitate=t.Subunitate and p.tert=t.tert
	inner join nomencl n on n.cod=p.cod
	where 
		p.tip in ('AP','AS','AN') or p.tip='AE' and p.Cont_venituri<>''
	group by p.subunitate,p.tip,p.numar,p.data,p.loc_de_munca,p.comanda,p.valuta,p.curs,
	...
	*/
	...
	max(p.data_operarii), max(p.ora_operarii), isnull(nullif(p.IndbugContcoresp,''),isnull(nullif(p.IndbugContStoc,''),p.indbug))
	from #pozdoc p
	inner join nomencl n on p.cod=n.cod
	...
	where 
		-- conditiile de generare:
		(p.Tip_miscare='E' or p.tip='TE' and p.Tip_miscare='V' and (isnull(g.tip_gestiune,'')='V' or isnull(gp.tip_gestiune,'')='V') or (p.Jurnal='MFX' or n.tip='F') and p.tip in ('AE','AP') or p.Tip_miscare='V' and p.tip='CM')
		-- exceptiile:
	...
	group by p.subunitate,p.tip,p.numar,p.data,p.loc_de_munca,p.comanda,p.valuta,p.curs,
	p.Cont_de_stoc,p.Cont_corespondent,p.cont_intermediar,p.valuta,p.curs,p.jurnal,isnull(nullif(p.IndbugContcoresp,''),isnull(nullif(p.IndbugContStoc,''),p.indbug))
	...
	**/
	...
	where 
		(p.TipInregistrare in ('VENFACTURA','ISTOC','ISTOC2') and 
			(p.Cont_creditor like '6%'
			or
			p.Cont_creditor like '7%' and cc.Tip_cont='A'
			))
		or
		(p.TipInregistrare like 'ISTOC%' and 
...
begin try	
	...
	update #pozdoc set tip='TE' where tip='TI' and gestiune=gestiune_primitoare --Modificarile de pret le facem inapoi TE
	/*	
	...
	*/
	...
	from pozincon p
		...
		where (@lista_lm=0 or exists (select 1 from LMFiltrare lu where lu.utilizator=@userASiS and lu.cod=p.Loc_de_munca ))
	...
	select Subunitate, Tip, Numar, Data, Cont_debitor, Cont_creditor, Suma, Valuta, Curs, Suma_valuta, 
		...
		, Numar_pozitie, Loc_de_munca, Comanda, left(Jurnal,3), isnull(Indbug,'')
		from #pozincon 
		order by tip,data
