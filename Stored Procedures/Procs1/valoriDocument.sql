...
-------------	completare valori in tabela temporara
	...
	set Cont_corespondent=g.Cont_corespondent, Cont_venituri=g.Cont_venituri, 
		Cantitate=g.cantitate, Valoare=g.Valoare, Valoare_furnizor=g.Valoare_furnizor, Tva_22=g.Tva_22, Tva_11=g.Tva_11, Valoare_valuta=g.Valoare_valuta, Valoare_valuta_tert=g.Valoare_valuta_tert, 
	...
	from (select p.subunitate, p.tip as tip, p.numar, max(p.data) as data, 
		...
		sum((case when p.Tip in ('RM','RS') then round(convert(decimal(17,5),p.Cantitate*p.Pret_valuta),@rotunjr_n) else 0 end)) as valoare_furnizor, 
	...
	begin
		...
		from (select p.subunitate, d.tip, p.numar, p.data, 
			...
			sum(case when p.valuta='' then 0 else round(convert(decimal(17,5), p.cantitate*p.pret_de_stoc*(1+p.discount/100)), 2)
				+(case when p.Valuta<>'' and p.curs>0 and p.procent_vama<>1 then (case when isnumeric(p.grupa)=1 then convert(float,p.grupa) else round(convert(decimal(17,5),p.TVA_deductibil/p.curs),2) end) 
					else 0 end) end) 
			as Valoare_valuta
		from pozdoc p
