...
--	select @q_loc_de_munca
	...
	select isnull(t.loc_de_munca,'') as cod,isnull(t.loc_de_munca,'') loc_de_munca, isnull(t.cont,'') as parinte,max(lm.denumire) as denumire,isnull(max(t.tip_cont),'B') as tip_cont,
				...
				convert(decimal(15,2),0) sold_initial_debit, convert(decimal(15,2),0) sold_initial_credit
				into #final
			from 	#tempsolduri t
	...
	select c.cont, '',
		...
		max(c.denumire_cont),max(c.tip_cont),0,0,0,0,0,0,max(c.nivel) as nivel, 0 as detaliu, 0 as parcurs, 0 as abssold, 0 as absrulaj, '' LMparinte,0,0,0
			from #conturi c
	group by c.cont
	union all
	select 'C'+left(c.cont,1), '',
		'Total','Clasa '+left(c.cont,1),'B',0,0,0,0,0,0,0 as nivel, 0 as detaliu, 0 as parcurs, 0 as abssold, 0 as absrulaj, '' LMparinte,0,0,0
			from #conturi c-- left join #tempsolduri t on c.cont=t.cont
	group by left(c.cont,1)
	union all
	select 'Total', '',
		null,'Total','B',0,0,0,0,0,0,-1 as nivel, 0 as detaliu, 0 as parcurs, 0 as abssold, 0 as absrulaj, '' LMparinte,0,0,0
...
-->	calculez totalurile:
	...
	update #final set sold_initial=sold_debit-sold_credit
		--> pentru totaluri care nu tin de conturi (len(cod)<3 sau nivel=-1) tipul contului nu e definit => si rulajul va fi corect distribuit:
	update #final set tip_cont=(case when len(cod)<3 or nivel<=0 then '' else tip_cont end)	
	
	--> calcul de solduri pe conturi:
	update #final 
		set sold_initial_debit=(case tip_cont when 'A' then sold_initial when 'P' then 0 else (case when sold_initial>0 then sold_initial else 0 end) end), 
			sold_initial_credit=-(case tip_cont when 'P' then sold_initial when 'A' then 0 else (case when sold_initial>0 then 0 else sold_initial end) end)		
		where nivel>0

	--update #final set sold_initial_debit=sold_debit, sold_initial_credit=sold_credit where tip_cont=''
	
	alter table #final add sold_final_debit decimal(15,2), sold_final_credit decimal(15,2)
	update #final set
	sold_final_debit=(case tip_cont when 'A' then sold_initial_debit-sold_initial_credit+rulajd_cumulat-rulajc_cumulat when 'P' then 0 else 
					(case when sold_initial_debit-sold_initial_credit+rulajd_cumulat-rulajc_cumulat>0 then sold_initial_debit-sold_initial_credit+rulajd_cumulat-rulajc_cumulat else 0 end)
				end),
	sold_final_credit=
					-(case tip_cont when 'P' then sold_initial_debit-sold_initial_credit+rulajd_cumulat-rulajc_cumulat when 'A' then 0 else
					(case when sold_initial_debit-sold_initial_credit+rulajd_cumulat-rulajc_cumulat>0 then 0 else sold_initial_debit-sold_initial_credit+rulajd_cumulat-rulajc_cumulat end)
				end)
		where nivel>0

	--> calcul solduri pe clase si total:
	update #final set sold_initial_debit=sold_debit, sold_initial_credit=sold_credit
		, sold_final_debit=sold_debit+rulajd_cumulat
		, sold_final_credit=sold_credit+rulajc_cumulat
	where nivel<=0

	update f set sold_initial_debit=i.sold_initial_debit, sold_initial_credit=i.sold_initial_credit,
			sold_final_debit=i.sold_final_debit, sold_final_credit=i.sold_final_credit
	from #final f
	inner join (select parinte
					, sum(sold_initial_debit) sold_initial_debit, sum(sold_initial_credit) sold_initial_credit
					, sum(sold_final_debit) sold_final_debit, sum(sold_final_credit) sold_final_credit
				from #final i where i.nivel=1 group by parinte) i on f.cod=i.parinte
	where f.nivel=0
	...
	inner join (select parinte
					, sum(sold_initial_debit) sold_initial_debit, sum(sold_initial_credit) sold_initial_credit
					, sum(sold_final_debit) sold_final_debit, sum(sold_final_credit) sold_final_credit
				from #final i where i.nivel=0 group by parinte) i on f.cod=i.parinte
	where f.nivel=-1

	select
			cod, loc_de_munca, parinte, denumire, tip_cont, sold_debit, sold_credit, rulajc_perioada, rulajd_perioada, 
			rulajc_cumulat, rulajd_cumulat, nivel, detaliu, sold_initial, sold_initial_debit, sold_initial_credit
			,sold_final_debit, sold_final_credit, f.nivel
			from #final f
		where (@q_centralizare=2 or detaliu=0)
			and (@q_cusoldrulaj=0 or
				 @q_cusoldrulaj=1 and absrulaj>0 or abssold>0)
		order by f.nivel, len(cod), cod
