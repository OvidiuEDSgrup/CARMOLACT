...
Begin try
	...
	@CalcITM int, @pITM decimal(5,2), @pSomaj decimal(5,2), @pFondGar decimal(5,2), @CoefCCI float, @CoefCAS float, 
...
--	daca tabela par este view inseamna ca se lucreaza cu parametrii pe locuri de munca (in aceeasi BD sunt mai multe unitati)	
	...
	set @CoefCAS=dbo.iauParN('PS','COEFCAS')
...
--		calcul contributii pe comanda 
		...
		if not(@GrupaM='O')
		Begin
			set @CASCom=round(@ValoarePoz*(case when @CoefCAS>0 and @CoefCAS<1 then @CoefCAS else 1 end)*((case when @GrupaMPontaj='S' then @pCasCS when @GrupaMPontaj='D' then @pCasCD else @pCasCN end)-@pCasIndiv)/100,2)
				+round(@ValPozCasCM*(case when @CoefCAS>0 and @CoefCAS<1 then @CoefCAS else 1 end)*((case when @GrupaMPontaj='S' then @pCasCS when @GrupaMPontaj='D' then @pCasCD else @pCasCN end)-@pCasIndiv)/100,0)
