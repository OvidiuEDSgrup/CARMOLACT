--***
CREATE procedure [dbo].[postcalcul] @ddatajos datetime,@ddatasus datetime,@comanda varchar(20),@cLM varchar(20), 
...
@Grupare varchar(1), @sursa varchar(1)='C'
as
...
select LM as lm_sup, Comanda as comanda_sup,'',SUM(suma) as valoare,(case when f.cont like '711%' then @art711 else Articol_de_calculatie end)
		,'',data,1
	from FisaPeCont f inner join conturi c on f.Cont=c.Cont
...
select s.lm_sup as lm,s.comanda_sup as comanda,isnull(sum(s.valoare*(case @RepartizareCost when '' then 1 when 'N' then isnull(pr.procent,0) 
	...
	as articol, max(isnull(a.denumire,'<inexistent>')) as denart, max(isnull(a.ordinea_in_raport,55)) as ordinea_in_raport, s.sursa,s.data, 
