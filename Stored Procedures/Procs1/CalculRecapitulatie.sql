...
CREATE PROCEDURE [dbo].[CalculRecapitulatie] @sesiune VARCHAR(50), @parXML XML
AS
IF EXISTS (
		SELECT *
		FROM sysobjects
		WHERE NAME = 'CalculRecapitulatieSP'
			AND type = 'P'
		)
BEGIN
...
DECLARE @elem VARCHAR(20), @nF INT, @cSQL VARCHAR(7000), @cFormula VARCHAR(7000), @nProcent INT, @nPas INT, @gPas INT, @cLinieUpdate 
	VARCHAR(8000), @nValImp FLOAT, @nivel INT, @id INT, @element VARCHAR(20), @procent FLOAT, @scriuPret BIT, @mesaj VARCHAR(500), 
	@parCalcul XML, @curs FLOAT, @valuta VARCHAR(10)
...
BEGIN TRY
	...
	BEGIN
		SET @parCalcul = (
				SELECT @nivel nivel, @calculat calculat, @id id, @curs curs, @valuta valuta
				FOR XML raw
				)

		EXEC CalculMaterialesiManopera @sesiune = @sesiune, @parXML = @parCalcul OUTPUT

		--@nivel, @calculat OUTPUT, @id
		SELECT @calculat = @parCalcul.value('(/*/@calculat)[1]', 'int')
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (CalculRecapitulatie)'
