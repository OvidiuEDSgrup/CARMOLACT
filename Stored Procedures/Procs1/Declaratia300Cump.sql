...
--	creare tabela temporara ce va prelucra rezultatul adus din jurnalul de cumparari si care va putea fi modificata printr-o procedura specifica.
	...
	create table #decontcump (numar char(20))
	exec CreazaDiezTVA '#decontcump'
...
--	exec Declaratia300Cumpdet @parXML
	...
	declare @parXMLJ xml
	set @parXMLJ=(select 1 tipcump, '' tipfact, 0 tvanx, @IAFTVA0 IAFTVA0, 0 tvaeronat, @CtNeimpoz CtNeimpoz, 1 as D300 for xml raw)
--	populare tabela #jtvacump prin apelul procedurii rapJurnalTVACumparari 
	...
	insert into #decontcump
		(numar, data, cod_tert, furnizor, codfisc, total, baza_19, tva_19, baza_9, tva_9, baza_5, tva_5, baza_19_9_reg, tva_19_9_reg, scutite, 
			...
			baza_oblig_2_19, tva_oblig_2_19, baza_oblig_2_9, tva_oblig_2_9, baza_oblig_2_5, tva_oblig_2_5, baza_oblig_2_reg, tva_oblig_2_reg)
	select numar, data, cod_tert, furnizor, codfisc, convert(decimal(15,3),d.total) as total
		,convert(decimal(15,3),d.baza_19)-convert(decimal(15,3),d.baza_19_reg)-convert(decimal(15,3),d.baza_5) as baza_19
		,convert(decimal(15,3),d.tva_19)-convert(decimal(15,3),d.tva_19_reg)-convert(decimal(15,3),d.tva_5)	as tva_19
...
--	regularizari aferente perioadelor anterioare + regularizari ce tin de cota TVA 24 inregistratata in perioada curenta dupa modificarea cotei de TVA la 20%.
		,convert(decimal(15,3),baza_19_reg)+convert(decimal(15,3),baza_oblig_2_reg) as baza_19_9_reg
		,convert(decimal(15,3),tva_19_reg)+convert(decimal(15,3),tva_oblig_2_reg) as tva_19_9_reg
		...
		,convert(decimal(15,3),d.baza_oblig_2)-convert(decimal(15,3),d.baza_oblig_2_reg) as baza_oblig_2
		,convert(decimal(15,3),d.tva_oblig_2)-convert(decimal(15,3),d.tva_oblig_2_reg) as tva_oblig_2
		...
		,convert(decimal(15,3),d.baza_oblig_2_19) as baza_oblig_2_19, convert(decimal(15,3),d.tva_oblig_2_19) as tva_oblig_2_19
		,convert(decimal(15,3),d.baza_oblig_2_9) as baza_oblig_2_9, convert(decimal(15,3),d.tva_oblig_2_9) as tva_oblig_2_9
		,convert(decimal(15,3),d.baza_oblig_2_5) as baza_oblig_2_5, convert(decimal(15,3),d.tva_oblig_2_5) as tva_oblig_2_5
		,convert(decimal(15,3),baza_oblig_2_reg) as baza_oblig_2_reg, convert(decimal(15,3),tva_oblig_2_reg) as tva_oblig_2_reg
	from #jtvacump d
	where (baza_19<>0 or tva_19<>0 or baza_9<>0 or tva_9<>0 or scutite<>0
			...
			or baza_oblig_1<>0 or tva_oblig_1<>0 or baza_oblig_2<>0 or tva_oblig_2<>0 or baza_19_reg<>0 or tva_19_reg<>0)
...
--	apelul procedurii specifice care permite modificarea continutului tabelei #decontcump
	...
	select isnull(convert(decimal(15),sum(total)),0) as total
		...
		,isnull(convert(decimal(15),sum(baza_oblig_2_19)),0) as baza_oblig_2_19, isnull(convert(decimal(15),sum(tva_oblig_2_19)),0) as tva_oblig_2_19
		,isnull(convert(decimal(15),sum(baza_oblig_2_9)),0) as baza_oblig_2_9, isnull(convert(decimal(15),sum(tva_oblig_2_9)),0) as tva_oblig_2_9
		,isnull(convert(decimal(15),sum(baza_oblig_2_5)),0) as baza_oblig_2_5, isnull(convert(decimal(15),sum(tva_oblig_2_5)),0) as tva_oblig_2_5
		,isnull(convert(decimal(15),sum(baza_oblig_2_reg)),0) as baza_oblig_2_reg, isnull(convert(decimal(15),sum(tva_oblig_2_reg)),0) as tva_oblig_2_reg
	from #decontcump
