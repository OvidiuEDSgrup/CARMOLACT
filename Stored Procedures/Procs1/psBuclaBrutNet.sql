...
Create procedure psBuclaBrutNet 
	@dataJos datetime, @dataSus datetime, @DataCor datetime, @marcaJos char(6), @locmJos char(9), @Precizie int, @pTipCorectie_venit char(2), @pTipCorectie char(2),
...
As
Begin
	declare @oreLuna float, @SalarMediu float, @pCasind float, @DedPersLCrt int, @ReglSalNetTN int, @AdVenNet_Q int, @Drumor int, @Colas int, @Velpitar int, 
	...
	@venitBrutCuDed float, @venitBrutFaraDed float, @DeducereLaOreLucrate int
	...
	from par_lunari
	where data=@datasus and tip='PS' and parametru in ('ORE_LUNA','SALMBRUT','CASINDIV')

	select 
		@DeducereLaOreLucrate=max(case when Parametru='CHINDPON' then Val_logica else 0 end),
		@DedPersLCrt=max(case when Parametru='CHINDLCRT' then Val_logica else 0 end),
		@ReglSalNetTN=max(case when Parametru='REGSALNTN' then Val_logica else 0 end),
		...
		@Velpitar=max(case when Parametru='VELPITAR' then Val_logica else 0 end)
	from par 
	where tip_parametru='PS' and parametru in ('CHINDPON','CHINDLCRT','CHINDPON','REGSALNTN','ADVNET-Q')
		or tip_parametru='SP' and parametru in ('DRUMOR','COLAS','VELPITAR')
...
--select @pVenitTotal, @pVenitBaza, @pImpozit, @pBazaCASInd
	...
	Begin
		...
		exec calcul_deducere @venitBrut=@BrutPtDeducere, @nrPersIntr=@NrPersIntr, @dedPers=@DedPers output, @oreJustificate=@oreJustificate, @oreLuna=@oreLuna, 
				@grupaMunca=@GrpMunca, @regimLucru=@regimLucru, @venitBrutCuDed=@venitBrutCuDed, @venitBrutFaraDed=@venitBrutFaraDed, @DeducereLaOreLucrate=@DeducereLaOreLucrate, @data=@dataSus
...
--	inlocuit @pSumaNeta cu @SumaNeta, pentru a functiona si in cazul in care se lucreaza cu salar net in lei (corectie H)
	...
	where data = @DataCor and marca = @marcaJos and Loc_de_munca=@locmJos and tip_corectie_venit=@pTipCorectie_venit
End
