...
Create procedure rapConcediiMedicale
	...
	@locm char(9)=null, @strict int=0, @activitate varchar(20)=null, @ordonare char(1), @alfabetic int, @CMmm30Zile int, @Luni_istoric int=6, 
...
begin try
	...
	@ord char(30), @ord1 char(30), @Continuare int, @ZileCMsusp int, @total_indemniz_marca decimal(10), @total_deducere decimal(10), @dataLuniiCM datetime,
	@venitBrutCuDed float, @venitBrutFaraDed float
...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	Create table #concedii_medicale
		...
		indemnizatie_unitate float, indemnizatie_fnuass float, indemnizatie_fambp float, indemnizatie_total float, indemnizatie_neta float, 
	...
	select a.data, a.marca, isnull(i.Nume,p.Nume), p.Cod_numeric_personal, isnull(i.Loc_de_munca,p.Loc_de_munca), lm.denumire, 
		...
		a.baza_calcul, p.vechime_totala, isnull(p.spor_cond_10,ip.spor_cond_10) as spor_cond_10, a.suma as data_risc, isnull(e.serie_certificat_cm_initial,'') as serie_certificat_cm_initial, 
	...
	where a.data between @dataJos and @dataSus and (@marca is null or a.marca=@marca)
		...
		and (a.tip_diagnostic<>'0-' or @tipdiagnostic='0-' and a.tip_diagnostic=@tipdiagnostic)
	...
	Begin
		set @dataLuniiCM=dbo.eom(@Data_inceput)
		if dbo.EOM(@dataJos)<>dbo.EOM(@dataSus)
		Begin
			set @ore_luna=dbo.iauParLN(@dataLuniiCM,'PS','ORE_LUNA')
			if @ore_luna=0
				set @ore_luna=dbo.zile_lucratoare(dbo.bom(@Data_inceput),@dataLuniiCM)*8
			set @salar_mediu=dbo.iauParLN(@dataLuniiCM,'PS','SALMBRUT')
			set @salar_minim=dbo.iauParLN(@dataLuniiCM,'PS','S-MIN-BR')
		End
...
--	calcul indemnizatie neta functie de contributiile sociale retinute pt. CM si impozit
		...
		Begin
			exec calcul_deducere @venitBrut=@total_indemniz_marca, @nrPersIntr=@nrpersintr, @dedPers=@total_deducere output, 
				@venitBrutCuDed=@venitBrutCuDed, @venitBrutFaraDed=@venitBrutFaraDed, @data=@dataLuniiCM
...
--	sfarsit calcul indemnizatie neta
		...
		if @CMmm30Zile=0 or @ZileCMsusp>0
			...
			select @Data, @Marca, @Nume, @cnp, @lm, @denlm, @vechime, @spor10, 
				...
				@Indemnizatie_unitate, @Indemnizatie_FNUASS, @Indemnizatie_FAAMBP, @Indemnizatie_bruta, @Indemnizatie_neta, 
	...
	select Data, Marca, Nume, Loc_de_munca, Denumire_lm, vechime, spor10, 
		...
		Baza_Stagiu, Zile_Stagiu, cnp, ordonare, indemnizatie_total
