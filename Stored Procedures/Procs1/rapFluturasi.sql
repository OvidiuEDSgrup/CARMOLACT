--***
create procedure rapFluturasi @datasus datetime, @locm char(20), @strict int=0, @tip_stat varchar(30),
...
declare @q_datajos datetime,@q_datasus datetime, @q_locm varchar(20), @q_strict int, @q_tip_stat varchar(100),
	...
	@denSistPrg varchar(100), -- parametru pentru denumire spor sistematic peste program
	...
	@SpCond1OL int, @SpCond2OL int, @SpCond3OL int, @SpCond4OL int, @SpCond5OL int
...
--	citesc perioada de impozitare a tichetelor pt. luna curenta
	...
	select @denSistPrg=MAX(case when Parametru='SPSISTPRG' then Val_alfanumerica else '' end),
		@spSistPrgLaOre=MAX(case when Parametru='SP-PR-ORE' then Val_logica else 0 end),
	...
	from par where Tip_parametru='PS' and Parametru in ('SP-PR-ORE','SPSISTPRG','SSPEC','INDC-SUMA','SCOND1','SCOND2','SCOND3','SCOND4','SCOND5')

	if nullif(@denSistPrg,'') is null
		set @denSistPrg='Spor sist. prg.'
...
/*	Pun datele de pontaj in tabela temporara pentru a le putea altera la nevoie (de. ex. ANAR, s-a suspendat sporul specific/inspectie, 
	dar s-au pastrat valorile procentelor in pontaj pentru a calcula valoarea sporurilor pentru a calcula provizioane. */
select * into #pontaj
from pontaj
where data between @q_datajos and @q_datasus

update po set 
	po.Spor_conditii_1=(case when @SpCond1OL=1 and ore__cond_1=0 or b.Spor_cond_1=0 then 0 else po.Spor_conditii_1 end),
	po.Spor_conditii_2=(case when @SpCond2OL=1 and ore__cond_2=0 or b.Spor_cond_2=0 then 0 else po.Spor_conditii_2 end),
	po.Spor_conditii_3=(case when @SpCond3OL=1 and ore__cond_3=0 or b.Spor_cond_3=0 then 0 else po.Spor_conditii_3 end),
	po.Spor_conditii_4=(case when @SpCond4OL=1 and ore__cond_4=0 or b.Spor_cond_4=0 then 0 else po.Spor_conditii_4 end),
	po.Spor_conditii_5=(case when @SpCond5OL=1 and ore__cond_5=0 or b.Spor_cond_5=0 then 0 else po.Spor_conditii_5 end)
from #pontaj po
inner join brut b on b.marca=po.marca and b.loc_de_munca=po.loc_de_munca and b.data=dbo.EOM(po.data)

if exists (select 1 from sysobjects where name='rapFluturasiSPIni' and xtype='P')
	exec rapFluturasiSPIni @datasus, @locm, @q_tip_stat, @marci, @functii, @str_nivel, @ordonare, @nrexemplare
...
from (select e.cod_functie,e.marca,e.grupa_de_munca,e.loc_ramas_vacant,e.data_plec,e.loc_de_munca, 
	e.Spor_de_functie_suplimentara Spor_de_functie_suplimentara
		from personal e 
		left outer join infopers ip on ip.marca=e.marca
		...
		and (isnull(@q_tip_stat,'')='' or isnull(e.tip_stat,ip.religia)=@q_tip_stat)
	) as e
...
p.marca=e.marca		-- p
	...
	--and charindex('|'+e.grupa_de_munca+'|',@q_gr_munca_str)<>0 
	and (e.loc_ramas_vacant=0 or e.data_plec>@q_datajos)
	and (isnull(e.loc_de_munca,p.loc_de_munca) like rtrim(@q_locm)+(case when @q_strict=1 then '' else '%' end) or @q_locm is null)
...
--		'R0'+--(case when c.ordine is null then 'Alte' else 
		   ...
		   ordine, 
		sum(r.retinut_la_lichidare+r.retinut_la_avans) as suma, isnull(max(c.ordine),1000) as ordin into #sumecfg
...
into #tichete
from fTichete_de_masa (@tich_datajos, @tich_datasus, @q_Marci, 'Centralizat', '1', 0, 0, null, @q_locm, @q_strict, null, null, 'T', '', '', 0, null)
...
select rtrim(isnull(e.nume,'')) as nume,
	...
	isnull(b.ore_invoiri,0)/isnull(s.regim_de_lucru,8) as zi_inv,
...
-----------------
	...
	isnull(n.avans,0)-isnull(ra.Retinut_la_avans,0) as avans,
...
from istpers e 
	...
	left join (select marca,sum((isnull(b.realizat__regie,0)+isnull(b.Salar_categoria_lucrarii,0))) as total_salar,
					...
					sum(Ind_c_medical_CAS+spor_cond_9) Ind_c_medical_CAS,
					...
					sum(ore_invoiri) as ore_invoiri,
...
--	am apelat #tichete unde am pus separat tichetele cuvenite si cele suplimentare
	...
	left join (select ra.Marca, sum(ra.Retinut_la_avans) as Retinut_la_avans
			from resal ra where ra.data between @q_datajos and @q_datasus and ra.Retinut_la_avans<>0 group by ra.marca) ra on ra.Marca=e.Marca
	where (e.marca=@q_marci or @q_marci is null) and (e.cod_functie=@q_functii or @q_functii is null)
	and (isnull(e.loc_de_munca,lm.cod) like rtrim(@q_locm)+(case when @q_strict=1 then '' else '%' end) or @q_locm is null) --and (s.loc_ramas_vacant=0 or e.data_plec>@q_datajos)
...
if exists (select 1 from sysobjects where name='rapFluturasiSP' and xtype='P')
	exec rapFluturasiSP @datasus, @locm, @q_tip_stat, @marci, @functii, @str_nivel, @ordonare, @nrexemplare
...
if exists (select 1 from sysobjects where name='rapFluturasiSP1' and xtype='P')
	exec rapFluturasiSP1 @datasus, @locm, @q_tip_stat, @marci, @functii, @str_nivel, @ordonare
...
select --nr_rand, randuri,
	...
	zi_bs, zi_cfs, zi_ne, zi_inv, ore_lucrate, ore_acord, indice, proc_ind_cond, ind_cond_suma, total_salar+realizat_acord as total_salar, realizat_acord, ind_co, ind_ev, suma_bo, suma_bs, suplCM_ore, suplCM_suma, 
	...
	rtrim(@denSistPrg)+(case when @spSistPrgLaOre=1 then ' '+rtrim(convert(varchar(10),proc_sist_prg))+'%' else '' end) as den_sist_prg, 
	rtrim(case when @spSistPrgLaOre=1 then convert(varchar(10),ore_sist_prg) else convert(varchar(10),proc_sist_prg)+'%' end) as proc_ore_sist_prg, proc_sist_prg, ore_sist_prg, sist_prg_suma, sp_vech_suma, venit_brut, 
	ret_CAS, ret_somaj, CASS, deduceri, diminuari, restituiri, corectii, baza_impozit, impozit, venit_net, avans, cuvenit_net, premiu, suma_incasata, premiu_la_avans, 
