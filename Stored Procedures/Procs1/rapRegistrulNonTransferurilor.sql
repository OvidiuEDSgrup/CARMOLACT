...
begin try
	...
	select data_plecarii data_plecarii, x.Numar numar_plecare, x.valoare valoare, cod_intrare, rtrim(x.cod) cod, rtrim(t.tert) tert,
		data_sosirii data_sosirii, numar_sosire, '('+rtrim(x.cod)+') '+rtrim(n.denumire) denumire_nomenclator, '('+rtrim(t.tert)+') '+rtrim(t.denumire) as nume_tert,
		--> date adaugate pentru test raport, trebuie identificate si completate corect:
			...
			(case @ordonare	when 'N' then ltrim(rtrim(isnull(n.denumire,'')))+'|'+ltrim(rtrim(x.cod))
								when 'T' then ltrim(rtrim(isnull(t.denumire,'')))+'|'+ltrim(rtrim(t.tert))
								when 'V' then isnull(space(10-len(rtrim(convert(varchar(10), convert(decimal(10,2),x.valoare)))))+rtrim(convert(varchar(10), convert(decimal(10,2),x.valoare))),'')
								when 'P' then isnull(convert(varchar(10),data_plecarii,112),'')
								when 'S' then isnull(convert(varchar(10),data_sosirii,112),'')
								else '' end) ordonare
		/*
		...
		into #registru
	from
	(select p.idpozdoc, P.numar, p.data data_plecarii , p2.data data_sosirii, p2.numar numar_sosire, p.cantitate * p.pret_de_stoc valoare, p.cod_intrare, rtrim(p.cod) cod
		...
		and (@sosire_datajos is null or p2.data >= @sosire_datajos or p2.data is null)
		and (@sosire_datasus is null or p2.data <= @sosire_datasus	or p2.data is null)	
	) x 
	...
	select ordonare, data_plecarii, numar_plecare, valoare, cod_intrare, cod, tert, data_sosirii, numar_sosire, denumire_nomenclator, nume_tert,
	...
	from #registru
		order by ordonare desc, cod, cod_intrare
