...
/*
	Exemplu apel:
		...
		insert #DocDeContat values ('1','PI','5314.EUR.1.4','2016-04-20')
		exec inregPlin '',''
		select * from pozincon where numar_document='5314.EUR.1.4' and data='2016-04-20'
...
*/
	...
	*/
	select 
		...
		p.Jurnal, isnull(c.Tip_cont,'') as tipcont,utilizator,
		...
		--'' ora_operarii--
		space(20) as indbug, isnull(p.detalii.value('(/row/@_contdifdec)[1]','varchar(40)'),'') as cont_dif_dec, isnull(p.detalii.value('(/row/@_difcursdec)[1]','float'),0) as dif_curs_dec, p.subtip
	--into #pozplin
	from pozplin p
	...
	select 
		...
		'' ora_operarii,space(20) as indbug, isnull(p.detalii.value('(/row/@_contdifdec)[1]','varchar(40)'),'') as cont_dif_dec, isnull(p.detalii.value('(/row/@_difcursdec)[1]','float'),0) as dif_curs_dec, p.subtip
	into #pozplin
	from pozplin p
	INNER JOIN #DocDeContat dc on p.subunitate=dc.subunitate and dc.tip='PI' and p.cont=dc.numar and p.data=dc.data
	left join terti t on p.subunitate=t.subunitate and p.tert=t.tert
	...
	*/
	...
	(case when sum(round(convert(decimal(17,5),p.Dif_curs_dec),2))>0 then (case when max(p.subtip)='IA' then p.ContCorespondent else p.cont end)
			else p.cont_dif_dec end),
	(case when sum(round(convert(decimal(17,5),p.Dif_curs_dec),2))>0 then p.cont_dif_dec
			else (case when max(p.subtip)='IA' then p.ContCorespondent else p.cont end) end), 
	abs(sum(round(convert(decimal(17,5),p.Dif_curs_dec),2))) as valoare,
	max(p.explicatii) as explicatii,
	max(p.data_operarii), max(p.ora_operarii), rtrim(p.Indbug)
	from #pozplin p
		left join conturi c on c.Subunitate=p.Subunitate and c.cont=p.cont
		left join conturi cc on cc.Subunitate=p.Subunitate and cc.cont=p.ContCorespondent
