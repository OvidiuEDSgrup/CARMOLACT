...
begin try
	...
	begin
		...
		insert into #TMPPADOC
				(Subunitate, Numar_document, Data, Tert, Tip, Factura_stinga, Factura_dreapta, Cont_deb, Cont_cred, Suma,TVA11, TVA22, Utilizator, Data_operarii, Ora_operarii, Tert_beneficiar, 
			...
			select @sub,@nrcorectii,@datacorectii,tert,'F'+@tipfact,(CASE WHEN @tipfact='B' THEN factura ELSE '' END), (CASE WHEN @tipfact='F' THEN FACTURA ELSE '' END), 
				(CASE WHEN @tipfact='B' THEN CONT_DE_TERT ELSE @ctcorectii END),(CASE WHEN @tipfact='B' THEN @ctcorectii ELSE cont_de_tert END),-SOLD,0,0,
				@userASiS,convert(datetime,convert(char(10),getdate(),104),104), replace(convert(char(8), getdate(), 114),':',''),
				(CASE WHEN @tipfact='B' THEN @cttvacol ELSE @cttvaded END),'Corectii facturi sold mic',valuta,curs,-Sold_valuta,'',0,loc_de_munca,'',@datacorectii,@datacorectii,0,0,0,@jurnalcorectii
			from facturi 
			where tip=(case when @tipfact='B' then 0x46 else 0x54 end) 
				and abs(round(convert(decimal(17,5), sold),2))>=0.01 
				and abs(round(convert(decimal(17,5), sold),2))<=abs(@soldfact) 
				and (@tertfiltru='' or tert=@tertfiltru) 
		...
		set @pozitiemax=isnull((select max(numar_pozitie) from pozadoc where subunitate=@sub and numar_document=@nrcorectii and data=@datacorectii),0)
		...
		INSERT INTO POZADOC
			...
			SELECT Subunitate,Numar_document,Data,Tert,Tip,Factura_stinga,Factura_dreapta,Cont_deb,
				...
				Data_fact,Data_scad,Stare,Achit_fact,Dif_TVA,Jurnal 
			FROM #TMPPADOC
