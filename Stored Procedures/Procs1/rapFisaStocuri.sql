--***
create procedure rapFisaStocuri(
	/*	Procedura pt rapoartele de Fisa stocuri (web).	*/
	...
	@tip_pret varchar(1)=0,	-->	0=stoc, 1=amanuntul, 2=pe tip gestiune, 3=vanzare, 4=cu amanuntul
	...
	@nivel1 varchar(2)=null, @nivel2 varchar(2)=null, @nivel3 varchar(2)=null, @nivel4 varchar(2)=null
		/**	@nivel[x]:	GE = Gestiuni
						...
						DA = Data
		*/
	,@umalt varchar(3)=null	--> daca se completeaza:	se recalculeaza cantitatile, fara a afecta valorile, folosind coeficientul de conversie din nomenclator;
													-->		pentru codurile de nomenclator care nu au respectiva unitate de masura se va pune cantitate 0
	,@locmg varchar(20)=null
	,@tip_raport varchar(10)='d'	--> 'c'=centralizat, 'd'=detaliat, 'fm'=fisa magazie
	,@afispretcont bit=0
	,@precizie_pret varchar(2)='5'
	)
as
	/*	test
...
select @eroare=''
begin try
	--> verificare sintactica precizie_pret:
	if not isnumeric(@precizie_pret)=1 or @precizie_pret not between 0 and 5
		raiserror('Precizia maxima a pretului trebuie sa fie numerica si intre 0 si 5!',16,1)
	...
	select @q_dDataJos=@dDataJos, @q_dDataSus=@dDataSus, @q_cCod=@cCod, @q_cGestiune=@cGestiune, @q_cCodi=@cCodi, @q_cCont=@cCont,
		...
		@precizie_pret=isnull(@precizie_pret,'5')	--> precizia maxima a zecimalelor in afisarea pretului
		,@precizie_pret_max='5'		--> precizia maxima a zecimalelor prin calcule
	if @furnizor is null
	...
	select	@s_cCod=(case when @q_cCod is null then 'null' else ''''+@q_cCod+'''' end),
			...
			@s_locmg=(case when @locmg is null then 'null' else ''''+@locmg+'''' end),
			...
			@expr_pret=(case when @q_tip_pret='0' then 'r.pret' when @q_tip_pret='2' then 'case when g.Tip_gestiune=''A'' or g.pret_am=1 then r.pret_cu_amanuntul else 0 end'
				when @q_tip_pret='1' then 'r.pret_cu_amanuntul' when @q_tip_pret='2' then 'case when g.Tip_gestiune=''A'' or g.pret_am=1 then r.pret_cu_amanuntul else 0 end'
	...
	select @comanda_str='
		...
		create table #stocuri(cont varchar(40), cod varchar(20), cod_intrare varchar(50), gestiune varchar(20), tert varchar(20),
			tip_document varchar(2), numar_document varchar(20), data datetime, stoci decimal(38,'+@precizie_pret_max+'), v_stoci decimal(20,6), intrari decimal(38,'+@precizie_pret_max+'), v_intrari decimal(20,6),
			iesiri decimal(38,'+@precizie_pret_max+'), v_iesiri decimal(20,6), DenGest varchar(500),
			loc_de_munca varchar(20), predator varchar(200), pretRaport varchar(200), comanda varchar(50), locatie varchar(30), tip_gestiune varchar(20),
			denumire_locatie varchar(500), valoare decimal(38,'+@precizie_pret_max+'), grupa varchar(200), grNivel1 varchar(200), grNivel2 varchar(200), grNivel3 varchar(200),
			grNivel4 varchar(200), gestiuneCustodie bit, lot varchar(2000), um varchar(100), pret decimal(38,'+@precizie_pret_max+') default 0)
		...
		create table #stocuri_grupat(
			...
			select @p=(select '+@s_dDataJos+' dDataJos, '+@s_dDataSus+' dDataSus, '+@s_cCod+' cCod, '+@s_cGestiune+' cGestiune, '+@s_cCodi+' cCodi
							...
							,'+@s_comanda+' Comanda, '+@s_locmg+' LM, "'+isnull(@furnizor,'')+'" Furnizor, '+@s_lot+' Lot, '+@s_grupGestiuni+' grupGestiuni
	...
	+	(case when @grUM is not null then '
			update d set um=p.detalii.value("(row/@um_um)[1]","varchar(100)")
					...
					,d.pret=(d.pret*d.cantitate)/p.detalii.value("(row/@cantitate_um)[1]","decimal(20,'+@precizie_pret_max+')")
	...
	(case when isnull(@q_den,'')<>'' then '
	delete s from #docstoc s inner join nomencl n on s.cod=n.cod
		where n.denumire not like ''%'+@q_den+'%''
	' else '' end)
	+'
	select g.subunitate, g.cod_gestiune, g.denumire_gestiune, g.tip_gestiune, ISNULL(g.detalii.value(''(/*/@custodie)[1]'',''int''),0) as custodie,pret_am into #gestiuni
	from gestiuni g
	...
	insert into #stocuri(cont, cod, cod_intrare, gestiune, tert, tip_document, numar_document, data, stoci, v_stoci, intrari, v_intrari,
		...
		grNivel2, grNivel3, grNivel4, gestiuneCustodie, lot, um, pret)
	select max(r.cont),max(r.cod),max(r.cod_intrare),max(r.gestiune),
		...
		, max(rtrim(case when r.tip_document in(''TE'',''TI'',''DF'',''CM'') then isnull(gp.denumire_gestiune,r.predator) 
	...
	from #docstoc r
		...
		left join gestiuni gp on r.predator=gp.cod_gestiune and r.subunitate=gp.Subunitate
	...
	'
		...
		exec CreazaDiezPreturi
			...
			update #stocuri set pret='+(case when @tip_pret=3 then 'pr.pret_vanzare' when @tip_pret=4 then 'pr.pret_amanunt' else 'pret' end)+'
	...
	-->	recalculare valori dupa inlocuirea pretului:
			update #stocuri set v_stoci=stoci*pret, v_intrari=intrari*pret, v_iesiri=iesiri*pret
	'
	--> aici mut zecimalele in partea intreaga ca sa nu pierd zecimale (prin '/' pierd din zecimale daca las impartirea sa functioneze clasic); dupa operatie mut zecimalele la loc prin inserarea caracterului '.':
	else select @comanda_str=@comanda_str+'update s set pret=(case when (abs(s.stoci)+abs(s.intrari)+abs(s.iesiri))=0 then 0 else s.valoare*1'+replicate('0',@precizie_pret_max)+'/(abs(s.stoci)+abs(s.intrari)+abs(s.iesiri)) end)
		from #stocuri s
		
		
		update s set pret=left(x.pret,len(x.pret)-'+@precizie_pret_max+')+"."+substring(x.pret,len(x.pret)-'+@precizie_pret_max+'+1,'+@precizie_pret_max+')
		from #stocuri s cross apply (select "'+replicate('0',@precizie_pret_max)+'"+convert(varchar(60),convert(decimal(38),pret)) pret ) x
		where abs(x.pret)>0.001
	
	'
	--> aici configuram zecimalele pentru afisarea in raport; se ia maximul lungimii zecimalelor, pana la @precizie_pret initiala
	select @comanda_str=@comanda_str+'
		declare @precizie_pret int
		
		update #stocuri set pretraport=pret
		
				--> identific precizia maxima din date:
		select @precizie_pret=max(case when x.precizie_pret=0 then 0 else len(x.precizie_pret) end)
		from #stocuri s
		cross apply (select floor(	--> elimin fosta parte intreaga
							reverse(	--> inversez lexicografic partea fractionara cu partea intreaga
								abs(convert(decimal(38,'+@precizie_pret+'),pretRaport))
						)) as precizie_pret
				) x	--> cross apply sa nu scriu expresia de doua ori la verificarea valorii 0
				
		if @precizie_pret>'+@precizie_pret+' set @precizie_pret='+@precizie_pret
		+'
		update s set pretRaport=
			left(convert(varchar(200),convert(money,floor(s.pretRaport)),1),charindex(".",
				 convert(varchar(200),convert(money,floor(s.pretRaport)),1))-1)
				 +(case when @precizie_pret=0 then "" else
						substring(s.pretRaport, charindex(".",s.pretRaport),@precizie_pret+1)
					end)
		 from #stocuri s
	...
	select @comanda_str=@comanda_str+'
		...
		--	@numeNivel1+','+@numeNivel2+','+@numeNivel3+','+@numeNivel4+',
			...
			,max(predator),
			'+(case when @afispretcont=1 then 'max(pretRaport), max(cont)' else '"",""' end)+'
		from #stocuri s	'	--+@listaTabele
		+char(10)+'group by grNivel1, grNivel2, grNivel3, grNivel4'+
		(case when @detalii=1 then ',s.tert,s.tip_document,s.numar_document,s.data'
			+(case when @afispretcont=1 then ',pretRaport, cont' else '' end)
			else '' end)+', gestiuneCustodie'
