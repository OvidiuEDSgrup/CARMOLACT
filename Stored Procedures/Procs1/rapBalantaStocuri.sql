...
exec sp_executesql N'exec rapBalantaStocuri @sesiune=@sesiune, @dDataJos=@dDataJos, @dDataSus=@dDataSus, @cCod=@cCod, @cGestiune=@cGestiune, @cCodi=@cCodi, @cCont=@cCont,
		@TipStocuri=@TipStocuri, @den=@den, @gr_cod=@gr_cod, @tip_pret=@tip_pret, @tiprap=@tiprap, @ordonare=@ordArticole, @grupare4=@grupare4,
		@comanda=@comanda, @centralizare=@centralizare, @grupare=@grupare, @categpret=@categpret, @locatie=@locatie,
		@furnizor_nomenclator=@furnizor_nomenclator, @furnizor=@furnizor, @grupGestiuni=@grupGestiuni, @locmg=@locmg, @lot=@lot',
	N'@sesiune nvarchar(13),@dDataJos datetime,@dDataSus datetime,@cCod nvarchar(3),@cGestiune nvarchar(1),@grupGestiuni nvarchar(4000),@cCodi nvarchar(4000),@cCont nvarchar(4000),@TipStocuri nvarchar(4000),
	@den nvarchar(4000),@gr_cod nvarchar(4000),@tip_pret nvarchar(1),@tiprap nvarchar(1),@ordArticole nvarchar(1),@grupare4 bit,@comanda nvarchar(4000),@centralizare nvarchar(1),@grupare nvarchar(1),
	@categpret nvarchar(4000),@locatie nvarchar(4000),@furnizor_nomenclator nvarchar(4000),@furnizor nvarchar(4000),@locmg nvarchar(4000),@lot nvarchar(4000)',
	@sesiune=N'B99E6A6761B84',@dDataJos='2016-02-01 00:00:00',@dDataSus='2016-02-29 00:00:00',@cCod=N'005',@cGestiune=N'1',@grupGestiuni=NULL,@cCodi=NULL,@cCont=NULL,@TipStocuri=N'',@den=NULL,@gr_cod=NULL,
	@tip_pret=N'0',@tiprap=N'D',@ordArticole=N'0',@grupare4=0,@comanda=NULL,@centralizare=N'3',@grupare=N'0',@categpret=NULL,@locatie=NULL,@furnizor_nomenclator=NULL,@furnizor=NULL,@locmg=NULL,@lot=NULL
...
begin try
	...
	select @q_dDataJos=@dDataJos, @q_dDataSus=@dDataSus, @q_cCod=@cCod, @q_cGestiune=@cGestiune, @q_cCodi=@cCodi, @q_cCont=@cCont,
		...
		@q_locm=(case when @tiprap='F' or @locmg is null then @locm else @locmg end), @q_lot = @lot
	...
	declare @pXML xml
	select @pXML=(select @sesiune as sesiune for xml raw)
	...
	select r.subunitate, r.cont,r.cod,r.cod_intrare,r.gestiune,
		...
		space(200) denumire_locatie, max(r.lot) as lot, max(n.um) as um
	into #stocuri
	...
	select @dDataJos dDataJos, @dDataSus dDataSus, @cCod cCod, @cGestiune cGestiune, @grupGestiuni grupGestiuni, @cCodi cCodi,
		...
		@furnizor furnizor, @locmg locmg, @lot lot, @parXML for xml raw)
	...
	select
		...
		,rtrim(numar_document) numar_document, data, stoci, intrari, iesiri, rtrim(isnull(DenGest,'')) DenGest
		,rtrim(n.denumire)+' ('+rtrim(isnull(nullif(r.um, ''), n.UM))+')' as DenProd
		,rtrim(isnull(nullif(r.um, ''), n.UM)) as um, rtrim(n.grupa) grupa, rtrim(gr.denumire) as nume_grupa
		,rtrim(c.denumire_cont) as nume_cont, rtrim(r.loc_de_munca) loc_de_munca
		,rtrim(isnull(p.nume,'')) as den_marca, rtrim(isnull(l.denumire,'')) as den_lm, 
	...
	*/
	...
	having abs(sum(stoci))>=0.001 or abs(sum(intrari))>=0.001 or abs(sum(iesiri))>=0.001 
		or abs(sum(stoci*pret))>=0.01 or abs(sum(intrari*pret))>=0.01 or abs(sum(iesiri*pret))>=0.01
