...
--	selectare salariati suspendati in luna.
	...
	Begin
		...
		if (@Plecat=0 or (@Avans_plecati_luna=0 and @Data_plecarii>@DataS or @Avans_plecati_luna=1 and @Data_plecarii>@DataJ)) 
			and (@NUavans_ang_luna=0 or @Data_ang<=dateadd(day,@Ziua_ang_luna,@DataJ) and (@suspendat=0 or @data_inc_susp<=dateadd(day,@Ziua_ang_luna,@DataJ) or @Gasit_avexcep=1))
			and (@suspendat=0 or (@Avans_plecati_luna=0 and (@data_sf_susp>@DataS or @Suma_avans<>0) or @Avans_plecati_luna=1 and @data_sf_susp between @DataJ and @DataS) or @Gasit_avexcep=1)
		Begin
