...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	select p.marca, p.nume, p.loc_de_munca as lm, lm.denumire as den_lm, p.cod_functie as functie, f.denumire as den_functie, 
		isnull(p.comanda,ip.Centru_de_cost_exceptie) as comanda, isnull(z.Descriere,'') as den_comanda, 
		...
		isnull(p.nr_contract,ip.nr_contract), dc.Data_inf as data_contract,
		...
		v.varsta as varsta, (case when LEFT(v.varsta,1)='*' then 0 else convert(int,left(v.varsta,CHARINDEX('a',v.varsta)-1)) end) as varsta_ani, 
		...
		(case when @ordonare=2 then p.loc_de_munca when @ordonare=3 then p.localitate when @ordonare=4 then right(convert(char(4),year(@dataSus-p.data_nasterii)),2) 
			when @ordonare=6 then isnull(p.comanda,isnull(ip.Centru_de_cost_exceptie,'')) else '' end) as grupare, 
		(case when @ordonare=2 then p.loc_de_munca when @ordonare=3 then p.localitate when @ordonare=6 then isnull(p.comanda,'') else '' end) as ordonare, 
	...
	from personal p
		...
		left outer join comenzi z on z.Subunitate=@sub and z.Comanda=isnull(p.comanda,ip.Centru_de_cost_exceptie)
	...
	where (@marca is null or p.Marca=@marca) and (@locm is null or p.Loc_de_munca like rtrim(@locm)+(case when @strict=0 then '%' else '' end)) 
		...
		and (@sex is null or p.sex=@sex) and (@tipstat is null or isnull(p.tip_stat,ip.Religia)=@tipstat) 
		...
		and (@codfunctie is null or p.cod_functie=@codfunctie) and (@comanda is null or isnull(p.comanda,ip.Centru_de_cost_exceptie)=@comanda)
