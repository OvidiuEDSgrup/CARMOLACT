...
-->	citire date din gridul de operatii
	...
	begin
		...
		SELECT 'Compensari facturi: Selectie facturi beneficiar'  nume, 'PI' codmeniu, 'D' tipmacheta, 'RE' tip,'PI' subtip, 'O' fel,
	...
	*/
	...
	if exists (select 1 from #xmlFacturi where factura='AVANS' and ISNULL(selectat,0)=1)
	...
	if exists (select 1 from sysobjects where [type]='P' and [name]='wOPPISelectivaSP1')
		exec wOPPISelectivaSP1 @sesiune=@sesiune, @parXML=@parXML
	...
	if exists (select 1 from #xmlFacturi where factura='AVANS') -- asta ar trebui in wOPPISelectivaSP2
	...
	begin
		declare @cotatva_std decimal(12,2) 
		EXEC luare_date_par 'GE', 'COTATVA', 0, @cotatva_std OUTPUT,'' --> citire cota TVA standard
		...
		--	Lucian: Discutat cu Ghita si stabilit ca la operatia selectiva nu trebuie sa tratam TVA la avans. Ramine tratat doar in wScriuPlin daca se completeaza cota de TVA la operare.
		--	Arobs-ul care face TVA la avans nu foloseste operatia selectiva. Daca va fi nevoie se va trata prin wOPPISelectivaSP2.
		--update pozplin set tva22=suma*@cotatva_std/(100.00+@cotatva_std), tva11=@cotatva_std
