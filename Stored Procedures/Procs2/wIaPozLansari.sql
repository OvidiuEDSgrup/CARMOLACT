...
CREATE PROCEDURE [dbo].[wIaPozLansari] @sesiune VARCHAR(50), @parXML XML
AS
IF EXISTS (
		SELECT *
		FROM sysobjects
		WHERE NAME = 'wIaPozLansariSP'
			AND type = 'P'
		)
BEGIN
...
SELECT @cod = ISNULL(@parXML.value('(/row/@comanda)[1]', 'varchar(20)'), ''), @codProdus = ISNULL(@parXML.value('(/row/@cod)[1]'
			, 'varchar(20)'), ''), @cantitateProdus = ISNULL(@parXML.value('(/row/@cantitate)[1]', 'float'), '')

SELECT @id = id, @idPozTehnologii = idp, @cantitateProdus = cantitate
FROM pozLansari
WHERE tip = 'L'
	AND cod = @cod
...
SELECT @denumireProdus = denumire, @umProdus = um, @pretProdus = Pret_stoc
FROM nomencl
WHERE cod = @codProdus
...
BEGIN
	...
	SET @doc = ''
	SET @doc = (
			SELECT @cod AS comanda, (
					CASE WHEN pl.tip = 'O' THEN 'Operatie' WHEN (
								pl.tip = 'M'
								AND n.tip = 'P'
								) THEN 'Semifabricat' WHEN pl.tip = 'R' THEN 'Reper' ELSE 'Material' END
					) AS tip, (
					CASE WHEN pl.tip IN ('M', 'Z') THEN rtrim(n.denumire) WHEN pl.tip = 'O' THEN 
								rtrim(c.Denumire) WHEN pl.tip = 'R' THEN RTRIM(isnull(t.denumire, pl.detalii.value(
											'(/row/@denumire)[1]', 'varchar(20)'))) END
					) AS denumire, (
					CASE WHEN pl.tip IN ('M', 'Z') THEN rtrim(n.um) WHEN pl.tip = 'O' THEN rtrim(c.um
								) END
					) AS um, pl.id AS id, rtrim(pl.cod) AS cod, CONVERT(DECIMAL(10, 2), pl.cantitate) AS cantitate, (CASE WHEN pl.tip = 'M' THEN 'MC' WHEN pl.tip = 'O' THEN 'OP' END
					) AS subtip, dbo.wfIaArboreLansare(pl.id), pl.detalii AS detalii
			FROM pozLansari pl
...
END
ELSE
	SELECT 'Produs' AS _grupare, rtrim(@codProdus) AS cod, rtrim(@denumireProdus) AS denumire, convert(DECIMAL(12, 5), 
			@cantitateProdus) AS cantitate, @umProdus AS um, convert(DECIMAL(12, 5), @pretProdus) AS pret
