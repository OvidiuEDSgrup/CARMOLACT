...
@COEV_macheta int, @MachetaCO int, @MachetaCO_TipOpMach varchar(100), @parXMLCalcul xml 

select	@MachetaCO=max(case when parametru='OPZILECOM' then Val_logica else 0 end)
		,@MachetaCO_TipOpMach=max(case when parametru='OPZILECOM' then Val_alfanumerica else '' end)
		,@COEV_macheta=max(case when parametru='COEVMCO' then Val_logica else 0 end)
from par where tip_parametru in ('PS','SP') 
	and parametru in ('OPZILECOM','COEVMCO')
...
begin try
	...
	begin
		...
		if @subtip='ON'
		begin
			select @data_inceput=@Datalunii_1, @data_sfarsit=@datalunii, @Zile_CO=0
		end
		else
		begin
		...
		if @tip_concediu in ('3','6') and abs(@xZile_CO)>@Zile_CO
		...
		set @parXMLCalcul=(select convert(varchar(10),@Datalunii_1,101) as datajos, convert(varchar(10),@Datalunii,101) as datasus, rtrim(@Marca) as marca, 
			convert(varchar(10),@data_inceput,101) as datainceput, @Zile_CO as zilelucr, @Indemnizatie_CO as indco for xml raw)
		exec CalculConcediiDeOdihna @sesiune=@sesiune, @parXML=@parXMLCalcul
...
--	calcul ore concediu de odihna si obligatii cetatenesti (CO eveniment) de pus in pontaj
		...
		where co.data=@Datalunii and co.Marca=@Marca
			...
			and @MachetaCO=1 and co.Tip_concediu not in ('2','E')
