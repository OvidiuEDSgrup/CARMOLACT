...
Declare @mesaj varchar(250), @tip char(2),@numarDVI varchar(13),@dataDVI datetime, @numar_receptie varchar(20), @data_receptie datetime,
	...
	@suma_ron_CIF float, @val_fara_comis float, @suma_com_vam float, @TipPlataTVAUnitate char(1)
select
	/* date receptie*/
	...
	@dataDVI=isnull(@parXML.value('(/parametri/@dataDVI)[1]','datetime'),isnull(@parXML.value('(/parametri/@data)[1]','datetime'),'')),
...
--	apelez validarea pentru receptiile de mijloace fixe cu DVI pentru cazul in care s-a inceput amortizarea pentru acestea
	...
	begin  				
		update dvi set 
			/*date CIF*/ Tert_CIF=@tert_CIF, Factura_CIF=@factura_CIF, Data_CIF=@data_CIF, Data_comis=@scadenta_CIF, Cont_CIF=@cont_CIF,
				Valoare_CIF=@suma_valuta_CIF,Valoare_CIF_lei=@suma_ron_CIF, Valuta_CIF=@valuta_CIF, curs=@curs, 
	...
	begin  
		...
		-->	Daca firma este cu TVA la incasare, atunci factura de TVA vama nu trebuie sa fie cu TVA la incasare.
		select top 1 @TipPlataTVAUnitate=tip_tva
		from TvaPeTerti where tipf='B' and tert is null and @data_receptie>dela
		order by dela desc

		if isnull(@TipPlataTVAUnitate,'P')='I'
			insert into TvaPeTerti(tipf, tert, dela, factura, tip_tva)
			select 'F', @tert_vama, @data_receptie, @factura_vama, 'P'
			where not exists (select 1 from TvaPeTerti tt where tt.tipf='F' and tt.tert=@tert_vama and tt.dela=@data_receptie and tt.factura=@factura_vama)
  	end  
