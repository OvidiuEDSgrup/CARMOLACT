...
CREATE PROCEDURE wIaContracteSP @sesiune VARCHAR(50), @parXML XML
AS
...
CROSS APPLY 
(
	select top 1 j.stare stare, s.denumire denstare, s.culoare culoare from JurnalContracte j JOIN StariContracte s on j.stare=s.stare and s.tipContract=ct.tip and j.idContract=ct.idContract 
	order by j.data desc, j.idJurnal desc
) st
