...
begin try
	...
	/* 		
		Cardurile se scaneaza, deci durata dintre prima tastare si <enter> trebuie sa fie <100ms.
	*/
	if @durataInput > 100 or @clipboardIsSimilar=1
		raiserror('Cardul de fidelizare nu a fost scanat.', 11, 1)
	
	set @puncte = isnull((select sum((case when p.tip='C' then -1 else 1 end) * puncte)
	...
	set @xmlFinal=
		( select c.UID uidCardFidelizare, c.Nume_posesor_card numeTitularCard, 
			...
			from CarduriFidelizare c where c.UID=@UID_Card
			for xml raw)
	
	--insert CarduriFidelizare(UID, Nume_posesor_card)
	--select '123456789012345678901', 'User test'	
