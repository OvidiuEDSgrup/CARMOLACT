...
as               
declare @subunitate char(9),@tip char(2),@numar char(20),@datadoc datetime,@tert char(13), @userASiS varchar(20),@mesaj varchar(8000), 
...
@err int, @eroare varchar(254), @faraMesaj int, @anularePI int, @docJurnal xml, @stareAnulat int
...
begin try
	...
	insert into docsters 
		(Subunitate, Tip, Numar, Data, Tert, Factura, Gestiune, Cod, Cod_intrare, Gestiune_primitoare, Cont,  Cont_cor, Cantitate, Pret, Pret_vanzare, Jurnal, 
		Utilizator, Data_operarii, Ora_operarii, Data_stergerii, Pret_valuta, Adaos, Pret_cu_amanuntul, TVA_deductibil, Cota_TVA, TVA_neexigibil, Pret_amanunt_predator, Tip_miscare,
		Locatie, Data_expirarii, Numar_pozitie, Loc_de_munca, Comanda, Barcod, Cont_intermediar, Cont_venituri, Discount, Numar_DVI, Stare, Grupa, Cont_factura,
		Valuta, Curs, Data_facturii, Data_scadentei, Procent_vama, Suprataxe_vama, Accize_cumparare, Accize_datorate, Contract, 
		detalii, idPozDoc, subtip, idIntrareFirma, idIntrare, idIntrareTI, lot, colet) 
	select Subunitate, Tip, Numar, Data, Tert, Factura, Gestiune, Cod, Cod_intrare, Gestiune_primitoare, Cont_de_stoc, Cont_corespondent, Cantitate,
		Pret_de_stoc, Pret_vanzare, Jurnal, @userASiS, Data_operarii, Ora_operarii, dateadd(ms,10*row_number() over(order by getdate()),getdate()), 
		Pret_valuta, Adaos, Pret_cu_amanuntul, TVA_deductibil, Cota_TVA, TVA_neexigibil, Pret_amanunt_predator, Tip_miscare,
		Locatie, Data_expirarii, Numar_pozitie, Loc_de_munca, Comanda, Barcod, Cont_intermediar, Cont_venituri, Discount, Numar_DVI, Stare, Grupa, Cont_factura,
		Valuta, Curs, Data_facturii, Data_scadentei, Procent_vama, Suprataxe_vama, Accize_cumparare, Accize_datorate, Contract, 
		detalii, idPozDoc, subtip, idIntrareFirma, idIntrare, idIntrareTI, lot, colet
	...
	--jurnalizare anulare document
	set @stareAnulat=9
	SELECT @docJurnal = 
		(SELECT @numar numar, @stareAnulat as stare, @datadoc data, @tip tip, 'Anulare document' explicatii
	...
	/*	Apelat SP2 pentru validari specifice. */
	if exists (select 1 from sysobjects where [type]='P' and [name]='wOPAnulareDocSP2')
		exec wOPAnulareDocSP2 @sesiune=@sesiune, @parXML=@parXML output
