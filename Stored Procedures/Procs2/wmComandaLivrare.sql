...
CREATE procedure [dbo].wmComandaLivrare @sesiune varchar(50), @parXML xml  
as  
...
begin try
	...
	*/
	...
	if (@stare=@stareBkFacturabil or @lVanSales=1) and @areMeniu=1 and @linietotal is not null and isnull(@tert,'')<>''
		set @linie_facturat=
			(
			select 
				'<FacturareComanda>' cod, 'Facturare comanda' denumire, 
				(
					select 
						'Total:'+(rtrim(convert(decimal(12,2),sum(pc.cantitate*pc.Pret*(1-pc.discount/100))))+' RON')
					from PozContracte pc where pc.idContract=@comanda
				) info,
				...
				(
					select
						...
						(select rtrim(n.Denumire) as denProdus,RTRIM(convert(char(20),(convert(decimal(12,2),pc.Pret)))) as pret,
							rtrim(LTRIM(convert(char(10),(convert(decimal(12,2),pc.Cant_aprobata))))) as cantitate, n.um as uMasura,
	...
	/** Linie stornare cod **/
	DECLARE @linie_strono xml	
	exec wAreMeniuMobile 'CStornare',@utilizator,@areMeniu output
	if @areMeniu=1 and isnull(@tert,'')<>''
		set @linie_strono=
		/** linie cod nou: trimit discount sugerat in cod... */
			(
			select 
				'Storneaza cod' denumire, '0x0000ff' as culoare,'C' as tipdetalii, 
				'wmAlegCodStornare' procdetalii,'server://Icoane/sterg32.png' as poza
			for xml raw, type
			)
	...
	SELECT
		@linie_facturat,@linie_inchide_com,@linie_inchide_com_dep,@linie_inchide_bon,@linie_tiparest_bon,@linie_adauga,@linie_strono,@linie_produse,
		@linie_discount
	for XML PATH('Date')

	select @numar_comanda=numar from Contracte where idContract=@comanda
	select 
		'wmComandaLivrare' as detalii,0 as areSearch, 'Comanda:'+isnull(@numar_comanda,'-') as titlu , @actiune actiune, '@cod' numeatr,'D' as tipdetalii,
