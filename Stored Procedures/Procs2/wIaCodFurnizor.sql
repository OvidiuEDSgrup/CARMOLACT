...
as
	...
	CREATE table #furnizori_cod 
		(tert varchar(20), pret float, data_pret datetime, cod_special varchar(20), nr_zile_livrare int, cant_minima float, tip varchar(1), prioritate int, detalii XML)

	/** Se iau furnizorii codului din PPRETURI  **/
	insert into #furnizori_cod (tert, pret, data_pret, cod_special,nr_zile_livrare, cant_minima, tip, prioritate, detalii)

	select 
		RTRIM(p.tert), rtrim(p.pret), p.Data_pretului, RTRIM(p.codfurn), RTRIM(p.nr_zile_livrare), RTRIM(p.cant_minima), 'P', p.prioritate, p.detalii
	from ppreturi p
