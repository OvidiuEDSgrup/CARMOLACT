...
BEGIN TRY
	...
	SELECT @idInventar, cod, null, @utilizator, GETDATE(), 
		convert(xml,'<row stoc_scriptic="'+rtrim(convert(decimal(15,0),sum(stoc)))+'" comanda="'+rtrim(comanda)+'" lot="'+rtrim(lot)+'" locatie="'+rtrim(locatie)+'" />')
	from stocuri 
	where Cod_gestiune=@gestiune and stoc>0.01
	group by Cod, Lot, Locatie, Comanda
	*/
