...
BEGIN TRY
	...
	IF EXISTS (SELECT * FROM sysobjects WHERE NAME = 'wPrelucrareComenziRezervariSP')
	BEGIN
		exec wPrelucrareComenziRezervariSP @sesiune = @sesiune, @parXML = @parXML
		RETURN
	END
	...
	delete #documente where abs(cantitate)<0.01
