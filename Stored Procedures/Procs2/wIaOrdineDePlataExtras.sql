...
CREATE PROCEDURE wIaOrdineDePlataExtras @sesiune VARCHAR(50), @parXML XML
	--> copie modificata dupa wIaOrdineDePlata pentru a se aduce total incasari si total plati
AS
DECLARE 
...
IF OBJECT_ID('tempdb..#LMOrdineDePlata') IS NOT NULL
	...
	SELECT 
		...
		,convert(decimal(15,2), isnull(sm.suma_plati,0)) suma_plati
		,convert(decimal(15,2), isnull(sm.suma_incasari,0)) suma_incasari
	FROM OrdineDePlata ant
	LEFT JOIN (
			SELECT idOP, isnull(SUM(suma),0) suma
				,sum(case when left(p.tip,1)='P' then isnull(p.suma,0) else 0 end) suma_plati
				,sum(case when left(p.tip,1)='I' then isnull(p.suma,0) else 0 end) suma_incasari
			FROM PozOrdineDePlata p where stare='1'
			GROUP BY idOP
