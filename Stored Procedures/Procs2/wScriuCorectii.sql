...
@userASiS varchar(20), @docXML xml, @docXMLIaDLSalarii xml, @eroare xml, @mesaj varchar(1000), @mesajEroare varchar(1000), @ptupdate int
...
begin try
	...
	isnull(Suma_neta, 0) as Suma_neta, 
	isnull(Suma_corectie, 0) as Suma_corectie, 
	isnull(Procent_corectie, 0) as Procent_corectie, 
	...
	begin
		...
		if @ptupdate=0 and exists (select 1 from corectii where data=@vData and marca=@marca and tip_corectie_venit=@tip_corectie_venit and loc_de_munca=@loc_de_munca
			and (case when @suma_neta<>0 then Suma_corectie else Suma_neta end)<>0)
		begin
			set @mesajEroare='Pe aceasta corectie s-a operat deja o suma '+(case when @suma_neta<>0 then 'bruta' else 'neta' end)+' pentru salariatul introdus!'
