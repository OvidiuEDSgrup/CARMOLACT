...
data_facturii datetime,data_scadentei datetime,loc_de_munca varchar(13),numar_pozitie int,cod varchar(20),barcod varchar(100),codcodi varchar(50),cantitate float,pret_valuta float,pret_vanzare float,
...
codgs1 varchar(1000), idPozDocRezervare int,idplaja int, nrp int identity, asociereconf varchar(20), tipPtNumar varchar(2))
...
select 
	...
	isnull(pret_amanunt_predator,0) as pret_amanunt_predator,
...
(
	...
	pret_amanunt_predator decimal(14, 5) '@pret_amanunt_predator', 
	...
	update poz set 
		...
		discount=isnull(poz.discount,isnull(antet.discount, 0))
	from #documente poz
	...
	*/
	update d set 
		cantitate = d.detalii.value('(/row/@cantitate_um)[1]','float')*u.coeficient, 
		pret_valuta=(case when d.detalii.value('(/row/@pret_um)[1]','float') IS NOT NULL then 
			round(convert(decimal(15,3),d.detalii.value('(/row/@pret_um)[1]','float') / (d.detalii.value('(/row/@cantitate_um)[1]','float')*u.coeficient)),2) else pret_valuta end)
	from #documente d
	JOIN UMProdus u on d.cod=u.cod and u.UM=d.detalii.value('(/row/@um_um)[1]','varchar(3)') and ISNULL(d.detalii.value('(/row/@cantitate_um)[1]','float'),0)>0.0
	...
	*/
	update d set tip_miscare=
		...
		tva_neexigibil=(case when (g.tip_gestiune in ('A','V') or g.pret_am=1) and d.tip in ('RM','AI','PP','TE') then isnull(nullif(d.tva_neexigibil,0),n.Cota_TVA) else 0 end)
	from #documente d
	inner join nomencl n on d.cod=n.cod 
	left outer join gestiuni g on g.Subunitate=@subunitate and d.gestiune=g.cod_gestiune
	--Atentie!!!! nu este tratat tva_neexigibil la gestiunile primitoare. Oricum speram sa se scoata in curand cu noile gestiuni de tip E
	...
	*/
	...
	where d.data_scadentei=d.data_facturii or convert(char(10),d.data_scadentei,101)=convert(char(10),getdate(),101) or d.data_scadentei is null or ptUpdate=1
	...
	*/
	...
	if exists(select 1 from #documente d join gestiuni g on g.cod_gestiune = d.gestiune where (d.tip_miscare='E' or d.tip_miscare='I' and d.cantitate<0) and d.cod_intrare<>'' 
		and not (d.tip='TE' and d.gestiune=d.gestiune_primitoare and ISNULL(g.pret_am,0)=0 )) and @StocuriNoi=0
	begin
			update d set pret_de_stoc=s.pret,pret_amanunt_predator=s.Pret_cu_amanuntul,cont_de_stoc=s.Cont,locatie=(case when d.tip in ('TE','DF','PF') then d.locatie else s.Locatie end),
				idIntrare=s.idIntrare,idIntrareFirma=s.idIntrareFirma,d.tva_neexigibil=(case when g.tip_gestiune='A' then s.tva_neexigibil else d.tva_neexigibil end)
			from #documente d
			inner join stocuri s on d.gestiune=s.cod_gestiune and d.cod=s.cod and d.cod_intrare=s.Cod_intrare
			left outer join gestiuni g on g.Subunitate=@subunitate and g.cod_gestiune=d.gestiune
	end

	if exists(select 1 from #documente d join gestiuni g on g.cod_gestiune=d.gestiune where (d.tip_miscare='E' or d.tip_miscare='I' and d.cantitate<0) and isnull(d.idIntrare,0)>0 
		and not (d.tip='TE' and d.gestiune=d.gestiune_primitoare and ISNULL(g.pret_am,0)=0)) and @StocuriNoi=1
	begin
	...
	if exists(select 1 from #documente d join gestiuni g on g.cod_gestiune = d.gestiune where (d.tip_miscare='E' or d.tip_miscare='I' and d.cantitate<0) and d.cod_intrare='' and d.ptUpdate=0 
		and not (d.tip='DF' and d.subtip='RO') and not (tip='TE' and gestiune=gestiune_primitoare and ISNULL(g.pret_am,0)=0)) 
	begin /*Spargere pentru iesiri*/
		...
		create table #pozd(Cantitate float,TVa_deductibil decimal(12,2),Tip varchar(2),Numar varchar(20),Cod varchar(20),Data datetime,Gestiune varchar(20),
			...
			Barcod varchar(100),Cont_intermediar varchar(40),Cont_venituri varchar(40),Discount decimal(12,5),Tert varchar(20),Factura varchar(20),
			...
			locatie_ceruta varchar(30), lot_cerut varchar(20))
		...
		OUTPUT (case when deleted.tip_miscare='I' then -1 else 1 end)*DELETED.Cantitate,DELETED.TVa_deductibil,DELETED.Tip,DELETED.NUMAR,DELETED.Cod,DELETED.Data,DELETED.Gestiune,
			...
			isnull(DELETED.detalii.value('(/row/@locatie_ceruta)[1]','varchar(30)'),''), isnull(DELETED.detalii.value('(/row/@lot_cerut)[1]','varchar(20)'),'')
		INTO #pozd
		from #documente d 
		...
		insert into #stoctotal
		select 
			row_number() over (partition by pd.cod_gestiune,s.cod order by pd.nrordine,s.data
			--(case when pd.cantitate<0 then -1 else 1 end)*datediff(day,s.data,'2100-01-01')
				) as nrord,
		...
		from stocuri s
		inner join
			(select p.gestiune as cod_gestiune,gt.gestiune_transfer,p.cod,gt.nrordine,p.locatie_ceruta,p.lot_cerut,sum(p.cantitate) as cantitate
			from #pozd p 
			left outer join #gesttransfer gt on gt.gestiune=p.Gestiune
			group by p.gestiune,gt.gestiune_transfer,p.cod,gt.nrordine,p.locatie_ceruta,p.lot_cerut
			) pd on s.cod_gestiune=isnull(pd.gestiune_transfer,pd.cod_Gestiune) and s.cod=pd.cod
		where 
			rtrim(s.Cod_intrare)!='' 
			and (pd.cantitate<0 or s.stoc>=0.001)
			and (nullif(pd.lot_cerut,'') is null or s.lot=pd.lot_cerut)
			and (nullif(pd.locatie_ceruta,'') is null or s.locatie=pd.locatie_ceruta)
		order by cod_gestiune,cod
		...
		from 
			(	select 
			s1.Cod_gestiune,s1.Cod,s1.nrord,s2.stoc as st 
			from #stoctotal s1
			outer apply (select sum(stoc) stoc from #stoctotal s2 where s1.Cod_gestiune=s2.Cod_gestiune and s1.Cod=s2.Cod and s2.nrord<=s1.nrord) s2
			) calcule 
		where 
		...
		**/
		...
		if @iauPretAmIesiri=1 
			update d set pret_valuta=isnull(p.pret_vanzare,0),
						 ...
						 discount=(case when ISNULL(d.discount,0)=0 and isnull(d.subtip,'') not in ('IF','AA') then ISNULL(p.discount,0) else d.discount end)
				from #documente d
	...
	from #documente d
	inner join infotert ext on ext.subunitate=@subunitate and d.tert=ext.tert and ext.identificator='' 
	where d.tip='RM' and d.valuta<>'' and ext.zile_inc=1 and d.facturanesosita=0

	-- Pentru receptii nesosite, tip_tva=0.
	update d set tip_tva=0
	from #documente d
	where d.tip='RM' and isnull(numar_dvi,'')='' and d.facturanesosita=1	--(am pastrat aici conditia de la validarea din CGplus cu numarul de DVI)
	...
	*/
	...
	if nullif(@cContCheltMarfuri,'') is null
		set @cContCheltMarfuri='601'

	update #documente set cont_intermediar='3'+substring(@cContCheltMarfuri,2,100)
	where tip='CM' and left(cont_de_stoc,3) in ('371','357') and tip_miscare='E' and left(cont_corespondent,3)<>'607' 
		and ptupdate=0 and nullif(cont_intermediar,'') is null
	...
	if nullif(@cContIntermediarMateriale,'') is null
	...
	update d set cont_corespondent=(case when left(d.cont_de_stoc,1)='8' then ''
				else isnull(rtrim(p.val_alfanumerica),'711')+
					(case when p.Val_logica=1 then substring(d.cont_de_stoc,len(rtrim(ltrim(c.cont_parinte)))+1,13) else '' end)
				end)
	from #documente d
	...
	*/
	...
	/*Pentru cod_intrare=Pret_de_stoc sau cod_intrare=cod_produs sau orice alta combinatie
		Se va pune in procedura SP
	*/
	
	/*
	update d set cod_intrare=ltrim(str(pret_de_stoc,12,2)),idIntrare=s.idIntrare,idIntrareFirma=s.idIntrareFirma
	from #documente d
	left join stocuri s on d.gestiune=s.cod_gestiune and d.cod=s.cod and s.Cod_intrare=ltrim(str(d.pret_de_stoc,12,2))
	where d.tip_miscare='I'
	*/
 ...
 -->Conditia privind storno avansuri este cuprinsa in primul update care determina valoarea in valuta stornata (ca sa nu dublez conditia).
	...
	begin
		...
		select top 1 
			@tipPentruNr=max(case when NULLIF(tipPtNumar,'') is not null then tipPtNumar when tip='RM' and jurnal='RC' then 'RC' else tip end),
			@lm=max(loc_de_munca),
			@jurnal=max(jurnal),
	...
	update #documente set cont_corespondent='' where tip='MI' and cont_corespondent is null
	...
	begin
		...
		IF OBJECT_ID('tempdb.dbo.#pozd') IS NOT NULL
			drop table #pozd

		IF OBJECT_ID('tempdb..#stoctotal') IS NOT NULL
			DROP TABLE #stoctotal

		
		IF isnull((select top 1 val_logica from par where tip_parametru='GE' and parametru='FARASTOC'),0)=1 -- Inseamna ca e putin haos adica LUCREZ CU STOC NEGATIV
			and EXISTS (select 1 from sysobjects where [type]='P' and [name]='RezolvaPotentialeStocuriNegative') -- Incearca sa remedieze din haos sa nu se ajunga la inchideri cu ciudatenii
			exec RezolvaPotentialeStocuriNegative @sesiune=@sesiune, @parXML=@parXML
