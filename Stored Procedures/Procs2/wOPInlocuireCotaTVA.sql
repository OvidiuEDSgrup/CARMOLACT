...
/*
	...
	exec wOPInlocuireCotaTVA @sesiune='7B6210EB8B055',@parXML=@p2
...
declare @utilizator varchar(20), @sub varchar(9), @data datetime, @dataJosPreturi datetime, 
	...
	*/
	...
	,@nivel_inlocuire varchar(100), @grupa varchar(100)
begin try
	...
	select 
		...
		@cotaveche=isnull(@parXML.value('(/*/@cotaveche)[1]','float'),20),
		@cotaNoua=isnull(@parXML.value('(/*/@cotanoua)[1]','float'),19),
		@inlocuireCotaTVaNomencl=isnull(@parXML.value('(/*/@cotanomencl)[1]','int'),1),
		...
		@nivel_inlocuire=isnull(@parXML.value('(/*/@nivel_inlocuire)[1]','varchar(100)'), 'G'),	--> daca se inlocuieste pe [T]oata baza de date sau doar pentru o [G]rupa
		@grupa=isnull(@parXML.value('(/*/@grupa)[1]','varchar(100)'), '')		--> [G]rupa pentru care se inlocuieste cota tva


	if @grupa='' and @nivel_inlocuire='G'
		raiserror('Nu s-a schimbat cota TVA! Completati grupa de nomenclator sau alegeti inlocuirea TVA "Pe toata baza de date" !',16,1)
		
	--> #nomenclFiltrat contine toate codurile pentru care se va modifica cota tva; scopul este de a unifica regulile de filtrare ca sa nu se repete peste tot unde se modifica ci sa se rezolve rapid cu un inner join
	select n.cod
		, (case when n.cota_tva=@cotaveche then 1 else 0 end) cota_veche	--in loc de filtru pun flag deoarece la stocuri se verifica cota din stocuri, nu cea din nomenclator
	into #nomenclFiltrat
	from nomencl n
	...
	begin transaction inlocuire_cota_tva
		--> modificarea in par si in tvacasam se face doar daca modificarea de tva se aplica la nivelul intregii baze de date:
		if @nivel_inlocuire='T'
		begin
			...
			SET @comandaSql = N'
				...
				' set val_numerica=@cotaNoua where tip_parametru=''GE'' and parametru=''COTATVA'' and val_numerica=@cotaveche'
			exec sp_executesql @statement=@comandaSql, @params=N'@cotaNoua float, @cotaVeche float', @cotaNoua=@cotaNoua, @cotaveche=@cotaveche

			update TVAcasam set Cota_TVA=@cotaNoua where Cota_TVA=@cotaveche
		end
		...
		if exists (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'tr_validNomencl') AND type='TR')
			alter table nomencl disable trigger tr_validNomencl
		update n
		set Cota_TVA=(case when @inlocuireCotaTVaNomencl=1 then @cotaNoua else n.Cota_TVA end), 
			Pret_vanzare=(case when @optiunePretAm='P' and @TabelaPreturi=0 then convert(decimal(18, 5), n.pret_cu_amanuntul/(1.00+@cotaNoua/100.00)) else n.Pret_vanzare end),
			Pret_cu_amanuntul=(case when @optiunePretAm='R' and @TabelaPreturi=0 then convert(decimal(18, 5), n.Pret_vanzare*(1.00+@cotaNoua/100.00)) else n.Pret_cu_amanuntul end)
		from nomencl n
			inner join #nomenclFiltrat f on n.cod=f.cod and cota_veche=1		--> filtrarea
			--and (pe o grupa and grupa='' or not pe o grupa)
		if exists (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'tr_validNomencl') AND type='TR')
			alter table nomencl enable trigger tr_validNomencl
		...
		begin
			...
			into #preturirecalc
			from preturi p
				inner join #nomenclFiltrat f on f.cod=p.cod_produs and cota_veche=1	--> filtrarea
				left outer join categpret c on c.categorie=p.UM
			where isnull(c.tip_categorie, 0)<>3 and (@CategorieFiltru is null or p.UM=@CategorieFiltru)
		...
		begin
			...
			*/
			...
			begin
				...
				into #tmpstoc
				from stocuri s --#docstoc s
					inner join #nomenclFiltrat f on f.cod=s.cod --> filtrarea; aici cota se verifica in stocuri, nu din nomenclator
				...
				set @input=(select 'TE' as '@tip', convert(varchar(10),@DataDocumente,101) as '@data', 
					rtrim(a.gestiune) as '@gestiune', rtrim(a.gestiune) as '@gestprim', 
						...
						rtrim(s.cont) as '@contcorespondent',
