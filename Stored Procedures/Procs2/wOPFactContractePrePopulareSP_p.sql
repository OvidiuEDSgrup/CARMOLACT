...
CREATE PROCEDURE wOPFactContractePrePopulareSP_p @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	...
	select @tipContract as tip, convert(char(10), @dataJos, 101) as datajos, convert(char(10), @dataSus, 101) as datasus, 
		'' tert, '' AS punct_livrare, '' as lm, '' as gestiune
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (wOPFactContractePrePopulareSP_p)'
