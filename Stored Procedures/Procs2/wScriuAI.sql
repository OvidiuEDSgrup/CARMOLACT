...
as
	...
	declare @Numar varchar(20) , @Data datetime , @Gestiune char(9), @Cod char(20), @Cantitate float, @barcod char(20), 
		@CodIntrare char(20), @CtStoc varchar(40), @CtCoresp varchar(40), @PretStoc float, @Valuta char(3), @Curs float, 
	...
	select @Numar=isnull(numar,''),@Data=data,@Gestiune=gestiune,@Cod=cod, @Cantitate=cantitate,@barcod=barcod,
		...
		@Valuta=valuta,@Curs=curs,@PretAm=pret_amanunt,@CotaTVA=cota_TVA,@TVAnx=TVA_neexigibil,@Tert=ISNULL(tert,''),@Locatie=ISNULL(locatie,''),
		@LM=lm,@Comanda=comanda_bugetari,@Explicatii=explicatii,@Serie=isnull(serie,''),@Utilizator=ISNULL(utilizator,''),
	...
	(
		...
		tva_neexigibil float '@tva_neexigibil',
