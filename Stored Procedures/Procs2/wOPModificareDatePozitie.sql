...
begin try
	declare 
		...
		@pamanunt decimal(17,5),@o_pamanunt decimal(17,5),@curs decimal(17,5),@pvaluta decimal(17,5),@o_pvaluta decimal(17,5),@pvanzare decimal(17,5),@o_pvanzare decimal(17,5),@disc decimal(17,5),@o_disc decimal(17,5),
	...
	select 
		...
		@disc=isnull(@parXML.value('(/parametri/@discount)[1]','decimal(17,5)'),0),
		@o_disc=isnull(@parXML.value('(/parametri/@o_discount)[1]','decimal(17,5)'),0),
	...
	set @xmlPozDoc.modify('replace value of (/row/row/@discount)[1] with sql:variable("@disc")')
	...
	begin
		set @detaliiPoz.modify('delete (/row/@taraorigine)[1]')
		set @detaliiPoz.modify('insert attribute taraorigine {sql:variable("@taraorigine")} into (/row)[1]')
	end
	
	if @indicatorBugetar is not null
	begin		
		set @detaliiPoz.modify('delete (/row/@indicator)[1]')
		set @detaliiPoz.modify('insert attribute indicator {sql:variable("@indicatorBugetar")} into (/row)[1]')
	end
		
	if @detaliiPoz is not null
	begin		
		set @detaliiPoz = (select @detaliiPoz for xml raw('detalii') , type)
