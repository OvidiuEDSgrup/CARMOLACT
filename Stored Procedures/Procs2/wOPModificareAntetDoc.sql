...
/* descriere... */
create procedure wOPModificareAntetDoc @sesiune varchar(50), @parXML xml
as     
begin
	...
	declare @tip char(2), @numar varchar(20),@o_numar varchar(20), @data datetime, @o_data datetime, @gestiune varchar(9), @o_gestiune varchar(9),
		...
		@categPretProprietate varchar(20), @stare int,	@eroare xml, @mesaj varchar(2000), @Bugetari int,@NrAvizeUnitar int,
		...
		@sql_dvi nvarchar(max),@sql_update_dvi nvarchar(max),@sql_where_dvi nvarchar(max), @tip_doc varchar(2),@observatii varchar(200),@binar varbinary(128)
	...
	begin tran
		...
		select 
			...
			,@detalii=detalii
			,@cont_TVA=cont_TVA
			,@observatii=ltrim(rtrim(observatii))
		...
		set @binar=cast('modificareantetdoc' as varbinary(128))
		set CONTEXT_INFO @binar

		-->	Facem si aici validarea existentei facturii (datei facturii) in loc de procedura validFactura, pentru a putea modifica data_facturii sau numarul de factura.
		-->	Am exceptat in procedura validFactura validarea in conditiile in care modificarile se fac prin procedura curenta.
		IF @tip in ('RM','RS','RP','AP','AS') and (@factura<>@o_factura or @data_facturii<>@o_data_facturii) and @factura<>'' 
			and exists (select 1 from facturi f 
				where f.subunitate=@sub and f.tert=@tert and f.factura=@factura and f.tip=(case when @tip in ('RM','RS','RP') then 0x54 else 0x46 end) and f.data<>@data_facturii
				and ABS(f.valoare)>0.01)
			-->	Daca se modifica factura atunci ajunge conditia de mai sus. Daca se modifica data facturii, atunci este necesara si de conditia de mai jos.
			and (@factura<>@o_factura or not exists (select 1 from facturi f 
				where f.subunitate=@sub and f.tert=@tert and f.factura=@o_factura and f.tip=(case when @tip in ('RM','RS','RP') then 0x54 else 0x46 end) and f.data=@o_data_facturii
				and ABS(f.valoare)>0.01))
		begin
			select 
				top 1 @mesaj = 'Factura cu numarul ' + rtrim(f.factura) + ' exista in sistem cu o alta data: ' + convert(varchar(10), f.data, 103)
				from facturi f 
				where f.subunitate=@sub and f.tert=@tert and f.factura=@factura and f.tip=(case when @tip in ('RM','RS','RP') then 0x54 else 0x46 end) and f.data<>@data_facturii and @factura<>'' 
					and ABS(f.valoare)>0.01
		...
		set @sql_update_doc='Subunitate=@sub'+char(13)+
			...
			+ ',detalii=@detalii'
		set @sql_where_doc='WHERE Subunitate=@sub and tip=@tip_doc and numar=@o_numar and data=@o_data'+ case when @tip='RC' then ' and jurnal=''RC''' else '' end
		...
		set @sql_update_pozdoc='Subunitate=@sub'+char(13)+	 
			...
			(case when @valuta<>@o_valuta then ',Valuta=@valuta'+char(13) else '' end)+
			(case when @curs<>@o_curs then ',Curs=@curs, '+'pret_vanzare=round(pret_valuta*(case when @valuta='''' then 1 else @curs end)*(1-discount/100),5), '
				+(case when @tip_doc='RM' then 'pret_de_stoc=round(pret_valuta*(case when @valuta='''' then 1 else @curs end)*(1+discount/100),4), ' else '' end)
				+'tva_deductibil=convert(decimal(16,2), cantitate*round(pret_valuta*(case when @valuta='''' then 1 else @curs end)*(1-discount/100),5)*cota_tva/100.0)'+char(13) else '' end)+
			(case when @discount<>@o_discount then ',Discount=@discount, pret_vanzare=round(pret_valuta*(case when @valuta='''' then 1 else @curs end)*(1-('+convert(varchar(10), convert(decimal(15,2),@discount))+')/100),5), 
				tva_deductibil=convert(decimal(16,2), cantitate*round(pret_valuta*(case when @valuta='''' then 1 else @curs end)*(1-('+convert(varchar(10), convert(decimal(15,2),@discount))+')/100),5)*cota_tva/100.0)'+char(13) else '' end)+
			(case when @data_facturii<>@o_data_facturii then ',Data_facturii=@data_facturii'+char(13) else '' end)+
			(case when @data_scadentei<>@o_data_scadentei then ',Data_scadentei=@data_scadentei'+char(13) else '' end)+	
		...
		set @sql_where_pozdoc='WHERE Subunitate=@sub and tip=@tip_doc and numar=isnull(@o_numar,@numar) /* de ce? and tert=@o_tert */and data=@o_data'	+
		...
		set @sql=@sql_doc +char(13)+char(13)+ @sql_pozdoc		
		...
		if @observatii is not null
		begin
			update doc 
			set detalii='<row/>' where Subunitate=@sub and tip=@tip_doc and numar=@o_numar and data=@o_data 
				and detalii is NULL and @observatii<>''

			update doc 
			set detalii.modify('replace value of (/row/@observatii)[1] with sql:variable("@observatii")')
			where Subunitate=@sub and tip=@tip_doc and numar=@o_numar and data=@o_data and detalii.value('(/row/@observatii)[1]','varchar(200)') is not null 

			update doc 
			set detalii.modify('insert attribute observatii {sql:variable("@observatii")} into (/row)[1]') 
			where Subunitate=@sub and tip=@tip_doc and numar=@o_numar and data=@o_data 
				and detalii.value('(/row/@observatii)[1]','varchar(200)') is null and @observatii<>''
		end
