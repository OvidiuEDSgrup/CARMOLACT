...
BEGIN TRY
	...
	(
		select 
			'TE' as tip, @data data, @numar numar, rtrim(a.Cod_gestiune) gestiune, rtrim(@gest_detinatar) gestprim, @detalii_doc detalii, rtrim(a.loc_munca) lm, rtrim(a.comanda) comanda,
			convert(int, a.discount_suma) as categpret, 
			(
				select
					rtrim(p.cod) cod, convert(decimal(17,5), p.cantitate) cantitate, rtrim(p.cod_intrare) codintrare, rtrim(NULLIF(p.cont_corespondent,'')) contintermediar,
					rtrim(p.Cont_de_stoc) contstoc, convert(decimal(17,5),p.pret_de_stoc) pstoc, rtrim(p.loc_de_munca) lm, rtrim(p.comanda) comanda, @detalii_poz detalii, p.gestiune gestiune,
					convert(int, p.accize_cumparare) as categpret, (case when g.tip_gestiune in ('A','V') or gp.tip_gestiune in ('A','V') then convert(decimal(12,2),p.Tva_neexigibil) end) tvaneexigibil
				from #tmp_pconfirm p
					left outer join gestiuni g on g.subunitate=p.subunitate and g.cod_gestiune=p.gestiune
					left outer join gestiuni gp on gp.subunitate=p.subunitate and gp.cod_gestiune=p.gestiune_primitoare
				for xml raw, type
			)
		from #tmp_confirm a
		for xml raw, type
