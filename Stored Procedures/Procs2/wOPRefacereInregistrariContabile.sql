...
begin try
	...
	*/
	...
	select 
		...
		@ALOP = ISNULL(@parXML.value('(/*/@ALOP)[1]', 'int'),0),
		@tipNC = ISNULL(@parXML.value('(/*/@tipnc)[1]', 'varchar(100)'),'')

	if @RM+@PP+@CM+@AP+@AS+@AC+@TE+@DF+@PF+@CI+@AF+@AI+@AE+@PI+@AD+@NC+@alop=0
	...
	select @listaTipuri=
		...
		(case @NC when 1 then isnull(nullif(@tipNC,'')+',','IC,MA,ME,MI,MM,NC,DP,AO,AT,PS,UA,') else '' end) +
		(case @alop when 1 then 'PB,RB,AB,AL,OP,CA,' else '' end)
	...
	**/
	...
	IF @RM+@PP+@CM+@AP+@AS+@AC+@TE+@DF+@PF+@CI+@AF+@AI+@AE>0 -- lipseste cel putin AF. Tratat sa prelucreze si AF.
		...
		where 
			...
			and (@lista_lm=0 or exists (select 1 from LMFiltrare lu where lu.utilizator=@userASiS and lu.cod=d.Loc_de_munca ))
	...
	if @alop>0 and exists(select 1 from sysobjects where name='insertptRefacereALOP')
		exec insertptRefacereALOP @sub,@datainf,@datasup,@listaTipuri,@datainch

	/*
		In acest punct avem in DocDeContat documentele a caror inregistrari le vom reface, dar probabil si altele, pe care nu vrem sa le "afectam"
	...
	*/
	select string as tip
	into #tipuridoc
	from dbo.fSplit(@listaTipuri,',')

	delete pn
	from pozincon pn
	inner join #tipuridoc on pn.Tip_document=#tipuridoc.tip
	where pn.Subunitate=@sub and 
	pn.Data between @datainf and @datasup
	and (@lista_lm=0 or exists (select 1 from LMFiltrare lu where lu.utilizator=@userASiS and lu.cod=pn.Loc_de_munca ))
