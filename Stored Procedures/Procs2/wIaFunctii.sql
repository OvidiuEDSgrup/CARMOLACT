...
begin try
	declare @utilizator varchar(20), @lista_lm int, 
			@filtruDenumire varchar(100), @filtruCOR varchar(100), @filtruDomeniu varchar(100), @mesaj varchar(1000)

	exec wIaUtilizator @sesiune = @sesiune, @utilizator = @utilizator OUTPUT
	set @lista_lm=dbo.f_areLMFiltru(@utilizator)

	set @filtruDenumire = isnull(@parXML.value('(/row/@denumire)[1]','varchar(100)'),'')
	...
	if OBJECT_ID('tempdb..#tmpFunctii') is not null drop table #tmpFunctii

	select *, nullif(detalii.value('(/row/@lm)[1]','varchar(20)'),'') as lm
	into #tmpFunctii
	from functii
	where rtrim(denumire) like '%'+@filtruDenumire+'%' 

	select top 100 rtrim(f.cod_functie) as cod, rtrim(f.denumire) as denumire, rtrim(f.nivel_de_studii) as nivelstudii, 
		rtrim(isnull(e.val_inf,'')) as codcor, rtrim(isnull(fc.denumire,'')) as denumirecor, 
		rtrim(isnull(p1.Valoare,'')) as descriere, rtrim(isnull(p2.Valoare,'')) as scop, 
		rtrim(isnull(p3.Valoare,'')) as studii, rtrim(isnull(p4.Valoare,'')) as experienta, 
		rtrim(isnull(p5.Valoare,'')) as id_domeniu, rtrim(isnull(d.Denumire,'')) as dendomeniu, 
		f.detalii, lm.denumire as denlm
	into #wFunctii
	from #tmpFunctii f
		...
		left outer join lm on lm.cod=f.lm
	where (rtrim(isnull(fc.Cod_functie,'')) like '%'+@filtruCOR+'%' or rtrim(isnull(fc.Denumire,'')) like '%'+@filtruCOR+'%') 
		and (rtrim(isnull(p5.Valoare,'')) like '%'+@filtruDomeniu+'%' or rtrim(isnull(d.Denumire,'')) like '%'+@filtruDomeniu+'%')
		and (@lista_lm=0 or f.lm is null or lu.cod is not null)
