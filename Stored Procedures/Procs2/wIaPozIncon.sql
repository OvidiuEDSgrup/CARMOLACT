...
begin
	...
	if @tip in ('RE','RV','EF','DE','DR') set @tip='PI'
	set @tabela=1
	if @tip='PI'
		set @tabela=2
	...
	*/
	...
	if exists (select 1 from DocDeContat where Subunitate=@subunitate and Tip=@tip and Numar=@nrdocument and Data=@data) 
		exec faInregistrariContabile @dinTabela=0,@Subunitate=@subunitate, @Tip=@tip, @Numar=@nrdocument, @Data=@data, @parXML=@parXMLinreg
