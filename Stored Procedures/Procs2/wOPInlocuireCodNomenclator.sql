...
BEGIN TRY
	if exists(select * from sysobjects where name='wOPInlocuireCodNomenclatorSP1' and type='P')      
		exec wOPInlocuireCodNomenclatorSP1 @sesiune=@sesiune,@parXML=@parXML
	...
	begin tran 
		...
		begin try
			...
			if exists (select 1 from sys.objects where name='PozTehnologii' and type='U')
			Update p
				set cod=t.cod_nou
			FROM PozTehnologii p
			JOIN ##tmp_corespc t on t.cod_vechi=p.cod and p.tip in ('M')

			if exists (select 1 from sys.objects where name='PozLansari' and type='U')
			Update p
				set cod=t.cod_nou
			FROM PozLansari p
			JOIN ##tmp_corespc t on t.cod_vechi=p.cod and p.tip in ('M')

			if exists (select 1 from sys.objects where name='pozAntecalculatii' and type='U')
			Update p
				set cod=t.cod_nou
