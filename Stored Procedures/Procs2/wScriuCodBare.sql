...
begin try
SELECT @PARXML
declare @cod varchar(20),@update int, @codbare varchar(100), @um int, @umprodus varchar(20)
set @cod = upper(@parXML.value('(/row/@cod)[1]','varchar(20)'))
set @codbare = upper(@parXML.value('(/row/row/@codbare)[1]','varchar(100)'))
...
if @update=1
 begin
 declare @o_cod varchar(20),@o_codbare varchar(100), @o_um int, @o_umprodus varchar(20)
	 select @o_cod  = upper(@parXML.value('(/row/@cod)[1]','varchar(20)')),
			@o_codbare  = upper(@parXML.value('(/row/row/@o_codbare)[1]','varchar(100)')),
