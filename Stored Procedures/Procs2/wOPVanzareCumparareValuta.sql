...
declare @mesaj varchar(500),@utilizator varchar(20),@sub varchar(9),@cont_dif_curs varchar(40),@cont_disp_lei varchar(40),@cont_disp_valuta varchar(40),
	...
	@ctCheltDifCF varchar(40),@CtVenDifcF varchar(40),@ctCheltDifCurs varchar(40),@CtVenDifCurs varchar(40)
begin try		
	...
	select 
		...
		@ctCheltDifCurs=ISNULL(@parXML.value('(/parametri/@cont_chelt_dif_curs)[1]', 'varchar(40)'), ''),
		@CtVenDifCurs=ISNULL(@parXML.value('(/parametri/@cont_venit_dif_curs)[1]', 'varchar(40)'), ''),
	...
	select	@ctCheltDifCurs=(case when @ctCheltDifCurs='' then isnull(nullif(@CtCheltDifcF,''),'665') else @ctCheltDifCurs end),
			@CtVenDifCurs=(case when @CtVenDifCurs='' then isnull(nullif(@CtVenDifcF,''),'765') else @CtVenDifCurs end)
	...
	set @cont_dif_curs=case when @tiptranzactie <>'CV' and @curs_BNR>@curs_BC OR @tiptranzactie='CV' and @curs_BNR<@curs_BC then @ctCheltDifCurs else @CtVenDifCurs end
