...
--	se parcurg salariatii care au completat campul (atributul) email din personal.detalii, in conditiile filtrelor, si fiecare document rezultat este trimis pe email
	if @tipoperatie=1
	Begin
		...
		select a.marca, p.nume, isnull(p.detalii.value('(/row/@email)[1]','varchar(100)'),ip.email), isnull(dbo.iauExtinfopVal (a.Marca, 'PAROLAFLUT'),'') 
--	informatia din extinfop va fi utilizata daca se va dori arhivarea documentului (7z) cu parola (GrupSapte a cerut in ASiSplus)
		...
		where a.data=@dataSus and (@marca='' or a.marca=@marca) and (@lm='' or a.loc_de_munca=@lm) 
			...
			and (@functie='' or i.Cod_functie=@functie) and isnull(p.detalii.value('(/row/@email)[1]','varchar(100)'),isnull(ip.email,''))<>''
