...
declare @subunitate varchar(9), @GenComPBK int, @fltComanda varchar(20), @fltTipComanda varchar(1), @fltDescriere varchar(80),
	@fltPerioadaJos_citit varchar(100), @fltPerioadaSus_citit varchar(50), @fltPerioadaJos datetime, @fltPerioadaSus datetime, @fltGrupa varchar(20), @fltDenGrupa varchar(30),
...
select @fltComanda=isnull(@parXML.value('(/row/@comanda)[1]', 'varchar(20)'), ''),
	...
	@fltPerioadaJos_citit=@parXML.value('(/row/@perioadajos)[1]', 'varchar(50)'),
	@fltPerioadaSus_citit=@parXML.value('(/row/@perioadasus)[1]', 'varchar(50)'),
...
select @fltPerioadaJos = (case when len(@fltPerioadaJos_citit)>4 and ISDATE(dbo.fSchimbaZiLuna (@fltPerioadaJos_citit))=1 then convert(datetime, dbo.fSchimbaZiLuna (@fltPerioadaJos_citit)) end)
select @fltPerioadaSus = (case when len(@fltPerioadaSus_citit)>4 and ISDATE(dbo.fSchimbaZiLuna (@fltPerioadaSus_citit))=1 then convert(datetime, dbo.fSchimbaZiLuna (@fltPerioadaSus_citit)) end)
...
and (@fltPerioadaJos is null or c.data_lansarii>=@fltPerioadaJos)
and (@fltPerioadaSus is null or c.data_lansarii<=@fltPerioadaSus)
