	...
	-- Ghita:... sau (EAE-alte iesiri) daca se returneaza toata cantitatea 
		...
		begin
			...
			select p.*, f.Valoare_de_inventar, f.Valoare_amortizata+f1.Amortizare_lunara as Valoare_amortizata, 
				...
				mf.Cod_de_clasificare as Cont_amortizare, convert(float,0) as pret_de_stoc_nou, 
				f.cantitate-p.cantitate-isnull((select sum(cantitate) from pozdoc pmis where pmis.numar<>@numar and pmis.Subunitate=p.Subunitate and pmis.Cod_intrare=p.Grupa and pmis.Data between dbo.BOM(p.data) and dbo.EOM(p.data) and pmis.subtip='TR'),0) as cant_noua
			into #pozdocMEP
			from pozdoc p
				inner join gestiuni g on g.Subunitate=p.Subunitate and g.Cod_gestiune=p.Gestiune and g.Tip_gestiune='I'
			...
			begin
				...
				where not exists (select 1 from mismf where subunitate=@subunitate and Numar_de_inventar=grupa and Data_lunii_de_miscare=dbo.EOM(data) 
					and Tip_miscare=(case when p.cant_noua<0.0001 then 'EAE' else 'MEP' end) and Numar_document=numar)
...
begin catch
	begin try close tmpMF deallocate tmpMF end try begin catch end catch	--< m-am asigurat ca nu ramane cursor aiurea in caz de eroare
