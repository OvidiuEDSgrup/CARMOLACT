...
CREATE PROCEDURE [dbo].[wIaLansari] @sesiune VARCHAR(50), @parXML XML
AS
...
SELECT TOP 100 RTRIM(pt.cod) AS comanda, RTRIM(c.descriere) AS descriere, RTRIM(produs.cod) AS cod, (CASE c.Tip_comanda WHEN 'P' THEN RTRIM(n.Denumire) WHEN 'X' THEN rtrim(tt.denumire) END
		) AS produs, pt.id AS id, convert(CHAR(10), c.data_lansarii, 101) AS dataLansare, convert(CHAR(10), c.data_inchiderii, 101) AS 
	dataInchidere, convert(DECIMAL(10, 2), pt.cantitate) AS cantitate, RTRIM(n.um) AS um, isnull(RTRIM(c.Tip_comanda), 'S') AS tipL, 
	isnull(RTRIM(t.Denumire), 'intern') AS tert, (CASE WHEN c.starea_comenzii = 'I' THEN '#00FF00' ELSE '#FF0000' END) AS 
	culoare
FROM pozLansari pt
INNER JOIN comenzi c ON pt.cod = c.Comanda
INNER JOIN pozTehnologii produs ON produs.id = pt.idp
	AND pt.tip = 'L'
	AND produs.tip = 'T'
INNER JOIN tehnologii tt ON tt.cod = produs.cod
LEFT JOIN nomencl n ON produs.cod = n.Cod
