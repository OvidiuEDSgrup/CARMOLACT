...
declare @subtip varchar(2), @codfiscal varchar(20), @tipdecl varchar(1), @numedecl varchar(150), @prendecl varchar(50), @functiedecl varchar(50), 
	...
	@userASiS varchar(10), @nrLMFiltru int, @LMFiltru varchar(9), @iDoc INT, @multiFirma int, @lmUtilizator varchar(9)
...
--	citire date din gridul de operatii
		...
		SELECT isnull(data,datalunii) as data, rand_decont, denindicator, valoare, tva, introdus_manual
		INTo #xmldeconttva
		...
		(
			...
			,denindicator varchar(1000) '@denindicator'
			,valoare FLOAT '@valoare'
			,tva FLOAT '@tva'
			,introdus_manual int '@introdus_manual'
		)
		EXEC sp_xml_removedocument @iDoc 

--	actualizez datele din tabela deconttva cu valorile din grid (daca s-au modificat)
		update d set d.Denumire_indicator=isnull(x.denindicator,d.Denumire_indicator), d.valoare=x.valoare, d.TVA=x.tva, d.introdus_manual=x.introdus_manual
		from deconttva d, #xmldeconttva x
		where d.Data=x.data and d.Rand_decont=x.Rand_decont and (@multiFirma=0 or d.Loc_de_munca=@lmUtilizator)
	End
	else 
	Begin
