...
isnull(ttva.tip_tva,'P') as tiptva,
...
(case when ff.tert is null and fb.tert is null then '#808080' -- fara sold
	WHEN GETDATE() BETWEEN t.detalii.value('(/row/@data_invalid_jos)[1]', 'datetime') AND t.detalii.value('(/row/@data_invalid_sus)[1]', 'datetime') THEN '#C0C0C0'
...
if exists (select * from sysobjects where name='wIaTertiSP1' and type='P')
	exec wIaTertiSP1 @sesiune, @parXML 
