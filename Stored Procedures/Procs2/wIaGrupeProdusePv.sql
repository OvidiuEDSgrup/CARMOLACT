...
begin try
	...
	*/
	...
	insert @grupe(nr, grupa, denumire, nrproduse, valoare, poza)
		select top(@nrElemente) nr, rtrim(g.Grupa), convert(varchar(30), nr)+' '+rtrim(g.Denumire), convert(varchar(30),g.nrprod), convert(varchar(30),g.nrprod)+' produse', 
			'/assets/img/folder_orange.png'
		from
			(select g.grupa, g.Denumire, n.nrprod,
					ROW_NUMBER() over(order by (g.denumire)) nr
				from grupe g
