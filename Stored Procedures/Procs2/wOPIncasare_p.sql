...
begin try
	...
	create table #dateinc (tert varchar(20), factura varchar(20), data datetime, valoare decimal(17,2), lm varchaR(20), comanda varchar(20), valuta varchar(20), curs decimal(15,4), contcasa varchar(40), formular varchar(100), generare int)

	insert into #dateinc (tert, factura, data, valoare, lm, comanda, valuta, curs, contcasa, formular,generare)
	select
		isnull(@parXML.value('(*/@tert)[1]','varchar(20)'),''),    
		isnull(@parXML.value('(*/@factura)[1]','varchar(20)'),''), 
		isnull(@parXML.value('(*/@data)[1]','datetime'),''),       
		isnull(@parXML.value('(*/@totalvaloare)[1]','decimal(17,2)'),0),     		
		isnull(@parXML.value('(*/@lm)[1]','varchar(9)'),''),         
		isnull(@parXML.value('(*/@comanda)[1]','varchar(20)'),''),				
		isnull(@parXML.value('(*/@valuta)[1]','varchar(20)'),''),				
		isnull(@parXML.value('(*/@curs)[1]','decimal(15,4)'),0),     		
		@contcasa, @formular ,1
