...
@tipsocietate varchar(60), @dentipsocietate varchar(200), @reprlegal varchar(100), 
...
@PrelAvans int, @PrelPersintr int, @PrelCorLm int, @PrelCONeefect int, @PrelPensiiFac int, @PrelParLunari int, @nRealizari int, 
...
select @dentipsocietate=rtrim(descriere) from CatalogRevisal where TipCatalog='TipSocietate' and cod=@tipsocietate
...
--	calcul salarii
	...
	,(case when @codMeniu in ('CS','CSAL') and @nRealizari=1 then 1 end) as calculacord
...
--	declaratii
	...
	,(case when @codMeniu='RV' then rtrim(@dentipsocietate) end) as dentipsoc
