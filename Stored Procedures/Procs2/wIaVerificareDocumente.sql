...
begin try
	...
	select rtrim(a.gestiune) as gestiune,b.tip_gestiune, a.tip as tip_doc, rtrim(a.numar) as numar, convert(char(10),a.data,101) as data, a.tip_miscare, a.cod, a.cod_intrare, 
		...
		into #pretIntrare
		from (select * from pozdoc where subunitate=@sub and data between @data_jos and @data_sus and data>@dDataIstoric and tip_miscare in ('I','E')) a 
		...
		where 
			...
			and abs(convert(decimal(15,2),a.cantitate*a.pret_de_stoc)-(convert(decimal(15,2),a.cantitate*b.pret)))>0.01
		...
		left join gestiuni g on g.Cod_gestiune=a.gestiune
		where a.subunitate=@sub and a.data between @data_jos and @data_sus and a.data>@dDataIstoric and a.tip_miscare in ('I','E')
	...
	IF OBJECT_ID('tempdb..#contStoc') IS NOT NULL drop table #contStoc
		...
		select rtrim(a.gestiune) as gestiune,  rtrim(a.tip) as tip_doc, rtrim(a.numar) as numar, convert(char(10),a.data,101)as data, 
			...
			convert(char(10),@data_jos,101) as datajos,convert(char(10),@data_sus,101) as datasus
			into #contStoc
		from (select * from pozdoc where subunitate=@sub and data between @data_jos and @data_sus and data>@dDataIstoric and tip_miscare in ('I','E') and
		...
		from (select * from pozdoc where Subunitate=@sub and data between @data_jos and @data_sus and data>@dDataIstoric and tip in ('TE','DF','PF') and tip_miscare in ('I','E'))a  
