...
begin try
	declare @Numar char(8), @Data datetime, @Tip char(2), 
		...
		@tiptva int, @idplaja int, @docInserate XML
	...
	select @Numar=isnull(numar,''),@Data=data,@Suma=suma,@Tert=tert,@FacturaStanga=factura_stanga,
		...
		@o_DifTVA=o_diftva,@tiptva=tiptva,@idplaja=NULLIF(idplaja,0)
	...
	if ISNULL(@LM,'')=''
		if @tip='FB' or @tip='CF' or @tip='SF' or @tip='C3'
	...
	begin
		...
		insert pozadoc
			...
			Loc_munca, Comanda, Data_fact, Data_scad, Stare, Achit_fact, Dif_TVA, Jurnal, idplaja)
		...
		values 
			...
			@LM, isnull(@Comanda,''), @DataFact, @DataScad, @tiptva, @AchitFact, @DifTVA, isnull(@Jurnal,''), @idplaja)
