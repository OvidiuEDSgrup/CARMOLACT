...
as
declare @codCateg varchar(10), @codInd varchar(10), @o_codInd varchar(10), @doc XML,@err varchar(100), @rand decimal(5,2), @parinte varchar(10)

set	@codCateg = rtrim(isnull(@parXML.value('(/row/@codCat)[1]', 'varchar(10)'), ''))	
set	@codInd = rtrim(isnull(@parXML.value('(/row/row/@cod)[1]', 'varchar(10)'), ''))
set	@o_codInd = rtrim(isnull(@parXML.value('(/row/row/@o_cod)[1]', 'varchar(10)'), ''))
set @rand= isnull(@parXML.value('(/row/row/@rand)[1]', 'decimal(5,2)'), 0)
set @parinte= rtrim(isnull(@parXML.value('(/row/row/@parinte)[1]', 'varchar(10)'), ''))
