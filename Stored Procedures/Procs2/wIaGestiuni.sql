...
begin try
	declare @subunitate varchar(9), @filtruGestiune varchar(9), @filtruDenumire varchar(30), @filtruTipGestiune char(1),
		@filtruTert varchar(13), @filtruDenTert varchar(80), @filtruCont varchar(40), @filtruDenCont varchar(80), @areDetalii bit, @filtruLocMunca varchar(80)
	...
	select 
		...
		@filtruLocMunca = isnull(@parXML.value('(/row/@fitrulm)[1]', 'varchar(80)'), '')
	...
	select @filtruDenumire = replace(@filtruDenumire,' ','%'),
		...
		@filtruLocMunca = replace(@filtruLocMunca, ' ', '%')
	...
	select 
		...
		isnull(ltrim(rtrim(lm.cod)),'')+' - '+ isnull(ltrim(rtrim(lm.denumire)),'') as denlm,
		...
		(CASE WHEN GETDATE() BETWEEN g.detalii.value('(/row/@data_invalid_jos)[1]', 'datetime')
			AND g.detalii.value('(/row/@data_invalid_sus)[1]', 'datetime') THEN '#C0C0C0' END) AS culoare,
		g.detalii as detalii, RTRIM(cp.Categorie) AS categpret, RTRIM(cp.Denumire) AS dencategpret,
		isnull(g.pret_am, 0) as pret_am
	from gestiuni g
		...
		left join lm on g.detalii.value('(//@lm)[1]','varchar(10)')=lm.Cod
	where g.subunitate=@subunitate
		...
		and (isnull(lm.cod, '') like @filtruLocMunca + '%' 
			or isnull(lm.denumire, '') like '%' +@filtruLocMunca + '%')
	order by 1
