...
CREATE PROCEDURE wScriuJurnalContracteSP @sesiune VARCHAR(50), @parXML XML
OUTPUT AS
...
BEGIN TRY
	...
	merge into con as cn 
		using @contracte as ct on ct.idcontract=cn.idcon
	when matched and isnull(cn.stare,0)<>isnull(ct.stare,0) then 
		update set stare=isnull(ct.stare,0);
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (wScriuJurnalContracteSP)'
