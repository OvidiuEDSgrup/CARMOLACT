...
begin try
	declare @fisier varchar(255), @meniu varchar(20), @database varchar(50), @doc xml
		,@mesaj varchar(5000)
		,@publicabil varchar(100)	--> daca sa se exporte si machete cu detalii.@nepublicabil=0
	...
	set @publicabil = isnull(@parXML.value('(/*/@publicabil)[1]','varchar(50)'),'0')
	...
	begin
		...
		--> identific nepublicabilele:
			
		--> tipuri asociate cu meniurile + tip + subtip; e nevoie pentru a controla generare popularii in functie de nepublicabilitate
		--> starea de nepublicabilitate este gasita de wOPGasesteRelatiiConfigurari si se returneaza in #tipuri.nepublicabil_actual
		...
		begin
			...
			exec wOPGasesteRelatiiConfigurari_tabela @sesiune=null, @parXML=null
		end
		
		insert into #tipuri(meniu, tip, subtip)
		select meniu, tip, subtip from @export
		
		declare @nr_anterior int, @nr_curent int
		select @nr_anterior=0, @nr_curent=1
		while @nr_anterior<>@nr_curent
		begin
			select @nr_anterior=@nr_curent
			exec wOPGasesteRelatiiConfigurari @sesiune=null, @parXML=null
			delete t from #tipuri t where t.tabela='webconfigtipuri_reftab'	--> nu vreau sa iau si configurarile care folosesc meniurile pe care le caut
			select @nr_curent=count(1) from #tipuri
		end
...
--			select 
			...
			cross apply (select (select max(wt.Meniu) Meniu, max(wt.Tip) Tip, max(isnull(wt.Subtip,'')) Subtip, max(Ordine) Ordine, max(Nume) Nume, max(Descriere) Descriere,
				...
				(select max(form.Meniu) Meniu, max(form.Tip) Tip, max(isnull(form.Subtip,'')) Subtip, max(Ordine) Ordine, max(Nume) Nume, max(TipObiect) TipObiect,
					...
					from webconfigform form
						left join #tipuri t on form.meniu=t.meniu and form.tip=t.tip and isnull(form.subtip,'')=t.subtip
							and t.tabela<>'webconfigmeniu'
					where form.meniu=max(wt.meniu) and
						...
						and (@publicabil<>1 or t.nepublicabil_actual<>1
							and isnull(form.detalii.value('(row/@nepublicabil)[1]','varchar(20)'),'')<>'1')
					group by form.meniu, form.tip, isnull(form.subtip,''), datafield
					order by form.tip, isnull(form.subtip,''), ordine for xml auto, type) formuri,
				(select max(filtre.Meniu) Meniu, max(filtre.Tip) Tip, max(Ordine) Ordine, max(convert(int,Vizibil)) Vizibil, max(TipObiect) TipObiect, max(Descriere) Descriere,
					...
					from webconfigfiltre filtre
						left join #tipuri t on filtre.meniu=t.meniu and filtre.tip=t.tip
							and t.tabela<>'webconfigmeniu'
					where filtre.meniu=max(wt.meniu) and max(isnull(wt.subtip,''))=''
						and (@publicabil<>1 or t.nepublicabil_actual<>1
							and isnull(filtre.detalii.value('(row/@nepublicabil)[1]','varchar(20)'),'')<>'1')
						and (replace(isnull(filtre.meniu,''),max(e1.tipmacheta)+'_','')=isnull(filtre.tip,'') and max(isnull(wt.tip,''))=''
					...
					group by filtre.meniu, filtre.tip, datafield1
					order by filtre.tip, ordine for xml auto, type) filtre,
				(select max(grid.Meniu) Meniu, max(grid.Tip) Tip, max(grid.Subtip) Subtip, max(convert(int,InPozitii)) InPozitii, max(NumeCol) NumeCol, max(DataField) DataField,
					...
					from webconfiggrid grid
						left join #tipuri t on grid.meniu=t.meniu and grid.tip=t.tip and grid.subtip=t.subtip
							and t.tabela<>'webconfigmeniu'
					where grid.meniu=max(wt.meniu)
						and (@publicabil<>1 or t.nepublicabil_actual<>1
							and isnull(grid.detalii.value('(row/@nepublicabil)[1]','varchar(20)'),'')<>'1')
						--isnull(grid.tip,'')=max(isnull(wt.tip,'')) 
						and  (case when max(isnull(e1.tipmacheta,''))='C' and (max(e1.areTipNecompletat)=0 or isnull(grid.tip,'')=isnull(grid.meniu,'')) and isnull(wt.tip,'')='' then '' else isnull(grid.tip,'') end)=max(isnull(wt.tip,''))
						and isnull(grid.subtip,'')=max(isnull(wt.subtip,''))
					group by grid.meniu, grid.tip, grid.subtip, datafield, inpozitii
					order by grid.tip, inpozitii, grid.subtip, ordine for xml auto, type) griduri,
				(select max(MeniuSursa) MeniuSursa, max(TipSursa) TipSursa, max(NumeTab) NumeTab, max(Icoana) Icoana, max(TipMachetaNoua) TipMachetaNoua,
						...
						from webconfigtaburi tab
							left join #tipuri t on tab.MeniuSursa=t.meniu and tab.tipsursa=t.tip
								and t.tabela<>'webconfigmeniu'
						where tab.meniusursa=max(wt.meniu)
							and (@publicabil<>1 or t.nepublicabil_actual<>1
								and isnull(tab.detalii.value('(row/@nepublicabil)[1]','varchar(20)'),'')<>'1')
						--isnull(tab.tipsursa,'')=max(isnull(wt.tip,''))
							and	(case when max(isnull(e1.tipmacheta,''))='C' and max(e1.areTipNecompletat)=0 and isnull(wt.tip,'')='' then '' else isnull(tab.tipsursa,'') end)=max(isnull(wt.tip,''))
			...
			from webconfigtipuri wt
				...
				left join #tipuri t on wt.meniu=t.meniu and wt.tip=t.tip and isnull(wt.Subtip,'')=t.subtip
					and t.tabela<>'webconfigmeniu'
			where isnull(wt.Meniu,'')=isnull((select Meniu from @export where ID=@linie_curenta),'')
				and (@publicabil<>1 or t.nepublicabil_actual<>1)
