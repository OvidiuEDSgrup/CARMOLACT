create procedure wIaPozdoc @sesiune varchar(50), @parXML xml  
	...
	---despartire camp comanda in comanda(primele 20 caractere) si indicator bugetar(ultimele 20 caractere)
		...
		(case when @tip_doc in ('RM','RS') and p.Procent_vama=0 then '0-TVA Deductibil'
			...
			when @tip_doc in ('RM','RS') and p.Procent_vama=3 then '3-TVA Neded. cont intrare'
		...
		convert(decimal(17, 5), p.pret_valuta) as pvaluta, convert(decimal(17, 5), p.pret_valuta*(1-p.discount/100)) as pvalutadisc, 
		convert(decimal(17, 2), round(convert(decimal(18,5),p.cantitate*p.pret_valuta),2)) as valvaluta, 
		convert(decimal(17, 2), round(convert(decimal(18,5),p.cantitate*p.pret_vanzare),2)) as valvanzare, 
		...
		(case when p.Pret_de_stoc>0.0 then convert(decimal(17, 2), (p.pret_vanzare/p.Pret_de_stoc-1)*100.0) end ) as adaos,  -- se recalculeaza mai jos pt est. C cu evaluare
	...
	---in campul numar_dvi se salveaza si punctul de livrare incepand cu caracterul 14--- 
		...
		convert(decimal(12, 2), p.TVA_neexigibil) as tvaneexigibil,
		...
		convert(char(10), p.Data_facturii, 101) as datafacturii,		
	...
	if exists(select 1 from #wPozDoc p inner join gestiuni g on p.Gestiune=g.cod_gestiune left join gestiuni gp on p.tip='TE' and p.gestprim=gp.Cod_gestiune 
		where isnull(gp.pret_am, g.pret_am)=1)
	begin
		select p.idpozdoc,p.cod,po.tip,po.gestiune
		into #preturiam
		from #wPozDoc p
		inner join pozdoc po on p.idpozdoc=po.idPozDoc
		inner join gestiuni g on p.Gestiune=g.cod_gestiune and g.pret_am=1
		where po.Gestiune<>po.Gestiune_primitoare
		union all 
		select p.idpozdoc,p.cod,'TE',po.Gestiune_primitoare
		from #wPozDoc p
		inner join pozdoc po on p.idpozdoc=po.idPozDoc
		inner join gestiuni g on po.Gestiune_primitoare=g.cod_gestiune and g.pret_am=1 and po.tip='TE'
		where po.Gestiune<>po.Gestiune_primitoare	
		...
		update p set 
			pamanunt =(case when pd.tip in ('RM','TE') then  pa.pret_amanunt else pamanunt end),
			adaos = (case when p.pstoc>0.0 and p.pamanunt>0.0 then convert(decimal(17, 2), ((p.pamanunt/(1.0+p.cotatva/100.0)/p.pstoc-1)*100.0)) 
			when p.pstoc>0.0 and p.pvanzare>0.0 then convert(decimal(17, 2), (p.pvanzare/p.pstoc-1)*100.0) else 0 end )
		from #wPozDoc p
		JOIN PozDoc pd on p.idpozdoc=pd.idPozDoc
		JOIN #preturiam pa ON pa.idpozdoc=p.idpozdoc
	end


	SET @doc = (
			SELECT ordonare, subunitate, tip, subtip, numar, data, cod, codcodi, gestiune, gestprim, tert, lm, comanda, dencomanda, indbug, 
				codintrare, tiptva, dentiptva, cantitate, pvaluta, pvalutadisc, valvaluta, valvanzare, valcutva, valstoc, pstoc, pvanzare, pamanunt,
				...
				dentert, tvaneexigibil, culoare, cod_de, utilizator, data_operarii, datafacturii, detalii, idpozdoc, colet, serii.query('.')
			FROM #wPozDoc
