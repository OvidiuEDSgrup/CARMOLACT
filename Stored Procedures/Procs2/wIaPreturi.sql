...
begin
	...
	(case when (p1.tip_pret in ('2','9') and @data between p1.Data_inferioara and p1.Data_superioara) or
		...
		end) as inPromotie, p1.tip_pret tipPretCategorie
	into #prCat
		from preturi p1
	...
	if @tipCategorie='3' --Discount
		update p set discount=pcat.pret_vanzare,calculat=1,inpromotie=pcat.inpromotie, tipPretCategorie=pcat.tipPretCategorie
	...
	if @tipCategorie='2' --Pret cu amanuntul
		update p set pret_amanunt=pcat.pret_cu_amanuntul,pret_amanunt_vechi=pcatold.pret_cu_amanuntul,tipPret='2',calculat=1,inpromotie=pcat.inpromotie, tipPretCategorie=pcat.tipPretCategorie
		from #preturi p
	...
	if @tipCategorie='1' --Pret vanzare
		update p set pret_vanzare=pcat.pret_vanzare,pret_vanzare_vechi=pcatold.pret_vanzare,calculat=1,inpromotie=pcat.inpromotie, tipPretCategorie=pcat.tipPretCategorie
		from #preturi p
		inner join #prCat pcat on p.cod=pcat.cod and isnull(p.umprodus,'')=isnull(pcat.umprodus,'') and pcat.ranc=1
		left join #prCat pcatold on p.cod=pcatold.cod and isnull(p.umprodus,'')=isnull(pcatold.umprodus,'') and pcatold.ranc=2
		where isnull(calculat,0)=0
...
if @tipCatBaza='2' --Pret cu amanuntul
	update p set pret_amanunt= pcat.Pret_cu_amanuntul,tipPret='2',calculat=1,pret_amanunt_vechi= pcatold.Pret_cu_amanuntul,inpromotie=pcat.inpromotie, tipPretCategorie=pcat.tipPretCategorie
	from #preturi p
...
if @tipCatBaza='1' --Pret vanzare
	update p set pret_vanzare=pcat.pret_vanzare,calculat=1,pret_vanzare_vechi=pcatold.Pret_vanzare,inpromotie=pcat.inpromotie,tipPretCategorie=pcat.tipPretCategorie
	from #preturi p
	inner join #prBaza pcat on p.cod=pcat.cod and pcat.ranc=1
