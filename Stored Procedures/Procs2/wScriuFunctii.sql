...
declare @iDoc int, @cod varchar(6), @descriere varchar(200), @scop varchar(200), @studii varchar(200), @experienta varchar(200), @id_domeniu int, @referinta int, @tabReferinta int, 
		@eroare xml, @mesaj varchar(1000), @mesajEroare varchar(254), @docDetalii xml
...
--	validari
	...
	if exists (select * from sysobjects where name ='wScriuFunctiiSP')
		exec wScriuFunctiiSP @sesiune=@sesiune, @parXML=@parXML

	insert into functii (cod_functie, denumire, nivel_de_studii, detalii)
	select x.cod, isnull(x.denumire,''), isnull(x.nivel_studii,''), x.detalii
	from #xmlfunctii x
	where x.ptupdate=0
	...
	update f set 
		f.Cod_functie=isnull(x.cod, f.Cod_functie), Denumire=isnull(x.denumire,f.Denumire), 
		f.Nivel_de_studii=isnull(x.nivel_studii,f.Nivel_de_studii),
		f.detalii=x.detalii
