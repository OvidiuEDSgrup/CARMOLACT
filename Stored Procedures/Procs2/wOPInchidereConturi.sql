...
declare @data datetime, @lm varchar(13),@com varchar(20),@indbug varchar(20),@stergTVA int,@inchidTVA int,@inchid4423 int,@sterg121 int,@inchid121 int,@inchid770 int,
	 @inv711_121 int,@subunitate char(9),@lunaBlocata int,@anBlocat int,@mesaj varchar(200),
	 @initializareAnSolduri bit, @inchidTLI bit, @corectiiDate bit, @rulajeLM bit, @Inlocm varchar(20), @userASiS varchar(100), @dataValidareTertiANAF datetime
/*
declare @p2 xml
...
begin try
	select 
		...
		@inchid770=isnull(@parXML.value('(/parametri/@inchid770)[1]', 'int'),0),
	...
	if exists(select 1 from lmfiltrare where utilizator=@userASiS) and @lm not in (select cod from lmfiltrare where utilizator=@userASiS)
		raiserror('Introduceti loc de munca corelat cu filtrarea!',16,1)

	--> se initializeaza soldurile contabile doar daca suntem in decembrie; se init pe anul urmator:
	set @initializareAnSolduri= (case when month(@data)=12 then isnull(@initializareAnSolduri,0) else 0 end)
	...
	/*	Verificam ca s-a executat operatia Validare terti ANAF pentru perioada selectata in ultimele 48 de ore.
		Daca nu s-a executat da eroare. Facem aceste validari pentru inchidere luna>=10.2016. */
	if @data>='10/01/2016'
	begin
		select top 1 @dataValidareTertiANAF=data from webJurnalOperatii where obiectSQL='wOPVerificareCifANAF' 
			and parametruXML.value('(/row/@luna)[1]','int')=month(@data) and parametruXML.value('(/row/@an)[1]','int')=year(@data)
			and (nullif(parametruXML.value('(/row/@lm)[1]','varchar(9)'),'') is null or parametruXML.value('(/row/@lm)[1]','varchar(9)')=@lm)
		order by data desc

		if @dataValidareTertiANAF is null
			raiserror('Nu ati rulat operatia Validare terti ANAF pentru perioada selectata! Rulati mai intai aceasta operatie si apoi efectuati Inchidere conturi!' ,16,1)
		if DateDiff(hour,@dataValidareTertiANAF,getdate())>48
			raiserror('Nu ati rulat operatia Validare terti ANAF pentru perioada selectata, in ultimele 48 de ore! Rulati mai intai aceasta operatie si apoi efectuati Inchidere conturi!' ,16,1)
	end

	-->	Mutat jurnalizarea aici. Daca da cel putin unul din mesajele de eroare de mai sus inseamna ca operatia nu a fost executata.
	...
	exec inchidere121lm @Data=@data,@Locm=@lm, @Inlocm=@Inlocm,
		...
		@Sterg121=@sterg121, @Inchid121=@inchid121, @Inv711_121=@inv711_121, @com=@com

	if exists (select 1 from sysobjects where [type]='P' and [name]='inchidereBugetari')
		exec inchidereBugetari @Data=@data,@Locm=@lm, @Inlocm=@Inlocm,
			@StergTVA=@stergTVA, @InchidTVA=@inchidTVA, @Inchid4423=@inchid4423,
			@Sterg121=@sterg121, @Inchid121=@inchid121, @Inv711_121=@inv711_121, @inchid770=@inchid770
	...
	if exists (select 1 from webConfigTipuri where meniu = 'OP_IN' and tip='IC' and subtip='NC')
		select @data Data, 'Inregistrari contabile la inchidere' nume,
			'OP_IN' codmeniu, 'IC' tip, 'NC' subtip,
			'1' as inchideFereastra,
			'O' tipmacheta,
			(select (select @data data for xml raw, type) parxml) dateInitializare
