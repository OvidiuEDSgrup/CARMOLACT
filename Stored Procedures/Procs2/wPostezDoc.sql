...
as
if exists (select 1 from sysobjects where [type]='P' and [name]='wPostezDocSP')
begin 
	declare @returnValue int -- variabila salveaza return value de la procedura specifica
	exec @returnValue = wPostezDocSP @sesiune, @parXML
	return @returnValue
end
	...
	**/
	declare 
		@subunitate varchar(9), @tip varchar(2), @numar varchar(20), @data datetime, @tabela int, @parXMLinreg xml 
	...
	set @tabela=1
	if @tip in ('CO','C3','CF','CB','SF','IF','FF','FB')
		set @tabela=3
	if @tip='NC'
		set @tabela=4
	...
	if exists (select 1 from DocDeContat where Subunitate=@subunitate and Tip=@tip and Numar=@numar and Data=@data)
		exec faInregistrariContabile @dinTabela=0, @Subunitate=@subunitate, @Tip=@tip, @Numar=@numar, @Data=@data, @parXML=@parXMLinreg
