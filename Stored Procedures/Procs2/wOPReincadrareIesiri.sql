...
begin try
	...
	declare @articoleReincadrate int,@articoleDeReincadrat int,@iteratieReincadrare int
	select @articoleReincadrate=0,@articoleDeReincadrat=1,@iteratieReincadrare=1
	...
	select top 0 
		Cantitate,TVa_deductibil,Subunitate,Tip,Numar,Cod,Data,Gestiune,Pret_valuta,Pret_de_stoc,Adaos,Pret_vanzare,Pret_cu_amanuntul,Cota_TVA,Utilizator,
			...
			CONVERT(FLOAT,0) as cumulat,0 as nrordmin,0 as nrordmax,convert(float,0) as tvaunit,0 as nrp, 0 as nrpozmax,0 idPozDoc,convert(xml,'<row />') as detalii,space(20) as colet
		into #pozd from pozdoc

	create table #idDeSters(idpozdoc int)
	
	while @articoleDeReincadrat>0 and @articoleReincadrate<>@articoleDeReincadrat and @iteratieReincadrare between 1 and 5
	begin
		begin transaction stoc
		--select @articoleReincadrate, @articoleDeReincadrat, @iteratieReincadrare
		/** Luam ID-urile din pozdoc care se vor sterge **/
		
		truncate table #idDesters
		truncate table #pozd

		IF OBJECT_ID('tempdb..#stoctotal') IS NOT NULL
				DROP TABLE #stoctotal

		insert into #idDeSters(idPozdoc)
		SELECT 
			idPozDoc
		from pozdoc
		...
		and not (tip='TE' and gestiune=gestiune_primitoare) --Excludem modificarile de pret

		IF EXISTS (SELECT *	FROM sysobjects	WHERE NAME = 'wOPReincadrareIesiriSP'	)
			exec wOPReincadrareIesiriSP @sesiune=@sesiune, @parXML=@parXML

		set @articoleDeReincadrat=@@ROWCOUNT
		...
		IF EXISTS (Select 1 from pozdoc d join #idDeSters id on d.idPozDoc=id.idPozDoc where d.tip='TE' and d.gestiune=d.Gestiune_primitoare)
			set @iteratieReincadrare=@iteratieReincadrare+1
		else
			set @iteratieReincadrare=0
		...
		OUTPUT DELETED.Cantitate,DELETED.TVa_deductibil,DELETED.Subunitate,DELETED.Tip,DELETED.Numar,DELETED.Cod,DELETED.Data,DELETED.Gestiune,
			...
			DELETED.idPozDoc,deleted.detalii,deleted.colet
		INTO #pozd
		from pozdoc 
		...
		(select 
			...
			(case when tip='TE' then 'TE'+LEFT(newid(),8)
			...
			pd.idPozDoc,s1.idIntrare,s1.idIntrareFirma,pd.detalii,pd.colet
			from #pozd pd
		...
		insert (
			...
			Data_operarii,Ora_operarii,Cod_intrare,Cont_de_stoc,Cont_corespondent,TVA_neexigibil,Pret_amanunt_predator,Tip_miscare,Locatie,Data_expirarii,
			Numar_pozitie,Loc_de_munca,Comanda,Barcod,Cont_intermediar,Cont_venituri,Discount,Tert,Factura,Gestiune_primitoare,Numar_DVI,Stare,Grupa,Cont_factura,
			Valuta,Curs,Data_facturii,Data_scadentei,Procent_vama,Suprataxe_vama,Accize_cumparare,Accize_datorate,Contract,Jurnal,idIntrare,idIntrareFirma,detalii,colet)
		VALUES(
			...
			pd.Data_scadentei,pd.Procent_vama,pd.Suprataxe_vama,pd.Accize_cumparare,pd.Accize_datorate,pd.Contract,pd.Jurnal,pd.idIntrare,pd.idIntrareFirma,pd.detalii,pd.colet)
		OUTPUT INSERTED.idPozdoc, pd.idPozDoc
		into #inserate(idPozDoc, idPozDocVechi);
		
		set @articoleReincadrate=@@ROWCOUNT
