--***
create procedure [dbo].wIaEfecteImpl @sesiune varchar(30), @parXML XML
AS  
begin  
	declare @data_implementare datetime,@data_jos datetime,@data_sus datetime,@an_impl int,@luna_impl int,@mod_impl int,@tiptert char(1),
		@lista_lm int, @userASiS varchar(20)
	...
	if exists (select * from LMFiltrare l where l.utilizator=@userASiS)
		...
		set @lista_lm=0
--select * from efimpl
	select top 100 (case a.tip when 'I' then 'B' when 'P' then 'F' else '' end) as tiptert,convert(decimal(17,4),sum(a.Valoare))as t_valoare,convert(decimal(17,4),sum(a.sold))as t_sold,
			convert(decimal(17,4),sum(a.Decontat))as t_decontat,COUNT(*) as nr_efecte,@data_sus as datasus,@data_jos as datajos,
			(case a.tip when 'I' then 'Efecte de incasat' when 'P'then 'Efecte de plata' else '' end) as dentiptert
	from efimpl a
