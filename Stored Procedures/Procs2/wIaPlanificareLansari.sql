...
CREATE PROCEDURE [dbo].[wIaPlanificareLansari] @sesiune VARCHAR(50), @parXML XML
AS
...
SELECT TOP 100 RTRIM(pt.cod) AS comanda, RTRIM(c.descriere) AS descriere, RTRIM(produs.cod) AS cod, (CASE c.Tip_comanda WHEN 'P' THEN RTRIM(n.Denumire) WHEN 'X' THEN rtrim(tt.denumire) END
		) AS denprodus, convert(CHAR(10), c.data_lansarii, 101) AS dataLansare, convert(CHAR(10), c.data_inchiderii, 101) AS 
