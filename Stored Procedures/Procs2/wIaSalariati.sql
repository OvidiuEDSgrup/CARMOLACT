...
declare @sub varchar(9), @LunaInch int, @AnulInch int, @DataInch datetime, @DataAnAnt datetime, @DataLunii datetime, --	data lunii pentru calcul zile CO cuvenite
	...
	@filtruCNP varchar(13), @filtruCuPlecati varchar(20), @filtruNrContract varchar(20), @codMeniu char(20), @dataCrt datetime
...
set @codMeniu=isnull(@parXML.value('(/row/@codMeniu)[1]','varchar(20)'),'')
set @dataCrt=convert(datetime,convert(char(10),GETDATE(),101),101)

/*	Daca se da adaugare (@marca='') si in acest caz nu trebuie sa se faca populare. Am pus procedura de populare wIaSalariati si la meniu SDS intrucat daca se da Accept intr-un tab, 
	apoi se revine in tabul Date salariat si se da Accept se pierd modificarile din efectuate in celelalte taburi cu scriere in personal. */
if @codMeniu='SDS' and rtrim(@marca)=''
	RETURN
