...
begin
	...
	@lmantet varchar(9), @f_denlm varchar(50), @f_salariat varchar(50), @userASiS varchar(10), @DateSalGrpPeLMNivel1 int, @NivelGrupare int
	...
	*/
	...
	@tip as tip, (case when @tip='TM' then 'T2' else @tip end) as subtip, 'Tichete' as densubtip, rtrim(rtrim(max(c.LunaAlfa))+' '+convert(char(4),max(c.An))) as luna, 
	left(rtrim(isnull(i.Loc_de_munca,'')),@NivelGrupare) as lmantet, rtrim(max(isnull(lm.denumire,''))) as denlmantet, count(distinct t.Marca) as nrsal, 
	...
	from Tichete t
		...
		left outer join lm lm on lm.Cod=left(i.Loc_de_munca,@NivelGrupare)
		...
		and (@lmantet is null or left(i.Loc_de_munca,@NivelGrupare)=@lmantet) 
		and (@f_salariat is null or i.Nume like '%'+@f_salariat+'%' or t.Marca like @f_salariat+'%')
		and (@lista_lm=0 or lu.cod is not null)
	group by t.Data_lunii, left(rtrim(isnull(i.Loc_de_munca,'')),@NivelGrupare)
	union all
	select distinct convert(char(10),@datasus,101) as data, @tip as tip, (case when @tip='TM' then 'T2' else @tip end) as subtip, 'Tichete' as densubtip,
	rtrim(rtrim(max(c.LunaAlfa))+' '+convert(char(4),max(c.An))) as luna, 
	left(p.loc_de_munca,@NivelGrupare) as lmantet, rtrim(max(isnull(lm.denumire,''))) as denlmantet,
	...
	from personal as p
		left outer join lm on left(p.loc_de_munca,@NivelGrupare)=lm.cod
	...
	where (@data is null or @datasus=@data)
		...
		and (@f_denlm is null or lm.denumire like '%'+@f_denlm+'%')
		and (@lmantet is null or left(p.Loc_de_munca,@NivelGrupare)=@lmantet) 
		and (@f_salariat is null or p.Nume like '%'+@f_salariat+'%' or p.Marca like @f_salariat+'%')
		and (@lista_lm=0 or lu.cod is not null)
	group by data, left(p.loc_de_munca,@NivelGrupare)
