...
@Serie_CM_ini varchar(10),@Numar_CM_ini varchar(10), @CM_initial varchar(20), @Cod_diagnostic varchar(10), @Cod_diagnostic_ant varchar(10), @Cod_urgenta varchar(10), @Cod_grupaA varchar(10), 
...
begin try
	...
	upper(isnull(Serie_CM, '')) as Serie_CM,
	upper(isnull(Numar_CM, '')) as Numar_CM,
	...
	begin
		if @ptupdate=0 and exists (select 1 from conmed where marca=@marca and data_inceput>@data_sfarsit)
		begin
			set @mesajAvertizare='Salariatul are introdus un CM ulterior celui curent. Daca acel CM este in continuare la CM-ul introdus, trebuie sa-l stergeti si sa-l reoperati!' 
...
--	citesc datele pt. concediile medicale care sunt in continuare
		...
		begin
			Select @Zile_luna_anterioara=a.Zile_lucratoare+a.Zile_luna_anterioara, 
				...
				@Cod_diagnostic_ant=a.Cod_diagnostic
			...
			if @Cod_diagnostic<>@Cod_diagnostic_ant and isnull(@Cod_diagnostic_ant,'')<>''
				set @mesajAvertizare='Codul de diagnostic pt. acest CM: '+rtrim(@Cod_diagnostic)+' difera de codul de diagnostic al CM-ului initial: '+rtrim(@Cod_diagnostic_ant)+'!'
				select @mesajAvertizare as textMesaj for xml raw, root('Mesaje')
		end

		if @Tip_diagnostic<>'0-' and @Ind_calc_manual=0
			select @Media_zilnica=(case when @Media_zilnica=0 then isnull((select round(sum(Baza_cci_plaf)/(case when sum(Zile_asig)=0 and sum(Baza_cci_plaf)=0 then 1 else sum(Zile_asig) end),4) 
				from dbo.fIstoric_cm(@datalunii,@Marca,@Tip_diagnostic,@Data_inceput,@Continuare,0,0)),0) else @Media_zilnica end)
