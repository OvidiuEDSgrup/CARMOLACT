...
BEGIN TRY
	...
	DECLARE @tert VARCHAR(20), @lm VARCHAR(20), @gestiune VARCHAR(20), @grupa VARCHAR(20), @mesaj VARCHAR(400), 
		...
		@utilizator varchar(50), @idContractFiltrat int,@dataAzi datetime, @factura varchar(20)
	...
	SET @idContractFiltrat = nullif(@parXML.value('(/*/@idContract)[1]', 'int'),0) -- filtrare un contract
	...
	declare @stareFacturabil int
	set @stareFacturabil=(select min(stare) from StariContracte s where s.tipContract=@tipContract and s.facturabil=1)

	-- filtrare contracte
	insert into @contracte(idContract,valuta,numar_contract,data_contract, tert)
	select idContract,valuta,c.numar as numar_contract,c.data as data_contract,c.tert
	from contracte c 
		cross apply (select top (1) j.stare from JurnalContracte j where j.idContract=c.idContract order by j.data desc, j.idJurnal desc) j
	where tip = @tipContract and j.stare=@stareFacturabil
		and c.data between @dataJos and @dataSus
		and (@idContractFiltrat is null or idContract = @idContractFiltrat)
	...
	declare @pozitii table(idPozContract int, idContract int, cod varchar(20), cantitate decimal(12,3), pret decimal(15,5), discount decimal(5,2), 
		cod_specific varchar(50),valuta varchar(3), numar_contract varchar(20), data_contract datetime,tert varchar(13))
select p.*,c.valuta,c.numar_contract,c.data_contract,c.tert from PozContracte p  /* fac acest subselect pt. ca sa ma asigur ca nu face optimizatorul SQL prostii */
			inner join @contracte c on p.idContract = c.idContract
	-- iau pozitii -> se iau separat pt. ca sa nu faca joinu-uri multe.
	insert into @pozitii(idPozContract, idContract, cod, cantitate, pret, discount, cod_specific, valuta
		,numar_contract, data_contract, tert)
	select 
		idPozContract,
		p.idContract,
		p.cod,
	...
	--date pentru form
	select @parXML--convert(varchar(10),@dataJos,101) as datajos ,convert(varchar(10),@dataSus,101)  datasus, convert(decimal(17,5),@curs) as curs, @valuta as valuta 
	for xml raw('Date')--, root('Date')	

	select * from @pozitii
if @sesiune='' SELECT
			p.idContract,
			--p.idPozContract,
			--case when row_number() over (partition by p.idContract order by n.denumire)=1 then p.numar_contract else '' end 
			rtrim(p.numar_contract) as numar_contract,
			convert(varchar(10),p.data_contract,101) as data_contract,
			row_number() over (order by p.data_contract, p.numar_contract) as nr_pozitie,
			--p.cod,
			...
			--case when row_number() over (partition by p.idContract order by n.denumire)=1 then RTRIM(t.Denumire) else '' end 
		...
		--WHERE p.cantitate<>0
		group by p.idContract, p.data_contract, p.numar_contract, p.tert, t.Denumire
		order by p.data_contract, p.numar_contract
	--date pentru grid
	SELECT (   
		SELECT
			p.idContract,
		...
		WHERE p.cantitate<>0
		group by p.idContract, p.data_contract, p.numar_contract, p.tert, t.Denumire
		order by p.data_contract, p.numar_contract--t.Denumire,p.numar_contract,row_number() over (partition by p.idContract order by n.denumire)
		FOR XML RAW, TYPE  
		)  
