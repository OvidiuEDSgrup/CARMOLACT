...
if nullif(@parXML.value('(/row/@discount)[1]', 'float'),0) is null
	select top 1 @discount=discount from pozdoc where subunitate='1' and tip=@n_tip and numar=@n_numar and data=@n_data
else
	select @discount= @parXML.value('(/row/@discount)[1]', 'float')
--if @parXML.value('(/row/@discount)[1]', 'float') is null
...
select  rtrim(@n_tert) n_tert,convert(char(10),@n_data,101) n_data,@n_tip n_tip,rtrim(@n_numar) n_numar,
	RTRIM(@cont_stoc) cont_stoc,rtrim(@dencontstoc) dencontstoc, convert(decimal(12,2),@discount) discount
for xml raw
