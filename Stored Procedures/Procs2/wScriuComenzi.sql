...
begin try
	declare 
		...
		@mesajEroare varchar(100), @UltComanda int, @docDetalii xml, @utilizator varchar(100), @propLM varchar(9)
	...
	-- Daca nu se completeaza locul de munca, se incearca 
	if exists(select 1 from #xmlcomenzi where isnull(loc_de_munca,'')='')
	begin
		select top 1 @propLM = rtrim(cod) from lmfiltrare where utilizator=@utilizator order by len(cod)
		update #xmlcomenzi set loc_de_munca=@propLM where isnull(loc_de_munca,'')=''
	end
	...
	from #xmlcomenzi x
	where (x.ptupdate=0 or x.ptupdate=1 and not exists (select 1 from pozcom pc where pc.subunitate='GR' and pc.Comanda=x.comanda)) and isnull(x.grupa, '')<>''

	insert pozcom (Subunitate, Comanda, Cod_produs, Cantitate, UM)
	select 
		@Sub, x.comanda, x.tehnologie, isnull(x.cantitate, 0), isnull(x.UM, '')
	from #xmlcomenzi x 
	where --x.tehnologie<>'' and (x.ptupdate=0 or x.ptupdate=1 and x.tehnologie_veche='')
		(x.ptupdate=0 or x.ptupdate=1 and x.tehnologie_veche='' and not exists (select 1 from pozcom pc where pc.subunitate=@Sub and pc.Comanda=x.comanda and pc.Cod_produs=x.tehnologie))
