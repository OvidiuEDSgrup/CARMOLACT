...
BEGIN TRY
	DECLARE
		...
		@stare int, @stareJurnal varchar(20), @tip varchar(2)
	...
	/** Creare xml si trimitere la wScriuPlin. Formez separat XML-ul pentru Plati dinspre tipul Salarii. */
	if @tip='SA'
		SET @xml =
		(
			SELECT
				...
				(
					SELECT
						...
						RTRIM(podp.detalii.value('(/row/@contcorespondent)[1]', 'varchar(40)')) as contcorespondent, 
						odp.detalii.value('(/row/@lm)[1]','varchar(9)') as lm, rtrim(podp.explicatii) as explicatii, 
						podp.suma AS suma, 'PD' AS subtip
	...
	SELECT
		...
		'S-a generat documentul '+(case when @tip='SA' then '"PD"' else '"PF"' end)+' in data de ' + CONVERT(varchar(10), @data, 103) + '.'  AS textMesaj
