...
declare @cod varchar(20),@cautare varchar(100), @lista_categpret int, @nrcateg int, @rankmax int
...
if LEN(@cautare)>2
	set @rankmax=999 -- daca foloseste cautarea se afiseaza istoricul de preturi
else
	set @rankmax=1
...
--and (@nrcateg<@rankmax or getdate() between data_inferioara and data_superioara)
and (@rankmax=999 or convert(datetime, convert(varchar(10), getdate(),101)) between data_inferioara and data_superioara)

select * from #pret
where ranc<=@rankmax --or ranc=1
order by catpret, ranc
for xml raw
