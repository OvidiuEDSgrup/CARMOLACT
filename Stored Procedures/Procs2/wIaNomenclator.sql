...
begin try
	...
	into #stocpecod
	from stocuri s 
		inner join nomencl on s.cod=nomencl.cod
	...
	select top 100 rtrim(nomencl.cod) as cod,
		isnull(s.stoc,0) as stoc,
		isnull(s.stocpropriu,0) as stocpropriu 
	into #n100
	from nomencl 
	left outer join #stocpecod s on s.cod=nomencl.cod
	...
	where
		...
		and isnull(s.stoc,0) between @filtruStocJ and @filtruStocS
	...
	select (
		...
		convert(decimal(12,3),#n100.stoc) as stoc,
		convert(decimal(12,3),#n100.stocpropriu) as stocpropriu,
		...
		(case 
			when getdate() between n.detalii.value('(/row/@data_invalid_jos)[1]', 'datetime') and n.detalii.value('(/row/@data_invalid_sus)[1]', 'datetime') then '#C0C0C0'
			when n.tip not in ('R','S','F') and #n100.stoc is null then '#808080' -- fara stoc
			when n.tip not in ('R','S','F') and #n100.stoc<0 then'#FF0000' -- stoc negativ
		...
		rtrim(um_2) as um_2,
		rtrim(c.denumire) dencpv,
		rtrim(n.gestiune) gestiune,
		rtrim(gest.Denumire_gestiune) dengestiune,
		n.detalii		
		into #wNomencl
	from #n100 
		inner join nomencl n on n.cod=#n100.cod
		left outer join grupe on n.grupa=grupe.grupa
		left outer join gestiuni gest on gest.subunitate=@cSub and gest.cod_gestiune = n.gestiune
		...
		left join cpv c on c.cod=n.detalii.value('(row/@cpv)[1]','varchar(100)')
	order by o, patindex('%'+@filtruDenumire+'%', n.denumire), n.denumire
	
	
	/* Apel SP1 cu posibilitatea de a altera #wNomencl- chiar si coloane in plus, etc	*/
	if exists(select * from sysobjects where name='wIaNomenclatorSP1' and type='P')
		exec wIaNomenclatorSP1 @sesiune=@sesiune, @parXML=@parXML
