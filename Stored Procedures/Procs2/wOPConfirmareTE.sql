...
begin try 
	...
	set @dataTE = isnull(@parXML.value('(/parametri/@dataTE)[1]', 'datetime'),convert(datetime,convert(char(10),GETDATE(),110),110))
	...
	set @tip = 'TE'
	set @datacharTE=convert(char(10),@dataTE,101)
	...
	begin transaction confTE
		...
		set @parXML.modify('delete (/*/@numarconf)[1]')
		set @parXML.modify('insert attribute numarconf {sql:variable("@numarTE")} into (/*)[1]')
		...
		set @input=(select rtrim(@subunitate) as '@subunitate','TE' as '@tip',
			@numarTE as '@numar', @dataTE as '@data',
				...
				case when left(cont_corespondent,1)='8' then rtrim(cont_corespondent) else	
					isnull((select g.Cont_contabil_specific from gestiuni g where g.subunitate=@subunitate and g.cod_gestiune=[Contract] and g.Cont_contabil_specific<>''),Cont_de_stoc) 
					end as '@contcorespondent', 
		...
		if exists (select 1 from sysobjects where [type]='P' and [name]='wScriuDoc')
			exec wScriuDoc @sesiune,@input
		else
			exec wScriuDocBeta @sesiune,@input
