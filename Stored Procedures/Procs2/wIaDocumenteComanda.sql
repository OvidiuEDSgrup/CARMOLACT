...
BEGIN TRY
	DECLARE 
		...
		@gestiuneRezervari varchar(20), @cuRez int, @ContAvizNefacturat varchar(20), @codAvans varchar(20)
	...
	**/
	...
	UNION
	/* Facturile de avans sunt legate de comanda, nu de vreo pozitie => idPozContract = null, cu legatura prin idJurnal	*/
	select
		NULL, lc.idPozDoc
	...
	JOIN pozdoc pd on pd.idPozDoc=lc.idPozDoc and pd.cod=@codAvans
	...
	WHERE (
			@f_numardoc IS NULL
			OR pd.Numar LIKE @f_numardoc
			)
		AND (
			@f_denumire IS NULL
			OR n.Denumire LIKE @f_denumire
			)
		AND (
			@f_cod IS NULL
			OR pc.cod LIKE @f_cod
			)
