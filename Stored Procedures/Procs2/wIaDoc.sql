...
begin try
	...
	*/
	...
	from doc d
		...
		left outer join @GestiuniUser gpd on @tip_doc='TE' and gpd.valoare=d.Contractul
	...
	where d.subunitate=@Sub --and d.Jurnal<>'MFX' 
		...
		and (@lista_lm=0 or exists (select * from LMFiltrare lu where lu.utilizator=@userASiS and (lu.cod=d.Loc_munca OR lu.cod=d.detalii.value('(/*/@lmdest)[1]','varchar(20)')))
			or gu.valoare is null and gpu.Valoare is not null -- TI-urile sa nu tina cont de locul de munca, nefiind modificabile
				and (gpd.valoare is not null or d.contractul='')) -- in plus, daca are gestiune destinatara trebuie sa fie si asta in lista de gestiuni
...
--select * from #d100
	alter table #d100 add cont_corespondent varchar(40),dencontcorespondent varchar(80),cont_venituri varchar(40),dencontvenituri varchar(80),
		cantitate decimal(17,3), valoare decimal(17,2), valoare_furnizor decimal(17,2), tva_11 decimal(17,2), tva_22 decimal(17,2), valoare_valuta decimal(17,2), valoare_pret_amanunt decimal(15,2),Numar_pozitii int,
	...
	begin
		...
		update d set d.Cont_corespondent=v.Cont_corespondent, d.Cont_venituri=v.Cont_venituri, 
			...
			d.cantitate=v.cantitate, d.valoare=v.valoare, d.valoare_furnizor=v.valoare_furnizor, d.Tva_11=v.Tva_11, d.Tva_22=v.Tva_22, d.Valoare_valuta=v.Valoare_valuta, d.Valoare_valuta_tert=v.Valoare_valuta_tert, 
	...
	select 
		...
		isnull(rtrim(gDest.denumire_gestiune),'') as dengestdest,
		...
		convert(decimal(17,2), d1.valoare_furnizor) as valfurnizor, 
	...
	from #d100 d1 
		...
		left outer join #gest gDest on gDest.cod_gestiune = d.contractul and gDest.tip_gestiune=(case when @tip_doc='TE' then 'G' else 'F' end)
		left outer join lm on lm.cod = d.loc_munca
		left outer join comenzi com on com.subunitate = @sub and com.comanda = left(d.comanda,20)  
