...
begin
	...
	select p.subunitate, p.tip as tip, p.numar_document as numar, p.data as data, p.tip  as subtip, 
		...
		p.numar_pozitie as numarpozitie, p.jurnal as jurnal, 
		p.Stare as tiptva, 
		(case p.Stare when 0 then '0-TVA Deductibil' when 1 then '1-TVA Compensat' when 2 then '2-TVA Nedeductibil' when 3 then '3-TVA Neded. cont intrare' else '' end) as dentiptva,
		p.idpozadoc as idpozadoc, 
		'#000000' as culoare
	into #wPozAdoc
	from pozadoc p
	...
	select rtrim(subunitate) as subunitate, 
		...
		numarpozitie as numarpozitie, rtrim(jurnal) as jurnal, tiptva as tiptva, dentiptva, idpozadoc as idpozadoc, 
		culoare as culoare
	from #wPozAdoc
