...
DECLARE @userASiS VARCHAR(10), @lista_lm BIT, @lista_conturi BIT, @DecGrCont INT, @CtAvFurn VARCHAR(40), @CtAvBen VARCHAR(40), @subunitate CHAR(9), @Bugetari INT, 
	...
	@numere_pozitii VARCHAR(max), @areDetalii BIT, @tipefect varchar(1), @jurnal varchar(20)
...
-->	mai jos am lasat Null la marca, decont, efect	
	...
	rtrim(CASE WHEN isnull(ca.sold_credit, 0) = 8	OR isnull(cc.sold_credit, 0) = 8 THEN p.efect /*ELSE ''*/ END) AS efect, 
...
from #wPozPlin p
outer apply (select max(pc.rk) nrpoz from #wPozPlin pc where pc.subtip=p.subtip and pc.numar = p.numar and pc.plataincasare=p.plataincasare and pc.tert=p.tert) pc
--where p.rk = 1
...
set @docXML = (
	...
	max(marca) as marca, max(denmarca) as denmarca,
	max(decont) as decont, 
	max(efect) as efect,
	(CASE WHEN poz.pozitii = 1 THEN max(tipTVA) END) tipTVA, 
	(CASE WHEN poz.pozitii = 1 THEN max(denTiptva) END) denTiptva, 
	(CASE WHEN poz.pozitii = 1 THEN max(idPozPlin) END) idPozPlin, convert(xml,max(convert(varchar(max),poz.detalii))) as detalii, 
	--culoare,
	(
		SELECT 
			tip,
