...
select rtrim(compcategorii.Cod_Ind) as "@indicator",
	...
	(select convert(int,Val_max)  as "@prag" from semnific s where s.Indicator=compcategorii.Cod_Ind 
		order by Val_max
		for xml path('praguri_indicator'),type),
	(select numar as "@nivel", rtrim(Denumire) "@denumire", Procedura "@procedura", tip_filtru "@tipfiltru" , Tip_grafic "@tipgrafic"
		from colind where Cod_indicator= compcategorii.Cod_Ind order by numar
