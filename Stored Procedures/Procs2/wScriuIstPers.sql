...
as  
Begin
	declare @userASiS varchar(10), @datajos datetime, @datasus datetime, @LunaInch int, @AnulInch int, @DataInch datetime, @DataLJ datetime
	...
	isnull((select count(1) from personal p 
		...
		left outer join LMfiltrare lu on lu.utilizator=@userASiS and lu.cod=p.Loc_de_munca
		where (p.loc_ramas_vacant=0 or p.data_plec>=@DataLJ or left(isnull(p.detalii.value('(/row/@lmnou)[1]','varchar(100)'),isnull(ip.Loc_munca_nou,'')),7)='DETASAT') and p.Data_angajarii_in_unitate<=@datasus
	...
	where (dbo.f_areLMFiltru(@userASiS)=0 or lu.cod is not null)
		and (p.loc_ramas_vacant=0 or p.data_plec>=@DataLJ or left(isnull(p.detalii.value('(/row/@lmnou)[1]','varchar(100)'),isnull(ip.Loc_munca_nou,'')),7)='DETASAT') and p.Data_angajarii_in_unitate<=@datasus 
			and p.Loc_de_munca<>i.Loc_de_munca),0)<>0)
