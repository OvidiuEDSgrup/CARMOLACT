...
BEGIN TRY
	DECLARE @tert VARCHAR(20), @mesaj VARCHAR(400), @dataJos DATETIME, @dataSus DATETIME, 
		...
		@date xml
		,@tipdoc varchar(2) --Poate fi AD de la Alte Documente sau null va fi considerat PozPlin
	...
	SET @valuta = isnull(nullif(@parXML.value('(/*/*/detalii/row/@valutafalta)[1]', 'varchar(3)'),''),isnull(@parXML.value('(/*/*/@valuta)[1]', 'varchar(3)'),''))
...
-->	detalii pozitii (pentru eventualele informatii de introdus in pozplin.detalii)
	...
	Begin
		create table #facturiPeConturi (tert varchar(13)) -- se creeaza aici tabela #facturiPeConturi, pentru a putea fi completata in wOPPISelectiva_pSP1
	...
	--               daca vor dori si nebugetarii (ex. SNC) se va face procedura wOPPISelectiva_pSP1, care va apela procedura FacturiPeConturi, vezi linia urmatoare:
	-- "CREATE PROCEDURE wOPPISelectiva_pSP3 @sesiune VARCHAR(50), @parXML XML output as if isnull(@parXML.value('(/*/*/@tipOperatiune)[1]', 'varchar(2)'),'') in ('PF','IB') exec FacturiPeConturi @sesiune=@sesiune, @parXML=@parXML output"
	-- "DROP PROCEDURE wOPPISelectiva_pSP3"
		exec FacturiPeConturi @sesiune=@sesiune, @parXML=@parXML output

	if exists (select 1 from sysobjects where [type]='P' and [name]='wOPPISelectiva_pSP3')
		exec wOPPISelectiva_pSP3 @sesiune=@sesiune, @parXML=@parXML output
	...
	where isnull(@tipdoc,'PI') not in ('AD')
	...
	select @date =
	(
		select @tipdoc as tipdoc,convert(varchar(10),@data,101) as data, @valuta as valuta, rtrim(@tert) as tert, rtrim(@tert)+' - '+rtrim(@dentert) as dentert, convert(decimal(17,2),@suma) as suma,
