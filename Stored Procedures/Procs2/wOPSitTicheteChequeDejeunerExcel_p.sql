...
create procedure [dbo].wOPSitTicheteChequeDejeunerExcel_p @sesiune VARCHAR(50), @parXML XML
AS
...
begin try
	...
	select convert(char(10),@datajos,101) as datajos, convert(char(10),@datasus,101) as datasus, 
		'sitTicheteChequeDejeunerExcel' as procedura, 
		'Comanda_tichete_'+dbo.fDenumireLuna(@dataSus)+'_'+convert(char(4),year(@datasus))+'.xlsx' as numefisier 
	for xml raw
end try

begin catch
	set @mesaj = ERROR_MESSAGE() + ' (wOPSitTicheteChequeDejeunerExcel_p)'
