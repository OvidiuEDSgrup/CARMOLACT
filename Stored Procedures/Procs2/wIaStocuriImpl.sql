...
begin
	...
	if exists (select 1 from sysobjects where [type]='P' and [name]='wIaStocuriImplSP')
	begin 
		declare @returnValue int -- variabila salveaza return value de la procedura specifica
		exec @returnValue = wIaStocuriImplSP @sesiune, @parXML output
		return @returnValue
	end 


	declare @userASiS varchar(20), @tip varchar(2),@fcod_gestiune varchar(13),@ftip_gestiune varchar(13),@data_jos datetime,@data_sus datetime,
		@an_impl int,@luna_impl int,@mod_impl int,@data_implementare datetime,@data_lunii datetime,@cod_gestiune varchar(13),@tip_gestiune varchar(1),@lista_gestiuni int
	...
	from istoricstocuri a
			...
			left outer join @GestiuniUser gu on gu.valoare=a.Cod_gestiune
	where ((g.Denumire_gestiune like '%'+@fcod_gestiune+'%' or (p.Nume like '%'+@fcod_gestiune+'%' and @tip='OF')) or a.Cod_gestiune like @fcod_gestiune+'%' or isnull(@fcod_gestiune,'')='')
		...
		and (a.Tip_gestiune='F' or @lista_gestiuni=0 or gu.valoare is not null)
