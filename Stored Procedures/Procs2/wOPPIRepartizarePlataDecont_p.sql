...
BEGIN TRY
	...
	if object_id('tempdb..#pozDeSters') is not null 
		drop table #pozDeSters
	if object_id('tempdb..#DecPeComponente') is not null 
		drop table #DecPeComponente
	if object_id('tempdb..#tmpComponente') is not null 
		drop table #tmpComponente

	-->	Verificam daca la nivelul decontului (marca+decont), exista mai multe componente (loc de munca, cont, indicator, valuta). Daca nu exista, atunci nu trebuie sa afisam acele pozitii in macheta de repartizare.
	select marca, decont, loc_de_munca, valuta, cont, indicator into #tmpComponente
	from #repdecont
	group by marca, decont, loc_de_munca, valuta, cont, indicator

	select marca, decont into #DecPeComponente
	from #tmpComponente
	group by marca, decont
	having count(1)=1

	select * into #pozDeSters 
	from (select * from #repdecont rp 
		where exists (select 1 from #DecPeComponente dc where dc.marca=rp.marca and dc.decont=rp.decont)) a
	...
	-->	Stergem acele pozitii care la nivel de marca+decont nu au mai multe componente.
	delete rp from #repdecont rp inner join #pozDeSters ds on ds.idPozitieDoc=rp.idPozitieDoc
	where rp.nrp<999
