...
begin
	...
	@data datetime, @dataj datetime, @datas datetime, @lPremiu_la_avans int, @SubTipCorectii int, @CorectiiNete int, @DateSalGrpPeLMNivel1 int, @NivelGrupare int, @cautare varchar(500)

	set @lPremiu_la_avans=dbo.iauParL('PS','PREMAVANS')
	set @SubTipCorectii=dbo.iauParL('PS','SUBTIPCOR')

	select @DateSalGrpPeLMNivel1=max(case when parametru='DSLGLMNIV' then Val_logica else 0 end)
		,@NivelGrupare=max(case when parametru='DSLGLMNIV' then Val_numerica else 0 end)
	from par where tip_parametru in ('PS') and parametru in ('DSLGLMNIV')
	set @NivelGrupare=isnull(nullif(@NivelGrupare,0),9)
	...
	Null as procent, Null as progravans, Null as progrlich, Null as retinutlich, Null as valtotala, Null as valretinuta, 
	...
	Null as orenoapte,  Null as orerealizate, Null as realizat, Null as coefacord, Null as salcatl, Null as oredetasare, Null as oredelegatii, Null as orelucrate, Null as oreregimnormal, 
	...
	Null as nrtichetecuv, Null as nrtichetesupl, Null as nrtichetestoc, Null as nrticheteret, Null as nrticheteprimite, Null as detalii, 
	...
	where (@tip='SL' and a.marca=xA.row.value('@marca','varchar(6)') or @tip='AV' and p.loc_de_munca like rtrim(@lmantet)+(case when @DateSalGrpPeLMNivel1=1 then '%' else '' end) 
	...
	a.salcatl as salcatl, a.oredetasare, a.oredelegatii, a.orelucrate, a.oreregimnormal,
	...
	procent as procent, Null as progravans, Null as progrlich, Null as retinutlich, Null as valtotala, Null as valretinuta, datainceput, orainceput, datasfarsit, orasfarsit,
	...
	Null as tipcor, Null as dentipcor, Null as codbenef, Null as denbenef, 
	Null as tiptichet, Null as serieinceput, Null as seriesfarsit, Null as nrtichete, Null as valtichet, Null as valtichete, 
	Null as nrtichetecuv, Null as nrtichetesupl, Null as nrtichetestoc, Null as nrticheteret, Null as nrticheteprimite, detalii as detalii, 
	...
	else 'Corectii ' end)+rtrim(isnull(stc.denumire,tc.Denumire)) as denumire, 
	'' as nrdoc, rtrim(c.Tip_corectie_venit) as codac, rtrim(isnull(stc.denumire,tc.Denumire)) as denac, 
	...
	from corectii c
		...
		left outer join subtipcor stc on @SubTipCorectii=1 and stc.subtip=c.Tip_corectie_venit
	...
	Null as sumaneta, Null as sumacorectie, Null as procentcorectie, Null as tipachitare, Null as dentipachitare, Null as sumaachitata, 
	convert(decimal(12,2),r.Procent_progr_la_lichidare) as procent, convert(decimal(12,2),r.Retinere_progr_la_avans) as progravans, convert(decimal(12,2),r.Retinere_progr_la_lichidare) as progrlich, 
	...
	Null as tiptichet, Null as serieinceput, Null as seriesfarsit, Null as nrtichete, Null as valtichet, Null as valtichete, 
	Null as nrtichetecuv, Null as nrtichetesupl, Null as nrtichetestoc, Null as nrticheteret, Null as nrticheteprimite, r.detalii as detalii, 
	...
	from tichete t 
		...
		, @parXML.nodes('row') as xA(row)
	where (@tip='SL' and t.marca=xA.row.value('@marca','varchar(6)') or @tip='TM' and p.loc_de_munca like rtrim(@lmantet)+(case when @DateSalGrpPeLMNivel1=1 then '%' else '' end) 
		and (t.Marca like @cautare+'%' or p.Nume like '%'+@cautare+'%'))
		and t.data_lunii between @dataj and @datas
	union all
	select convert(char(10),t.data_lunii,101) as data, rtrim(t.marca) as marca, rtrim(max(p.nume)) as densalariat, 
	14 as numarpozitie, @tip as tip, @tip as subtip, 
	'Tichete' as denumire, '' as nrdoc, rtrim(max(p.Loc_de_munca)) as codac, 'Tichete' as explicatii,
	convert(decimal(12,2),sum(t.Nr_tichete)) as cantitate, convert(decimal(12,2),sum(t.Nr_tichete*t.Valoare_tichet)) as valoare, 
	Null as oreavans, Null as sumaavans, Null as premiuavans, 
	Null as sumaneta, Null as sumacorectie, Null as procentcorectie, Null as tipachitare, Null as dentipachitare, Null as sumaachitata, 
	Null as procent, Null as progravans, Null as progrlich, Null as retinutlich, Null as valtotala, Null as valretinuta, 
	Null as datainceput, Null as orainceput, Null as datasfarsit, Null as orasfarsit, Null as tipconcediu, Null as denconcediu, 
	...
	Null as nravizme, Null as mediazilnica, Null as bazastagiu, Null as zilestagiu, Null as calculmanual, 
	Null as zileco, Null as indemnizatieco, Null as indnetaco, Null as dataop, Null as zileca, Null as oreca, 
	Null as nrcrt, rtrim(max(p.loc_de_munca)) as lm, rtrim(max(lm.denumire)) as denlm, Null as comanda, Null as dencomanda, 
	Null as tipsal, Null as oreregie, Null as oreacord, Null as oresupl1, Null as oresupl2, Null as oresupl3, Null as oresupl4, Null as orespor100, 
	Null as orenoapte,  Null as orerealizate, Null as realizat, Null as coefacord, Null as salcatl, Null as oredetasare, Null as oredelegatii, Null as orelucrate, Null as oreregimnormal, 
	Null as oreco, Null as orecm, Null as oreintr1, Null as oreintr2, Null as oreobligatii, Null as oreinvoiri, Null as orenemotivate, Null as orecfs, Null as orenelucrate, 
	Null as sporspecific, Null as orepesteprogr, Null as sppesteprogr, Null as ore1, Null as sp1, Null as ore2, Null as sp2, 
	Null as ore3, Null as sp3, Null as ore4, Null as sp4, Null as ore5, Null as sp5, Null as ore6, Null as sp6, Null as sp7, Null as sp8, 
	Null as tipcor, Null as dentipcor, Null as codbenef, Null as denbenef, 
	null as tiptichet, null as serieinceput, null as seriesfarsit, 
	sum(convert(decimal(12,2),t.Nr_tichete*(case when t.Tip_operatie in ('C','P','S') then 1 else -1 end))) as nrtichete, convert(decimal(12,2),max(t.Valoare_tichet)) as valtichet, 
	...
	sum(convert(decimal(12),t.Nr_tichete*(case when t.Tip_operatie in ('C','P','S') then 1 else -1 end))) as nrticheteprimite, Null as detalii, 
	rtrim((case when 1=0 then t.marca else max(p.nume) end)) as Ordonare, 
	'#000000' as culoare 
	from tichete t 
		left outer join personal p on p.marca=t.marca
		left outer join lm lm on lm.cod=p.Loc_de_munca
		, @parXML.nodes('row') as xA(row)
	where @tip='TC' and p.loc_de_munca like rtrim(@lmantet)+(case when @DateSalGrpPeLMNivel1=1 then '%' else '' end) 
		and (t.Marca like @cautare+'%' or p.Nume like '%'+@cautare+'%') and t.data_lunii between @dataj and @datas
