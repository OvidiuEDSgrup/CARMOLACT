...
create procedure [dbo].[wIaConturi] @sesiune varchar(50), @parXML XML
	as
	declare 
	...
	select 
		...
		,@filtruContParinte = isnull(@parXML.value('(/row/@contparinte)[1]', 'varchar(40)'), '')
	...
	select top 100 
		...
		(CASE WHEN GETDATE() BETWEEN c.detalii.value('(/row/@data_invalid_jos)[1]', 'datetime')
			AND c.detalii.value('(/row/@data_invalid_sus)[1]', 'datetime') THEN '#C0C0C0' END) AS culoare
	from conturi c
	...
	where 
		...
		and cont_parinte like rtrim(@filtruContParinte)+'%' 
