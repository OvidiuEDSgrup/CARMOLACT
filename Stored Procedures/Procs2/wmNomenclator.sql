...
declare @FltStocPred int, @searchText varchar(80), @subunitate varchar(9), @tip varchar(2), @gestiune varchar(20),@gestutiliz varchar(20), @categoriePret int,
		...
		@grupa varchar(20), @lista_gestiuni int,@tert varchar(20),@grupapenivele int,@contract varchar(20), @tipdetalii varchar(50), @meniuDetalii varchar(50)
...
select c.cod as cod, rtrim(max(nomencl.denumire)) as denumire,
	...
	+ltrim(CONVERT(varchar(20), max(convert(decimal(15, 2), 
		isnull((case when @pretCuAmanuntul=1 then p.pret_amanunt_discountat else p.Pret_vanzare_discountat end),0)))))+' lei / '+rtrim(max(nomencl.um))  as info,  
		
	max(convert(decimal(15, 2), isnull((case when @pretCuAmanuntul=1 then p.pret_amanunt else p.Pret_vanzare end),0))) as pret,
	1 as cantitate,
	(case when c.nrap=-1 then '0xd3d3d3' when c.nrap>0 then '0x00ff00' else '0xffffff' end) as culoare
...
group by c.cod,nomencl.Denumire,nomencl.tip,c.nrap
...
if len(@procDetalii)=0 --Probabil e apelat din meniu
	...
	select @procDetalii as detalii,1 as areSearch, 'Alege produs' as titlu,1 as _neimportant,
