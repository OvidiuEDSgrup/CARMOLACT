...
DECLARE @userASiS VARCHAR(10), @lista_lm BIT, @lista_conturi BIT, @DecGrCont INT, @CtAvFurn VARCHAR(40), @CtAvBen VARCHAR(40), @subunitate CHAR(9), @Bugetari INT, 
	...
	@numere_pozitii VARCHAR(max), @areDetalii BIT, @tipefect varchar(1), @lista_jurnale bit, @jurnal varchar(20)
...
BEGIN
	...
	SELECT rtrim(p.subunitate) AS subunitate, @tip AS tip, rtrim(p.cont) AS cont, rtrim(isnull(ca.denumire_cont, '')) AS dencont, 
		...
		convert(DECIMAL(10, 4), p.curs_la_valuta_facturii) AS cursvalutafact, 
		...
		convert(DECIMAL(15, 2), p.Achit_fact) AS achitfact, 
...
/*		isnull(isnull(convert(CHAR(10), e.Data_document, 101), convert(CHAR(10), p.data, 101)), '01/01/1901') AS ext_datadocument, 
	...
	WHERE p.subunitate = @subunitate
		AND (
			...
			OR ISNULL(@cautare, '') = ''
			OR ISNUMERIC(@cautare)=1 AND RTRIM(LTRIM(STR(p.Suma,15,2)))=RTRIM(LTRIM(STR(@cautare,15,2)))
			)
		...
		AND (
			...
			OR @tip = 'DE'
				AND p.marca = @marca
				AND (CASE WHEN @DecGrCont = 1 THEN p.cont+ltrim(p.valuta) ELSE p.decont END) = @decont)
			OR @tip = 'EF'
				AND p.tert = @tert
		...
		AND
		 ( ISNULL(@lista_jurnale,0) = 0 OR EXISTS (select 1 from PropUtiliz pu where pu.utilizator=@userASIS and pu.proprietate='JURNAL' and pu.valoare=p.jurnal))
