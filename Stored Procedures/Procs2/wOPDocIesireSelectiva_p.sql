...
BEGIN TRY
	...
	SELECT ROW_NUMBER() OVER (ORDER BY p.Data) AS nr,
		...
		RTRIM(s.Cod_intrare) AS cod_intrare, CONVERT(decimal(17,5), s.Pret) AS pret, CONVERT(decimal(15,3), s.Stoc) AS stoc,
	...
	SELECT (   
		SELECT
			...
			CONVERT(decimal(17,3), @cantitate) AS cantitateFixaPoz,
