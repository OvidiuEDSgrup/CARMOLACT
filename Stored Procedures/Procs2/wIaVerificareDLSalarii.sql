...
begin try
	...
	declare @tip varchar(2), @dataJos datetime, @dataSus datetime, @dataJosNext datetime, @dataSusNext datetime, 
		@utilizator varchar(20), @sub varchar(9), @tipValidare varchar(100), 
...
--	citire date din xml
	...
	select @dataJosNext=dbo.BOM(DateADD(month,1,@dataSus)), @dataSusNext=dbo.EOM(DateADD(month,1,@dataSus))
	...
	create table #necorelatii(
		...
		Continut char(50) not null,
		Explicatii char(500) not null,
...
--	validare marci eronate
	...
	union all
	select 'CE', 'Retineri', 'Marca', r.data, r.marca, r.marca, 'Marca inexistenta!', isnull(i.Loc_de_munca,'') as lm, 0, 0
	from resal r 
...
--	pun in tabela temporara doar salariatii pt. care trebuie validat CNP-ul (nu e cazul la salariatii de Nationalitate straina, decat cei cu Carte de rezidenta).
	...
	where isnull(nat.Val_inf,'')='' or isnull(nat.Val_inf,'')='România' or isnull(ta.Val_inf,'')='CarteDeRezidenta'
...
--	validare comenzi
	insert into #necorelatii
	select 'CE', 'Realcom', 'Comanda', r.data, r.marca, r.Comanda, 'Comanda inexistenta!', r.Loc_de_munca as lm, 0, 0 
	from realcom r 
		left outer join istpers i on r.data=i.data and r.marca=i.marca
	...
	union all
	select distinct 'CE', 'Realcom', 'Comanda', r.data, '', r.Comanda, 'Comanda inexistenta!', r.Loc_de_munca as lm, 0, 0 
	from realcom r 
...
--	validare rest de plata din net, cu valori negative
	insert into #necorelatii
	select distinct 'RN', 'Net' as tabela, 'Rest de plata' as camp, n.data as data, n.marca as marca, max(n.Rest_de_plata) as continut, 
		'Rest de plata < 0!' as explicatii, n.Loc_de_munca as lm, 0, 0
	from net n
		left outer join istpers i on n.marca=i.marca and n.data=i.data
	where n.data between @dataJos and @dataSus and n.Rest_de_plata<0
	group by n.data, n.marca, n.loc_de_munca
...
--	pana la 18 ani poti fi trecuti in intretinere la ambii parinti
	union all 
	select 'PI', 'Persintr', 'Participare la deducere' as camp, p.data, p.marca, p.nume_pren as continut, 
		'Pers. in intretinere va implini 18 ani luna viitoare. De stabilit la care parinte va ramine in intretinere', 
		isnull(i.loc_de_munca,''), 0 as valoare_1, 0 as valoare_2
	from persintr p
	...
	where p.data between @datajos and @dataSus and p.tip_intretinut in ('C','U')
		and DateADD(year,18,p.data_nasterii) between @dataJosNext and @dataSusNext
...
--	returnare date
	...
	(select (case when Tip_validare='CE' then 'Coduri eronate' when Tip_validare='SI' then 'Date eronate pt. salariati inactivi' 
		...
		when Tip_validare='RN' then 'Rest de plata negativ' when Tip_validare='PE' then 'Pontati eronat' 
		when Tip_validare='PI' then 'Persoane in intretinere' end) as validare, 
	...
	where (dbo.f_areLMFiltru(@utilizator)=0 or lu.cod is not null)	
		...
		and (@filtruValidare='' 
			or (case when Tip_validare='CE' then 'Coduri eronate' when Tip_validare='SI' then 'Date eronate pt. salariati inactivi' 
				...
				when Tip_validare='NP' then 'Necorelatie pontaj-concedii' when Tip_validare='SN' then 'Salariati nepontati' 
				when Tip_validare='RN' then 'Rest de plata negativ' when Tip_validare='PE' then 'Pontati eronat' 
				when Tip_validare='PI' then 'Persoane in intretinere' end) like '%'+REPLACE(@filtruValidare,' ','%')+'%')
