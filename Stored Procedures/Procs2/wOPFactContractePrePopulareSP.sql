...
CREATE PROCEDURE wOPFactContractePrePopulareSP @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	...
	(
		select convert(char(10), @dataJos, 101) as datajos, convert(char(10), @dataSus, 101) as datasus,
			@valuta as valuta, @tipContract as tip, @idContractFiltrat as idContract
			, @tert as tert, @punct_livrare as punct_livrare,
			@lm as lm, @gestiune as gestiune
		for xml raw ,type--root('row')
	)
	
	SELECT 'Operatie pentru generare facturi contracte'  nume, 'CL' codmeniu, 'D' tipmacheta,'CL' tip,'FC' subtip,'O' fel,
	 (SELECT @dateInitializare ) dateInitializare FOR XML RAW('deschideMacheta'), ROOT('Mesaje')

END TRY
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (wOPFactContractePrePopulareSP)'
