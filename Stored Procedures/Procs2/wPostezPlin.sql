...
as
	...
	**/
	declare 
		@sb varchar(9), @tip varchar(2), @numar varchar(40), @data datetime, @parXMLinreg xml 
	...
	set @parXMLinreg=(select 2 tabela for xml raw)
	if exists (select 1 from DocDeContat where Subunitate=@sb and Tip='PI' and Numar=@numar and Data=@data)
		exec faInregistrariContabile @dinTabela=0, @Subunitate=@sb,@Tip='PI',@Numar=@numar, @Data=@data, @parXML=@parXMLinreg
