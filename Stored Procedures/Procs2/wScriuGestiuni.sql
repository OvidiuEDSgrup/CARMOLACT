...
begin try
	select isnull(ptupdate, 0) as ptupdate, upper(gestiune) as gestiune, upper(isnull(gestiune_veche, gestiune)) as gestiune_veche, 
		...
		isnull(categpret, '') as categpret, isnull(pret_am, 0) as pret_am, detalii
	into #xmlgestiuni
	...
	insert gestiuni
	(Subunitate, Tip_gestiune, Cod_gestiune, Denumire_gestiune, Cont_contabil_specific, detalii, pret_am)
	select @Sub, x.tip_gestiune, x.gestiune, isnull(x.denumire, space(30)) + isnull(x.tert, ''), isnull(x.cont, ''), x.detalii, x.pret_am
	from #xmlgestiuni x
	where x.ptupdate=0
	...
	set cod_gestiune=isnull(x.gestiune, g.cod_gestiune), tip_gestiune=isnull(x.tip_gestiune, g.tip_gestiune), 
		...
		cont_contabil_specific=isnull(x.cont, g.cont_contabil_specific), g.detalii=x.detalii, g.pret_am = x.pret_am
