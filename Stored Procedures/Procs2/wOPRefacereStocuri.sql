...
@PretMediu int, @nExcepPM int, @cGestExcepPM char(1000), @GestFaraPM int, -- o singura gestiune, fara pret mediu
@recalcpretmediu int, @exceptiefolpretmediu int--, @userASiS varchar(10)
...
set @GestFaraPM=(case when @gest<>'' and sign(charindex(','+rtrim(@gest)+',',@cGestExcepPM))!=sign(@nExcepPM) then 1 else 0 end)
...
begin try
	...
	if @GestFaraPM=0 and @inlocpreturi=0 and @PretMediu=1 and @recalcpretmediu=0
	...
	if @cod='' set @cod=null
	set @PretMediu=(case when @GestFaraPM=1 then 0 else @PretMediu end) -- daca o singura gestiune si fara pret mediu sa nu intre in procedura respectiva
