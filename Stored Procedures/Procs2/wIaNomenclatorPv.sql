...
begin try
	...
	insert @nomencl(nr, cod, denumire, um)
	select top(@nrElemente) nr, rtrim(n.cod), convert(varchar(30), nr)+' '+rtrim(n.Denumire), rtrim(n.UM)
	from
		(select n.cod as cod, n.Denumire, n.um, 
				ROW_NUMBER() over(order by (n.denumire)) nr
			from nomencl n
			--inner join grupe on n.grupa=grupe.grupa and (@grupa='' or grupe.grupa = @grupa)
