...
begin try
	...
	select @tip=ISNULL(@parXML.value('(/*/@tip)[1]', 'varchar(2)'), ''),
		...
		@idpozdoc = coalesce(@parXML.value('(/row/docInserate/row/@idPozDoc)[1]', 'int'), @parXML.value('(/row/row/@idpozdoc)[1]', 'int'),0), 
	...
	begin
		...
		declare @idpozdoc_xml xml
		select @idpozdoc_xml=(select @idpozdoc idpozdoc FOR XML raw )
	...
	begin
		if (select detalii from pozdoc where idPozDoc=@idpozdoc) is null
			update pozdoc set detalii='<row />'  where idPozDoc=@idpozdoc
		--@nrluni
		update pozdoc set detalii.modify ('insert attribute nrluni {sql:variable("@nrluni")} into (/row)[1]') where idPozDoc=@idpozdoc and detalii.value('(/row/@nrluni)[1]', 'float') is null
		update pozdoc set detalii.modify ('replace value of (/row/@nrluni)[1] with sql:variable("@nrluni")')  where idPozDoc=@idpozdoc and detalii.value('(/row/@nrluni)[1]', 'float') is not null
		--@datainceput  
		update pozdoc set detalii.modify ('insert attribute datainceput {sql:variable("@datainceput")} into (/row)[1]') where idPozDoc=@idpozdoc and detalii.value('(/row/@datainceput)[1]', 'datetime') is null
		update pozdoc set detalii.modify ('replace value of (/row/@datainceput)[1] with sql:variable("@datainceput")')  where idPozDoc=@idpozdoc and detalii.value('(/row/@datainceput)[1]', 'datetime') is not null
		--@contavans
		update pozdoc set detalii.modify ('insert attribute contavans {sql:variable("@contavans")} into (/row)[1]') where idPozDoc=@idpozdoc and detalii.value('(/row/@contavans)[1]', 'varchar(40)') is null
		update pozdoc set detalii.modify ('replace value of (/row/@contavans)[1] with sql:variable("@contavans")')  where idPozDoc=@idpozdoc and detalii.value('(/row/@contavans)[1]', 'varchar(40)') is not null
		--
		IF @fara_mesaje<>1
