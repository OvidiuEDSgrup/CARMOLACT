--***
Create procedure wScriuBenret @sesiune varchar(50), @parXML xml
as 
...
begin try  
	select isnull(ptupdate, 0) as ptupdate, cod, isnull(cod_vechi, cod) as cod_vechi, 
		...
		contdebitor, contcreditor, analiticmarca, detalii
	into #xmlbenret
	...
	insert into benret 
		(Cod_beneficiar, Tip_retinere, Denumire_beneficiar, Obiect_retinere, Cod_fiscal, Banca, Cont_banca, Permane, Cont_debitor, Cont_creditor, Analitic_marca, detalii)
	select x.cod, isnull(x.tipret,''), isnull(x.denumire,''), isnull(x.obiectret,''),
		...
		isnull(x.analiticmarca,0), x.detalii
	from #xmlbenret x
	where x.ptupdate=0
