...
'	/*	'''' xmlu e locul in care se va repeta @recursiva, cu replace
	...
	*/
	...
	if object_id('tempdb..#configurarimachete') is not null	--> tabela de extragere configurari - pt stergere meniuri de exemplu
	begin
		select @iterativa='
			if object_id(''tempdb..#configurarimachete'') is null create table #configurarimachete(p xml)
			insert into #configurarimachete(p)
			select (
			'+@iterativa+'
			)'
	end
