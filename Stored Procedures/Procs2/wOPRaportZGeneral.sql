...
begin try  
	...
	select bp.idantetbon,(case when n.tip='S' then n.cont else '707' end) as cont,sum(total) as total
	into #vanzari
	from bp
	...
	group by idantetbon,(case when n.tip='S' then n.cont else '707' end)
	...
	create table #cross (idantetbon int,cont varchar(20),cont_casa varchar(20),suma float)

	insert into #cross
	select v.idantetbon,v.cont,i.cont_casa,v.total/v.totalg*i.valinc as suma
	from #vanzari v
	inner join #incasari i on v.IdAntetBon=i.idantetbon

	select ab.data_bon as data,ab.gestiune,ab.loc_de_munca,cr.cont_casa,cr.cont,sum(cr.suma) as total
	into #descris
	from #cross cr
	inner join antetbonuri ab on cr.idantetbon=ab.idantetbon
	group by ab.data_bon,ab.gestiune,ab.loc_de_munca,cr.cont_casa,cr.cont
	order by ab.data_bon,ab.gestiune,ab.loc_de_munca,cr.cont_casa,cr.cont
	...
	--Fiind vorba de o operatie cu multe linii vom insera direct date in PozPlin
		  ...
		  sc.cont as cont_corespondent,sc.total,'', 0, 0, 0, 24.00,round(sc.total*(1.00-1.00/(1.00+24.00/100)),2),'Vanzari PVRIA',isnull(sc.loc_de_munca,gc.loc_de_munca),
