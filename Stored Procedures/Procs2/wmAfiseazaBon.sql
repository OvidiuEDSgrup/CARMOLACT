...
LTRIM(str(bp.Cantitate))+' '+RTRIM(n.UM)+' x '+LTRIM(str(bp.Pret))+' lei = '+
ltrim(convert(varchar(20),convert(money,(1-Factura_chitanta)*convert(decimal(15,2),bp.tva)+convert(decimal(15,2), bp.Total*case when Factura_chitanta=1 then 1 else (100-Discount)/100 end)),1))+
	' lei'
	 as info
from bp
