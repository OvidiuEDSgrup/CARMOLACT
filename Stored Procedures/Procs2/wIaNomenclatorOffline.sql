...
begin try
	...
	select rtrim(preturi.Cod_produs), rtrim(preturi.UM) , convert(varchar(100),convert(decimal(12,5),min(preturi.Pret_vanzare))), 
		convert(varchar(100),convert(decimal(12,5),min(preturi.Pret_cu_amanuntul)))
		from preturi 
		inner join 
	...
	select 
		...
		RTRIM(n.Tip) as "@tip",
		RTRIM(n.UM) as "@um",
		convert(decimal(12,2), n.cota_tva) as "@cota_tva",
