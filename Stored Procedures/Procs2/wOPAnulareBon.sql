...
declare @numarbon varchar(20),@data datetime, @factura varchar(20),@tert varchar(20),@tip varchar(2), @valoare float,@casamarcat varchar(10),
		...
		@inputAnulareAP xml, @inputAnulareAC xml, @inputAnulareTE xml, @eroare varchar(2000), @idAntetBon int, @eBon bit
...
begin try
	...
	if @tip='BY' -- factura
	begin
		set @inputAnulareAP=(select 'AP' as tip, @factura as numar, @data as datafacturii, 1 as faraMesaj for xml raw('parametri'))
	...
	begin
		...
		begin
			...
			set @inputAnulareAC = (select 'AC' as tip, @nrdoc as numar, @data as datafacturii, 1 as faraMesaj for xml raw('parametri'))
			exec wOPAnulareDoc @sesiune=@sesiune, @parXML=@inputAnulareAC
			
			if @NuTEAC=0
			begin
				set @NrTE=@NrDoc
  				set @inputAnulareTE = (select 'TE' as tip, @nrTE as numar, @data as datafacturii, 1 as faraMesaj for xml raw ('parametri'))
