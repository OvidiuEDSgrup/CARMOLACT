...
select rtrim(p.tert) as tert,isnull(rtrim(t.denumire), '') as dentert,rtrim(p.factura) as factura, RTRIM(p.Cont_factura) as contfactura,
	...
	convert(decimal(12,0),p.cantitate) as cantitate, RTRIM(p.Utilizator) AS utilizator, CONVERT(char(10), p.Data_operarii, 101) AS data_operarii,
	convert(decimal(17,2), round(convert(decimal(18,5), p.cantitate * p.pret_valuta), 2)
			+ convert(decimal(17,2), p.TVA_deductibil/(case when p.valuta = '' or p.curs = 0 then 1 else p.curs end))) AS valtotala
into #pozPrestari
from pozdoc p  
