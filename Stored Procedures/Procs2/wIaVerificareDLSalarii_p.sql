...
begin try  
	...
	SELECT convert(char(10),@dataJos,101) AS datajos, convert(char(10),@datasus,101) AS datasus, @tipvalidare AS tipvalidare, 
		(case when @tipvalidare='TV' then 'Toate' 
			...
			when @tipvalidare='RN' then 'Rest de plata negativ' when @tipvalidare='PE' then 'Pontati eronat' 
			when @tipvalidare='PI' then 'Persoane in intretinere' end) as dentipvalidare
