CREATE TABLE [dbo].[pozadoc] (
    ...
    FOREIGN KEY ([idplaja]) REFERENCES [dbo].[docfiscale] ([Id])
);
...
GO
CREATE NONCLUSTERED INDEX [missing_index_416]
    ON [dbo].[pozadoc]([Subunitate] ASC, [Tert] ASC, [Tip] ASC, [Data] ASC);


GO
--***
create trigger adocctatr on pozadoc for insert, update
as
if (
(select count(cont_deb) from inserted i inner join conturi c on i.cont_deb=c.cont 
where i.tip = 'FF' and sold_credit<>0)>0)
begin
RAISERROR ('Contul debitor al FF trebuie sa fie neatribuit!', 16, 1)
rollback transaction 
end
if (
(select count(cont_deb) from inserted i inner join conturi c on i.cont_deb=c.cont 
where i.tip in ('CO','CF','C3','SF') and sold_credit<>1)>0)
begin
RAISERROR ('Contul debitor trebuie sa fie atribuit furnizori', 16, 1)
rollback transaction 
end
if (
(select count(cont_cred) from inserted i inner join conturi c on i.cont_cred=c.cont 
where i.tip in ('CF','FF','SF') and sold_credit<>1)>0)
begin
RAISERROR ('Contul creditor trebuie sa fie atribuit furnizori', 16, 1)
rollback transaction 
end
if (
(select count(cont_deb) from inserted i inner join conturi c on i.cont_deb=c.cont 
where i.tip in ('CB','FB','IF') and sold_credit<>2)>0)
begin
RAISERROR ('Contul debitor trebuie sa fie atribuit beneficiari', 16, 1)
rollback transaction 
end
if (
(select count(cont_cred) from inserted i inner join conturi c on i.cont_cred=c.cont 
where i.tip in ('CO','CB','C3','IF') and sold_credit<>2)>0)
begin
RAISERROR ('Contul creditor trebuie sa fie atribuit beneficiari', 16, 1)
rollback transaction 
end
if (
(select count(cont_cred) from inserted i inner join conturi c on i.cont_cred=c.cont 
where i.tip = 'FB' and sold_credit<>0)>0)
begin
RAISERROR ('Contul creditor al FB trebuie sa fie neatribuit!', 16, 1)
rollback transaction 
end
...
IF ABS(@val)>=0.01 or ABS(@vtva)>=0.01 or ABS(@vtva9)>=0.01
begin
	IF OBJECT_ID('tempdb.dbo.#facturi') IS NOT NULL
		DROP TABLE #facturi	
	
	SELECT distinct tert, factura_dreapta factura, data_fact data, 'F' tip 
		INTO #facturi
...
data=(case when @gdataf>'01/01/1901' and (@gtip in ('SF') or @gdataf<data) then @gdataf else data end),
...
begin try
	...
	BEGIN
		create table #formulecontabile (cont_credit varchar(40), cont_debit varchar(40), tip varchar(2), numar varchar(40), data datetime)
	...
	select DISTINCT data, loc_munca from inserted
	union all
	select DISTINCT data, loc_munca from deleted
