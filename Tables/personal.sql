CREATE TABLE [dbo].[personal] (
    ...
    [tip_stat]                      VARCHAR (25) NULL,
    [vechime_la_intrare]            VARCHAR (6)  NULL,
    [Spor_cond_7]                   FLOAT (53)   NULL,
    [Spor_cond_8]                   FLOAT (53)   NULL,
    [Spor_cond_9]                   FLOAT (53)   NULL,
    [Spor_cond_10]                  FLOAT (53)   NULL
);
...
GO
CREATE NONCLUSTERED INDEX [missing_index_174]
    ON [dbo].[personal]([Activitate] ASC);


GO
--***
create  trigger tr_validPersonal on personal for insert, update NOT FOR REPLICATION as
DECLARE @nrRanduri int, @mesaj varchar(1000), @categsal int, @NCAnActiv int, @Somesana int, @ExceptieCONeefectuat int
SET @nrRanduri=@@ROWCOUNT
set @Somesana=dbo.iauParL('SP','SOMESANA')
set @ExceptieCONeefectuat=dbo.iauParL('PS','EXVCONEEF')
IF @nrRanduri=0 
	RETURN
...
begin try
	...
	begin
		...
		begin
			select top 1 @mesaj='Eroare operare: Cod functie '+rtrim(i.cod_functie)+' neintrodus sau inexistent in catalogul de functii (marca:'+rtrim(i.marca)+'!'
			from inserted i left join functii f on i.Cod_functie=f.Cod_functie
			where f.cod_functie is null
