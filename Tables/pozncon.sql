...
GO
CREATE NONCLUSTERED INDEX [missing_index_923]
    ON [dbo].[pozncon]([Subunitate] ASC, [Tip] ASC, [Data] ASC, [Loc_munca] ASC)
    INCLUDE([Numar], [Nr_pozitie]);


GO
CREATE NONCLUSTERED INDEX [missing_index_82]
    ON [dbo].[pozncon]([Tip] ASC, [Numar] ASC, [Data] ASC)
    INCLUDE([Nr_pozitie]);


GO
--***
create trigger nconctatr on pozncon for insert, update
as
if (
	(select count(cont_debitor) from inserted i inner join conturi c on i.cont_debitor=c.cont 
	where i.tip in ('NC'/*,'PS'*/) and sold_credit<>0)>0)
begin
	RAISERROR ('Contul debitor trebuie sa fie neatribuit! (pozncon.nconctatr)', 16, 1)
	rollback transaction 
end
if (
	(select count(cont_creditor) from inserted i inner join conturi c on i.cont_creditor=c.cont 
	where i.tip in ('NC'/*,'PS'*/) and (sold_credit<>0 and Sold_credit<>6))>0)
begin
	RAISERROR ('Contul creditor trebuie sa fie neatribuit! (pozncon.nconctatr)', 16, 1)
	rollback transaction 
end
/*if (
(select count(cont_creditor) from inserted i inner join conturi c on i.cont_creditor=c.cont 
where i.tip = 'MA' and left(i.cont_creditor,2) in ('28','46') and sold_credit<>5)>0)
begin
RAISERROR ('Contul creditor trebuie sa fie atribuit 5-Amortizare MF! (pozncon.nconctatr)', 16, 1)
rollback transaction 
end
if (
(select count(cont_debitor) from inserted i inner join conturi c on i.cont_debitor=c.cont 
where i.tip = 'MA' and sold_credit<>0)>0)
begin
RAISERROR ('Contul debitor trebuie sa fie neatribuit! (pozncon.nconctatr)', 16, 1)
rollback transaction 
end*/
...
begin try
	declare 
		@rulajelm int
	set @rulajelm=0 --Valoarea implicita

	select top 1 @rulajelm=val_numerica from par where tip_parametru='GE' and parametru = 'RULAJELM'
	...
	begin
		...
		select distinct @utilizator,loc_munca,data from inserted	
			where (@rulajelm=1 or left(cont_debitor,1) not in ('8','9') and left(cont_creditor,1) not in ('8','9'))
		exec validLM
	end
