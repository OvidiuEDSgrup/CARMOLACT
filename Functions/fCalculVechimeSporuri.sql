...
--	set @dataJos=DATEADD(day,1, @dDataInch)
	...
	Select p.Marca, p.Loc_de_munca, p.Zile_concediu_de_odihna_an, p.Salar_lunar_de_baza, p.Data_angajarii_in_unitate, p.Loc_ramas_vacant, p.Data_plec, p.Tip_colab, 
		p.Vechime_totala, isnull(nullif(p.Vechime_la_intrare,''),ip.Vechime_la_intrare), isnull(p.detalii.value('(/row/@vechimemeserie)[1]','varchar(6)'),ip.Vechime_in_meserie), 
		p.Spor_vechime, p.Spor_specific, p.Spor_cond_8, isnull(s.Coef_invalid,0), 
...
--		determin vechimea in unitate functie de vechimea la intrare (din personal) si perioada de la data angajarii pana la zi.
...
--		determin procent spor vechime conform grilei si gradatia
		...
		if @Tip_colab in ('DAC','CCC','ECT') 
			set @vSporVechime=0
--		daca zile de CO se determina functie de vechimea in unitate sau daca [X]Vechimea la intrare se incrementeaza, campul Vechime_la_intrare din personal se incrementeaza la inchiderea de luna (+1)	
--		in acest caz vechimea in unitate pt. determinare spor specific sa nu mai tina cont de data angajarii (se tine cont de aceasta la inceput de while)
		if @ZileCOVechUnit=1 or @IncrementVechIntr=1
		Begin
--			determin vechimea in ani/luni pornind de la vechimea la intrare din personal (ca sa nu ma fac convert in datetime din vechime la intrare)
...
--		determin procent spor specific conform grilei
		...
		if @IncrementVechMeserie=1
		Begin
--			determin vechimea in ani/luni pornind de la vechimea in meserie din personal.detalii (ca sa nu ma fac convert in datetime din vechime in meserie)
...
/*
	select a.Marca, a.Vechime_totala as pVechime_totala, a.Vechime_la_intrare as pVechime_la_intrare, a.Spor_vechime as pSpor_vechime, a.Spor_specific as pSpor_specific,
		p.Vechime_totala, p.Spor_vechime, p.Spor_specific, isnull(p.Vechime_la_intrare,ip.Vechime_la_intrare) --into sporuri0911 
