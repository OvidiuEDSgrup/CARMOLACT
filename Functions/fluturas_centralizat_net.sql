...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	declare @perTich table (datalunii datetime, datajos datetime, datasus datetime)
	insert into @perTich
	...
	declare @tichete table (marca varchar(6), data_salar datetime, data datetime, numar_tichete int, valoare_tichete float)
	...
	Begin
			...
			select marca, @Datalunii, data, numar_tichete, valoare_tichete from dbo.fNC_tichete (@dataImpozJos, @dataImpozSus, isnull(@MarcaJ,''),1)
			fetch next from tmpTich into @Datalunii, @dataImpozJos, @dataImpozSus
	End
	...
	(case when a.Data>'06/30/2010' then max(isnull(ti.numar_tichete,0)) --isnull((select numar_tichete from fNC_tichete (dbo.iauParLD(a.Data,'PS','DJIMPZTIC'), dbo.iauParLD(a.Data,'PS','DSIMPZTIC'), a.Marca, 1)),0)
	...
	round((case when a.Data>'06/30/2010' then max(isnull(ti.Valoare_tichete,0)) --isnull((select Valoare_tichete from fNC_tichete (dbo.iauParLD(a.Data,'PS','DJIMPZTIC'), dbo.iauParLD(a.Data,'PS','DSIMPZTIC'), a.Marca, 1)),0)
	when not(@lOPTICHINM=1 or @lNC_tichete=1 and @cTabela='2') 
	...
	and (@Grupare in (/*'AN',*/'LUNA','MARCA') and dbo.eom(convert(datetime,substring(c.parametru,4,2)+'/01/'+substring(c.parametru,6,4),102))=a.data 
		or @Grupare in ('','AN') and dbo.eom(convert(datetime,substring(c.parametru,4,2)+'/01/'+substring(c.parametru,6,4),102)) between @dataJos and @dataSus)),0) end), 
	...
	where a.data between @dataJos and @dataSus and a.data=dbo.eom(a.data)
		...
		and (@lTipPers=0 or @cTipPers='N' and isnull(p.detalii.value('(/row/@functpublic)[1]','int'),0)=1 or @cTipPers='C' and isnull(p.detalii.value('(/row/@functpublic)[1]','int'),0)=0) 
		and (@lTipStat=0 or p.tip_stat=@cTipStat) 
...
----------*/
	...
	sum((case when a.somaj_5<>0 and not(@Buget=1 and convert(char(1),isnull(p.detalii.value('(/row/@functpublic)[1]','int'),0))='1') then a.asig_sanatate_din_cas else 0 end)),
	sum((case when a.somaj_5<>0 and @Buget=1 and convert(char(1),isnull(p.detalii.value('(/row/@functpublic)[1]','int'),0))='1' then a.asig_sanatate_din_cas else 0 end)),
...
/*		dbo.fluturas_centralizat_net1(@dataJos,@dataSus,@MarcaJ,@MarcaS,@LocmJ,@LocmS,@lGrupaM,@cGrupaM, @lTipSalarizare,@cTipSalJos,@cTipSalSus,@lTipPers,@cTipPers,
	...
	where a.data between @dataJos and @dataSus and a.data=dbo.eom(a.data)
		...
		and (@lTipStat=0 or p.tip_stat=@cTipStat) and (@lDreptCond=0 or (@AreDreptCond=1 and (@cListaCond='T' or @cListaCond='C' and p.pensie_suplimentara=1 
...
--	pentru contorizare salariati plecati si corelatie in nr. salariati la finalul lunii si nr. salariati la inceputul lunii urmatoare
	...
	where i.Data between @dataJos and @dataSus and p.Loc_ramas_vacant=1 and p.Data_plec=dbo.bom(i.Data)
		...
		and (@lSirMarci=0 or charindex(','+rtrim(ltrim(i.Marca))+',',@cSirMarci)>0) and (@lTipStat=0 or p.tip_stat=@cTipStat) 
		...
		and (@activitate is null or p.Activitate=@activitate) and i.grupa_de_munca<>'O'
