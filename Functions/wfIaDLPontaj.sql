...
returns @wIaDLPontaj table (Data varchar(10), Marca varchar(6), Nume varchar(50), numar_pozitie int, subtip varchar(20), tip varchar(20), denumire varchar(40), nrdoc varchar(10), codac varchar(2), 
	...
	realizat decimal(12,2), coefacord decimal(12,4), salcatl decimal(12,3), oredetasare int, oredelegatii int, orelucrate int, oreregimnormal int, 
...
Begin
	...
	select convert(char(10),p.Data,101) as data, p.marca as marca, ps.nume as nume, 
		...
		convert(decimal(12,2),p.ore_regie+p.ore_acord
			-(case when @OSNRN=1 then p.ore_suplimentare_1+p.ore_suplimentare_2+p.ore_suplimentare_3+p.ore_suplimentare_4 else (case when @ORegieFaraOS2=1 then p.Ore_suplimentare_2 else 0 end) end)+
			-(case when @O100NRN=1 then p.Ore_spor_100 else 0 end)) as oreregimnormal, 
	...
	select 
		...
		p.spor_conditii_1 as sp1, p.ore__cond_2 as ore2, p.spor_conditii_2 as sp2, p.ore__cond_3 as ore3, p.spor_conditii_3 as sp3, p.ore__cond_4 as ore4, 
		p.spor_conditii_4 as sp4, p.ore__cond_5 as ore5, p.spor_conditii_5 as sp5, p.ore_donare_sange as ore6, p.spor_conditii_6 as sp6, p.spor_cond_7 as sp7, p.spor_cond_8 as sp8,
		Null as nrtichete
	from 
	pontaj p
		left outer join realcom r on r.data=p.data and r.marca=p.marca and r.numar_document='PS'+rtrim(convert(char(10),p.numar_curent)) and r.loc_de_munca=p.loc_de_munca
