--***
Create
function fDeclaratia112 (@DataJ datetime, @DataS datetime, @oMarca int, @Marca char(6), @Lm char(9), @Strict int, @unSirMarci int, @SirMarci char(200), @TagAsigurat char(20))
...
begin
	...
	from pontaj a
		left outer join istpers i on i.Data=@DataS and i.Marca=a.Marca
	where a.data between @DataJ and @DataS and (@oMarca=0 or a.Marca=@Marca) and i.grupa_de_munca<>'O' 
	...
	from pontaj a
		...
		left outer join dbo.fDeclaratia112TagAsigurat (@DataJ, @DataS, @Lm) ta on dbo.eom(a.Data)=ta.data and a.marca=ta.marca
	...
	(case when @InstPubl=1 or n1.somaj_5=0 then 0 else (case when YEAR(a.Data)<=2011 or n1.CM_incasat=0 then isnull(n.Asig_sanatate_din_CAS,0) else n1.CM_incasat end) end) as Baza_FG, 
	ta.Regim_de_lucru
	from istpers a 
