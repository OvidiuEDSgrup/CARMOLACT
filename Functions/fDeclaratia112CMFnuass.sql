--***
Create
function fDeclaratia112CMFnuass (@DataJ datetime, @DataS datetime, @oMarca int, @Marca char(6), @Lm char(9), @Strict int)
...
--	creare cursor pt. verificare existenta CM pe acelasi CNP, alta marca si aceeasi perioada
	...
	where a.data_inceput between @DataJ and @DataS and (@oMarca=0 or a.marca=@Marca) 
...
--	grupare pe cnp si diagnostic
	...
	select a.Data, p.Cod_numeric_personal, (case when (a.tip_diagnostic='1-' or a.tip_diagnostic='5-' or 
		...
		round((case when max(c.NrCMCNP)=1 then max(zile_lucratoare*(case when a.Tip_diagnostic='10' then 0.25 else 1 end)) else sum(zile_lucratoare*(case when a.Tip_diagnostic='10' then 0.25 else 1 end)) end),0), 
		...
		else sum((zile_lucratoare-zile_cu_reducere)*(case when tip_diagnostic='10' then 0.25 else 1 end)) end),0), 
