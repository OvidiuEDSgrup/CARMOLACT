--***
Create
function fDeclaratia112Impozit (@DataJ datetime, @DataS datetime, @ImpozitPL int, @lm char(9)='')
...
Begin
	...
	--	inserez impozitul pt. zilieri
		...
		where s.Data between @DataJ and @DataS and @ImpozitPL=0 and (@lm='' or s.Loc_de_munca like rtrim(@lm)+'%')
		...
		where s.Data between @DataJ and @DataS and @ImpozitPL=1 and @AnPLImpozit=0 and @Colas=0 
			and (@lm='' or s.Loc_de_munca like rtrim(@lm)+'%')
