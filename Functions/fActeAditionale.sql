...
returns @aActe_aditionale table
	(marca char(8), numar int, anul char(4), luna char(4), ziua int, nr_act char(50), data_act datetime, datavig_act datetime, modificare char(1000))
as 
Begin
	...
	(case when dbo.iauExtinfopVal(p.Marca,'CNTRITM')='' then isnull(p.Nr_contract,ip.Nr_contract) else dbo.iauExtinfopVal(p.Marca,'CNTRITM') end),
