...
Create 
function fLista_istoric_venituri
...
Begin
	...
	select a.data, a.marca, max(n.loc_de_munca), max(i.Nume), max(i.grupa_de_munca), max(isnull(d.regim_de_lucru,8)),
		sum(a.total_ore_lucrate), round(sum(a.total_ore_lucrate/isnull(d.regim_de_lucru,8)),0), sum(a.ore_nemotivate), 
		round(sum(a.ore_nemotivate/isnull(d.regim_de_lucru,8)),0), sum(a.ore_concediu_medical), round(sum(a.ore_concediu_medical/isnull(d.regim_de_lucru,8)),0), 
		sum(a.ore_concediu_de_odihna), round(sum(a.ore_concediu_de_odihna/isnull(d.regim_de_lucru,8)),0), 
		...
		sum(a.ore_invoiri), round(sum(a.ore_invoiri/isnull(d.regim_de_lucru,8)),0), sum(a.ore_intrerupere_tehnologica), 
		round(sum(a.ore_intrerupere_tehnologica/isnull(d.regim_de_lucru,8)),0), sum(a.ore_obligatii_cetatenesti), 
		round(sum(a.ore_obligatii_cetatenesti/isnull(d.regim_de_lucru,8)),0), sum(a.ore_concediu_fara_salar), 
		(case when isnull(max(ca.Zile_CFS),0)<>0 and max(a.Spor_cond_10)<1 then isnull(max(ca.Zile_CFS),0) else round(sum(a.ore_concediu_fara_salar/isnull(d.regim_de_lucru,8)),0) end), 
		sum(a.realizat__regie), sum(a.realizat_acord), sum(a.indemnizatie_ore_supl_1), sum(a.indemnizatie_ore_supl_2), sum(a.indemnizatie_ore_supl_3), sum(a.indemnizatie_ore_supl_4), 
		sum(a.indemnizatie_ore_spor_100), sum(a.ind_ore_de_noapte), sum(a.ind_obligatii_cetatenesti), sum(a.ind_concediu_de_odihna), 
		sum(a.ind_c_medical_CAS), sum(a.ind_c_medical_unitate), sum(a.premiu), max(n.VENIT_TOTAL), max(n.VEN_NET_IN_IMP), 
		max(n.Impozit), max(n.VENIT_NET), max(d.Loc_de_munca), 
		(case when @ordonare=3 then n.loc_de_munca else '' end), max(i.salar_de_baza), max(lm.denumire), 
		max(isnull(f2.Cod_functie,i.Cod_functie)), max(f.Denumire), 
		(select sum(convert(int,h.data_sfarsit-h.data_inceput+1)) from conmed h where a.data=h.data and a.marca=h.marca) as td_zile_calend_cm, 
	...
	where a.data between @DataJos and @DataSus and  a.marca between @MarcaJos and @MarcaSus 
		and n.loc_de_munca between @LocmJos and @LocmSus 
		and (@Filtru_grupa=0 or i.grupa_de_munca=@Grupa) and (@Filtru_sex=0 or p.sex=@Sex) 
		and (@lTip_salarizare=0 or (@Tip_salarizare='T' and i.tip_salarizare in ('1','2')) or (@Tip_salarizare='M' and i.tip_salarizare in ('3','4','5','6','7'))) 
		and (@drept_conducere=0 or (@drept_conducere=1 and @drept=1 and (@liste_drept='T' or @liste_drept='C' and p.pensie_suplimentara=1 or @liste_drept='S' and p.pensie_suplimentara<>1)) or (@drept_conducere=1 and @drept=0 and @liste_drept='S' and p.pensie_suplimentara<>1))
