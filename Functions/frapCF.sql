CREATE function [dbo].[frapCF](@dataInceput datetime,@dataSfarsit datetime/*, @gestiune char(200)*/)
returns @rapCF table
(factura char(20),  data  datetime,  cod_tert char(13), denumire_tert char (40),
...
select *
from(
select d.factura as factura,
		(select min(ff.data) from facturi ff where ff.factura=d.factura and ff.tert=d.tert and ff.subunitate='1' and ff.tip=(0x46)) as data,
		--min(d.data) as data,
		d.tert as cod_tert,
...
where d.tip in ( 'ap', 'as', 'ac' )
		...
		and not (factura  is null) and (factura!='')
	
		and d.subunitate = '1'
		and tip_miscare in ('E', 'V')
		--and factura in (select factura from doc as dd where dd.tip in ( 'ap', 'as', 'ac' )
		--							and dd.data_facturii between @dataInceput and @dataSfarsit
		--							and dd.subunitate = '1' and dd.cod_gestiune in (@gestiune))
		--and d.gestiune in (@gestiune)
                               -- and d.gestiune not  in ('25')

	--	and grupa <> '4428'
		group by  d.factura, d.tert, gestiune--, d.numar_dvi		
) as rezultat

order by  data, factura
