...
Create 
function fDeclaratii_fiscale_salarii 
...
begin
	...
	CASIAlte decimal(7), SomajIAlte decimal(7), ImpozitDAC decimal(10), ImpozitCCC decimal(10), CassAngFambp float, CassFambp float) 
	...
	if @pLmsus='' and @pLmjos<>''
		set @pLmsus=rtrim(@pLmjos)+'ZZZ'
	...
	where n.data=@DataSus and i.Grupa_de_munca='O' and i.Tip_colab in ('CCC','DAC')
		and (@pLmjos='' or i.Loc_de_munca like rtrim(@pLmjos)+'%')
	...
	if exists (select * from sysobjects where name ='SalariiZilieri')
		select @ImpozitZilieri=isnull(sum(Impozit),0) from SalariiZilieri where Data between @DataJos and @DataSus
			and (@pLmjos='' or Loc_de_munca like rtrim(@pLmjos)+'%')

	insert into @sume
	select @DataJos, @DataSus, isnull(sum(n.Impozit+n.Diferenta_impozit)-max(isnull(@ImpozitDAC,0))-max(isnull(@ImpozitCCC,0)),0), 
	...
	from @sume a where CassAngFambp<>0
	union all 
	...
	from @sume a where SomajIAlte<>0
	union all 
...
/*
	select * from fDeclaratii_fiscale_salarii ('10/01/2011', '10/31/2011', '12/25/2011', '112', '', 'ZZZ')
*/