--***
create function  fBenef(@Sb char(9),@IstFactImpl int,@DataJ datetime,@DataS datetime,@Tert char(13),@Fact char(20),@cCt char(13),
			@4428 int,@GenUni int,@DocSch int,@LME int,@FactBil int,@DataJMF datetime, @locm varchar(20))
returns @docbenef table
(subunitate char(9),tert char(13),factura char(20),tip char(2),numar char(20),data datetime,valoare float,tva float,achitat float,valuta char(3),curs float,total_valuta float,achitat_valuta float,loc_de_munca char(13),comanda char(40),cont_de_tert char(20),fel int,cont_coresp char(20),explicatii char(50),numar_pozitie int,gestiune char(13),data_facturii datetime,data_scadentei datetime,punct_livrare char(5),barcod char(30),
...
declare @userASiS varchar(10), @lista_gestiuni bit, @contTvaDeductibil varchar(20)
...
insert @docbenef
select i.subunitate,i.tert,i.factura,'SI',i.factura,i.data,i.valoare,i.tva_11+i.tva_22,i.achitat,i.valuta,i.curs,i.valoare_valuta,i.achitat_valuta,
		i.loc_de_munca,i.comanda,i.cont_de_tert,'1','','Sold initial',0,'',i.data,i.data_scadentei,'','','' contTVA,'' as cod,0 as cantitate,
...
where @IstFactImpl=1 and i.subunitate=@Sb and i.tip='B' and i.data_an=@DataJ and i.tert like rtrim(@Tert) and i.factura like rtrim(@Fact) and i.cont_de_tert like rtrim(@cCt)+'%'
...
select fi.subunitate,fi.tert,fi.factura,'SI',fi.factura,fi.data,fi.valoare,fi.tva_11+fi.tva_22,fi.achitat,fi.valuta,fi.curs,fi.valoare_valuta,
		...
		isnull((select max(i.serie_doc) from incfact i where i.subunitate=fi.subunitate and i.numar_factura=fi.factura and i.tert=fi.tert), ''),
...
where @IstFactImpl=2 and fi.subunitate=@Sb and fi.tip=0x46 and fi.tert like rtrim(@Tert) and fi.factura like rtrim(@Fact) and fi.cont_de_tert like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
...
where p.subunitate=@Sb and p.tip in ('AP','AS') and (@FactBil=1 or p.cont_factura<>'' or left(p.cont_de_stoc,1)<>'8') and p.data between @DataJ and @DataS and p.tert like rtrim(@Tert) and p.factura like rtrim(@Fact) and p.cont_factura like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
	and (@locm='%' or p.Loc_de_munca like @locm)
union all 
select p.subunitate,p.tert,p.factura,p.plata_incasare,p.numar,p.data,0,0,
		(case when p.plata_incasare in ('PS','IS') then -1 else 1 end)*(p.suma-suma_dif),p.valuta,p.curs,0,
...
where p.subunitate=@Sb and p.plata_incasare in ('IB','IR','PS') and p.data between @DataJ and @DataS and p.tert like rtrim(@Tert) and p.factura like rtrim(@Fact) and p.cont_corespondent like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
	and (@locm='%' or p.Loc_de_munca like @locm)
union all 
select	p.subunitate,p.tert,factura_stinga,p.tip,p.numar_document,p.data,(case when p.tip='CB' then 0 else suma end),
		(case when p.tip='CB' or p.stare in (1, 2) then 0 else tva22 end),(case when p.tip='CB' then -suma else 0 end),p.valuta,p.curs,
...
left outer join terti t on p.subunitate=t.subunitate and p.tert=t.tert 
where p.subunitate=@Sb and p.tip in ('FB','IF','CB') and p.data between @DataJ and @DataS and p.tert like rtrim(@Tert) and p.factura_stinga like rtrim(@Fact) and p.cont_deb like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
...
left join lmfiltrare pr on pr.cod=p.loc_munca and pr.utilizator=@userASiS
left outer join terti t on p.subunitate=t.subunitate and p.tert=t.tert 
where p.subunitate=@Sb and p.tip in ('CO','IF','CB') and p.data between @DataJ and @DataS and p.tert like rtrim(@Tert) and p.factura_dreapta like rtrim(@Fact) and p.cont_cred like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
	and (@locm='%' or (p.Loc_munca) like @locm)
union all 
...
where p.subunitate=@Sb and p.tip='C3' and p.data between @DataJ and @DataS and p.tert_beneficiar like rtrim(@Tert) and p.factura_dreapta like rtrim(@Fact) and p.cont_cred like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
	and (@locm='%' or (p.Loc_munca) like @locm)
union all 
...
where procent_inchiriere not in (1,6,9) and m.subunitate=@Sb and tip_miscare='EVI' and data_miscarii between @DataJMF and @DataS and m.tert like rtrim(@Tert) and m.factura like rtrim(@Fact) and loc_de_munca_primitor like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
	and (@locm='%' or (m.Loc_de_munca_primitor) like @locm)
union all
select p.subunitate,p.tert,(case when p.cod_intrare='' then 'AVANS' else left(p.cod_intrare,@LFact) end),'AX',numar,p.data,0,0,	
		round(convert(decimal(18,5),cantitate*p.pret_vanzare),2)+
		@4428*(case when p.tip in ('AP','AS') and @GenUni=0 and (@DocSch=0 or p.tip='AS') and p.procent_vama in (1, 2) then 0 else 1 end)*tva_deductibil,
		p.valuta,p.curs,0,(case when p.valuta<>'' and p.curs>0 then round(convert(decimal(18,5),cantitate*pret_valuta*(1-p.discount/100)),2)+
							round(convert(decimal(18,5),@4428*(case when p.tip in ('AP','AS') and @GenUni=0 and (@DocSch=0 or p.tip='AS') 
								and p.procent_vama in (1, 2) then 0 else 1 end)*tva_deductibil/p.curs),2) else 0 end),	
...
left outer join nomencl n on n.cod=p.cod
where p.subunitate=@Sb and p.tip in ('AP','AS') and p.data between @DataJ and @DataS and p.tert like rtrim(@Tert) and 
	(case when p.cod_intrare='' then 'AVANS' else left(p.cod_intrare,20) end) like rtrim(@Fact) and cont_de_stoc like rtrim(@cCt)+'%'
	and (dbo.f_areLMFiltru(@userASiS)=0 or pr.utilizator=@userASiS)
	and (@locm='%' or (p.Loc_de_munca) like @locm)

if exists (select 1 from sysobjects o where o.type='TF' and o.name='fBenefUA')
	insert @docbenef select f.*, '','',0,'',f.data from dbo.fBenefUA(@Sb,@DataJ,@DataS,@Tert,@Fact,@cCt,@LFact) f
