...
declare @datajos datetime, @marca varchar(50), @functie varchar(50), @benret varchar(50), @subtipret varchar(50), @tipcorectie varchar(50),
		...
		@nreprpers varchar(50), @freprpers varchar(50), @tipstat varchar(30)
...
--	nume si functie pt. persoana care intocmeste declaratiile de TVA
		...
		@tipstat=@parXML.value('(row/@tipstat)[1]','varchar(30)')
...
insert into @denumiri(parametru,valoare)
	...
	select '@tipstat', rtrim(isnull((select max(ts.denumire) from TipStatPlata ts where ts.Tip_stat_plata=@tipstat),'<nu exista>')) 
		where @tipstat is not null union all
