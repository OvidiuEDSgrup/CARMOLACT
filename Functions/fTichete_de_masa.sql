...
Create function fTichete_de_masa 
	...
	@serieinceput char(13)=null, @seriesfarsit char(13)=null, @Serii int=0, @tipopexcep char(1)=null) 
returns @tichete_de_masa table
	(Data datetime, Marca char(6), Nume char(50), CNP char(13), Loc_de_munca char(9), Denumire_lm char(30), Cod_functie char(6), Denumire_functie char(80), 
...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	select a.Data_lunii, a.Marca, max(isnull(i.Nume,p.Nume)) as Nume, max(p.Cod_numeric_personal) as Cod_numeric_personal, 
		...
		(case when @Serii=1 then a.Serie_inceput else '' end), max(a.Serie_sfarsit), 
		sum((case when /*@tiplista<>'' and*/ @tiplista<>'Tip operatie' and @tiplista<>'Balanta' and a.tip_operatie in ('R') then -1 else 1 end)*a.Nr_tichete) as Nr_tichete, 
		sum(a.Valoare_tichet) as Valoare_tichet, sum(a.Valoare_imprimat) as Valoare_imprimat, sum(a.TVA_imprimat) as TVA_imprimat, 
		sum((case when /*@tiplista<>'' and*/ @tiplista<>'Tip operatie' and @tiplista<>'Balanta' and a.tip_operatie in ('R') then -1 else 1 end)*a.Nr_tichete*a.Valoare_tichet) as Valoare_tichete, 0 as impozit, 
	...
	where @ParcurgTichete=1
		...
		and (@tipoperatie is null or @tipoperatie='M' or a.tip_operatie=@tipoperatie)
		and (@tipoperatie!='M' or a.tip_operatie!='S')
		and (@tipopexcep is null or a.tip_operatie<>@tipopexcep)
		...
		and (@mandatar is null or m.mandatar=@mandatar) and (@tipstat is null or isnull(p.tip_stat,ip.Religia)=@tipstat) 
	...
	group by a.Data_lunii, a.Marca, (case when @Serii=1 then a.Serie_inceput else '' end), (case when @tiplista='Tip operatie' or @tiplista='Balanta' then a.tip_operatie else '' end)
	union all 
