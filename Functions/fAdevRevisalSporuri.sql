...
--	Set @HostID='2640'
	...
	select a.Marca, (case when a.TipSpor='TipSporAngajator' then a.CodSpor else c.Descriere end), 
		...
		isnull(p.Nr_contract,ip.Nr_contract), isnull((select max(data_inf) from extinfop e where e.marca=a.marca and e.cod_inf='DATAINCH'),'01/01/1901')
...
--	inlocuiesc caracterele speciale intrucat da eroare la deschidere document rezultat
	update @AdevRevisalSporuri set DenumireSpor=REPLACE(DenumireSpor,'î','i')
	update @AdevRevisalSporuri set DenumireSpor=REPLACE(DenumireSpor,'â','a')
	update @AdevRevisalSporuri set DenumireSpor=REPLACE(DenumireSpor,'â','a')
