...
Begin
	...
	select @pMarca=isnull(@pMarca,'')

	insert into @fDate_pontaj_automat
--	determin regimul de lucru
	select 'RL', 'Regim de lucru', '', p.marca, 
	(case when p.Grupa_de_munca in ('O','P') or (p.Grupa_de_munca in ('C') or p.Grupa_de_munca in ('N','D','S') and @PlataCuOra=0) and @RegimLV=0 
		then (case when p.Salar_lunar_de_baza=0 then (case when p.Grupa_de_munca in ('O','P') then 3 else 8 end) else p.Salar_lunar_de_baza end) 
		else (case when @RegimLV=1 and p.Salar_lunar_de_baza<>0 then round(p.Salar_lunar_de_baza*8/(case when p.Tip_salarizare in ('1','2') then @OreLuna else @NrmLuna end),
			(case when @Dafora=1 then 2 else 0 end)) else 8 end) end) as RL, 
	(case when (month(p.Data_angajarii_in_unitate)=month(@dataSus) and year(p.Data_angajarii_in_unitate)=year(@dataSus) or p.Loc_ramas_vacant=1 and month(p.Data_plec)=month(@dataSus) 
		and year(p.Data_plec)=year(@dataSus)) and @PontajZilnic=0 
			then dbo.Zile_lucratoare((case when month(p.Data_angajarii_in_unitate)=month(@dataSus) and year(p.Data_angajarii_in_unitate)=year(@dataSus) 
				then p.Data_angajarii_in_unitate else dbo.bom(@dataSus) end),
			(case when p.Loc_ramas_vacant=1 and month(p.Data_plec)=month(@dataSus) and year(p.Data_plec)=year(@dataSus) then DateAdd(day,-1,p.Data_plec) 
				else dbo.eom(@dataSus) end)) else @OreLuna/8.00 end) as Zile, (case when isnull(e.Procent,0)<>0 then isnull(e.Procent,0) else 0 end) as Ore, @dataJos, @dataSus, '', ''			
	from personal p
		left join extinfop e on e.marca=p.marca and e.cod_inf='TIPINTREPTM' and e.val_inf='OrePeLuna' and e.Procent<>0
	where (@Tip='' or @Tip='RL') and (@pMarca='' or p.Marca=@pMarca) and @FormularPontaj=0
	union all
--	concedii medicale	
