...
Create 
function fLista_personal
	(@DataJos datetime, @DataSus datetime, @MarcaJos char(6), @MarcaSus char(6), @LocmJos char(9), @LocmSus char(9), @Cod_functie_Jos char(6), @Cod_functie_Sus char(6), 
	@GrupaJos char(1), @GrupaSus char(1), @unTipStat int, @Tipstat char(10), @l_drept char(1), @User char(30), @User_windows int, @Ordonare char(1), @Alfabetic int, 
	@lTipSalarizare int=0, @cTipsalarizare char(1))
...
begin
	...
	select a.marca, a.nume, a.cod_functie, a.denumire_functie, a.loc_de_munca, a.denumire_locm, a.grupa_de_munca, a.categoria_salarizare, 
		...
		(case when @lbaza_sp4=1 then 0 else a.sp4_calc end)+(case when @lbaza_sp5=1 then 0 else a.sp5_calc end)+(case when @lbaza_sp6=1 then 0 else a.sp6_calc end)+a.sp7_calc+spnoapte_calc, 
	...
	(select p.marca,isnull(i.Nume,p.nume) as Nume,isnull(i.cod_functie,p.cod_functie) as cod_functie, f.denumire as denumire_functie, isnull(i.loc_de_munca,p.loc_de_munca) as loc_de_munca, lm.denumire as denumire_locm,
		...
		(case when isnull(x.Val_inf,'') in ('Inegal','OreDeNoapte') or 0=1 then round(d.sal_inc_calc*p.Spor_de_noapte/100 ,0) else 0 end) as spnoapte_calc
	from personal p
		left outer join istpers i on i.data=@datasus and i.marca=p.marca
	...
	where p.marca between @MarcaJos and @MarcaSus and p.data_angajarii_in_unitate<=@DataSus
		and isnull(i.cod_functie,p.cod_functie) between @Cod_functie_Jos and @Cod_functie_Sus and isnull(i.loc_de_munca,p.loc_de_munca) between @LocmJos and @LocmSus and isnull(i.grupa_de_munca,p.grupa_de_munca) between @GrupaJos and @GrupaSus
		and (@unTipStat=0 or @Tipstat=e.religia)
		and (@lTipSalarizare=0 or (@cTipSalarizare='T' and isnull(i.tip_salarizare,p.Tip_salarizare)  in ('1','2')) or (@cTipSalarizare='M' and isnull(i.tip_salarizare,p.Tip_salarizare) in ('3','4','5','6','7'))) 
