...
declare 
	...
	(	
		select 
			...
			(case when tip='R' then 'RS' when tip='M' then 'MT' when tip='O' then 'OP' else 'TT' end)as subtip ,
			...
			isnull(convert(decimal(10,2),ordine_o),0) as ordine,rtrim(cod) as cod, idp as idParinte,id as idReal,ISNULL(cantitate_i,0) as cant_i,
			rtrim(resursa) as lm,CONVERT(decimal(10,2),pret) as pret ,tip as tip,
			(case when @cantitate>0 then CONVERT(decimal(10,2),cantitate*@cantitate) else convert(decimal(10,2),cantitate) end) as cantitate,
