...
returns @wIaConcedii table (Data varchar(10), Marca varchar(6), Nume varchar(50), numar_pozitie int, subtip varchar(2), tip varchar(2), 
	...
	seriecm varchar(10), numarcm varchar(10), cminitial varchar(15), dencminitial varchar(50), 
	...
	lm varchar(9), denlm varchar(100), comanda varchar(20), dencomanda varchar(100),
	zileco int, indemnizatieco decimal(10), indnetaco decimal(10), dataoperarii datetime, zileconalte int, oreconalte int, detalii xml)
As
Begin
	...
	isnull(nullif(cm.detalii.value('(/row/@bazastagiu)[1]','decimal(10)'),0),isnull((select Baza_Stagiu 
		from dbo.stagiu_cm (cm.Data, cm.Marca, cm.Data_inceput, dbo.data_inceput_cm(cm.Data, cm.marca, cm.Data_inceput, 1), (case when cm.Zile_luna_anterioara>0 or i.Serie_certificat_cm_initial<>'' then 1 else 0 end), 6)),0)) as bazastagiu, 
	isnull(nullif(cm.detalii.value('(/row/@zilestagiu)[1]','int'),0),isnull((select Zile_Stagiu 
		from dbo.stagiu_cm (cm.Data, cm.Marca, cm.Data_inceput, dbo.data_inceput_cm(cm.Data, cm.marca, cm.Data_inceput, 1), (case when cm.Zile_luna_anterioara>0 or i.Serie_certificat_cm_initial<>'' then 1 else 0 end), 6)),0)) as zilestagiu, 
	...
	Null as zileco, Null as indemnizatieco, Null as indnetaco, Null as dataoperarii, Null as zileconalte, Null as oreconalte, cm.detalii as detalii
	from conmed cm
	...
	9 as numarpozitie, (case when @tip='SL' then 'O1' when co.tip_concediu in ('3','6') then 'ON' else 'O2' end) as subtip, @tip as tip, 'CO' as denumire, 
	...
	Null as zileconalte, Null as oreconalte, Null as detalii
	from concodih co
	...
	Null as nrcrt, isnull(po.loc_de_munca,i.Loc_de_munca) as lm, lm.denumire as denlm, po.comanda as comanda, c.descriere as dencomanda,
	Null as zileco, Null as indemnizatieco, Null as indnetaco, Null as dataoperarii, ca.Zile as zileconalte, indemnizatie as oreconalte, Null as detalii
	from conalte ca
