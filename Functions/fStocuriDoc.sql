--***
create function  fStocuriDoc(@DIst datetime,@DJPdoc datetime,@DSus datetime,@Cod char(20),@Gest char(20),@Codi char(20),@Grupa char(13),@TipStoc char(1),@Cont char(13),@Corel int,
	@Loc char(30),@LM char(9),@Com char(40),@Cntr char(20),@Furn char(13),@Lot char(20))
returns @docstoc table
(subunitate char(9),gestiune char(20),cont char(20),cod char(20),data datetime,cod_intrare char(20),pret float,tip_document char(2),numar_document char(9),cantitate float,cantitate_UM2 float,tip_miscare char(1),in_out char(1),predator char(20),codi_pred char(20),
jurnal char(3),tert char(13),serie char(20),pret_cu_amanuntul float,tip_gestiune char(1),locatie char(30),data_expirarii datetime,TVA_neexigibil int,pret_vanzare float,accize_cump float,loc_de_munca char(9),comanda char(40),[contract] char(20),furnizor char(13),lot char(20),numar_pozitie int,cont_corespondent char(13),schimb int)
as
begin
...
union all
select a.subunitate,a.gestiune_primitoare,a.cont_corespondent,a.cod,a.data,(case when a.grupa='' then a.cod_intrare else a.grupa end),(case when a.tip='TE' and @PrestTE=1 and a.accize_datorate<>0 then accize_datorate else a.pret_de_stoc end)*(case when a.tip='DF' and a.procent_vama>0 then (1-convert(decimal(12,3),a.procent_vama/100)) else 1 end),(case a.tip when 'DF' then 'DI' when 'PF' then 'PI' else 'TI' end),a.numar,a.cantitate,(case when @UM2=0 and isnull(n.UM_1,'')<>'' and isnull(n.coeficient_conversie_1,0)<>0 then round(convert(decimal(17,5),a.cantitate/n.coeficient_conversie_1),3) when a.tip in ('DF','PF') or @UM2=0 or left(isnull(n.UM_2,''),1)<>'Y' then 0 else a.suprataxe_vama end),'I','2',a.gestiune,a.cod_intrare,a.jurnal,'','',a.pret_cu_amanuntul,(case when a.tip in ('DF','PF') then 'F' else isnull(tip_gestiune,'') end),a.locatie,a.data_expirarii,a.TVA_neexigibil,a.pret_amanunt_predator,a.accize_cumparare,a.loc_de_munca,a.comanda,(case when a.tip='TE' then a.factura else '' end),'','',a.numar_pozitie,a.cont_de_stoc,a.procent_vama
from pozdoc a
...
union all
select a.subunitate,a.tert,cont_corespondent,a.cod,a.data,cod_intrare,a.pret_de_stoc,a.tip,numar,a.cantitate,(case when @UM2=0 and isnull(n.UM_1,'')<>'' and isnull(n.coeficient_conversie_1,0)<>0 then round(convert(decimal(17,5),a.cantitate/n.coeficient_conversie_1),3) else 0 end),(case when tip_miscare='E' then 'I' else 'E' end),(case when tip_miscare='E' then '1' else '2' end),a.gestiune,a.cod_intrare,jurnal,'','',0,'T',locatie,data_expirarii,TVA_neexigibil,a.pret_vanzare,0,a.loc_de_munca,a.comanda,(case when a.tip='AP' then a.contract else '' end),'',(case when a.tip='AI' then a.grupa else '' end),numar_pozitie,a.cont_de_stoc,a.procent_vama
from pozdoc a 
