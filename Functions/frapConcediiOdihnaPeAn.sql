...
--	verific daca utilizatorul are/nu are dreptul de Salarii conducere (SALCOND)
	...
	isnull(ba.Zile_co_efectuat_an,0) 
		+ isnull((select sum(c.zile_co) from concodih c where c.data between @dataJos and @dataSus and p.marca=c.marca and (c.tip_concediu='3' or c.tip_concediu='6')),0) as zile_co_efectuat_an, 
	...
	isnull(ia.coef_invalid,0)+(case when @tipzileramase=1 then isnull(zcl.zile,0) else isnull(zcan.zile,0) end)
		-(isnull(ba.Zile_co_efectuat_an,0)+ isnull((select sum(c.zile_co) from concodih c where c.data between @dataJos and @dataSus and p.marca=c.marca and (c.tip_concediu='3' or c.tip_concediu='6')),0)) 
		as zile_co_ramase,
