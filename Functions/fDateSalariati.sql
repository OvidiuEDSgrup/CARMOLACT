...
begin
	...
	select p.marca, p.nume, p.cod_numeric_personal, 
		...
		isnull(p.Nr_contract,ip.Nr_contract) as nr_contract, dc.Data_inf as data_contract, 
		...
		convert(int,left(isnull(p.detalii.value('(/row/@vechimestudii)[1]','varchar(6)'),ip.Vechime_studii),2)) as vechime_studii_ani,
		convert(int,substring(isnull(p.detalii.value('(/row/@vechimestudii)[1]','varchar(6)'),ip.Vechime_studii),3,2)) as vechime_studii_luni,
		convert(int,substring(isnull(p.detalii.value('(/row/@vechimestudii)[1]','varchar(6)'),ip.Vechime_studii),5,2)) as vechime_studii_zile
	from personal p 
