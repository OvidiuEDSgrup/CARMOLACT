...
--	daca tabela par este view inseamna ca se lucreaza cu parametrii pe locuri de munca (in aceeasi BD sunt mai multe unitati)	
	...
	from tichete a 
		...
		left outer join (select marca, sum(cantitate*(case when r.Categoria_salarizare='L' and Tarif_unitar=0 then 1 else tarif_unitar end)) as Realizat 
			from realcom r where data between @dataJos and @dataSus group by marca) r on r.marca=a.marca
	...
	Begin
		if @DeLaCalculLichidare=0 or @DeLaCalculLichidare=2
		Begin
			if @TVenitR_LM<>0 and @NCTichComenzi=1
			Begin
				Declare cursor_realcom cursor For
				select r.marca, r.loc_de_munca, r.comanda, sum((r.cantitate)*(case when r.Categoria_salarizare='L' and Tarif_unitar=0 then 1 else r.tarif_unitar end)) 
					from realcom r
				...
				Begin
					update @dateNCtichete set Numar_tichete=Numar_Tichete+@NumarTicheteDif, Valoare_tichete=Valoare_Tichete+@ValoareTicheteDif 
