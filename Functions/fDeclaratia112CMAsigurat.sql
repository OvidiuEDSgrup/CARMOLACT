--***
Create
function fDeclaratia112CMAsigurat 
	(@DataJ datetime, @DataS datetime, @lm char(9))
...
--	creare cursor pt. verificare existenta CM pe acelasi CNP, alta marca si aceeasi perioada
	...
	where a.marca=i.marca and a.data_inceput between @DataJ and @DataS and a.data=i.data 
		and not(a.tip_diagnostic in ('2-','3-','4-') or (a.tip_diagnostic='10' or a.tip_diagnostic='11') and a.suma=1)
		and a.tip_diagnostic<>'0-' and (@lm='' or i.Loc_de_munca like rtrim(@lm)+'%')

	insert into @DateCM
	select a.Data, max(ta.TagAsigurat), a.Marca as Marca, max(p.cod_numeric_personal) as CNP, 
		isnull((select round(sum(b.ore_concediu_medical/b.regim_de_lucru),0) from pontaj b where b.data between @DataJ and @DataS and b.marca=a.marca),0) as Zile_prestatii, 
		isnull((select round(sum(m.zile_lucratoare*(case when m.Tip_diagnostic='10' then 0.25 else 1 end)),0) from conmed m where m.data_inceput between @DataJ and @DataS and m.marca=a.marca and m.tip_diagnostic in ('1-','2-','3-','4-','5-','6-','12','13','14') and m.marca in (select t.marca from pontaj t where t.marca=m.marca and year(t.data)=year(m.data) and month(t.data)=month(m.data) and t.grupa_de_munca='D' and t.ore_concediu_medical<>0)),0) as Zile_prestatii_CD, 
		isnull((select round(sum(m.zile_lucratoare*(case when m.Tip_diagnostic='10' then 0.25 else 1 end)),0) from conmed m where m.data_inceput between @DataJ and @DataS and m.marca=a.marca and m.tip_diagnostic in ('1-','2-','3-','4-','5-','6-','12','13','14') and m.marca in (select t.marca from pontaj t where t.marca=m.marca and year(t.data)=year(m.data) and month(t.data)=month(m.data) and t.grupa_de_munca='S' and t.ore_concediu_medical<>0)),0) as Zile_prestatii_CS, 
		sum((case when not(a.tip_diagnostic in ('2-','3-','4-') or (a.tip_diagnostic='10' or a.tip_diagnostic='11') and a.suma=1) then round(a.Zile_lucratoare*(case when a.Tip_diagnostic='10' then 0.25 else 1 end),0) else 0 end)) as Zile_CM_fnuass, 
	...
	from conmed a 
		...
		left outer join dbo.fDeclaratia112TagAsigurat (@DataJ, @DataS, @lm) ta on a.data=ta.data and a.marca=ta.marca
