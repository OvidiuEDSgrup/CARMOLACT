...
Begin
	...
	Begin
		...
		round((case when r.Parametru='RSPVECH' then i.Spor_vechime
			...
			when r.Parametru='RSCOND7' then isnull(p.Spor_cond_7,ip.Spor_cond_7) else 0 end),0) as Valoare, 
		1 as Versiune
		from istPers i  
...
/*
	...
	when r.Parametru='RSCOND7' then i.Salar_de_baza*p.Spor_cond_7/100 else 0 end),0) as Valoare, 
	1 as Versiune
	from istPers i  
		left outer join personal p on p.Marca=i.Marca
		left outer join lm l on l.Cod=p.Loc_de_munca
