...
--	momentan asta functioneaza doar pt. GrupSapte. Cu timpul poate se va folosi si de altii.
	...
	max(isnull(p.Nr_contract,ip.Nr_contract)), isnull((select max(data_inf) from extinfop e where e.marca=a.marca and e.cod_inf='DATAINCH'),'01/01/1901')
	from conalte a
...
--	selectez suspendarile de contract din extinfop (datele introduse in macheta salariati - CTRL+J)
	...
	isnull(p.Nr_contract,ip.Nr_contract), isnull((select max(data_inf) from extinfop e3 where e3.marca=e.marca and e3.cod_inf='DATAINCH'),'01/01/1901')
	from Extinfop e 
...
--	inlocuiesc caracterele speciale intrucat da eroare la deschidere document rezultat
	update @RevisalSuspendari set DescriereTemeiLegal=REPLACE(DescriereTemeiLegal,'î','i')
	update @RevisalSuspendari set DescriereTemeiLegal=REPLACE(DescriereTemeiLegal,'â','a')
	update @RevisalSuspendari set DescriereTemeiLegal=REPLACE(DescriereTemeiLegal,'â','a')
