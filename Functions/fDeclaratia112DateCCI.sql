--***
Create
function fDeclaratia112DateCCI 
...
begin
	...
	select @TipD as TipD, (case when @TipD=1 then 'M' else '' end)  as Tip_rectif, a.Data, max(a.Marca), max(a.Tip_diagnostic), 
		...
		round(max(a.Zile_cu_reducere*(case when a.Tip_diagnostic='10' then 0.25 else 1 end)),0) as Zi_PRE_ANG, 
		round(max((a.Zile_lucratoare-a.Zile_cu_reducere)*(case when a.Tip_diagnostic='10' then 0.25 else 1 end)),0) as Zi_FN, 
		round(max(a.Zile_lucratoare*(case when a.Tip_diagnostic='10' then 0.25 else 1 end)),0) as Zi_PRE, 
